...
class MultiSelectQuestionWidget:
   ...
   def __init__(self,
      ...
      scroll = widgets.Box([self.submit_button, self.printed_output], layout=widgets.Layout(overflow='scroll hidden', border='1px solid black', width='500px', height=''))
...
def MultiSelect(descriptions, options, types_):
    ...
    for question, options in zip(descriptions, options):
        ...
        # Modify the HTML style to ensure text wraps within the specified width
        description_widget = widgets.HTML(
            value=f"<hr style='border:1px solid lightgray; width:100%;'>"
                  f"<div style='text-align: left; width: {description_width}; word-wrap: break-word;'>"
                  f"<b>{question}</b></div>")
