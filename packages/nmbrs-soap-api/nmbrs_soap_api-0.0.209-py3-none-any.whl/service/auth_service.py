...
class AuthService(Service):
    ...
    def authenticate_using_sso(self, username: str, password: str) -> dict:
        token = self.sso_service.service.GetToken(**{"Username": username, "Password": password})
        return self.authenticate_using_standard_token(username, token)

    @nmbrs_exception_handler(resources=["SingleSignOn:GetToken2"])
    def refresh_sso_token(self, username: str, token: str) -> str:
