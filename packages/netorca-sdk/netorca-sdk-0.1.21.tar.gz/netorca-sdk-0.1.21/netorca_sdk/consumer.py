...
from netorca_sdk.exceptions import NetorcaException, NetOrcaWrongYAMLFormat
...
class ConsumerSubmission:
    ...
    def load_yaml_file(file_path: str) -> dict:
        ...
        with open(file_path, "r") as stream:
            try:
                ...
                raise NetOrcaWrongYAMLFormat(f"Error while parsing file: `{file_name}`. Exception: {exc.problem}")
    ...
    def validate(self, pretty_print=False, partial=False) -> Tuple[bool, dict]:
        ...
        if response.status_code != 200:
            return False, {"error": f"Validation failed due to response status_code: {response.status_code}"}
