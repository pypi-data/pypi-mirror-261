...
class TestRelSca(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.m1dwr99 = np.loadtxt(Path(__file__).parent.joinpath('./testdata/rel_sca_m1_dwr_a0.99.dat'))
        ...
        self.m1dwr4 = np.loadtxt(Path(__file__).parent.joinpath('./testdata/rel_sca_m1_dwr_a0.4.dat'))
        ...
        self.m2dwr99 = np.loadtxt(Path(__file__).parent.joinpath('./testdata/rel_sca_m2_dwr_a0.99.dat'))
        ...
        self.m2dwr4 = np.loadtxt(Path(__file__).parent.joinpath('./testdata/rel_sca_m2_dwr_a0.4.dat'))
    ...
    def test_non_rel_limit(self):
        ...
        for i, sp in enumerate(self.spi):
            # m=1
            self.assertEqual(self.rs.omega_real(1,0,sp,1), 0)
            # m=2
            self.assertEqual(self.rs.omega_real(2,0,sp,1), 0)

        """\Delta\omega_R"""
        # alpha -> 0
        for i, sp in enumerate(self.spi):
            # m=1
            self.assertEqual(self.rs._deltaomega(1,0,sp), 0)
            # m=2
            self.assertEqual(self.rs._deltaomega(2,0,sp), 0)
        ...
        for i,al in enumerate(self.alp):
            for j,sp in enumerate(self.spi):
                for k,mc in enumerate(mcl):
                    ...
                    dwr = self.rs.domegar_dmc(1,al,sp,mc)
                    wr -= mc*dwr
                    self.assertEqual(wr, self.rs.omega_real(1,al,sp,0))
        # m=2
        for i,al in enumerate(self.alp):
            for j,sp in enumerate(self.spi):
                for k,mc in enumerate(mcl):
                    ...
                    wr -= mc*dwr
                    self.assertEqual(wr, self.rs.omega_real(2,al,sp,0))
        ...
        for i,al in enumerate(self.alp):
            for j,sp in enumerate(self.spi):
                ...
                if (al>alsw[0] and sp>spsw[0]):
                    ...
                    self.assertFalse(np.isnan(self.rs._f1dwr(y, sp)))
                    self.assertFalse(np.isnan(self.rs._f1wi(y, sp)))
                else:
                    self.assertTrue(np.isnan(self.rs._f1wr(y, sp)))
                    self.assertTrue(np.isnan(self.rs._f1dwr(y, sp)))
        ...
        for i,al in enumerate(self.alp):
            for j,sp in enumerate(self.spi):
                ...
                if (al>alsw[1] and sp>spsw[1]):
                    ...
                    self.assertFalse(np.isnan(self.rs._f2dwr(y, sp)))
                    self.assertFalse(np.isnan(self.rs._f2wi(y, sp)))
                else:
                    self.assertTrue(np.isnan(self.rs._f2wr(y, sp)))
                    self.assertTrue(np.isnan(self.rs._f2dwr(y, sp)))
    ...
    def test_omegaI_against_v1_data(self):
        ...
        for i,wit in enumerate(witest):
            wic = self.rs.omega_imag(1,altest[i],0.99)
            if (abs((wic-wit)/wic)>tol):
                if (errplt):
                    ...
                    for j,al in enumerate(altest):
                        wiltest[j] = self.rs.omega_imag(1,al,0.99)
        ...
        witest = self.m1wi4[:,1]
        for i,wit in enumerate(witest):
            wic = self.rs.omega_imag(1,altest[i],0.4)
            if (abs((wic-wit)/wic)>tol): 
                if (errplt):
                    plt.plot(altest, witest, label='wi_v1, sp=0.4,m=1')
                    wiltest = np.zeros_like(altest)
                    for j,al in enumerate(altest):
                        wiltest[j] = self.rs.omega_imag(1,al,0.4)
    ...
    def test_deltaomega_against_v1_data(self):
        """Testing \Delta\omega_R(alpha,spin) against values obtained for v1 to within tolerance"""
        tol = 1e-6
        errcount = 0
        errplt = self.errpltglb

        # m=1, a=0.99
        altest = self.m1dwr99[:,0]
        dwrtest = self.m1dwr99[:,1]
        for i,dwrt in enumerate(dwrtest):
            dwrc = self.rs._deltaomega(1,altest[i],0.99)
            if (abs((dwrc-dwrt)/dwrc)>tol):
                if (errplt):
                    plt.title('Scalar: Test delta_omega_R (dwr) against v1 data')
                    plt.plot(altest, -dwrtest, label='dwr_v1, sp=0.99,m=1')
                    dwrltest = np.zeros_like(altest)
                    for j,al in enumerate(altest):
                        dwrltest[j] = self.rs._deltaomega(1,al,0.99)
                    plt.plot(altest,-dwrltest, label='dwr_SuperRad, sp=0.99,m=1')
                    plt.plot(altest,np.abs((dwrtest-dwrltest)/dwrtest), label='rel_diff, sp=0.99,m=1')
                    plt.yscale('log')
                    plt.xscale('log')
                    plt.legend()
                errcount += 1
        ...
        # m=1, a=0.4
        altest = self.m1dwr4[:,0]
        dwrtest = self.m1dwr4[:,1]
        for i,dwrt in enumerate(dwrtest):
            dwrc = self.rs._deltaomega(1,altest[i],0.4)
            if (abs((dwrc-dwrt)/dwrc)>tol):
                if (errplt):
                    plt.plot(altest, -dwrtest, label='dwr_v1, sp=0.4,m=1')
                    dwrltest = np.zeros_like(altest)
                    for j,al in enumerate(altest):
                        dwrltest[j] = self.rs._deltaomega(1,al,0.99)
                    plt.plot(altest, -dwrltest, label='dwr_SuperRad, sp=0.4,m=1')
                    plt.plot(altest,np.abs((dwrtest-dwrltest)/dwrtest), label='rel_diff, sp=0.4,m=1')
                    plt.yscale('log')
                    plt.xscale('log')
                    plt.legend()
                errcount += 1
        ...
        # m=2, a=0.99
        altest = self.m2dwr99[:,0]
        dwrtest = self.m2dwr99[:,1]
        for i,dwrt in enumerate(dwrtest):
            dwrc = self.rs._deltaomega(2,altest[i],0.99)
            if (abs((dwrc-dwrt)/dwrc)>tol):
                if (errplt):
                    plt.title('Scalar: Test delta_omega_R (dwr) against v1 data')
                    plt.plot(altest, -dwrtest, label='dwr_v1, sp=0.99,m=2')
                    dwrltest = np.zeros_like(altest)
                    for j,al in enumerate(altest):
                        dwrltest[j] = self.rs._deltaomega(2,al,0.99)
                    plt.plot(altest,-dwrltest, label='dwr_SuperRad, sp=0.99,m=2')
                    plt.plot(altest,np.abs((dwrtest-dwrltest)/dwrtest), label='rel_diff, sp=0.99,m=2')
                    plt.yscale('log')
                    plt.xscale('log')
                    plt.legend()
                errcount += 1
        ...
        # m=2, a=0.4
        altest = self.m2dwr4[:,0]
        dwrtest = self.m2dwr4[:,1]
        for i,dwrt in enumerate(dwrtest):
            dwrc = self.rs._deltaomega(2,altest[i],0.4)
            if (abs((dwrc-dwrt)/dwrc)>tol):
                if (errplt):
                    plt.plot(altest, -dwrtest, label='dwr_v1, sp=0.4,m=2')
                    dwrltest = np.zeros_like(altest)
                    for j,al in enumerate(altest):
                        dwrltest[j] = self.rs._deltaomega(2,al,0.99)
                    plt.plot(altest, -dwrltest, label='dwr_SuperRad, sp=0.4,m=2')
                    plt.plot(altest,np.abs((dwrtest-dwrltest)/dwrtest), label='rel_diff, sp=0.4,m=2')
                    plt.yscale('log')
                    plt.xscale('log')
                    plt.legend()
                errcount += 1
        ...
        if (errcount>0):
            print('\n')
            print('Test_deltaomega_against_v1_data: Error count = %d/4' % (errcount))
            self.assertTrue(False)
        ...
        for l in range(0, 2): # m loop
            for k in range(0, 2): # spin loop
                ...
                for i in range(0,len(wrtest)):
                    wrc = self.rs.omega_real(aziin[l],altest[i],spinin[k],0)
                    wrt = wrtest[i]
                    wic = self.rs.omega_imag(aziin[l],altest[i],spinin[k])
                    wit = witest[i]
                    if (abs((wrc-wrt)/wrt)>tol or abs((wic-wit)/wit)>tol):
                        if (errplt):
                            ...
                            wrltest = np.zeros_like(altest)
                            for j,al in enumerate(altest):
                                wrltest[j] = self.rs.omega_real(aziin[l],al,spinin[k],0)
                            ...
                            wiltest = np.zeros_like(altest)
                            for j,al in enumerate(altest):
                                wiltest[j] = self.rs.omega_imag(aziin[l],al,spinin[k])
