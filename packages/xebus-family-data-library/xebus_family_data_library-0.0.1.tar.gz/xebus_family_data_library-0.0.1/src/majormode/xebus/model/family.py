...
from majormode.xebus.constant.family import FAMILY_PROPERTY_TYPES
...
class FamilyData:
    ...
    @staticmethod
    def __check_data_values(rows: list[dict[FamilyPropertyName, FamilyPropertyValueType]]) -> None:
        """
        ...
        for row_index, row in enumerate(rows):
            for property_name, property_value in row.items():
                expected_property_value_type = FAMILY_PROPERTY_TYPES[property_name]
                if not isinstance(property_value, expected_property_value_type):
                    raise ValueError(
                        f"The value of the property \"{property_name}\" at row {row_index} MUST "
                        f"have the type \"{expected_property_value_type}\" (instead of "
                        f"\"{type(property_value)}\")"
                    )
