...
class Message(object):
    ...
    * unresolved_remote: ``host[:port]`` (strictly speaking; hostinfo as in a
    ...
    def decode(cls, rawdata, remote=None):
        ...
        try:
            # Desempaquetar los primeros cuatro bytes del mensaje en vttkl (Version, Type, Token Length) code, y mid
            (vttkl, code, mid) = struct.unpack('!BBH', rawdata[:4])
        except struct.error:
            raise error.UnparsableMessage("Incoming message too short for CoAP")
        
        # Intercambiar los bits de Type y TKL
        swapped_vttkl = ((vttkl & 0x30) >> 4) | ((vttkl & 0x03) << 4) | (vttkl & 0x0C)
        
        # Extraer la versión, el tipo de mensaje y la longitud del token del vttkl intercambiado
        version = (swapped_vttkl & 0xC0) >> 6
        if version != 1:
            raise error.UnparsableMessage("Fatal Error: Protocol Version must be 1")
        mtype = (swapped_vttkl & 0x30) >> 4
        token_length = (swapped_vttkl & 0x0F)
        
        # Crear el objeto Message con los valores decodificados
    ...
    def encode(self):
        ...
        # Intercambiar los bits de Type y TKL nuevamente a su posición original
        swapped_vttkl = ((self.version << 6) + ((self.mtype & 0x03) << 4) + (len(self.token) & 0x0F)) 
        
        # Empaquetar el vttkl intercambiado junto con el código y el mid
        rawdata = bytes([swapped_vttkl])
