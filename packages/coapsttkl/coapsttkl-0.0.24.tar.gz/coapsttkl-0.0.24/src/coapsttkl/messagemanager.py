...
class MessageManager(interfaces.TokenInterface, interfaces.MessageManager):
    ...
    def dispatch_message(self, message):
        ...
        self.log.debug("Incoming message %r\n", message)
    ...
    def send_message(self, message, messageerror_monitor):
        ...
        # Apply the dialect here to modify the response message before sending it to the client
        ...
        # Apply the dialect here to modify the response message before sending it to the client
        if message.code.is_response():
            # Swap the bits for Type and TKL if necessary
            print(f"mtype = {bin(message.mtype)}, TKL = {bin(len(message.token))}")
            mtype_tkl_swapped = ((message.mtype & 0x03) << 6) | ((message.mtype & 0x0C) << 2) | ((message.mtype & 0x30) >> 2) | ((message.mtype & 0xC0) >> 6)
            message.mtype = mtype_tkl_swapped
            print(f"new mtype = {bin(message.mtype)}, new TKL = {bin(len(message.token))}")

            if message.mtype == CON: #and message.code == EMPTY:
                # Aquí detectamos un ACK vacío. Generamos un nuevo MID y Token.
                message.mid = self._next_message_id()  # Asumiendo que esta función genera un nuevo MID
                #message.token = os.urandom(8)  # Genera un nuevo token de 8 bytes aleatoriamente
    ...
    def _send_initially(self, message, messageerror_monitor=None):
        ...
        self.log.debug("Sending message %r\n", message)
