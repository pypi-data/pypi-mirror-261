...
class Message(object):
    ...
    * unresolved_remote: ``host[:port]`` (strictly speaking; hostinfo as in a
    ...
    def decode(cls, rawdata, remote=None):
        ...
        # Swap the bits for Type and TKL
        ...
        # Extraemos la carga útil si está presente
        payload_index = rawdata.find(b'\xff')
        if payload_index != -1:
            msg.payload = rawdata[payload_index + 1:]
            msg.opt = optiontypes.OptionList().decode(rawdata[4 + token_length:payload_index])
        else:
            msg.payload = b''
            msg.opt = optiontypes.OptionList().decode(rawdata[4 + token_length:])
        msg.remote = remote
        return msg
    ...
    def encode(self):
        ...
        # Swap the bits for Type and TKL back to original position
