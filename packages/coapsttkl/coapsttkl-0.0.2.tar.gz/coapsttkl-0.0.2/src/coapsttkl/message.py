...
class Message(object):
    ...
    * unresolved_remote: ``host[:port]`` (strictly speaking; hostinfo as in a
    ...
    def _encode(self):
        ...
        # Crea la cabecera
        header_bytes = bytes([
            (self.version << 6) + ((self.mtype & 0x03) << 4) + (len(self.token) & 0x0F)
        ]) + struct.pack('!BH', self.code, self.mid)

        # Convert bytes to integer for bitwise XOR
        header_int = int.from_bytes(header_bytes, byteorder='big')

        # Intercambia los bits de Type y TKL
        mask = 0b11000000
        new_header_int = header_int ^ mask

        # Convert integer back to bytes
        new_header_bytes = new_header_int.to_bytes(len(header_bytes), byteorder='big')

        # Codifica el resto del mensaje
        rawdata = new_header_bytes + self.token + self.opt.encode()
        if len(self.payload) > 0:
            rawdata += bytes([0xFF]) + self.payload
        return rawdata
