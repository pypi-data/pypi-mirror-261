...
def create_webcon_body(parameters, xml_b64, pdf_b64, xml_file_data, id, cui, template_xml_path, wfd_id_duplicate):
    ...
    if parameters['webcon_parent_wfdid'] is not None and parameters['webcon_parent_wfdid'] not in ['', 0]:
        body["parentInstanceId"] = parameters['webcon_parent_wfdid']
    
    if wfd_id_duplicate != 0 and parameters['duplicate_wfdid_field_guid'] is not None and parameters['duplicate_wfdid_field_guid'] not in ['', 0]:
        body["formFields"].append({"guid": parameters['duplicate_wfdid_field_guid'], "svalue": wfd_id_duplicate})
...
def send_to_WebCon(parameters, token , messages, xml_template_file_path):
    ...
    for message in messages:
        ...
        try:
            ...
            body = create_webcon_body(parameters, base64_string_xml, pdf_content, xml_text, invoice_id_element, company_id, xml_template_file_path, wfd_id_duplicate)
        ...
        for parent_node in parent_nodes:
            ...
            for column_path, column_name in columns:
                if '@' in column_path:
                    ...
                    value = child[0].text.strip() if child and child[0].text is not None else None
