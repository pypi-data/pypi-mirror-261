...
def send_to_WebCon(parameters, token , messages, xml_template_file_path):
    ...
    for message in messages:
        ...
        try:
            ...
            body = create_webcon_body(parameters, base64_string_xml, pdf_content, xml_text, invoice_id_element, company_id, xml_template_file_path, wfd_id_duplicate, namespaces)
...
def startRunning(json_file_path, xml_template_file_path):
...
# Function to convert received XPath to ElementTree-compatible format
def adjust_xpath(received_xpath, namespaces):
    adjusted_xpath = received_xpath
    for prefix, uri in namespaces.items():
        # Replace prefix with namespace URI in braces
        adjusted_xpath = adjusted_xpath.replace(f"{prefix}:", f"{{{uri}}}")

    # Check if the path starts with './' indicating it's relative to the root, and then remove the root element
    if adjusted_xpath.startswith("./"):
        adjusted_xpath = adjusted_xpath[2:]  # Remove './' to process the rest
    
    # Split the XPath by '/' and remove the first element (the root element)
    parts = adjusted_xpath.split('/')
    if len(parts) > 1:
        # Re-join the path without the first element
        adjusted_xpath = '/'.join(parts[1:])
    else:
        # If there's only one part after removing './', it means there was only the root specified, so return an empty string
        adjusted_xpath = ''
