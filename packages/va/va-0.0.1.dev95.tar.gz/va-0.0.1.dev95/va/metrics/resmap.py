...
def run_locres(mapone, maptwo, output_path, resmappy_path=None):
    ...
    try:
        ...
        if bindisplay:
            ...
            resmap_cmd = '{} --i {} --i2 {} --o {} --locres --locres_sampling 50'.format( respath, run_mapone_name, run_maptwo_name, locres_map)
            print(resmap_cmd)
        else:
            if resmappy_path == None:
                raise Exception('No ResMap.py path provided.')
            else:
                resmap_cmd = '{} --i {} --i2 {} --o {} --locres --locres_sampling 60'.format( resmappy_path, run_mapone_name, run_maptwo_name, locres_map)
                print(resmap_cmd)
