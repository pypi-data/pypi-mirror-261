...
if [[ $IS_COLAB -eq 0 ]] && ! command -v {cmd} >/dev/null; then
  ...
  exit 1
    ...
    build_prefix = "/build"  # All operations will be relative to this build directory.
...
if [[ -d "/content" ]]; then
  ...
  IS_COLAB=0
    ...
    script.append(f"mkdir -p .{build_prefix}/{model_name}")
...
if [[ $IS_COLAB -eq 0 ]]; then
    ...
    if [ ! -d ".{build_prefix}/{env_name}" ]; then
        $PYTHON_CMD -m venv .{build_prefix}/{env_name} || handle_error
    fi

    export VENV_PYTHON=.{build_prefix}/{env_name}/bin/python
fi
    """.strip()
...
if [[ "$OS" = "Darwin" ]] || [[ "$OS" = "Linux" ]] || [[ -d "/content" ]]; then
    ...
    if directories_map:
        for directory_path in directories_map.values():
            ...
            script.append(f"mkdir -p .{build_prefix}/{model_name}/{formatted_path}")
    ...
    for git in config.get("git", []):
        ...
        if from_source and to_destination and branch:
            repo_path = f".{build_prefix}/{model_name}/{to_destination.replace('/home/content/', '')}"
            git_clone = f"""
...
if [ -f {repo_path}/requirements.txt ]; then
    ...
    for item_type in ["dataset", "file"]:
        for item in config.get(item_type, []):
            ...
            if from_source and to_destination:
                destination_path = f".{build_prefix}/{model_name}/{to_destination.replace('/home/content/', '')}"
    ...
    for run in run_vec:
        ...
        if command and args:
            replaced_args = args.replace("/home/content/", f".{build_prefix}/{model_name}/")
