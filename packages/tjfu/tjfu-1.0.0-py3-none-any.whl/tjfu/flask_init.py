...
class FlaskInit:
    def __init__(
        ...
        self._jwt_access_token_expires = jwt_refresh_token_expires
        ...
        if jwt_secret_key is not None:
            self._app.config['JWT_SECRET_KEY'] = self._secret_key
            self._app.config['JWT_ACCESS_TOKEN_EXPIRES'] = timedelta(days=365)
            self._app.config['JWT_REFRESH_TOKEN_EXPIRES'] = timedelta(days=366)
