...
class Autolog:
    ...
    def _log_vectice_asset(self, asset: TVecticeObjects) -> None:
        if isinstance(asset["vectice_object"], Dataset):
            dataset = asset["vectice_object"]
            dataset_name = dataset.name if dataset.name else asset["variable"]
            self._iteration_service.log_dataset(asset["vectice_object"])
            _logger.info(f"Dataset {dataset_name!r} logged in iteration {self._iteration.index!r}.")
        elif isinstance(asset["vectice_object"], Table):
            table = asset["vectice_object"]
            table_name = table.name if table.name else asset["variable"]
            self._iteration_service.log_table(asset["vectice_object"])
            _logger.info(f"Table {table_name!r} logged in iteration {self._iteration.index!r}.")
        else:
            model = asset["vectice_object"]
            model_name = model.name if model.name else asset["variable"]
            self._iteration_service.log_model(asset["vectice_object"])
            _logger.info(f"Model {model_name!r} logged in iteration {self._iteration.index!r}.")
