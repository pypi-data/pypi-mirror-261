...
class BotBase(ABC):
    ...
    # @classmethod
    # def _get_all_local_bot_filenames(cls) -> list[Path]:
    #     suffix = ".yaml"
    #     files = []
    ...
    def _get_all_resources_bot_filenames(cls) -> list[Path]:
        ...
        resources_folder = Path(
            importlib.resources.files("bots")._paths[0]
        )  # TODO: HACK: test _paths[0] across Python versions.
    ...
    def _get_all_valid_bot_names(cls, include_system: bool = False) -> list[str]:
        ...
        # all_bot_filenames = cls._get_all_local_bot_filenames()
    ...
    def _get_bot_yaml_file(cls, bot_folder: str, bot_name: str) -> Path:
        ...
        raise AppUsageException(
            ...
            f"\nCheck the bot name, or try one of: {cls._get_all_valid_bot_names()}"
            "\n"
        )
