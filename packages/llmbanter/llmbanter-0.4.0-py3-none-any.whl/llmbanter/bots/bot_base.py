...
class BotBase(ABC):
    ...
    @classmethod
    def _wip_get_yaml_file(cls, bot_folder: str, bot_name: str):
        from llmbanter.library import consts

        if len(bot_name.split("/")) == 2:
            # Format: bot_type/bot_name
            bot_path = bot_name.split("/")[0]
            bot_name = bot_name.split("/")[1]
            local_path = Path(bot_folder) / bot_path / f"{bot_name}.yaml"
            resources_file = Path(importlib.resources.files("bots").joinpath(f"{bot_path}/{bot_name}.yaml"))
            print(f"XXX {local_path=}")
            print(f"XXX {resources_file=}")
        elif len(bot_name.split("/")) == 1:
            # Format: bot_name
            local_path = Path(bot_folder) / f"{bot_name}.yaml"
            resources_file = Path(importlib.resources.files("bots").joinpath(f"{bot_name}.yaml"))
            print(f"YYY {local_path=}")
            print(f"YYY {resources_file=}")
        else:
        ...
        if local_path.is_file():
            ...
            print(f"Using local yaml file: {local_path}")
            return local_path
        elif resources_file.is_file():
            logger.info(f"Using resources yaml file: {resources_file}")
            print(f"Using resources yaml file: {resources_file}")
        ...
        # raise Exception(
        #     f"Could not find bot yaml file corresponding to bot name '{bot_name}' either locally or in the {consts.package_name} resources folder. "
    ...
    def get_bot(cls, bot_name: str, bot_folder: str = "./bots") -> "BotBase":
        # print(f"XXX {bot_name=}")
        # file_path = None
        file_path = cls._wip_get_yaml_file(bot_folder, bot_name)
        bot_filename = file_path.name.replace(file_path.suffix, "")
        print(f"ZZZ {bot_filename=}")
        assert file_path.exists(), f"Bot file {file_path} does not exist as expected."
        # assert False, f"{bot_name=}, {file_path=}"

        try:
            # if len(bot_name.split("/")) == 2:
            #     bot_name_part0 = bot_name.split("/")[0]
            #     bot_name_part1 = bot_name.split("/")[1]
            #     bot_filename = bot_name_part1
            #     file_path = Path(bot_folder) / bot_name_part0 / f"{bot_name_part1}.yaml"
            # elif len(bot_name.split("/")) == 1:
            #     file_path = Path(bot_folder) / f"{bot_name}.yaml"
            #     bot_filename = bot_name
            # else:
            #     raise AppUsageException(
            #         f"Bot name should be in the format 'bot_type/bot_name' e.g. 'evangelist/instagram'. You specified bot name of '{bot_name}'."
            #     )
