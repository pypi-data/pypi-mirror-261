...
CSV_FIELD_NAME_SIS_ACCOUNT_ID = 'SIS ID'
...
CSV_FIELD_NAMES = {
    CSV_FIELD_NAME_SIS_ACCOUNT_ID: True,
...
def __build_card_image_default_file_name(id_card):
    ...
    return __clean_file_name(file_name)
...
def __build_card_image_formatted_file_name(id_card, file_name_format):
    ...
    field_name_attribute_mapping = {
        CSV_FIELD_NAME_SIS_ACCOUNT_ID.lower(): id_card.account_id,
...
def __clean_file_name(file_name: str) -> str:
    cleansed_file_name = ''.join([
        c if c.isalnum() else '-'
        for c in file_name
    ])
    return cleansed_file_name
...
def generate_id_card_image(
        ...
        header_image: Image,
        font_name: str,
        font_path: str or PathLike = None,
        padding: int = 20
):
    """
    ...
    """
    logging.debug(f"Generating the image of ID card for {id_card.full_name} ({id_card.account_id})...")
    ...
    qrcode_value = id_card.account_id
    ...
    if multiline_required:
        x = math.ceil((text_area_width - get_text_size(font, id_card.first_name)[0]) / 2) + padding
        image_draw.text((x, y), id_card.first_name, font=font, fill='black')

        x = math.ceil((text_area_width - get_text_size(font, id_card.last_name)[0]) / 2) + padding
...
def get_text_size(font, text: str) -> (int, int):
...
def read_id_cards_info_csv_file(
        csv_file_path_name,
        card_type: CardType or None = None,
        delimiter_character: str = DEFAULT_CSV_DELIMITER_CHARACTER,
        escape_character: str = DEFAULT_CSV_ESCAPE_CHARACTER,
        quote_character: str = DEFAULT_CSV_QUOTE_CHARACTER
) -> list[IdCardInfo]:
    """
    ...
    with open(csv_file_path_name, 'rt') as fd:
        ...
        id_cards = [
            IdCardInfo(
                row[CSV_FIELD_NAME_SIS_ACCOUNT_ID],
                row[CSV_FIELD_NAME_FULL_NAME],
                card_type=row.get(CSV_FIELD_NAME_CART_TYPE) or card_type,
