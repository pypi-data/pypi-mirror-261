...
def get_client_id(in_europe: bool, using_api_key: bool):
    if using_api_key:
        # TODO: Replace with real values after deployment.
        if in_europe:
...
def login():
    ...
    if not os.path.exists(PRL_PATH):
        ...
        os.chmod(PRL_PATH, mode=777)

    with open(CREDS_PATH, "w") as f:
        os.chmod(CREDS_PATH, mode=777)
        f.write(auth_json)
...
def get_auth_token():
    ...
    if "access_expiry" not in auth_dict or time.time() > auth_dict["access_expiry"]:
        ...
        if not os.path.exists(PRL_PATH):
            os.makedirs(PRL_PATH, mode=777)

        print(os.stat(CREDS_PATH), os.stat(PRL_PATH))
