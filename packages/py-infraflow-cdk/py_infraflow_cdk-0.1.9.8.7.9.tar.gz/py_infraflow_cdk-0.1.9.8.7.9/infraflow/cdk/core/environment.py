...
class Env(IEnv):
    ...
    def _get_endpoints(
        ...
        #     "Values": ["Gateway"],
        #     # str(endpoint_type).lower().capitalize()], #this needs to either be "Gateway" or "Interface"
        # },
        # {
        #     "Name": "service-name",
    ...
    def interface_endpoints(self, service_names: Optional[list[str]] = None) -> dict[str, InterfaceVpcEndpoint]:
        ...
        for ep in endpoints:
            ...
            if ep.get("VpcEndpointType") == "Interface":
                print(f"**** IS Interface Endpoint")
                endpoint_types[ep.get("ServiceName")].append(InterfaceVpcEndpoint.from_interface_vpc_endpoint_attributes(
