...
class AutoTask(DtComponent, HTTPService):
    ...
    def filter_ids(self, id_field: str, items: pd.DataFrame, chunk_size=None):
        data = items[id_field].dropna().unique().astype(int)

        if chunk_size:
            split_n = math.ceil(len(data) / chunk_size)
    ...
    async def get_dataframe_from_entity(self, zone, entity, payload, id_column_name):
        args = {
        ...
        while True:
            ...
            if error:
                self._logger.error(f"{__name__}: Error getting {entity}")
                raise ComponentError(f"{__name__}: Error getting {entity}") from error
