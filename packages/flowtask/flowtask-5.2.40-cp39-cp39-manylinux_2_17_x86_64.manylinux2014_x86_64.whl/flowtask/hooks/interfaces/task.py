...
from flowtask.exceptions import FlowTaskError, TaskFailed
...
class TaskSupport:
    def __init__(self, task: str, *args, **kwargs):
        ...
        self.logger = logging.getLogger(f"Task.{self.program}.{self.task}")
        self._args = args
        self._kwargs = kwargs
        ...
        try:
            if self._no_worker is False:
                self.wrapper = TaskWrapper(
                    program=self.program, task=self.task, **self._kwargs
                )
            else:
                self.task = Task(task=self.task, program=self.program, debug=True)
        except Exception as exc:
