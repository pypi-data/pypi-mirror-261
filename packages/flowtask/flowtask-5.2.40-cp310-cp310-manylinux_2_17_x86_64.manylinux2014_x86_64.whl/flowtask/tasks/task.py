...
from flowtask.models import TaskState, setTaskState
...
class Task(AbstractTask):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.debug(f"Using Events: {self._no_events}")
        ...
        self.logger.debug(f"CURRENTLY NEW CONDS: {self._conditions}")
        self.worker = worker
        if kwargs:
            # remain args go to kwargs:
            self._kwargs = {**kwargs}
        # add also the Log execution for InfluxDB
        self.event_defaults(LogExecution(disable_notification=self._no_notify))
        ## set the Task State:
    ...
    def get_component(self, step, prev):
        ...
        try:
            comp = component(
                job=depends, loop=self._loop, stat=stat, **params  # stats object
            )
    ...
    async def run(self):
        ...
        for step in self._pile:
            self.logger.debug(f"Step: {step.name}, Task: {self.task_id}")
            step_name = step.name
