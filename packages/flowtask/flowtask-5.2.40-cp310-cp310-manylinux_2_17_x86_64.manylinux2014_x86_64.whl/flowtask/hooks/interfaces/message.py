...
from notify.models import Actor, Chat, Channel, TeamsChannel, TeamsCard
...
from .masks import MaskInterface


class Notification(MaskInterface):
    def __init__(self, *args, **kwargs):
        super(Notification, self).__init__(*args, **kwargs)
        self.provider: str = kwargs.pop("type", "dummy")
        self.list_attachment: list = []
        self.account: str = self.set_account(kwargs.pop("account", {}))
    ...
    def get_notify(self, **kwargs):
        if self.provider == "telegram":
            recipient = Chat(**{"chat_id": EVENT_CHAT_ID, "chat_name": self._channel})
            ...
            if not self._recipient:
                ...
                recipient = [Actor(**user) for user in self._recipient]
