...
class LineageController:
    ...
    def get_matching_schemas_from_matching_tables(self,
                                                  matching_tables: List[Tuple[Table, Table]]
                                                  ) -> List[Tuple[Schema, Schema]]:
        # Schema names do not have to match for their to be a relationship, if tables match between schemas and both
        # schemas are passed in through the selectors then schema relationship should exist as well
        schema_matches: List[Tuple[Schema, Schema]] = []
        schema_id_matches: List[Tuple[int, int]] = []
        # Get all schema relationships based on matching tables
        for mt in matching_tables:
            schema_id_pair = (mt[0].schema_id, mt[1].schema_id)
            if schema_id_pair not in schema_id_matches:
                schema_id_matches.append(schema_id_pair)
                up_schema = [sc for sc in self.client.get_schemas(warehouse_id=[mt[0].warehouse_id]).schemas
                             if sc.id == mt[0].schema_id][0]
                down_schema = [sc for sc in self.client.get_schemas(warehouse_id=[mt[1].warehouse_id]).schemas
                               if sc.id == mt[1].schema_id][0]
                schema_matches.append((up_schema, down_schema))
    ...
    def infer_column_level_lineage_from_tables(self, tables: List[Tuple[Table, Table]]):
        # Create edges for all schemas
        matching_schemas = self.get_matching_schemas_from_matching_tables(
            tables
        )
        for sm in matching_schemas:
            up_schema = sm[0]
            down_schema = sm[1]
            try:
                self.create_edges(
                    up_schema, down_schema, DataNodeType.DATA_NODE_TYPE_SCHEMA
                )
            except Exception as e:
                log.warning(
                    f"Failed to create relationship between upstream schema: {up_schema.name} and "
                    f"downstream schema: {down_schema.name}. Exception {e}"
                )
        ...
        for match in track(tables, description="Creating relationships..."):
            upstream = match[0]
            downstream = match[1]
            try:
                self.create_edges(
                    upstream, downstream, DataNodeType.DATA_NODE_TYPE_TABLE
                )
