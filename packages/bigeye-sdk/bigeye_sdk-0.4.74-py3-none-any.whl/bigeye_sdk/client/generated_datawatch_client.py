...
class GeneratedDatawatchClient(abc.ABC):
    ...
    def create_lineage_edge(self,
        ...
        # If edge already exists, the call fails - catch and warn on this error instead
        try:
            response = self._call_datawatch(Method.POST, url=url, body=request.to_json())
            return LineageEdgeV2().from_dict(response)
        except Exception as e:
            exists_err = f"Edge from ({request.upstream_data_node_id}, {request.downstream_data_node_id}) already exists"
            if exists_err not in str(e):
                raise e
            else:
                log.warning(exists_err)
