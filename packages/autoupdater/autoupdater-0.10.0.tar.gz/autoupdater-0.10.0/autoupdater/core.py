...
def ensure_digest_installed(venv: Venv, target_digest: bytes) -> None:
    ...
    with open(venv.spec.requirements_file, "rb") as f:
        requirements_to_install = [line for line in f.read().split(b"\n") if line]
    installed_requirements = [line for line in pip_freeze.stdout.split(b"\n") if line]
    if pip_freeze.stdout:
        requirements_to_uninstall = [
            l for l in installed_requirements if l not in requirements_to_install
        ]
        with tempfile.TemporaryDirectory() as tmp_dir:
            requirements_file = path.join(tmp_dir, "requirements.txt")
            with open(requirements_file, "wb") as f:
                f.write(b"\n".join(requirements_to_uninstall))
            subprocess.run(
                [
    ...
    requirements_to_install = [
        line for line in requirements_to_install if line not in installed_requirements
    ]
