...
def ensure_digest_installed(venv: Venv, target_digest: bytes) -> None:
    ...
    with open(venv.spec.requirements_file, "r") as f:
        target_requirements = [line.strip() for line in f.readlines() if line.strip()]
    ...
    installed_requirements = [
        line.strip() for line in pip_freeze.stdout.split("\n") if line.strip()
    ]

    requirements_to_remove = [
        line for line in installed_requirements if line not in target_requirements
    ]

    requirements_to_install = [
        line for line in target_requirements if line not in installed_requirements
    ]
