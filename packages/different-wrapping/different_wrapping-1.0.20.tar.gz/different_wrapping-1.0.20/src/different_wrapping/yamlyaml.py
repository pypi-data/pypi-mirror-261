...
def build_kustomize_manifest(challenges, folder):
    ...
    manifest = {
        "apiVersion": "kustomize.config.k8s.io/v1beta1",
        "kind": "Kustomization",
        "resources": ["./%s" % challenge.relative_path() for challenge in filter(lambda challenge: challenge.has_containers and challenge.has_static_challenges(), challenges)],
    }
...
def generate_challenge_ingresses(challenge, outdir, args):
    ...
    for service_name, service in challenge.containers.items():
        ...
        if service.has_label("no.cyberlandslaget.http"):
            ...
            if args.ingress_type == "ingress":
                raise RuntimeError("Ingress type ingress not supported with TCP challenges")
            else:
                ingresses = ingresses + generate_tcp_gateway(service_name, service, challenge, args)
        else:
