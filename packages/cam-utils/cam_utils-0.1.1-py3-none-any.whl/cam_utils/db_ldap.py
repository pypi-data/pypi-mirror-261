...
class DBLdap:
    ...
    def __init__(self, ipaddress: str, port: int, scheme: bool, user: str, secret: str):
        ...
        if (ValidaInfo().ipv4address(ipaddress) == False):
        ...
        if (ValidaInfo().portaTCP(port) == True):
            self.__port = port
        else:
            self.__port = 389
    ...
    def search_dn_objects(self, base_dn, search_filter):
        try:
            ...
            if (ValidaInfo().texto(base_dn) == False):
                raise ValueError(f"{self.__libtag} BASE DN Não válido {base_dn}.")
        
            self.__search_filter = search_filter
            self.__base_dn = base_dn
            search_results = self.__connection.search_s(base_dn, ldap.SCOPE_SUBTREE, search_filter)
