...
class DBRabbitMQ: 
    ...
    def __init__(self, host: str, port: str, user: str, secret: str, virtual: str):
        ...
        if (ValidaInfo().ipv4address(host) == True):
            self.__ipaddress = host
        elif (ValidaInfo().texto(host) == True):
            self.__ipaddress = host
        else:
            raise ValueError(f"{self.__libtag} Endereço Informado não é hostname ou endereço IP  {host}.")
        ...
        if (ValidaInfo().texto(virtual) == False):
    ...
    def publish(self, queue: str , info: json):
        try:
            ...
            print(f"{self.__libtag} Publish - Validando processo  Abrindo Canal {queue}")
            ...
            if not self.isOpenChan():
                self.openChanPublish(queue)
                if not self.isOpenChan():
                    raise ValueError(f"{self.__libtag} Chan Publish Não foi aberto {queue}.")
    ...
    def consume(self, queue: str):
        try:
            ...
            print(f"{self.__libtag} Consumo - Validando processo  Abrindo Canal {queue}")

            if self.__queueConsume != queue: 
                print(f"{self.__libtag} Consumo - Fila nao definida - Abrindo Canal {queue}")
                self.openChanConsume(queue)
            
            if self.isOpenChan() == False:
                self.openChanConsume(queue)
                if self.isOpenChan() == False:
                    raise ValueError(f"{self.__libtag} Chan Consumo Não foi aberto {queue}.")
            ...
            self.__channel.basic_consume( queue = self.__queueConsume
...
def callbackConsume(channel: BlockingChannel, method_frame: Basic.Deliver , header_frame : BasicProperties, body: bytes, rabbitMQ : DBRabbitMQ):
    try:
