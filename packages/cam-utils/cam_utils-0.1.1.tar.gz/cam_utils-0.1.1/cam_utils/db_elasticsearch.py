from lib.valida_info import ValidaInfo
...
class DBElasticSearch:
    ...
    def __init__(self, url: str, port: str, user: str, secret: str, security: str):
        ...
        if (ValidaInfo().url(url) == True):
            self.__url = url
        elif (ValidaInfo().ipv4address(url) == True):
            self.__url = url
        else:
            raise ValueError(f"{self.__libtag} Endereço Informado não é URL ou endereço IP  {url}.")
        
        if (port is None or port == "" or port == "0"):
            self.__port = 0
        elif (ValidaInfo().portaTCP(port) == True):
            self.__port = int(port)
        else:
            raise ValueError(f"{self.__libtag} Porta TCP nao válida {user}.")
        
        if (ValidaInfo().texto(user) == False):
            raise ValueError(f"{self.__libtag} Username nao valido {user}.")
        
        if (ValidaInfo().texto(secret) == False):
        ...
        try:
            ...
            if (self.__scheme == "http"):
                self.__connection = Elasticsearch(self.__url , http_auth=(self.__username, self.__password) )
            else:
                self.__connection = Elasticsearch(self.__url, connection_class=RequestsHttpConnection, http_auth=(self.__username, self.__password), use_ssl=True, verify_certs=False, ssl_show_warn=False)
    ...
    def update_alldocuments(self, indexname, update_field, new_value):
        try:
            ...
            documents = scan(self.__connection, index=indexname, query={"query": {"match_all": {}}})
    ...
    def write_document(self, indexname: str, doc_id: str, doc_body: dict):
        try:
            ...
            if (ValidaInfo().texto(doc_id) == False):
            ...
            err = str(e) 
            self.__resmsg = f"{self.__libtag} INSERT index error: {err} {indexname} {doc_id}"
            print (self.__resmsg)
            return False
    ...
    def get_document(self, indexname: str, doc_id: str):
        try:
            ...
            err = str(e) 
            self.__resmsg = f"{self.__libtag} GetDocument index error: {err} {indexname} {doc_id}"
            print (self.__resmsg)
            return None
    ...
    def search_document(self, indexname: str, query: dict):
        try:
            ...
            if (ValidaInfo().keyExists(query, 'query') == False):
    ...
    def delete_document(self, indexname: str, query: dict):
        try:
            ...
            # Search documents based on the query
            deleteResult = self.__connection.delete_by_query(index=indexname, body=query)
            return deleteResult
    ...
    def bulk_documents(self, indexname: str, documents):
        try:
            ...
            if (ValidaInfo().is_list(documents) == False):
                raise ValueError(f"{self.__libtag} DOCUMENTS Não é Lista :: Registros: {len(documents)}.")
