...
Don't forget to change this folder to your profile folder.  
...
Then, we make sure that copy-to reads from the correct file by rerunning `copy-to set-git file confs.json` (path structure different for different systems), then we run:  
