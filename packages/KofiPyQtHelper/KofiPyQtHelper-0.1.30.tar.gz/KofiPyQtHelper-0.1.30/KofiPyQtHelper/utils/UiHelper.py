...
class UiHelper(Command, LayoutHelper, ComponentHelper):
    ...
    def init(self, parent: QWidget, infos):
        for info in infos:
            ...
            if ("content" in info or "contentJson" in info) and box is not None:
                if "content" in info:
                    content = info["content"]
                elif "contentJson" in info:
                    path = os.path.abspath(
                        "./config/interface/{0}/{1}.json".format(
                            info["contentJson"]["category"], info["contentJson"]["name"]
                        )
                    )
                    content, _, _ = self.get_layout_data(path)
