        ...
        if container.image.id == image_id:
            ...
            if port_data:
                for port, bindings in port_data.items():
                    ports.add(port.split('/')[0])  # get only the port number, not the protocol
    return ', '.join(sorted(ports)), err_msg
...
def list_local_images() -> PrettyTable:
    ...
    id_width = max(len(image.id) for image in images) // 2 + 1  # divide by 2 for two lines
    name_width = max(len(image.tags[0].split(':')[0]) if image.tags else 0 for image in images)
    tags_width = (terminal_width - id_width - name_width - 7) // 2  # divide by 2 to leave space for the Ports column
    ports_width = tags_width
    table.align["Image ID"] = "l"
    table.align["Image Name"] = "l"
    table.align["Tags"] = "l"
    table.align["Ports"] = "l"
    table._max_width = {"Image ID": id_width, "Image Name": name_width, "Tags": tags_width, "Ports": ports_width}
...
def compress_image(image_name: str, project_name: str):
    ...
    print_text("Start compressing the tar image into gzip ..")
