...
def enrich_compose(
    ...
    base_domain: str = prompt_user(
        f"Indicate the base domain for your app {app_name}?\n"
    ...
    msg += "\n\n[yellow]We will tag services by their usage[/yellow]: frontend, database, api...\n"
    ...
    for service_name in services:
        ...
        while prompt_done is False:
            type_nbr:str = prompt_user(f"Choose a type number for {service_name}",
                default=stype)
            prompt_done = True
            match type_nbr:
                case "1":
                ...
                case _:
                    ...
                    if type_nbr == "":
                        ...
                        prompt_done = True
                    elif type_nbr.upper() in SERVICE_TYPES:
                        stype = type_nbr.upper()
                        prompt_done = True
                    else:
                        prompt_done = False
        ...
        msg = f"Public URL for service [yellow]{service_name}[/yellow] "
        msg += "- leave empty if none"
        service_url: str = prompt_user(msg)
...
def check_for_enrichment(compose_path:str
    ) -> (Path, dict, dict):
    ...
    if docker_compose_path.is_dir():
        ...
        logging.debug("exiting as provider docker compose path is wrong")
        sys.exit(4)
    ...
    if selected_path.is_file():
        ...
        logging.debug("exiting as no docker compose file found")
        sys.exit(2)

    return None, None, {}


def upload_images(compose_path: Path):
    """Upload the enriched compose file to the Celestical Cloud."""

    cli_panel("Let's start uploading your stack to Celestical")

    compose:dict = read_docker_compose(fullpath=compose_path)
    ...
    for service_name in compose["services"]:
        ...
        image_name = compose["services"][service_name]["image"].split(':')[0]
        #typer.echo(image_name)
        #typer.confirm("Do you wish to proceed compressing?")
        compress_image(image_name=image_name, project_name=compose["celestical"]["name"])
...
def upload_compose(compose_path:str, call_nbr:int = 1) -> dict:
    ...
    if user_data is not None and isinstance(user_data, dict):
        ...
        apiconf.api_key['Authorization'] = \
          user_data.get("access_token", "")
        apiconf.api_key_prefix['Authorization'] = \
        ...
        apiconf.access_token = user_data.get("access_token", "")
    else:
    ...
    with api.ApiClient(apiconf) as api_client:
        ...
        try:
            ...
            if call_nbr > 1:
                ...
                return None
            # else
            cli_panel("You have to login again, Your token expired.")
            ...
            logging.error("Error during post of enriched compose file")
            logging.error(oops)
            return None
