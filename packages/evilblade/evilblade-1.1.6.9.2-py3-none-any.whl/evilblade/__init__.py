...
def cyccat(your_exp, offset_you_know, overflow_key, debug=0):
    """
    ...
    offset_you_know : the offset can make overflow you know
    ...
    while min_range <= max_range:
        ...
        if overflow_key == 0:
            condition_met = "Got EOF" in out
        else:
            condition_met = overflow_key in out
        ...
        if debug:
            print("{YELLOW}----------------------{END}")
            print("{YELLOW}|{END}    debug info       {YELLOW}|{END}")
            print("{YELLOW}----------------------{END}")
            print(f"[!] debug output: {out}")

            if condition_met : print(f"[!] too large")
            else : print(f"[!] too small")
            print(f"[!] min_range: {min_range}, max_range: {max_range}")
    ...
    if min_range-1 == offset_you_know:
