...
class Upstox(Broker):
    ...
    def __init__(
        ...
        self.configuration.access_token = self.access_token
        if load_instrument:
            self.load_instrument("upstox_instruments")
    ...
    def get_ltp(self, exchange=None, tradingsymbol=None):
        ...
        try:
            ...
            api_request = f"{self.get_exchange(symbol_info['exchange'])}|{symbol_info['exchange_token']}"
            api_response = api_instance.ltp(api_request, api_version)
    ...
    def account_summary(self):
        ...
        response = {
            "margin_used": float(margins.data["equity"].used_margin),
            "margin_available": float(margins.data["equity"].available_margin),
