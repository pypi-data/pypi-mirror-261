...
import shutil
from snippets import jdump_lines, jload_lines, log_cost_time, read2list, jdump, jload, jdumps, deep_update
...
from xagents.model.api import EMBD, get_embd_model, get_rerank_model
...
class KnowledgeBase:
    ...
    def update_info(self, config:dict):
        logger.debug(f"update kb:{self.name} with config:{jdumps(config)}")
        origin_config =self.get_info()
        deep_update(origin_config, config)
        
        
        jdump(origin_config, self.config_path)
