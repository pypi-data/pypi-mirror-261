...
The main advantages you will find when using autolms are the following:
...
```
pip install git+https://github.com/lenguajenatural-ai/autolms.git
```
...
```
git clone https://github.com/lenguajenatural-ai/autolms.git
cd autolms
pip install -e .
```
...
You can run tests after installing the library with `pytest`. It is already installed when installing `autolms`. Inside the main `autolms` repository directory (where README is), run:
...
Here is a list of the tasks supported by `autolms`.
...
```python
from autolms import ModelConfig
...
from autolms import DatasetConfig
...
```python
from transformers import EarlyStoppingCallback
from autolms import DatasetConfig, joinpaths
...
AutoTrainer is the main class in `autolms`, but is almost purely configured via lists of `DatasetConfig` and `ModelConfig`. The full configuration of AutoTrainer, given that you already have a `DatasetConfig` and a `ModelConfig`, would be the following:

```python
from autolms import AutoTrainer
...
from autolms.augmentation import NLPAugPipeline, NLPAugConfig
...
```python
from autolms import DatasetConfig, ModelConfig, AutoTrainer
from autolms.augmentation import NLPAugConfig
from autolms.default_param_spaces import hp_space_base
