...
def put_aggregation(client, project_id, aggregation_file):
    ...
    handle_aggregation_upload(client, project_id, aggregation_file.name, agg_meta, imex_map)
...
def handle_aggregation_upload(client, project_id, filename, agg_meta, imex_map):
    ...
    if is_edition(agg_meta):
        ...
        tguri = upload_file(client, project_id, work_path, imex_map)
    ...
    for ore_aggregates in agg_xml_root.findall(".//ore:aggregates", NAMESPACES):
        ...
        meta = metafile_to_object(data_path)

        if is_aggregation(meta):
            tguri = handle_aggregation_upload(client, project_id, data_path, meta, imex_map)
        else:
...
def metafile_to_object(filename) -> TextgridObject:
...
def upload_file(client: TGclient, project_id: str, data_path: PurePath, imex_map: dict) -> str:
