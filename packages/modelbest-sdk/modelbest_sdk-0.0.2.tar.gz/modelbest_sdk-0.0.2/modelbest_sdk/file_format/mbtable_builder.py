...
from modelbest_sdk.file_format.mbtable import MAX_KEY, MAX_KEY_LEN, ByteArray
...
class MbTableBuilder:
    ...
    def push(self, value):
        if self.system_key >= MAX_KEY:
            raise Exception("Key has reached its maximum value.")
        formatted_key = str(self.system_key).zfill(MAX_KEY_LEN)
        self.add(formatted_key, value)
        self.system_key += 1
...
if __name__ == '__main__':
    ...
    builder = MbTableBuilder('test/data/test_no_key.mbt', 'kZlib')
    builder.push('value1')
    builder.push('value2')
    builder.push('value3')
    builder.add_metadata('meta1', 'value1')
