...
It use system key (padding with leading zeros up to MAX_KEY_LENGTH) by default.
'''
...
class MbTablePartitionIterator:
    def __init__(self, mbtable_partition: MbTablePartition, start_key=0, max_iterations=None):
        # TODO: add support for system key
        self.mbtable_partition = mbtable_partition
        self.start_key = start_key
        self.max_iterations = max_iterations
        self.iterations_count = 0
        assert start_key >= 0 and start_key <= mbtable_partition.get_total_count()
        self.current_file_index, self.current_pos = mbtable_partition.get_file_index_and_position(start_key)
        self.iterator = None
    ...
    def __next__(self):
        ...
        try:
            _, value = self.iterator.__next__()
...
if __name__ == '__main__':
    ...
    with MbTablePartitionIterator(mbtable_partition, start_key=9, max_iterations=11) as iterator:
        for row in iterator:
            print(f"row: {row}")