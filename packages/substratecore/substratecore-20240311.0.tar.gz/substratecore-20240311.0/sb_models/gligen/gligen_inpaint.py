...
BASE_ID = "anhnct/Gligen_Inpainting_Text_Image"
CACHE_DIR = "/cache"
OUT_PATH = "/root/outs"
GPU_TYPE = gpu.A100()
MODEL_FOLDER = "gligen"
...
stub = Stub("gligen_inpaint", image=image)
...
class GLIGEN:
    ...
    def generate(
        ...
        input_image = load_image(image_uri)
        # gligen_images = [load_image(uri) if uri != "placeholder" else uri for uri in object_image_prompts]
        ...
        images = pipe(
            ...
            # gligen_images=gligen_images, # NOTE: not supporting image-prompts
...
async def local() -> None:
    ...
    generation = sd.generate.remote(
        ...
        image_uri=room_uri,
        object_text_prompts=["red backpack"],
        object_bboxes=[[0.2676, 0.4088, 0.4773, 0.7183]],
        # guidance_scale=7.5,
        # store="hosted",
        # seed=88888,
        negative_prompt="blur, lowres, bad anatomy, bad hands, cropped, worst quality",
    )
    output_path = os.path.expanduser(f"~/Downloads/gligen_inpaint-{int(time.time())}.jpg")
