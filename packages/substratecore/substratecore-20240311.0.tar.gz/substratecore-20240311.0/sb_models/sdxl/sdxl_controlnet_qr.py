...
CONTROLNET_MODELIDS = {
    "qrcode_monster": "monster-labs/control_v1p_sdxl_qrcode_monster",
}
...
stub = Stub("sdxl_controlnet_qr", image=image)
...
class SDXL:
    def __init__(self):
        t0 = time.time()
        self.pipe = load_pipe("qrcode_monster")
        t1 = time.time()
    ...
    def generate(
        ...
        control_image = load_image(image_uri, req=req)
        control_image = resize_to_resolution(control_image, output_resolution)
...
async def local() -> None:
    ...
    output_path = os.path.expanduser(f"~/Downloads/sdxl_controlnet_qr-{int(time.time())}.jpg")
