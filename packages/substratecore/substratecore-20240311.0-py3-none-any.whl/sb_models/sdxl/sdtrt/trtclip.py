...
class TRTClip:
    ...
    def load(self):
        ...
        self._tensors["input_ids"] = torch.empty((1, 77), dtype=torch.int32, device="cuda")
        text_embeddings_dims = (1, self._projection_dims) if self._is_clip2 else (1, 77, self._projection_dims)
        self._tensors["text_embeddings"] = torch.empty(text_embeddings_dims, dtype=torch.float, device="cuda")
        self._tensors["hidden_states"] = torch.empty((1, 77, self._projection_dims), dtype=torch.float, device="cuda")
    ...
    def __call__(self, *args, **kwargs):
        ...
        hf_model_hidden_states = [torch.empty((1, 77, self._projection_dims), dtype=torch.float, device="cuda")] * 13
