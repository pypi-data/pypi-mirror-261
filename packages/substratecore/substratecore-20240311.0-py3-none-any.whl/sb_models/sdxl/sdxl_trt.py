...
ENGINE_DIR = "engine-sdxl"
REFINER_ENGINE_DIR = "engine-sdxl-refiner"
...
class SDXLTRT:
    @build()
    def build(self):
        snapshot_download(repo_id="substrate-labs/sdxl-trt-9.3.0.post12.dev1", local_dir=CACHE_DIR)
        ...
        self.refiner.text_encoder_2 = _wrap_in_trtclip2(self.refiner.text_encoder_2, REFINER_ENGINE_DIR, cuda_stream)
        self.refiner.unet = _wrap_in_tunet(self.refiner.unet, REFINER_ENGINE_DIR, cuda_stream)
    ...
    def load(self):
        ...
        start_time = time.time()
        self.pipe(prompt="don quixote", num_inference_steps=20)
        end_time = time.time() - start_time
...
def sample(prompt: str = "don quixote"):
    img = SDXLTRT().generate.remote({"prompt": prompt, "num_inference_steps": 30})
