...
stub = Stub("sdxl_inpaint", image=image)
assets_nfs = NetworkFileSystem.persisted("assets-nfs")


def load_pipe(inpaint: bool = False):
    import torch
...
class SDXL:
    def __init__(
        ...
        t0 = time.time()
        self.pipes = {}
        self.pipes["img2img"] = load_pipe(inpaint=False)
        self.pipes["inpaint"] = load_pipe(inpaint=True)
        t1 = time.time()
    ...
    def generate(
        ...
        pipe = self.pipes["img2img"] if mask_image_uri is None else self.pipes["inpaint"]
        if mask_image_uri:
...
async def local() -> None:
    ...
    output_path = os.path.expanduser(f"~/Downloads/sdxl_inpaint_output-{int(time.time())}.jpg")
