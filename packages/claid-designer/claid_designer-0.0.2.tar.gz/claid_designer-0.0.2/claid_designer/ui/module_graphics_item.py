...
class ModuleGraphicsItem(QGraphicsObject):
    ...
    def showProperties(self):
        ...
        while(len(self.property_names) > 0 and len(empty_properties) != 0):

            property_dialog = PropertyInputDialog(self.property_names, self.property_descriptions, self.property_hints, defined_properties=self.properties)
            property_dialog.exec_()

            if(property_dialog.cancelled()):
                return
            
            defined_properties = property_dialog.get_defined_properties()
            empty_properties = property_dialog.get_empty_properties()
    ...
    def repaint(self, painter):
        ...
        #TODO: Need to check ModuleAnnotations if module is Java/C++/... and then draw the text accordingly
        # Draw smaller rectangle at the top for the module_type text
        x = rect.x()
        y = rect.y()
        width = rect.width()
        height = rect.height()
        text_dimensions_type = painter.boundingRect(QRectF(), Qt.AlignLeft, self.type)#+ " | Java")
        text_rect_height = 1.35 * text_dimensions_type.height()
        text_rect = QRectF(x, y, width, text_rect_height)
        path = QPainterPath()
        path.moveTo(text_rect.left(), text_rect.top() + self.corner_radius)
        path.arcTo(text_rect.left(), text_rect.top(), 2 * self.corner_radius, 2 * self.corner_radius, 180, -90)
        path.lineTo(text_rect.right() - self.corner_radius, text_rect.top())
        path.arcTo(text_rect.right() - 2 * self.corner_radius, text_rect.top(), 2 * self.corner_radius, 2 * self.corner_radius, 90, -90)
        ...
        text_dimensions_type = painter.boundingRect(QRectF(), Qt.AlignLeft, self.type) #+ " | Java")
        x_position = x + (width / 2) - (text_dimensions_type.width() / 2)
        y_position = y + text_dimensions_type.height()
        painter.drawText(int(x_position), int(y_position), self.type) #+ " | Java")        
        
        # Set color for module_id text
        painter.setPen(Qt.black)

        # Draw module_id text
        text_dimensions_id = painter.boundingRect(QRectF(), Qt.AlignLeft, self.id)
        x_position = x + (width / 2) - (text_dimensions_id.width() / 2)
        y_position = y + ((height + text_rect_height) / 2)
        painter.drawText(int(x_position), int(y_position), self.id)
