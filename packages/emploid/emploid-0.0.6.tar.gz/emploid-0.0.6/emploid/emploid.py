...
class Emploid:
    ...
    def logger(func):
        def wrapper(*args, **kwargs):
            try:
                logging.info(f"emploid started '{func.__name__}', parameters : {args} and {kwargs}")
    ...
    def __init__(
            self, 
            _driver_type=SETTINGS_USE_APPIUM,
            _autoconnect=True,
    ...
    @logger
    def query(self, _db_con=None, _query=""):
        """executes a query on the database and returns the result"""

        if("SELECT".lower() in _query.lower()):
            # if(_db_con==None):
            ...
            raise Exception("only SELECT queries are supported at the moment.")
    ...
    def set_log_path(self, _path):
        if not os.path.exists(_path):
            os.makedirs(_path)
        self.log_path = _path
        # logging.basicConfig(filename=self.log_path, encoding='utf-8', format='%(asctime)s %(message)s', level=logging.DEBUG)
        ...
        try:
            ...
            if(self.browser_type==BROWSER_TYPE_CHROME):
                ...
                if platform.system()=="Windows":
                    ...
                    self.driver_path = self.file_path + "/drivers/chromedriver.exe"
                    self.show("chromedriver path:", self.file_path)
                    self.service = Service(executable_path=driver_path)#self.file_path+'..\s_py.exe') 
    ...
    def get_generated_face(self):
        ...
        def func_(self, img_name):
            src = self.driver.find_element(SeleniumBy.XPATH, "//img[@id='avatar']").get_attribute("src")
            
            if(self.grab_file(src, img_name)):
                self.show("img saved!")
    ...
    @logger
    def highlight(self, _elm, _color="red"):
    ...
    @logger
    def pause(self):
        print("emploid is paused. press ENTER continue.")
        input()
