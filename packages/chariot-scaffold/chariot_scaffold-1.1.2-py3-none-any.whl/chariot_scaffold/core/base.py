        ...
        for i in range(len(self.__params_name)):
            if re_params_name[i] != 'self':
                ...
                if i < defaults_length:
                    ...
                    # 默认值映射枚举, 枚举类型默认值第一个
                    if str(type(self.input[re_params_name[i]]["default"])) == "<class 'enum.EnumMeta'>":
                        self.input[re_params_name[i]]["enum"] = [i.value for i in list(self.input[re_params_name[i]]["default"])]
                        self.input[re_params_name[i]]["default"] = self.input[re_params_name[i]]["enum"][0]
