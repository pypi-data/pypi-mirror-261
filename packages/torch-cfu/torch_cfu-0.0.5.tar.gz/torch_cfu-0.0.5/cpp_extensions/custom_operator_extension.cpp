...
static struct DummyCustomAllocator global_custom_alloc;
...
// which is directly set using the cuda API calls cudaGetDevice/cudaSetDevice.  static uint16_t CURR_DEVICE = -1;
...
#if 1
void test_ops() {
  std::cout << "Custom device test_ops() called!" << std::endl;
}
#endif
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("test_ops", &test_ops, "test ops");
}
