...
cfu_module = torch.utils.cpp_extension.load(
    name="custom_operator_extension",
    sources=[
        "cpp_extensions/custom_operator_extension.cpp",
    ],
...
class CfuDeviceMode(TorchFunctionMode):
    def __torch_function__(self, func, types, args=(), kwargs=None):
        if kwargs is None:
            kwargs = {}
        if 'device' in kwargs and 'cfu' in kwargs['device']:
            device_and_idx = kwargs['device'].split(':')
            if len(device_and_idx) == 1:
                # Case 1: No index specified
                kwargs['device'] = cfu_module.custom_device()
            else:
                # Case 2: The user specified a device index.
                device_idx = int(device_and_idx[1])
                kwargs['device'] = cfu_module.custom_device(device_idx)
