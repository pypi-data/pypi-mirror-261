...
class XMLApi:
    ...
    def get_devices(self, connected=False, verify=None):
        entries = self.raw_get_devices(connected=connected, verify=verify).xpath(
            ".//devices/entry"
        )
    ...
    def get_devicegroups_name(self, parents=None, with_devices=None, verify=None):
        """
        ...
        """
        devices = self.raw_get_devicegroups(verify=verify).xpath(
            ".//devicegroups/entry"
        )
        if with_devices:
            names = [d.attrib["name"] for d in devices if d.xpath("./devices/entry")]
        else:
            names = [d.attrib["name"] for d in devices]
        if parents:
    ...
    def automatic_download_software(self, version: Optional[str] = None):
        ...
        if not version:
            logging.error(f"Version {version_str} not found")
            return exit(1)

        # Already downloaded: Nothing to do
        if version.downloaded:
            logging.info(f"Version {version.version} already downloaded")
            return version
        ...
        if not base_version.downloaded:
            logging.info(f"Launching download of minor version {base_version.version}")
        ...
        logging.info(f"Launching download of version {version.version}")
    ...
    def automatic_software_upgrade(
        ...
        if version.current:
            logging.info(f"Version {version.version} is already installed")
            return version
        if not install:
            return version
        ...
        logging.info(f"Launching install of version {version.version}")

        job_id = self.install_software(version)
        if not job_id:
            logging.error("Install has not started")
            exit(1)
        job = self.wait_job_completion(job_id)
        logging.info(job.details)

        if restart:
            logging.info("Restarting the device")
            restart_response = self.restart()
            logging.info(restart_response)
        return version
