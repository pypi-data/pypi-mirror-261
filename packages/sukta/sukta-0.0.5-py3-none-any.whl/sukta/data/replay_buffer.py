...
class ReplayBuffer:
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        ...
        for key, value in stu.iter_path(
            root["data"], delim="/", dict_type=dict | zarr.Group
        ):
            assert (
                value.shape[0] == root["meta"]["episode_ends"][-1]
            ), f"Expected {key} T={root['meta']['episode_ends'][-1]} but got T={value.shape[0]}"
        self.root = root
    ...
    def copy_from_store(
        ...
        **kwargs,
        ...
        if store is None:
            ...
            data = dict()
            for path, arr in stu.iter_path(
                src_root["data"], dict_type=dict | zarr.Group
            ):
                if keys is None or "/".join(path) in keys:
                    data = stu.dict_set_path(data, path, arr[:])
            ...
            for key, value in stu.iter_path(
                src_root["data"], dict_type=zarr.Group, delim="/"
            ):
    ...
    def save_to_store(
        ...
        **kwargs,
        ...
        for path, value in stu.iter_path(
            self.root["data"], dict_type=dict | zarr.Group
        ):
            key = "/".join(path)
            data_group.require_group("/".join(path[:-1]))
    ...
    def keys(self):
        return [
            k
            for k, _ in stu.iter_path(self.data, delim="/", dict_type=dict | zarr.Group)
        ]

    def values(self):
        return [
            v
            for v, _ in stu.iter_path(self.data, delim="/", dict_type=dict | zarr.Group)
        ]

    def items(self):
        return stu.iter_path(self.data, delim="/", dict_type=dict | zarr.Group)

    def __getitem__(self, key):
        if self.backend == "zarr":
            return self.data[key]
        else:
            return stu.dict_get_path(self.data, key.split("/"))

    def __contains__(self, key):
        if self.backend == "zarr":
            return key in self.data
        else:
            return stu.dict_has_path(self.data, key.split("/"))
    ...
    def add_episode(
        ...
        episode_length = None
        for _, value in stu.iter_path(data, dict_type=dict | zarr.Group):
        ...
        for key, value in stu.iter_path(data, delim="/", dict_type=dict | zarr.Group):
            path = key.split("/")
            ...
            if is_zarr:
                ...
                has_key = stu.dict_has_path(self.data, key.split("/"))
            if not has_key:
                if is_zarr:
                    ...
                    stu.dict_set_path(self.data, path, arr)
            else:
                if is_zarr:
                    arr = self.data[key]
                else:
                    arr = stu.dict_get_path(self.data, path)
    ...
    def drop_episode(self):
        ...
        for _, value in stu.iter_path(self.data, dict_type=dict | zarr.Group):
    ...
    def get_steps_slice(self, start, stop, step=None, copy=False):
        ...
        result = dict()
        for path, value in stu.iter_path(self.data, dict_type=dict | zarr.Group):
            x = value[_slice]
            if copy and isinstance(value, np.ndarray):
                x = x.copy()
            result = stu.dict_set_path(result, path, x)
        return result
    ...
    def get_chunks(self) -> dict:
        ...
        chunks = dict()
        for key, value in stu.iter_path(self.data, dict_type=zarr.Group, delim="/"):
    ...
    def get_stats(self, key):
        if self.backend == "zarr":
            ...
            arr = stu.dict_get_path(self.data, key.split("/"))
        return {
