...
async def get_client(
    ...
    key = ClientKey(loop, api_url, auth_key, hint, tuple(sorted(headers.items())), timeout, max_keepalive_connections, keepalive_expiry)
    ...
    if key not in _GLOBAL_CLIENTS:
        ...
        transport = AsyncRetryTransport(max_keepalive_connections=max_keepalive_connections, keepalive_expiry=keepalive_expiry)
...
class ArraylakeHttpClient:
    ...
    @retry_on_exception((httpx.RemoteProtocolError, httpx.ConnectError), int(config.get("http.retries", 5)))
...
class AsyncRetryTransport(httpx.AsyncBaseTransport):
    ...
    def __post_init__(self) -> None:
        ...
        self.wrapped_transport = httpx.AsyncHTTPTransport(limits=limits)
