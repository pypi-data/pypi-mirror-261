...
from arraylake import config as config_obj  # type: ignore
from arraylake.asyn import gather_with_throttled_concurrency, paged_iterator, sync
...
class AsyncRepo:
    ...
    async def get_commits(self) -> Iterable[Commit]:
        async def wrapper(limit: int, **kwargs) -> tuple[Iterable[Commit], CommitPager]:
            ...
            return commits, partial(wrapper, **next_kwargs)
    ...
    async def commit_data(self, refresh: bool = False) -> CommitData:
        ...
        if refresh or self._commit_data is None:
            # refresh commit data
            commit_list, (tags, branches) = await asyncio.gather(self.get_commits(), self.db.get_refs())
