...
df = pd.read_csv('../reinforce/regbot_v69_validation.csv')
...
print(df['targets'].tail(10).value_counts().get(1, 0))
...
df['result'] = df.apply(lambda row: getSignal(  row['open'], row['high'], row['a'], row['b'], row['vc'], row['ema-26'], row['ema-12'], row['macd'],
                                                ...
                                                row['close-gradient-neg'], row['grad-sma-25'], row['close-grad-mean'], row['long_kdj'], row['long_grad_kdj'],
