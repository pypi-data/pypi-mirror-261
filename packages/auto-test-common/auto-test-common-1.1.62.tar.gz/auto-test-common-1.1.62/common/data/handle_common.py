...
def convert_json_dict(_json):
    ...
    for key in _json.keys():
        if isinstance(_json.get(key), str):
            if _json.get(key).find(Constant.DATA_NO_CONTENT) < 0:
...
def convert_json_bank(content, data):
    ...
    return json.dumps(convert_json_dict(_json))


if __name__ == '__main__':
    content='{"notifyDeptId": ${notifyDeptId},"eventId": ${eventId},"sysUserId": "${sysUserId}", "orderNum": "orderNum",' \
            '"oierer":{"383434":"78883","eventId":"${eventId}","sysUserId": "${sysUserId}"}}'
    os.environ['notifyDeptId']="13343434"
    data= {'eventId':'78883434'}
    print(convert_json_bank(content, data))
