...
class ProtectionJobRequestBody(object):
    ...
    Attributes:
        ...
        full_protection_start_time (TimeOfDay): Specifies the time of day to
            ...
            deprecated: true
        ignorable_errors_in_error_db (list of int): Specifies the errors
            which we can ignore from showing to the user.
        ...
        modification_time_usecs (long| int): Specifies the last time this Job
            was updated.  If this ispassed into a PUT request, then the backend
            will validate that the timestamp passed in matches the time that
            the protection group was actually last modified. If the two
            timestamps do not match, then the request will be rejected with a
            stale error.
