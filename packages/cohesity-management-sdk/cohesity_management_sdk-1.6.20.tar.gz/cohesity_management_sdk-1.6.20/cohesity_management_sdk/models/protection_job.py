...
class ProtectionJob(object):
    ...
    Attributes:
        ...
        modification_time_usecs (long|int): Specifies the last time this Job
            was updated.  If this is passed into a PUT request, then the
            backend will validate that the timestamp passed in matches the
            time that the protection group was actually last modified. If the
            two timestamps do not match, then the request will be rejected
            with a stale error.
