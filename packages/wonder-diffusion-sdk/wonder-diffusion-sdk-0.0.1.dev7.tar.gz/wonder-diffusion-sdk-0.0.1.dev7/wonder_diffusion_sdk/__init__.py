...
class WonderDiffusionSdk:
    ...
    def initialize_pipeline(self, model_config: WonderDiffusionModelConfig):
        ...
        self.logger.info(f'DIFFUSION SDK LOG: Using precision {model_config.precision}, kwargs: {kwargs}')
        self.pipeline = PIPELINE_MAP[model_config.pipeline_type](
            model_config.pretrained_model_name_or_path, **kwargs)
        self.logger.info(f'DIFFUSION SDK LOG: Using pipeline {model_config.pipeline_type}, pipeline class: {type(self.pipeline)}')
        self.pipeline.scheduler = SCHEDULER_MAP[model_config.initial_scheduler](
            self.pipeline.scheduler.config)
        self.logger.info(f'DIFFUSION SDK LOG: Using initial scheduler {model_config.initial_scheduler}, scheduler class: {type(self.pipeline.scheduler)}')
        if model_config.use_half_precision_vae:
            self.half_precision_vae(self.pipeline)
    ...
    def half_precision_vae(self, pipeline: DiffusionPipeline):
