...
def start(
    ...
    file_monitor_thread = Thread(
        ...
        args=(
            "./",  # Replace this with the actual path you want to monitor
            lambda event: (
                update_file_hashes(serve_id, file_hashes_dict, config, False)
                if not (isinstance(event, FileCreatedEvent) and event.src_path.endswith('_cerebrium_predict.json')) ##don't want to show reload with request
    ...
    #         args=(start_event, stop_event, serve_id, serve_id, "serve"),
    #     )
    ...
    try:
        while True:
            ...
            if build_status in ["build_failure", "init_failure"] or file_monitor_stop_event.is_set():
                start_event.wait()
                live.update(Text(""))
                live.stop()
                __graceful_shutdown(
...
def __graceful_shutdown(
    ...
    - exits the program
    """
    current_thread = threading.current_thread() ##needed since timeout is done from another thread
    ...
    if file_monitor_thread and file_monitor_thread is not current_thread and file_monitor_thread.is_alive():
