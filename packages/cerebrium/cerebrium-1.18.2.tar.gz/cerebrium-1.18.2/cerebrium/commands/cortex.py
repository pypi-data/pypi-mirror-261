...
def init(
    ...
    name = name if name else os.path.basename(os.path.abspath(init_dir))
...
def deploy(
    ...
    if update_config_from_files(
        config=cerebrium_config, archive_files=True, config_file=config_file
    ):
        cerebrium_log(
...
def package_app(
    ...
    if utils.files.check_deployment_size(cerebrium_config.file_list, 100):
...
def do_partial_upload(
    cerebrium_config: datatypes.CerebriumConfig, type: datatypes.OperationType
) -> Dict[str, Any]:
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        temp_file_list.extend(
            [os.path.join(temp_dir, f) for f in constants.INTERNAL_FILES]
        )
