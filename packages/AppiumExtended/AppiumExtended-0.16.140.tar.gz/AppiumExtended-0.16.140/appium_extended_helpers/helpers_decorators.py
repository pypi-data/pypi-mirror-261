...
def wait_until_dom_change(poll_frequency: float = 0.1):
    ...
    def func_decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if not func_result:
                self.logger.info(f"{func.__name__}() > {func_result}. Изменение DOM: False")
                return False
...
def wait_for_dom_change(poll_frequency: float = 0.5):
    ...
    def func_decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if not func_result:
                # Записать сообщение о том, что изменение DOM не было обнаружено
                self.logger.info(
                    f"{func.__name__}() > {func_result}. Изменение DOM: False")
                # Вернуть False для обозначения отсутствия обнаружения изменения экрана
                return False
