...
import functools
import shlex
from typing import Any, Callable, Generator, Iterator, TypeAlias, Union
...
def dumpsys(
    ad: typing_utils.AdbDevice, service: str, keyword: str,
    grep_argument: str = ""
) -> str:
  ...
  return ad.adb.shell(
      shlex.split(
          'dumpsys {} | grep {} "{}"'.format(
              shlex.quote(service), grep_argument, shlex.quote(keyword)
          )
      )
  ).decode()
...
def follow_logcat_within(
    ad: typing_utils.AdbDevice, time_sec: float = 5, wait_time_sec: float = 2.0
) -> Generator[str, None, None]:
