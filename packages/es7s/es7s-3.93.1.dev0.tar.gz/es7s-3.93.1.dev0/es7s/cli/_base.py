...
from es7s_commons import (
    NamedGroupsRefilter as _BaseNamedGroupsRefilter,
    RegexValRefilter as _BaseRegexValRefilter,
)
from es7s_commons import Regex, URL_REGEX, format_attrs
...
class HelpStyles(Styles):
    ...
    TEXT_URL = FS(fg=pt.cv.GRAY_50, underline_color=pt.Color256.get_by_code(4),  underlined=True)
                                                           #  https://...  |   | auto-detection                  # noqa
                                                           # BLUE, remove after updating pytermor to 2.212
# fmt: on
...
class NWMarkup:  # @REFACTOR ME  ●︿●
    ...
    class NamedGroupsRefilter(_BaseNamedGroupsRefilter):
        def __init__(self, pattern: PTT[str], group_st_map: dict[str, FT]):
            super().__init__(pattern, group_st_map, renderer=get_stdout().renderer)

    class RegexValRefilter(_BaseRegexValRefilter):
        def __init__(self, pattern: pt.filter.PTT[str], val_st: pt.FT):
            super().__init__(pattern, val_st, renderer=get_stdout().renderer)
    ...
    def __init__(self):
        self._filters: t.List[pt.StringReplacer] = [
            ...
            # -------------------------------------
            # [ 0] CMD_ARGUMENT
            # causes too many problems in the cases where it is not needed
            # self.RegexValRefilter(
...
class HelpFormatter(click.HelpFormatter):
    ...
    def write_dl(
        self, rows: t.Sequence[tuple[str, str]], col_max=30, col_spacing=2, dynamic_col_max=False
    ):
        if dynamic_col_max:
            col_max = self.width // 2
...
class CliBaseCommand(click.Command):
    ...
    def format_common_options(self, ctx: Context, formatter: HelpFormatter, add_header: bool):
        if not self._include_common_options_epilog:
            return
...
class CliCommand(CliBaseCommand):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.update(
            {
                "name": self._make_command_name(name),
