...
def get_server_info(username):
    ...
    if username.startswith('sophgo'):
        ...
        for server in server_list:
            ...
            lock_output(True)
            (_, ret) = is_sftp_supported(0, server)
            lock_output(False)
            if ret is True:
...
def get_filezilla_installation_path():
    if sys.platform.startswith('win'):
        import winreg
        reg_paths=[r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\FileZilla Client",r"SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\FileZilla Client"]
        ...
        for item in reg_paths:
            filezilla_path = winreg_find_path_form_uninstall(winreg.HKEY_LOCAL_MACHINE, item)
            if filezilla_path != "":
                fzpath = os.path.join(filezilla_path, 'filezilla.exe')
                return fzpath
        for item in reg_paths:
            filezilla_path = winreg_find_path_form_uninstall(winreg.HKEY_CURRENT_USER, item)
            if filezilla_path != "":
                fzpath = os.path.join(filezilla_path, 'filezilla.exe')
                return fzpath
        return ""
        ...
        try:
            process = subprocess.Popen(["which", "filezilla"], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
...
def get_winscp_installation_path():
    if sys.platform.startswith('win'):
        import winreg
        reg_paths=[r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\winscp3_is1",r"SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\winscp3_is1"]
        def winreg_find_path_form_uninstall(winreg_root,path):
            try:
                reg_key = winreg.OpenKey(winreg_root, path)
                install_path, _ = winreg.QueryValueEx(reg_key, "InstallLocation")
        ...
        winscp_path = ""
        for item in reg_paths:
            winscp_path = winreg_find_path_form_uninstall(winreg.HKEY_LOCAL_MACHINE, item)
            if winscp_path != "":
                fzpath = os.path.join(winscp_path, 'winscp.exe')
                return fzpath
        for item in reg_paths:
            winscp_path = winreg_find_path_form_uninstall(winreg.HKEY_CURRENT_USER, item)
            if winscp_path != "":
                fzpath = os.path.join(winscp_path, 'winscp.exe')
                return fzpath
        return ""
...
def sftp_login(username):
    ...
    if os.path.exists(fzpath):
        ...
        cmdstr = '"{}"'.format(fzpath) + ' sftp://'+ username + '@' + server_info['hostname'] + ":" + str(server_info['port'])
    else:
        ...
        if os.path.exists(wcpath):
            ...
            cmdstr = '"{}"'.format(wcpath) + ' sftp://'+ username + '@' + server_info['hostname'] + ":" + str(server_info['port'])
        else:
            print("sftp start ...")
