...
class IBaseETL(ITask, ABC):
    ...
    def __init__(self, time_zone: str = None, temp_folder: str = "/tmp", **kwargs):
        """
        ...
        super(IBaseETL, self).__init__(**kwargs)
        self.temp_folder = temp_folder
        self.time_zone = time_zone

    def execute(self, *args, **kwargs) -> TaskStatus:
        super(IBaseETL, self).execute()
        ...
        try:
            ...
            self.save_logs(TaskStatus.SUCCESS, processed_elements=res)
    ...
    def save_logs(
            self, execution_status: TaskStatus, processed_elements: int = 0, error_type: str = "",
            error_message: str = "", error_traceback: List[Dict] = None, **kwargs):
