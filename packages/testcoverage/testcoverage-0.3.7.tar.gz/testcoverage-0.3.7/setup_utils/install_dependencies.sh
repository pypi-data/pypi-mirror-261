...
InstallCGAL () {
	...
	tar -xf ${MAIN_DIR}/src/CGAL-${CGAL_VERSION}-library.tar.xz -C ${MAIN_DIR}/src/
...
InstallGMP () {
	echo "Setting up gmp"
	wget https://gmplib.org/download/gmp/${GMP_TAR_NAME}.tar.xz -P ${MAIN_DIR}/src
	...
	make -C ${MAIN_DIR}/src/${GMP_TAR_NAME} install -j$(nproc)
	if [ $? -eq 0 ]; then
