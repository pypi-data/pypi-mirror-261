...
@click.pass_obj
def setup(env):
    """subcommands for manage workspaces for bigplanner"""
...
def _service(env, command):
    """List existing workspaces for bigplanner"""
    ...
    service_unit = f"""
[Unit]
Description=bigplanner Supervisor Service
...
def service(env, command):
    """Install a reset service bigplanner"""
...
def autocomplete(env):
    ...
    subprocess.run(
        [
            "_BIGPLANNER_COMPLETE=bash_source bigplanner > ~/.bigplanner-complete.bash"
        ],
        shell=True,
    )
    ...
    print("source ~/.bigplanner-complete.bash")
