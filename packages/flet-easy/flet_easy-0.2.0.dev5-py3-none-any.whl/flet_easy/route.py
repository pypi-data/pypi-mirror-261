...
from flet_easy.extras_jwt import SecretKey
...
class FletEasyX:
    ...
    def __route_change(self, route: RouteChangeEvent):
        ...
        if pg_404:
            if self.__page_404:
                ...
                self.__add_events_params(self.__page_404.view, self.__page_404.title)
            else:
                self.__page.views.append(self.__view_404.view(self.__page))
                self.__page.update()

    def __add_events_params(self, view: View, title: str, url_params: dict = None):
    ...
    def __disconnect(self, e):
        self.__data._login_done = False

    def app_lifecycle_change(self, e: ft.AppLifecycleStateChangeEvent):
        if e.state == ft.AppLifecycleState.RESUME:
            print("Update UI with fresh data!")
        if e.state == ft.AppLifecycleState.SHOW:
            print("Update UI with show")
        if e.state == ft.AppLifecycleState.HIDE:
            print("Update UI with hide")
        if e.state == ft.AppLifecycleState.INACTIVE:
            print("Update UI with inactive")
        if e.state == ft.AppLifecycleState.PAUSE:
            print("Update UI with pause")
        if e.state == ft.AppLifecycleState.DETACH:
            print("Update UI with detach")
        if e.state == ft.AppLifecycleState.RESTART:
            print("Update UI with restart")
    ...
    def run(self):
        ...
        self.__page.on_app_lifecycle_state_change = self.app_lifecycle_change
            ...
            if route_check:
                ...
                try:
                    if page.protected_route:
                        ...
                        if await self.__config_login(self.__data):
                            if page.clear:
                                self.__page.views.clear()
                            if not page.share_data:
                                self.__data.share.clear()
    ...
    async def __disconnect_async(self, e):
        self.__data._login_done = False
        await self.__page.pubsub.send_others_on_topic_async(
            self.__page.client_ip,
            Msg("updateLoginSessions", value=self.__data._login_done),
        )

    # -- initialize updateLoginSession
    ...
    async def run_async(self):
        ...
        self.__page.on_disconnect = self.__disconnect_async
        self.__page.on_error = lambda e: print("Page error:", e.data)
