...
class _ClientBase:
    ...
    def tester_start_using_test_file(
        ...
        os.chdir(current_directory)
        if blocked:
            return self._get_tester_status(namespace, address)
        return None

    def _get_tester_status(self, namespace: str, address: str) -> json:
        '''
        ...
        if "error" in tester_status and tester_status["error"] is not None:
            raise Exception(f"Test failed: {tester_status['error']}")

        if "message" in tester_status and tester_status["message"] is not None:
            raise Exception(f"Test failed: {tester_status['message']}")

        if "results" in tester_status:
            test_results = tester_status["results"]
            return TestStatus({"test_results": test_results})
        return None
