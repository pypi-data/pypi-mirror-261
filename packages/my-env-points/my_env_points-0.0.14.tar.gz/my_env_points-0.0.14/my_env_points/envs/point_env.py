...
class MyGymPointsEnv(gym.Env):
    ...
    def __init__(self, a: int = 10, b: int = 10, max_size=10):
        ...
        self.size = (a, b)
        self.points: np.ndarray = np.random.rand(self.max_size, 4)
        self.low = np.tile(
            np.array([-a / 2, -b / 2, -max(a, b), -np.pi]), (self.max_size, 1)
        )
        self.high = np.tile(
            np.array([a / 2, b / 2, max(a, b), np.pi]), (self.max_size, 1)
        )
    ...
    def step(self, action):
        ...
        target = int(
            np.abs(action[0] - (-1.0), dtype=np.float32) * (self.max_size - 1) / 2
        )
        print(f"action: \n{action}")
        print(f"target: \n{target}")
