...
def _parse_directory_tree(directory, indent=0):
    ...
    for item in os.listdir(directory):
        ...
        if indent == 2 and "readme" in item_path.lower():
            ...
            tree["parsed"] = True
        elif indent < 2 and os.path.isdir(item_path) and "docker" not in item:
...
def _prompt_user_for_example_choice(examples):
    ...
    frameworks = [fw for fw in examples.keys()]
...
def examples(name=None):
    ...
    if name:
        ...
        try:
            ...
            raise PloomberCloudRuntimeException(
                "Invalid example choice. Command should be in the format:\n\n \
