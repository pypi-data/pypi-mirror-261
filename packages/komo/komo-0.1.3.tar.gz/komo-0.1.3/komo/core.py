...
def login(email: str, password: str):
    api_key = APIClient.get_api_key(email, password)
...
def print_job_logs(job_id, follow: bool = False):
    api_client = APIClient()
    api_client.print_job_logs(job_id, follow)
...
def _get_private_key_file():
    key_file = os.path.join(os.path.expanduser("~"), ".ssh/komodo-key")
