...
class APIClient:
        ...
        if workdir:
            with tempfile.TemporaryDirectory() as td:
                ...
                with open(workdir_zipfile, "rb") as f:
                    ...
                    if workdir == ".":
                        printing.info("Uploading current working directory")
                    else:
                        printing.info(f"Uploading directory {workdir}")
                    response = requests.request(
                        "POST",
                        url=f"{KOMODO_API_URL}/api/v1/jobs/upload_workdir",
                        headers=headers,
                        files=files,
                    )
                    printing.info("Finished uploading workdir")

                response.raise_for_status()
                workdir_upload_id = response.json()
    ...
    def _get_log_chunk(
        ...
        url = f"{KOMODO_API_URL}/api/v1/jobs/{job_id}/logs"
    ...
    def print_job_logs(self, job_id: str, follow: bool):
        ...
        while True:
            ...
            response = self._get_log_chunk(job_id, next_token, end_time=end_time)
    ...
    def get_machine(self, machine_name: str) -> Machine:
        machine_dict = self.api_request(
            "GET", f"{KOMODO_API_URL}/api/v1/machines/{machine_name}"
        )
        machine = Machine.from_dict(machine_dict)
        return machine
