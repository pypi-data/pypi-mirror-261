...
class SilicaComponent {
            ...
            if (data.hasOwnProperty(modelName)) {
                if (el.type === 'checkbox') {
                    if (Array.isArray(data[modelName])) {
                        el.checked = data[modelName].includes(el.value);
                    } else {
    ...
    handleModelInputEvent(event) {
        ...
        if (el.type === 'checkbox') {
            if (this.data.hasOwnProperty(modelName) && Array.isArray(this.data[modelName])) {
                let value = el.checked ? (el.getAttribute('value') || true) : false;

                if (value) {
                    this.data[modelName].push(value)
                } else {
                    let valueToRemove = el.getAttribute('value') || true
                    this.data[modelName] = this.data[modelName].filter(v => v !== valueToRemove)
                }

                this.setProperty(modelName, this.data[modelName]);
            } else {
                let value = el.checked ? (el.getAttribute('value') || true) : false;
    ...
    setListeners() {
        ...
        models.forEach((element) => {
            if (element.type === "checkbox") {
                element.addEventListener("change", this.boundMethods.handleModelInputEvent);
            } else {
