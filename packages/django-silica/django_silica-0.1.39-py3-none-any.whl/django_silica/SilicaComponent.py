    ...
    def set_property(self, attr, value):
        ...
        if '.' not in attr:
            if hasattr(self, base_attr):
                ...
                # Invoke hooks for single-level attribute
                self._invoke_hooks(base_attr, '', value)
            else:
                raise SetPropertyException(f'"{base_attr}" does not exist on property "{attr}".')
            return
        ...
        if isinstance(obj, dict) and final_segment in obj or hasattr(obj, final_segment):
            ...
            # Invoke hooks with nested attr, excluding the first segment
