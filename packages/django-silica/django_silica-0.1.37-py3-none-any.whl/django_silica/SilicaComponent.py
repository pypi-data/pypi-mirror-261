...
class SilicaComponent(TemplateView, ValidatesProperties):
    ...
    def set_value(self, path_str, value):
        segments = path_str.split('.')
        if not self._is_public(segments[0]):
            print("First level attribute is not public.")
            raise PropertyDoesNotExist()
        ...
        for i, segment in enumerate(segments[:-1]):
            if isinstance(obj, dict):
                if segment in obj:
                    ...
                    print(f"Dictionary key '{segment}' does not exist.")
                    raise PropertyDoesNotExist(f'Dictionary key {segment} does not exist.')
            else:
                if hasattr(obj, segment):
                    obj = getattr(obj, segment)
                    if i == 0 and not self._is_public(segment):
                        print(f"Attribute '{segment}' is not public.")
                        raise PropertyDoesNotExist(f'Attribute {segment} is not public.')
                else:
                    print(f"Attribute '{segment}' does not exist.")
                    raise PropertyDoesNotExist(f'Attribute {segment} does not exist.')

            if not isinstance(obj, (dict, type(self))):
                print("Cannot navigate through non-dict/non-class object.")
                raise PropertyDoesNotExist('Cannot navigate through non-dict/non-class object.')

        # Attempt to set the value on the final segment
        final_segment = segments[-1]
        if isinstance(obj, dict):
            if final_segment in obj:
                obj[final_segment] = value
                return True
            else:
                raise PropertyDoesNotExist(f'"{final_segment}" does not exist on property "{segment}".')
        else:
            if hasattr(obj, final_segment):
