...
class VXScheduler:
    ...
    def __init__(self) -> None:
        ...
        self._executor = ThreadPoolExecutor(thread_name_prefix="vxsched")
    ...
    def trigger_event(self, event: Optional[VXEvent] = None) -> Any:
        """触发消息（同步触发）
        ...
        if isinstance(event, VXEvent):
            list(
                self._executor.map(
                    lambda h: self._run_handler(h, event), self._handlers[event.type]
                )
            )
            return
