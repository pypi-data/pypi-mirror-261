        ...
        if self._result is _NOSET:
            try:
                ...
                if self._on_error == "raise":
                    ...
                    logging.error(
                        "async_task error: %s", err, exc_info=True, stack_info=True
                    )
                self._result = None
        return self._result
...
class async_task:
    ...
    __executor__ = ThreadPoolExecutor(max_workers=10, thread_name_prefix="async_task")

    def __init__(
        self, on_error: Literal["ignore", "raise", "logging"] = "logging"
    ) -> None:
        self._on_error = on_error
    ...
    def __call__(self, func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> VXAsyncResult:
            fn = self.__executor__.submit(func, *args, **kwargs)
...
if __name__ == "__main__":
    ...
    results = [test(i) for i in range(1)]
