...
KrigingProcessor::KrigingProcessor(KrigingSettings &settings, const DataDescription &dimensions) : LatticeModel(2) {
    threshold.Setup(settings.ThresholdParams);

    Point L(0.0, 0.0, 0.0);
    Point U(static_cast<double>(dimensions.W),
            static_cast<double>(dimensions.H),
...
void KrigingProcessor::BuildCDFs(const ContainerType &input_image, CDF& phase_0_cdf, CDF& phase_1_cdf) {
  const int histogram_length = 256;
  Histogram hist(input_image, histogram_length);
  phase_0_cdf = CDF(hist);
  phase_1_cdf = CDF(hist);

  this->StatsNL(input_image, threshold.HighThresholds(), threshold.LowThresholds(), this->means, this->vars);
  ...
  if (abs(this->vars[0] + this->vars[1]) < EPS_THINY) {
    ...
    middle_threshold_point = threshold.HighThresholds()[0] + (this->vars[0]) / (this->vars[0] + this->vars[1]) *
...
void KrigingProcessor::BuildVariograms(const ContainerType &input_image, ContainerType &segmented_image,
...
void KrigingProcessor::Proceed(const ContainerType &input_image,
  ...
  size_t unsegmented_voxels_count = InitSeedRegions(input_image, segmented_image);
  ...
  BuildVariograms(input_image, segmented_image, cdf_of_phase_0, cdf_of_phase_1, var_phase_0, var_phase_1);
  ...
  SolveOrdinaryKriging(kriging_matrix_phase_0, kriging_coeffs_phase_0, kriging_vector_len, var_phase_0, neighborhood);
  SolveOrdinaryKriging(kriging_matrix_phase_1, kriging_coeffs_phase_1, kriging_vector_len, var_phase_1, neighborhood);
  delete[] neighborhood;

  ApplyKrigingModel(input_image, segmented_image, kriging_vector_len, cdf_of_phase_0, cdf_of_phase_1, kriging_coeffs_phase_0, kriging_coeffs_phase_1);
}
 
void KrigingProcessor::ApplyKrigingModel(const ContainerType &input_image, ContainerType &segmented_image, size_t kriging_vector_len,
                                         const CDF& cdf_of_phase_0, const CDF& cdf_of_phase_1,
                                         const DynamicArray<double>& rhs_phase_0, const DynamicArray<double>& rhs_phase_1) {
  ...
  std::copy(segmented_image.begin(), segmented_image.end(), presegmented_image.begin());
  ...
  for (size_t idx = 0; idx < segmented_image.size(); ++idx) {
    ...
    if (input_image[idx] < threshold.LowThresholds()[0] || input_image[idx] > threshold.LowThresholds()[1]) {
      continue;
    }
    CollectValuesFromNeighborhood(input_image, presegmented_image, idx, intermediate_data_phase_0.begin_pointer(), cdf_of_phase_0);
    CollectValuesFromNeighborhood(input_image, presegmented_image, idx, intermediate_data_phase_1.begin_pointer(), cdf_of_phase_1);

    double phase_0_prob = std::inner_product(rhs_phase_0.begin(), rhs_phase_0.begin() + kriging_vector_len,
                                             intermediate_data_phase_0.begin(), 0.0);
    double phase_1_prob = std::inner_product(rhs_phase_1.begin(), rhs_phase_1.begin() + kriging_vector_len,
...
size_t KrigingProcessor::InitSeedRegions(const ContainerType &input_image, ContainerType &segmented_image) {
...
void KrigingProcessor::SolveOrdinaryKriging(DynamicArray<double>& matrix_data, DynamicArray<double>& kriging_weights, size_t kriging_vector_len,
                                            Variogram& variogram, const Point *neighborhood) {
    size_t rank = kriging_vector_len + 1;
    matrix_data.resize(rank * rank);
    kriging_weights.resize(rank);

    SetupOrdinaryKriging(matrix_data, kriging_vector_len, variogram, neighborhood);
    ...
    double sum = std::accumulate(kriging_weights.begin(), kriging_weights.begin() + kriging_vector_len, 0.0);
    if ((fabs(sum - 1.0) > EPS_THINY)) {
      std::cout << "Ordinary kriging system solution does not converge" << std::endl;
    }
    CoerceNegativeWeights(kriging_weights.begin_pointer(), kriging_vector_len, variogram, neighborhood);
}

void KrigingProcessor::CoerceNegativeWeights(double *weights, size_t kriging_vector_len, const Variogram &covariance, const Point *neighborhood) {
  ...
  for (size_t i = 0; i < kriging_vector_len; ++i) {
    ...
    else if (covariance(neighborhood[i].abs()) < average_covariance_of_neg && weights[i] < average_neg_weight) {
      weights[i] = 0.0;
    }
    stat_sum += weights[i];
  }
...
void KrigingProcessor::FillCDF(CDF &cdf, double lower_boundary, double upper_boundary) {
...
void KrigingProcessor::SetupOrdinaryKriging(std::vector<double> &mat, size_t kriging_vector_len, 
                                            const Variogram &covariance, const Point *neighborhood) {
  size_t rank = kriging_vector_len + 1;
...
void KrigingProcessor::CollectValuesFromNeighborhood(const ContainerType &input_image, ContainerType &segmented_image,
  ...
  for (int64_t z = cz - radius; z <= cz + radius; ++z) {
    for (int64_t y = cy - radius; y <= cy + radius; ++y) {
      for (int64_t x = cx - radius; x <= cx + radius; ++x) {
        if ((z - cz) * (z - cz) + (y - cy) * (y - cy) + (x - cx) * (x - cx) > radius * radius) {
          continue;
        }
        if ((z == cz) && (y == cy) && (x == cx)) {
          continue;
