...
class scheduleitem:
    def __init__(self,json,main) -> None:
        ...
        for action in self.json["actions"]:
            self.actions.append(scheduleaction(action,self.session))
        timezone = pytz.timezone(datetime.datetime.now().astimezone().tzname())
        self.start = datetime.datetime.fromtimestamp(self.json["start"]).astimezone(timezone)
        self.end = datetime.datetime.fromtimestamp(self.json["end"]).astimezone(timezone)
    ...
    def __repr__(self) -> str:
        return f"subjects={self.subjects},teachers={self.teachers},start={self.start},end={self.end},cancelled={self.cancelled},id={self.id}"