...
async def create_pro(
    ...
    try:
        install_chainmaker_go = await ConfirmPrompt(
            _("部署长安链?"), default_choice=True
        ).prompt_async(style=CLI_DEFAULT_STYLE)
    except CancelledError:
        ctx.exit()
    # 假设你的shell脚本路径是 /path/to/your/script.sh

    if install_chainmaker_go:
        ...
        des_dir = Path(os.getcwd()) / f"{project_name}" / "chainmaker-go"

        TEMPT_CHAIN_CRYPTOGEN = Path(__file__).parent.parent.parent / "template" / "pro_project" / "chainmaker-cryptogen"
        project_name = context.variables["project_name"]
        des_dir2 = Path(os.getcwd()) / f"{project_name}" / "chainmaker-cryptogen"

        import shutil

        # 源文件夹路径
        source_folder = TEMPT_CHAIN

        # 目标文件夹路径（必须事先创建好）
        target_folder = des_dir

        # 源文件夹路径
        source_folder2 = TEMPT_CHAIN_CRYPTOGEN

        # 目标文件夹路径（必须事先创建好）
        target_folder2 = des_dir2
        ...
        try:
            ...
            print("长安链部署失败。错误信息：", str(e))
        except Exception as e:
            print("发生了其他未知错误。错误信息：", str(e))

    try:
        install_storage = await ConfirmPrompt(
            _("部署存储层服务?"), default_choice=True
        ).prompt_async(style=CLI_DEFAULT_STYLE)
    except CancelledError:
        ctx.exit()
    ...
    if install_storage:
        ...
        TEMPT_STORAGE = Path(__file__).parent.parent.parent / "template" / "pro_project" / "storage"
        project_name = context.variables["project_name"]
        des_dir = Path(os.getcwd()) / f"{project_name}" / "storage"

        import shutil

        # 源文件夹路径
        source_folder = TEMPT_STORAGE
