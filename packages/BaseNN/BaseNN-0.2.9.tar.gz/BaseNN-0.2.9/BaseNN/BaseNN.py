...
class nn:
    ...
    def load_img_data(self, data_path, classes=None, batch_size=32,train_val_ratio=1.0, shuffle=True,batch_mode=False,transform=None,color="RGB",num_workers=0,**kw ):
        ...
        dataset = ImageFolderDataset(data_path, transform=self.transform,color=color, batch_mode=batch_mode)
        ...
        if self.rnn:
            ...
            if isinstance(data,str): # 推理文件路径
                ...
                if file_type == "csv":  # 推理csv
                    ...
                    # img = cv2.imread(data)
                    if color == "grayscale":
                        img = cv2.imread(data,cv2.IMREAD_GRAYSCALE) /255.0
                        img = np.expand_dims(img, 2)                 
                    else:
                        img = cv2.imread(data) / 255.0
                    # print(img.shape,img)
                    # img = np.array(Image.open(data))
                    # transform = torch.load(checkpoint,map_location=torch.device('cpu'))['meta']['transform']
                    if transform is not None:
                        # img = transform(img)
                        img = np.array(transform(Image.fromarray((img*255).astype(np.uint8)))) /255.0
