...
def mobilenet_v2():
    ...
    model = nn()
    # 载入数据
    model.load_img_data('../../dataset/cls/mnist/training_set/', batch_size=2048,shuffle=True)
    # # 定义模型结构：扩散模型
    model.add('mobilenet')
    model.add('Linear', size=(10,20), activation='relu')
    model.add('Linear', size=(20,10))
    model.add(activation='Softmax')
    # 指定优化器（可省略）
    model.add(optimizer='Adam')
    ...
    import time
    a = time.time()
    model.train(lr=1e-3, epochs=1,checkpoint='mobilenet_ckpt/basenn.pth') # 直接训练
    print(time.time() - a)
    # 反向去噪过程：高斯噪声->生成图像
    generated_imgs = model.inference(data='../../dataset/cls/mnist/training_set/4/2.png')
    model.print_result()
    print(generated_imgs.shape,generated_imgs)
