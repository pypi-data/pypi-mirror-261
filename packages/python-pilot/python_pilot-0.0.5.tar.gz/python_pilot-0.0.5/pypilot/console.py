...
class InteractiveConsoleAgent(InteractiveConsoleWithHistory):
                ...
                try:
                    ...
                    if llm:
                        ...
                        if self.token_count_limit is not None and token_count > self.token_count_limit:
                            utils.print_color(f"LLM[tokens:{token_count}] exceeds token count limit of {self.token_count_limit}", 'red')
                            llm = 0
                            continue
                    ...
                    if llm:
                        ...
                        else: # otherwise convert the chat response to string expression 
                            line = f"'''{line}'''"
                    self._history.append(line)
    ...
    def llm_input(self, instruction_line):
        ...
        selection = self.agent.select(instruction=instruction_line)
