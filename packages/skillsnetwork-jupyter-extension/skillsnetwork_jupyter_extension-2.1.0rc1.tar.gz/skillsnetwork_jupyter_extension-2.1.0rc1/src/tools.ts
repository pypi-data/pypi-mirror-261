...
export const openLearnerLab = async (
  ...
  const strippedFilePath = PathExt.removeSlash(filePath); // Some functions don't expect the path to start with a slash

  await deleteIfEmptyFile(strippedFilePath, contentsManager, docManager);
