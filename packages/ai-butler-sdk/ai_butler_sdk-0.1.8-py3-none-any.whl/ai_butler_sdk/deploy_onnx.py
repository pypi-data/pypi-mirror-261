...
class DeployOnnxInfer:
    ...
    def __init__(self, deploy_task_id, token, train_task_result_url, base_dir: str = ""):
        ...
        self.docker_image_name = settings.DEPLOY_ONNX_DOCKER_IMAGE
    ...
    def docker_run(self, port: int) -> str:
        ...
        container_id = client.containers.run(
            ...
            volumes={self.onnx_weight_path: {"bind": f"/srv/app/{settings.DEPLOY_ONNX_WEIGHT_DIR}", "mode": "rw"}},
            ports={"8000/tcp": port},  # 端口映射，这里将容器的80端口映射到主机的8080端口
            name=f"ai-butler-onnx-infer-{port}",  # 容器名称
        )
        return container_id
