...
class Polygon:
    ...
    async def aggregates(self, ticker, multiplier:str='1', timespan:str='day', date_from=None, date_to=None, limit:str=500, sort:str='desc'):
        ...
        endpoint = f"https://api.polygon.io/v2/aggs/ticker/{ticker}/range/{multiplier}/{timespan}/{date_from}/{date_to}?sort={sort}&apiKey={self.api_key}"
        print(endpoint)
        if limit == 50000:
    ...
    async def rsi_snapshot(self, ticker):
        ...
        if price is not None:
            ...
            if month_rsi is not None and hasattr(month_rsi, 'rsi_value') and month_rsi.rsi_value is not None and len(month_rsi.rsi_value) > 0:
                ...
                rsis.update({'month_rsi': week_rsi})
