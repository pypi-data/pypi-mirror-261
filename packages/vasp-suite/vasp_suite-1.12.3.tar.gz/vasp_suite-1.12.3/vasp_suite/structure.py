...
class Molecule(Structure):
    ...
    def find_molecule(self) -> None:
        ...
        '''
        cart_coords, atom_list = self.translate()
        atom_list = atom_list.reshape(-1, 1)
        origin = self.origin_index(cart_coords)
        coords = np.concatenate((atom_list, cart_coords), axis=1).reshape(-1, 4)

        molecule = np.array([coords[origin]])
        while True:
            total_coords = []
            for i in molecule:
                if not i[0] == 'H':
            ...
            if len(total_coords) == len(molecule):
                break
            else:
                molecule = total_coords

        self.cart_coords = np.array(molecule[:, 1:], dtype=float)
        self.atom_list = molecule[:, 0]
    ...
    def get_coordination_sphere(self, coordination_sphere: int):
        ...
        """
        cart_coords, atom_list = self.translate()
        coords = np.concatenate((np.array(atom_list).reshape(-1, 1), cart_coords), axis=1)
        sphere_coords = np.array([coords[self.origin_index(cart_coords)]])
        ...
        if coordination_sphere == 1:
            ...
            self.atom_list = sphere_coords[:, 0]
            return
