...
def register_artifact(
    ...
    **kwargs,
    ...
    Args:
        ...
        organism: The organism to register the artifact for.
    ...
    for feature_name, field in fields.items():
        ...
        if hasattr(registry, "organism_id"):
            ...
            query_kwargs["organism"] = organism
        df = data.obs if isinstance(data, ad.AnnData) else data
        labels = registry.from_values(df[feature_name], field=field, **query_kwargs)
...
def register_labels(
    ...
    check_organism(kwargs, registry)
    verbosity = ln.settings.verbosity
    ...
    log_registered_labels(labels_registered, validated_only)


def check_organism(kwargs: Dict, registry: Registry):
    """Check if organism is applicable for a registry."""
...
def log_registered_labels(labels_registered: Dict, validated_only: bool = True):
