...
if args.a:
    ...
    if fuzzy_action == "i":
        ...
        #subprocess.call(cmd_nix, shell=True)
        target_data = yaml.safe_load(open(os.path.join(HOME, actual_target[0]), 'r').read())
        PP(target_data)

    if fuzzy_action == "e":
        cmd_nix = """/bin/bash -c '$EDITOR %s/%s'""" % (HOME, actual_target[0])
        cmd_win = """echo NOT IMPLEMENTED"""
        subprocess.call(cmd_nix, shell=True)
    ...
    if fuzzy_action == "l":
        cmd_nix = """/bin/bash -c 'cd %s && docker-compose -f %s logs -f'""" % (HOME, actual_target[0])
        cmd_win = """echo NOT IMPLEMENTED"""
        subprocess.call(cmd_nix, shell=True)

    if fuzzy_action == "b":
        cmd_nix = """/bin/bash -c 'cd %s && docker-compose -f %s build'""" % (HOME, actual_target[0])
        cmd_win = """echo NOT IMPLEMENTED"""
        subprocess.call(cmd_nix, shell=True)

    if fuzzy_action == "bnc":
        cmd_nix = """/bin/bash -c 'cd %s && docker-compose -f %s build --no-cache'""" % (HOME, actual_target[0])
        cmd_win = """echo NOT IMPLEMENTED"""
        subprocess.call(cmd_nix, shell=True)
