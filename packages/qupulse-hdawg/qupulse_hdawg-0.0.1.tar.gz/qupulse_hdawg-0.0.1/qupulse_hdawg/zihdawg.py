...
class HDAWGChannelGroup(AWG):
    ...
    def _wait_for_compile_and_upload(self):
        ...
        for i in range(self.num_channels // 2):
            self._master_device._device.awgs[self.awg_group_index + i].write_to_waveform_memory(
                self._program_manager._waveform_memory._zhinst_waveforms_tuple[i])
        if self.SLEEP_PARANOIA > 0: time.sleep(self.SLEEP_PARANOIA)

        for i in range(self.num_channels // 2):
            self._master_device._device.awgs[self.awg_group_index + i].commandtable.upload_to_device(
                self._current_ct_dict[i])
        if self.SLEEP_PARANOIA > 0: time.sleep(self.SLEEP_PARANOIA)
        ...
        sequencer_ready_status = self._master_device.api_session.getInt(
            '/{}/AWGS/{}/READY'.format(self.master_device.serial, self.awg_group_index))
    ...
    def get_commandtable_schema(self, awg_int=None) -> dict:
        node_base_path = '/{}/awgs/{}'.format(self.master_device.serial,
                                              awg_int if awg_int is not None else self.awg_group_index)
        ...
        return tree_dict[str(self.master_device.serial).lower()]["awgs"][
            str(awg_int if awg_int is not None else self.awg_group_index)]["commandtable"]["schema"][0]["vector"]
...
class MDSChannelGroup(HDAWGChannelGroup):
    ...
    def enable(self, status: bool = None) -> bool:
        ...
        if status is not None:
            ...
            self.master_device.api_session.get(
                '/{}/awgs/{:d}/module'.format(self.master_device.serial, self.awg_group_index))
...
class SingleDeviceChannelGroup(HDAWGChannelGroup):
    ...
    def amplitudes(self) -> Tuple[float, ...]:
        ...
        for ch, zi_amplitude in zip(self._channels(),
                                    _amplitude_scales(self.master_device.api_session, self.master_device.serial)):
        ...
        if self._compile_job is None:
            ...
            if compiler_start:
                ...
                if compiler_status == -1:
                    ...
                    logger.warning("AWG %d: Compilation finished with warning: %s", self.awg_module.index,
                                   status_string)
