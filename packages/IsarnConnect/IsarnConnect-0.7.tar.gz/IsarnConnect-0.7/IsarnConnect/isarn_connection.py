...
class IsarnConnection:
    # ELS *60
    # LGZ *50
    def __init__(self, password, username, address, port, database):
        self.username = username
        self.password = password
        self.address = address
        self.port = port
        self.database = database
        self.engine = create_engine('postgresql+psycopg2://' + username + ':' + password + '@' + address + ':' + port +
                                    '/' + database, echo=False)
    ...
    def getByDate(self, date: datetime, meter: Meter):
        ...
        if meter.value == "els":
            ...
            items = session.query(LGZ).filter_by(c092=date_code).all()
            return tableRowToRowArray(items, Meter.LGZ)
        elif meter.value == "slb":
            items = session.query(SLB).filter_by(c092=date_code).all()
