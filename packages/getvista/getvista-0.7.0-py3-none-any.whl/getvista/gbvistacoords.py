...
def reformat(collected_features):
    ...
    for feature in collected_features:
        ...
        if transcript_id.startswith('NP') or transcript_id.startswith('XP') or transcript_id.startswith('YP'):
...
def download_fasta(accession, start, end, fasta_output_file, apply_reverse_complement, abs_start_coord, abs_end_coord):
    ...
    if not apply_reverse_complement:
        ...
        fasta_record = SeqIO.SeqRecord(
            ...
            id=f"{accession}_{abs_start_coord}:{abs_end_coord}:-1",
            description="",
        )
...
def useflanks(collected_features, start_position, end_position, flank, abs_start_coord, startdiff):
    ...
    while True:  # Keep looping until a valid gene name is entered or the user chooses to exit  
        ...
        if gene_name_input1.lower() in gene_locations_lower:
    ...
    while True:  # Keep looping until a valid gene name is entered or the user chooses to exit  
        ...
        # Check if the transcript name is present in the dictionary
        if gene_name_input2.lower() in gene_locations_lower:
...
def gbcoords(
    ...
    # Function to remove excess blank lines
    ...
    # Function to remove headers surrounded by blank lines (no UTRs or exons in region)
    def remove_surrounded_empty_lines(text):
        lines = [''] + text.split('\n')
        result = []
        i = 0
        while i < len(lines):
            # Check if the previous and next lines are empty
            if i > 0 and i < len(lines) - 1 and lines[i - 1] == '' and lines[i + 1] == '':
                i += 2  # Skip the current empty line and the next line
            else:
                result.append(lines[i])
                i += 1
        return '\n'.join(result)
    
    formatted_coordinates1 = remove_surrounded_empty_lines(formatted_coordinates)
    formatted_coordinates = remove_empty_lines(formatted_coordinates1)
