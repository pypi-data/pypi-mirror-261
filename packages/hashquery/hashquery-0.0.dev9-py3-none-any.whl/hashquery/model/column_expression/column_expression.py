...
class ColumnExpression(ABC, Identifiable, Serializable):
    ...
    def is_this_week(self) -> "ColumnExpression":
        ...
        # TODO(HB-8410): Support project-specified start of week.
        # .weekday() by default starts on Monday. We shift it to start on Sunday.
        start_of_week = datetime.date.today() - datetime.timedelta(
            days=(datetime.date.today().weekday() + 1) % 7
        )
        return self._is_between_timestamps(
            start_of_week,
    ...
    def is_this_quarter(self) -> "ColumnExpression":
        ...
        start_of_quarter = start_of_month - relativedelta(
            months=(start_of_month.month - 1) % 3
        )
        return self._is_between_timestamps(
            start_of_quarter,
            start_of_quarter + relativedelta(months=3),
        )
    ...
    def is_this_year(self) -> "ColumnExpression":
        """Filters the target time column to the previous quarter."""
        start_of_year = datetime.date.today().replace(month=1, day=1)
        return self._is_between_timestamps(
            start_of_year,
