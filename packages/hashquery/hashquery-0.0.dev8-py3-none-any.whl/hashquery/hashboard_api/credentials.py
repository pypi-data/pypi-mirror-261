...
def set_default_credentials(credentials: HashboardClientCredentials):
    """Sets the credentials to use if no other credentials are found in environment variables."""
...
def get_client_credentials() -> Optional[HashboardClientCredentials]:
    ...
    # Manual env vars
        ...
        credentials_from_file = _get_credentials_from_filepath(credentials_filepath)
        if credentials_from_file:
            return credentials_from_file
        if _default_credentials:
            return _default_credentials
        raise RuntimeError(f"Credentials file does not exist ({credentials_filepath})")


def _get_credentials_from_filepath(credentials_filepath: str):
