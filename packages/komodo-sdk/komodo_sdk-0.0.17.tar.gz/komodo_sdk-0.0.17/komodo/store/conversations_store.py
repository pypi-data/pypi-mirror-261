...
class ConversationStore:
    ...
    def add_header(self, guid, title, created_at, user_email, agent_id):
        conversation = Conversation(guid=guid, title=title, created_at=created_at, user_email=user_email,
                                    agent_shortcode=agent_id)
        header_data = conversation.SerializeToString()
        header_key = f"guid:{guid}:conversation"
        self.redis.set(header_key, header_data)
        return conversation

    def add_message(self, conversation, sender, sender_type, text):
        now = datetime.utcnow().isoformat() + 'Z'
        message = Message(guid=str(uuid.uuid4()), sender_type=sender_type, sender=sender,
    ...
    def get_conversation_headers(self, user_email, agent_id):
        conversations_data = []
