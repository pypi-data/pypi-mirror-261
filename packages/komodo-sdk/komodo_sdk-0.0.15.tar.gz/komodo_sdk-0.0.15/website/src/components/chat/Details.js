...
const Details = () => {
    ...
    useEffect(() => {
        if (location?.pathname) {
            ...
            setNewDetails({ ...newDetails, gId: location.pathname.split("/details/")[1] })
        }
    }, [location?.pathname])
    ...
    return (
        <PrimaryLayout>
            <div className="w-full flex items-center flex-col">
                <div className='mt-10 ms-7 pe-8 h-[calc(100vh-218px)] overflow-auto scrollbar w-[1017px] xl:w-[950px] lg:w-[800px]'>
                    {loader ? <div className='h-full bg-white'></div> :
                        <>
                            {allChatData && allChatData.length > 0 ? allChatData.map((val, i) => {
                                return (
                                    <div className='mb-10' key={i}>
                                        ...
                                        {val?.sender === context?.reactSelect?.shortcode && <div>
                                            <div className='mt-10 flex gap-5'>
                                                <div>
                                                    <img src={wave} alt="wave" className='w-[30px] h-[30px]' />
                                                </div>
                                                <div className='w-fit text-[#495057]'>
                                                    <p className='font-cerebriregular text-[15px] leading-[34px] text-justify' dangerouslySetInnerHTML={{ __html: val?.text }}></p>
                                                </div>
                                            </div>
                                        </div>}
                                    </div>
                                ...
                                <div className='text-2xl font-cerebriregular text-[#495057] ' >No Conversation Found</div>
                            </div>}
                ...
                <div>
                    <div className="flex gap-4 items-center justify-center ps-6 py-5 border-t-[0.5px] border-[#CDCDCD] w-full pe-8">
