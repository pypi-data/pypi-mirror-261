...
@sentry_trace
def process_actions_gpt_streaming(response_id, metadata, tools, tool_calls) -> list:
    print("Processing actions. Response Id: " + response_id)
    metadata = metadata or {}
    metadata['run_id'] = response_id
    outputs = get_tools_outputs(tools, metadata, tool_calls=tool_calls, timeout=TOOLS_TIMEOUT)
    for output in outputs:
