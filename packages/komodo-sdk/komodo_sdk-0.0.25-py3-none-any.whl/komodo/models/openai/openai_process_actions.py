...
def get_tools_output_legacy(tools, metadata, tool_calls):
    ...
    metadata['run_id'] = str(uuid.uuid4()) if 'run_id' not in metadata else metadata['run_id']
    print("Processing actions. Run Id: " + metadata['run_id'])
    outputs = get_tools_outputs(tools=tools, metadata=metadata, tool_calls=tool_calls)
    return outputs
...
def process_tool_call(tools, call, metadata):
    f = call.function
    print("Call Id: " + call.id + " Type: " + call.type + " Function: " + f.name + " Arguments: " + f.arguments)
    args = json.loads(f.arguments)
    args['metadata'] = metadata
    args['run_id'] = metadata['run_id']
    args['f.name'] = f.name

    output = "Requested function not supported by this agent. Do not retry this action."

    for tool in tools:
        if isinstance(tool, KomodoTool):
            print("Checking tool: " + str(tool.shortcode))
            if tool.shortcode == f.name:
                print("Invoking tool object: " + tool.name)
                output = tool.action(args)
                break
        elif isinstance(tool, str):
            print("Checking tool: " + str(tool))
            if tool == f.name:
                print("Invoking tool by name: " + t.name)
                t = KomodoToolRegistry.get_tool_by_shortcode(tool)
                if not t:
                    return f"Requested tool {tool} is not available"
                t.action(args)
                break
