...
def openai_chat_response_streamed(request: AssistantRequest):
    ...
    instructions = {'role': 'system', "content": request.instructions()}
    caution = {"role": "system", "content": "Do not make up fake data or hallucinate information."}
    guidance = {"role": "system", "content": "Use the tools provided to you intelligently to answer the questions."}
    history = [] if request.history is None else request.history
...
def openai_invoke_streamed(client, prompt, model=OPENAI_GPT4_MODEL, tools=None, messages=None, metadata=None,
    ...
    if tool_calls:
        outputs = process_actions_gpt_streaming(str(uuid.uuid4()), metadata, tools, tool_calls)
...
if __name__ == '__main__':
    client = openai_client()
    for response in openai_invoke_streamed(client, prompt="whats up in nyc today?", tools=["komodo_tavily_search"]):
        print(response)
