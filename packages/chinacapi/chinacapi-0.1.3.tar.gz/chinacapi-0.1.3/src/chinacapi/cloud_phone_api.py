...
from .utils import camel_key_to_snake, request
...
class CloudPhoneApi:
    def __init__(self, chinac_api: 'ChinacApi'):
        self.request = chinac_api.request

    def list_cloud_phone_product(self) -> dict:
        """
        列出云手机产品
        :return: 产品列表
        """
        res = self.request('ListCloudPhoneProduct', method='POST')
        if res['code'] != 10000:
            raise Exception(f'list cloud phone product error, code: {res["code"]}, message: {res["message"]}')
        return camel_key_to_snake(res['data'])

    def list_cloud_phone_region(self) -> List[CloudPhoneRegion]:
        """
        列出云手机地区
        :return: 地区列表
        """
        ...
        if res['code'] != 10000:
            raise Exception(f'list cloud phone product error, code: {res["code"]}, message: {res["message"]}')
        regions: List[dict] = res['data']['Vregion']
        regions: List[CloudPhoneRegion] = [CloudPhoneRegion(region['Region']) for region in regions]
        return regions

    def list_cloud_phone(self) -> List[CloudPhone]:
        """列出云手机"""
        res = self.request('ListCloudPhone', method='POST')
        if res['code'] != 10000:
    ...
    def describe_cloud_phone(self, region: CloudPhoneRegion, phone_id: str) -> CloudPhone:
        ...
        res = self.request('DescribeCloudPhone', body, method='POST')
        if res['code'] != 10000:
    ...
    def reboot_cloud_phone(self, cloud_phones: Union[CloudPhone, List[CloudPhone]], remark: str = None) -> str:
        ...
        res = self.request('RebootCloudPhone', body, method='POST')
        if res['code'] != 10000:
    ...
    def list_cloud_phone_adb_white_ip(self, region: CloudPhoneRegion) -> List[str]:
        ...
        res = self.request('ListCloudPhoneAdbWhiteIp', body, method='POST')
        if res['code'] != 10000:
    ...
    def set_cloud_phone_adb_white_ip(self, region: CloudPhoneRegion, ips: List[str]) -> str:
        ...
        res = self.request('SetCloudPhoneAdbWhiteIp', body, method='POST')
        if res['code'] != 10000:
            raise Exception(f'set cloud phone adb white ip error, code: {res["code"]}, message: {res["message"]}')
        return res['data']['TaskId']

    def _operate_cloud_phone(self, cloud_phones: Union[CloudPhone, List[CloudPhone]], operate: str, remark: str = None) -> str:
        """
        操作云手机
        :param cloud_phones: 手机列表
        :param operate: 操作类型
        ...
        res = self.request('OperateCloudPhone', body, method='POST')
        code = res['code']
        if code != 10000:
            message = res['message']
    ...
    def start_cloud_phone(self, cloud_phones: Union[CloudPhone, List[CloudPhone]], remark: str = None) -> str:
        ...
        :return: 任务ID
        """
        return self._operate_cloud_phone(cloud_phones, 'start', remark)
    ...
    def stop_cloud_phone(self, cloud_phones: Union[CloudPhone, List[CloudPhone]], remark: str = None) -> str:
        ...
        :return: 任务ID
        """
        return self._operate_cloud_phone(cloud_phones, 'stop', remark)
