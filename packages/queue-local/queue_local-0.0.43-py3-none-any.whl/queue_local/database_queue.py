...
class DatabaseQueue(OurQueue, GenericCRUD, metaclass=MetaLogger, object=queue_logger_object):
    ...
    def __init__(self, schema_name: str = "queue", table_name: str = "queue_item_table",
        ...
        """
        self.logger.start("Initializing DatabaseQueue", object={
        ...
        self.logger.end("DatabaseQueue initialized successfully", object={
            "schema_name": schema_name, "table_name": table_name,
            "view_name": view_name, "id_column_name": id_column_name})
    ...
    def push(self, data_json: dict) -> int:
        ...
        return self.insert(data_json=data_json)
    ...
    def get(self, action_ids: tuple = ()) -> dict:
        ...
        self.update_by_where(where=update_where, order_by=self.id_column_name, limit=1, data_json=data_json)
        ...
        if queue_item:
            ...
            params = (StatusEnum.RETRIEVED.value, updated_user_id, self.user_context.get_user_jwt(),
    ...
    def peek(self, action_ids: tuple = ()) -> dict:
        ...
        where = f"process_id IS NULL AND queue_status_id = {StatusEnum.NEW.value}" + \
                (f" AND action_id IN {action_ids}" if action_ids else "")
        return self.select_one_dict_by_where(where=where, order_by=self.id_column_name)
