...
__all__ = ['DimChars', 'DimType', 'DimName', 'dimchars', 'size2str', 'OTMethod', 'SDENoiseType', 'SDETypes', 'DynamicsMethod',
           ...
           'RecurrentLayer', 'SliceFormat', 'Geodesic', 'Framework']
...
class OTMethod(ModuleEnum, module = 'ot', default = 'emd'):
    ...
    def loss(
        ...
        **kwargs: P.kwargs
        ...
        if istensor(pi): pi = pi.clone().detach()
        if isarray(pi): pi = torch.tensor(pi)

        loss = torch.sum(to(pi, other=source) * to(M, other=source))
        return loss
