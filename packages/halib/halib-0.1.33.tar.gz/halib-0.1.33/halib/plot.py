...
def verify_csv(csv_file, required_columns=REQUIRED_COLUMNS):
    ...
    for col in required_columns:
        if col not in df.columns:
            raise ValueError(f"Required columns are: {REQUIRED_COLUMNS}, but found {df.columns}")
...
def get_valid_tags(csv_files, tags):
    if tags is not None and len(tags) > 0:
        ...
        assert(all(len(tag) > 0 for tag in tags)), "tags must be a list of non-empty strings"
        valid_tags = tags
    else:
        valid_tags = []
...
def actual_plot_seaborn(frame, csv_files, axes, tags, log):
    ...
    for df_item, tag in zip(ls_df, valid_tags):
        ...
        df_item.columns = [f"{tag}_{col}" if col != "epoch" else col for col in df_item.columns]
    ...
    for i, metric in enumerate(ls_metrics):
        ...
        tag2palette = {tag: palette for tag, palette in zip(valid_tags, sequential_palettes)}
        plot_colors = []
        for tag in valid_tags:
            ...
            for idx, col in enumerate(cols):
                if tag in col:
                    chosen_color = ls_colors[subarr[int(idx%num_part)].mean().astype(int)]
        ...
        axes[i].set(
            xlabel="epoch", ylabel=metric, title=title
        )
        axes[i].legend()
        axes[i].grid()
...
def actual_plot(frame, csv_files, axes, tags, log):
    ...
    for i, metric in enumerate(metric_values):
        ...
        tags_str = "+".join(valid_tags) if len(valid_tags) > 1 else valid_tags[0]
        metric_ax.set(xlabel="epoch", ylabel=metric, title=f"{tags_str}_{metric}-by-epoch")
        metric_ax.legend()
        metric_ax.grid()


def plot_csv_files(
    csv_files, outdir="./out/plot", tags=None, log=False, save_fig=False, update_in_min=1):
    ...
    if isinstance(tags, str):
        tags = [tags]
    valid_tags = get_valid_tags(csv_files, tags)
    assert len(valid_tags) == len(csv_files), "Unable to determine tags for each csv file"
...
@click.option('--outdir', '-o', type=str, help='output directory for the plot', default=str(desktop_path))
@click.option('--tags', '-t', multiple=True, type=str, help='tags for the csv files', default=[])
@click.option('--log', '-l', is_flag=True, help='log the csv files')
@click.option('--save_fig', '-s', is_flag=True, help='save the plot as a file')
@click.option('--update_in_min', '-u', type=float, help='update the plot every x minutes', default=0.0)
def main(
    csvfiles,
    outdir,
    tags,
