...
class Database:
    def __init__(self, index_dir: Optional[str] = None, content: Optional[Union[str, Tuple, List]] = None,
        ...
        if self.content is not None:
            ...
            self.TextStyles.say(message=str(f"Detected {len(self.content)} entries."), style="dim")
            self._create_index_from_content()
        elif self.index_dir is not None:
            self.TextStyles.say(message="Building Index...", style="dim")
            self.TextStyles.say(message=str(f"Directory: {self.index_dir}"), style="dim")
