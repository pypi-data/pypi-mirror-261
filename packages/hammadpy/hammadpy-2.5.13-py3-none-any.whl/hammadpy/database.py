...
class Database:
    def __init__(self, index_dir: Optional[str] = None, content: Optional[Union[str, Tuple, List]] = None,
        ...
        if self.content is not None:
            TextStyles.say("Creating Database Index from content...", color="yellow" style="dim")
            TextStyles.say(f"Detected {len(self.content)} entries.", style="dim")
            self._create_index_from_content()
        elif self.index_dir is not None:
            TextStyles.say("Building Index...", style="dim")
            TextStyles.say(f"Directory: {self.index_dir}", style="dim")
            self._use_existing_index()
        TextStyles.say("Database loaded.", color="green", style="bold")
