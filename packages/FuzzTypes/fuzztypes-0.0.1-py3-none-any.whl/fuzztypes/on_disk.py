...
class OnDiskStorage(abstract.AbstractStorage):
    ...
    def get_by_fuzz(self, key: str) -> List[Match]:
        query = f'"{self.normalize(key)}"~3'
        return self.run_query(key, vector=query)
    ...
    def run_query(self, key, where=None, vector=None) -> List[Match]:
        # query_type = "hybrid" can be passed into search()
        # which does hybrid fts and vector query
        qb = self.table.search(query=vector, vector_column_name="vector")

        # todo: how to handle hybrid? does LanceDB figure out by query type?
