...
usage: main.py [-h] [-w WHITE_LIST] [-b BLOCK_LIST] [-lv MAX_VT_IP_COUNT] [-lr MAX_RT_IP_COUNT] [-ls MAX_ST_IP_COUNT] [-p PORT] [-dr] [-dv] [-ds]
               [-o OUTPUT] [-f] [-s SPEED] [-r RTT] [-l LOSS] [-c CONFIG]
               source_file
...
options:
  ...
  -dr, --disable_rt     是否禁用RTT
  -dv, --disable_vt     是否禁用可用性
...
复制`config.ini.example` 为 `config.ini` ，将`config.ini`与`ip-check` 放到同一路径即可覆盖代码中默认配置。

`config.ini` 中所有配置都有注释，可以按需求更改
