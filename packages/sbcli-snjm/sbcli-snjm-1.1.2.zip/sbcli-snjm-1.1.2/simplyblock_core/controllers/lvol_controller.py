...
def add_lvol_ha(name, size, host_id_or_name, ha_type, pool_id_or_name, use_comp, use_crypto,
                distr_vuid, distr_ndcs, distr_npcs,
                max_rw_iops, max_rw_mbytes, max_r_mbytes, max_w_mbytes,
                distr_bs=None, distr_chunk_bs=None, with_snapshot=True, max_size=0):
    ...
    if cluster.prov_cap_crit and cluster.prov_cap_crit < size_prov:
        msg = f"Cluster provisioned cap critical, util: {size_prov}% of cluster util: {cluster.prov_cap_crit}"
        logger.error(msg)
        return False, msg

    elif cluster.prov_cap_warn and cluster.prov_cap_warn < size_prov:
        logger.warning(f"Cluster provisioned cap warning, util: {size_prov}% of cluster util: {cluster.prov_cap_warn}")
    ...
    lvol.distr_bs = distr_bs
    lvol.distr_chunk_bs = distr_chunk_bs
    lvol.distr_page_size = cluster.page_size_in_blocks
    ...
    if with_snapshot:
        ...
        lvol.bdev_stack.append({"type": "ultra_lvol", "name": lvol.top_bdev})
...
def delete_lvol_from_node(lvol, node_id):
    ...
    for bdev in lvol.bdev_stack[::-1]:
        ...
        if type == "ultra_lvol":
            ...
            if not ret:
                logger.error(f"failed to delete ultra lvol: {name}")
            continue
        if type == "alceml":
