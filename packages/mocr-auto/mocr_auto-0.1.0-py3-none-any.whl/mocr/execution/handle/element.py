...
from mocr.exceptions import ElementHandleError, NetworkError
...
class ElementHandle(JavascriptHandle):
    def __init__(
        ...
        Args:
            context (ExecutionContext): `mocr.execution.ExecutionContext` of the
                `mocr.frame.FrameManager` that spawned this.
            ...
            page (Page): The `mocr.browser.Page` the element was located in.
            frame_manager (FrameManager): The parent `mocr.frame.FrameManager`
    ...
    async def content_frame(self) -> Frame | None:
        ...
        Returns:
            Frame | None: Target `mocr.frame.Frame` if within iframe.
        """
    ...
    async def type_text(self, text: str, delay: int | float = 0) -> None:
        ...
        `ElementHandle.type_text("mocr")` will not type "MOCR" into the it.
    ...
    async def press(self, key: str, delay: int | float | None = None) -> None:
        ...
        `mocr.input.Keyboard.down` and `mocr.input.Keyboard.up` events, with
        the given `delay` in between.

        Modifier keys effect this method, meaning sending
        `mocr.input.Keyboard.press("shift")` and then
        `mocr.input.Keyboard.press("m")` will type an uppercase "M".

        Args:
            key (str): Name of key. See `mocr.input.Keyboard.key_definitions`.
    ...
    async def screenshot(
        ...
        Raises:
            ...
            ElementHandleError: Raised if element is not visible (has no
                `mocr.execution.ElementHandle.bounding_box`).
