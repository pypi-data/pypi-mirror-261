...
from mocr.frame.frame import Frame
...
class FrameManager(EventEmitter):
    def __init__(
        ...
        """
        This class is an assistant class that helps with `mocr.frame.Frame`
        management by creating them and by managing
        `mocr.execution.ExecutionContext`s.

        Args:
            client (DevtoolsConnection): `mocr.execution.ExecutionContext` of
                the `mocr.browser.Page` that spawned this.
            frame_tree (dict): A representation of the hierarchy of remote
                frames on a page.
            page (Page): The spawning `mocr.browser.Page` object.
        """
        super().__init__()
        self._client = client
        self._page = page
    ...
    def frames(self) -> list[Frame]:
        """
        A list of all `mocr.frame.Frame` objects under this manager.

        Returns:
            list[Frame]: All `mocr.frame.Frame`s controlled by this class.
        """
    ...
    def frame(self, frame_id: str) -> Frame | None:
        ...
        Returns:
            Frame | None: `mocr.frame.Frame`, if any match given `frame_id`.
        """
        return self._frames.get(frame_id)

    def execution_context_by_id(self, context_id: str) -> ExecutionContext:
        """
        Get a `mocr.execution.ExecutionContext` under this manager.
        ...
        Returns:
            ExecutionContext: `mocr.execution.ExecutionContext` created
    ...
    def create_javascript_handle(
        ...
        """
        Create a `mocr.execution.JavascriptHandle` for given context.

        Args:
            context (ExecutionContext): `mocr.execution.ExecutionContext` to
                pass into initialisation.
            remote_object (dict, optional): Remote object to be represented by
                new `mocr.execution.JavascriptHandle`. Defaults to None.
