...
if TYPE_CHECKING:
    ...
    from mocr.browser.context import BrowserContext
...
class Target():
    def __init__(
        ...
        Args:
            ...
            browser_context (BrowserContext): The `mocr.browser.BrowserContext`
                that this spawned from.
            session_factory (Callable): Callable that creates a new
                `mocr.connection.DevtoolsConnection` for this target.
            ignore_https_errors (bool): Whether network security errors should
                be ignored or not. Inherited from parent `mocr.browser.Browser`.
            ...
            screenshot_task_queue (list): The screenshot task queue, empty by
                default. Inherited from parent `mocr.browser.Browser`.
            loop (asyncio.AbstractEventLoop): The `asyncio` loop that is tracked
                under the `mocr.browser.Browser`'s `mocr.connection.Connection`.
    ...
    def browser(self) -> Browser:
        """
        The `mocr.browser.Browser` that owns the `mocr.browser.BrowserContext`
    ...
    def browser_context(self) -> BrowserContext:
        """`mocr.browser.BrowserContext` this target was initialised from."""
    ...
    async def create_devtools_connection(self) -> DevtoolsConnection:
        """
        Initialise the `mocr.connection.DevtoolsConnection` that will be bound
        to this `Target`.

        Returns:
            DevtoolsConnection: The new `mocr.connection.DevtoolsConnection`.
        """
        return await self._session_factory()

    async def page(self) -> Page | None:
        """
        Get the `mocr.browser.Page` object attached to this `Target`.
        Only applicable if `Target.kind` is "page" or "background_page".

        Returns:
            Page | None: Associated `mocr.browser.Page`, if any.
        """
        if (
