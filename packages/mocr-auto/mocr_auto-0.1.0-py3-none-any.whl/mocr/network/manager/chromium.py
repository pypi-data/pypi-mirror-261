...
from mocr.frame import FrameManager
from mocr.network.event import NetworkEventManager
from mocr.network.manager.base import NetworkManager
from mocr.network.request import Request
from mocr.network.response import Response

if TYPE_CHECKING:
    from mocr.browser.page import Page


LOGGER = logging.getLogger(__name__)


class ChromiumNetworkManager(NetworkManager):
    def __init__(
        self,
        page: Page,
        ...
        """
        Class to handle requests in a given `mocr.browser.Page`.
        ...
        `mocr.browser.Page` is shared with spawned `mocr.network.Request`
        ...
        - Events are stored via the `mocr.network.NetworkEventManager`
        ...
        Args:
            page (Page): Parent `mocr.browser.Page`.
            client (DevtoolsConnection): A `mocr.connection.DevtoolsConnection`
                spawned by the parent `mocr.browser.Page`.
            frame_manager (FrameManager): The `mocr.frame.FrameManager`
                from the `mocr.browser.Page` that spawned this element.
            interception_callback_chain (list[Callable]): A list of callbacks
                to be passed into new `mocr.network.Request`s that will be run
                during "request" event interception, by that object. Inherited
                from the parent `mocr.browser.Page`.
        """
        ...
        super().__init__(page, client, frame_manager, interception_callback_chain)
    ...
    async def _on_request_without_network_instrumentation(
        ...
        mocr_request_uuid = self._inspect_stack_for_mocr_uuid(event)
        ...
        if mocr_request_uuid:
            request._mocr_request_uuid = mocr_request_uuid
    ...
    async def _on_request(
        ...
        if event.get('redirectResponse'):
            ...
            # `mocr.network.Request` via a "NetworkManager.Request" event
        ...
        if (
            not mocr_request_uuid
            and any(r._mocr_request_uuid for r in redirect_chain)
        ):
            mocr_request_uuid = next(
                iter(r._mocr_request_uuid for r in redirect_chain)
            )
        request = Request(
            self._page,
            self._client,
    ...
    def forget_request(self, request: Request, events: bool) -> None:
        """
        Drop a request from the `mocr.network.NetworkEventManager` and
        ...
        Args:
            request (Request): `mocr.request.Request` to forget.
    ...
    def get_in_flight_requests_count(self) -> int:
        ...
        Wraps `mocr.network.NetworkEventManager.get_in_flight_requests_count`.
    ...
    async def set_request_interception(self, choice: bool) -> None:
        ...
        See `mocr.browser.Page.on` for details on using the request interception
        callback chain.
