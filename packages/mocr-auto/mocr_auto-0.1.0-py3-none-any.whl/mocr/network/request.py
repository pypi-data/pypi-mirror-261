...
from mocr.exceptions import NetworkError
from mocr.frame import Frame
...
class Request():
    def __init__(
        ...
        by a `mocr.network.NetworkManager`.
        ...
        Args:
            ...
            client (DevtoolsConnection): A `mocr.connection.DevtoolsConnection`
                spawned by the parent `mocr.network.NetworkManager`.
            ...
            frame (Frame | None): The `mocr.frame.Frame` this request originated
                from.
            ...
            interception_callback_chain (list[Callable]): A list of callbacks
                shared with the parent `mocr.network.NetworkManager` and its
                parent `mocr.browser.Page`. These callbacks wil run sequentially
                during "request" event interception. See `mocr.browser.Page` for
                more information.
            httpx_request: (httpx.Request | None, optional): If created by a
                `mocr.network.HttpDomain`, the original `httpx.Request` that
    ...
    def response(self) -> Response | None:
        """
        The `mocr.network.Response` object created from the remote response
    ...
    def frame(self) -> Frame | None:
        """The corresponding `mocr.frame.Frame`, if any."""
        return self._frame
    ...
    def httpx_request(self) -> bool:
        """
        If created by a `mocr.network.HttpDomain`, this will hold the original
    ...
    async def fulfill(
        ...
        Args:
            response (Response | None, optional): A completed
                `mocr.network.Reponse` object to extract data from. Passing
    ...
    async def abort(self, error_reason: str = "failed") -> None:
        ...
        which can cause a `mocr.exceptions.PageError` to raise later if
