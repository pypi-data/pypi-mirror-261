...
from mocr.exceptions import MocrTimeoutError
from mocr.frame import Frame, FrameManager
from mocr.utils.remote import add_event_listener, remove_event_listeners
...
class NavigationWaiter:
    def __init__(
        ...
        """
        Class to listen to a `mocr.frame.FrameManager` for specific navigation
        event(s) (`LIFECYCLE_EVENTS`) in the target `mocr.frame.Frame` for up
        to a given `timeout`. If all events not seen before timeout, raises
        a `MocrTimeoutError` exception.

        Args:
            frame_manager (FrameManager): The `mocr.frame.FrameManager` to watch
                for lifecycle events.
            frame (Frame): The `mocr.frame.Frame` that seen events count toward.
    ...
    async def _timeout_func(self) -> None:
        ...
        self._maximum_timer.set_exception(MocrTimeoutError(error_message))
