...
from mocr.constants import BROWSER_CLOSE, MOCR_VERSION, TARGET_GET_CONTEXTS
from mocr.launch import ChromiumLauncher, FirefoxLauncher
from mocr.utils import get_ws_endpoint


version = MOCR_VERSION
...
def launch(
    ...
    """
    Launch a browser process and create a `mocr.browser.Browser`.
    Wrapper for `mocr.launch.Launcher.launch`.

    Args:
        browser_type (Literal["chromium", "firefox"]): The type of browser to,
            use. One of "chromium" or "firefox". Note that Firefox is not fully
    ...
    Raises:
        ValueError: Raised if `browser_type` isn't of "chromium" or "firefox".

    Returns:
        Browser: A newly created `mocr.browser.Browser` instance.
    """
    launcher_classes = {
...
async def connect(
    ...
    Args:
        browser_type (Literal["chromium", "firefox"]): The type of browser to
            connect to. One of "chromium" or "firefox". Note that Firefox is not
    ...
    Raises:
        ValueError: Raised if `browser_type` isn't of "chromium" or "firefox" or
    ...
    if log_level is not None:
        logging.getLogger('mocr').setLevel(log_level)
