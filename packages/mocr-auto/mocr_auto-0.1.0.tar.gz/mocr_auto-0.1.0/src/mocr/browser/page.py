...
from mocr.browser.console import ConsoleMessage
from mocr.browser.viewport import ViewportManager
from mocr.browser.worker import WebWorker
from mocr.connection import DevtoolsConnection
from mocr.constants import (
    CLOSE,
    DOM_LOADED,
...
from mocr.exceptions import FirefoxNotImplementedError, PageError
from mocr.execution import ElementHandle, JavascriptHandle
from mocr.frame import Frame, FrameManager
from mocr.input import Dialog, Keyboard, Mouse, Touchscreen
from mocr.network import (
    ChromiumNetworkManager,
    FetchDomain,
...
from mocr.waiters import EventWaiter, NavigationWaiter

if TYPE_CHECKING:
    from mocr.browser import Browser
    from mocr.browser.target import Target
...
class Page(EventEmitter):
    def __init__(
        ...
        this `Page`'s `mocr.network.NetworkManager` when passed to `Page.on`.
        ...
        Args:
            browser (Browser): Parent `mocr.browser.Browser`.
            client (DevtoolsConnection): From parent `mocr.browser.Target`.
            target (Target): Spawning `mocr.browser.Target`.
            frame_tree (dict): Frame tree structure from remote connection.
            ignore_https_errors (bool): Ignore site security errors.
                Inherited from parent `mocr.browser.Browser`.
            user_agent (str): User agent for this page.
                Inherited from parent `mocr.browser.Browser`.
            screenshot_task_queue (list, optional): The screenshot task queue,
                empty by default. Inherited from parent `mocr.browser.Browser`.
    ...
    async def create(
        ...
        network_manager_classes = {
            "chromium": ChromiumNetworkManager,
    ...
    def target(self) -> Target:
        """Parent `mocr.browser.Target` this `Page` was spawned from."""
        return self._target

    @property
    def browser(self) -> Browser:
        """Parent `mocr.browser.Browser` this `Page` belongs to."""
        return self._target.browser

    @property
    def main_frame(self) -> Frame | None:
        """This `Page`'s `mocr.frame.FrameManager`'s main `mocr.frame.Frame`."""
        return self._frame_manager._main_frame

    @property
    def network_manager(self) -> NetworkManager | None:
        """This `Page`'s `mocr.network.NetworkManager`."""
        return self._network_manager

    @property
    def fetch_domain(self) -> FetchDomain:
        """This `Page`'s `mocr.fetch.FetchDomain`."""
        return self._fetch_domain

    @property
    def http_domain(self) -> HttpDomain:
        """This `Page`'s `mocr.network.HttpDomain`."""
    ...
    def keyboard(self) -> Keyboard:
        """This `Page`'s `mocr.input.Keyboard`."""
        return self._keyboard

    @property
    def mouse(self) -> Mouse:
        """This `Page`'s `mocr.input.Mouse`."""
        return self._mouse

    @property
    def touchscreen(self) -> Touchscreen:
        """This `Page`'s `mocr.input.Touchscreen`."""
        return self._touchscreen

    @property
    def frames(self) -> list[Frame]:
        """List of all `mocr.frame.Frames` within this `Page`."""
        return list(self._frame_manager.frames())

    @property
    def workers(self) -> list[WebWorker]:
        """List of all `mocr.browser.WebWorker`s within this `Page`."""
    ...
    def is_closed(self) -> bool:
        ...
        If a parent `mocr.browser.BrowserContext` is closed, all child `Page`s
        will close, too.
        """
        return self._closed
    ...
    def on(self, event: str, method: Callable | None) -> None:
        ...
        adds the method to this `Page`'s `mocr.network.NetworkManager`'s
        ...
        intercepted `mocr.network.Request` is `Request.released`, `.aborted`, or
    ...
    def make_http_domain(
        ...
        **httpx_client_kwargs,
        ...
        """
        Create a `mocr.network.HttpDomain` bound to this page.
    ...
    async def tap(self, selector: str) -> None:
        ...
        running `mocr.execution.ElementHandle.tap` on the resultant
    ...
    async def evaluate_handle(
        ...
        *args: Any,
        ...
        Runs this `Page`'s default `mocr.frame.Frame.evaluate_handle`.

        Args:
            page_function (str): JavaScript function to run.

        Raises:
            PageError: Raised if the `Page`'s main `mocr.frame.Frame` fails to
                find its `mocr.browser.ExecutionContext`.

        Returns:
            JavascriptHandle: `mocr.execution.JavascriptHandle`.
        """
        frame = self._ensure_frame()
    ...
    async def query_objects(
        ...
        """
        Query all objects with the given `mocr.execution.JavascriptHandle`'s
        `get_property("objectId")`.
        Wrapper for this `Page.main_frame`'s
        `mocr.execution.ExecutionContext.query_objects`.

        Args:
            javascript_handle (JavascriptHandle): A valid (not disposed)
                `mocr.execution.JavascriptHandle` object.
        ...
        Returns:
            JavascriptHandle: A `mocr.execution.JavascriptHandle` initialised
    ...
    async def goto(
        ...
        Raises:
            ...
            MocrTimeoutError: Raised if timeout exceeded before all expected
    ...
    async def go_back(
        ...
        Returns:
            Response | None: History `mocr.network.Response`, if any.
        """
    ...
    async def wait_for_navigation(
        ...
        yield any `mocr.network.Response` objects.
    ...
    async def wait_for_request(
        ...
        """
        Wait for a `mocr.network.Request` whose `url` matches the `url` or
        ...
        Args:
            ...
            method (Callable | None, optional): Callable to pass
                `mocr.network.Request`s emitted by `Page.network_manager` to.
                Defaults to None.
        ...
        Returns:
            Request: First matching `mocr.network.Request`.
        """
        if url:
    ...
    async def wait_for_response(
        ...
        """
        Wait for a `mocr.network.Response` whose `url` matches the `url` or
        that evaluates to True when passed to `method`.

        Args:
            url (str | None, optional): URL to wait. Defaults to None.
            method (Callable | None, optional): Callable to pass
                `mocr.network.Response`s emitted by `Page.network_manager` to.
                Defaults to None.
            timeout (int | None, optional): Time in milliseconds to wait.
                Defaults to 30000.
        ...
        Returns:
            Request: First matching `mocr.network.Response`.
        """
        if url:
    ...
    async def evaluate(
        ...
        *args: Any,
        ...
        """
        Execute a JavaScript function with given arguments.
        Runs this `Page`'s default `mocr.frame.Frame.evaluate`.
        ...
        Raises:
            `mocr.exceptions.NetworkError` if an unhandled error occurs
            either evaluating the function or requesting the resulting object.

        Returns:
            dict | None: The decoded object (dict) via
                `mocr.execution.JavascriptHandle.json` or None if a known
    ...
    async def set_request_cache(self, choice: bool = True) -> None:
        ...
        the browser, not `mocr.network.Request` objects.
    ...
    async def click(
        ...
        This method is a shortcut for running `Page.query_selector` and then
        running `mocr.execution.ElementHandle.click` on the resultant
        ElementHandle. In either case, an element will be scrolled into view
    ...
    async def type_text(
        ...
        `mocr.input.Keyboard.press("shift")` and typing
        `Page.type_text("input", "mocr")` will not type "MOCR" into the it.
    ...
    def wait_for_selector(
        ...
        Raises:
            MocrTimeoutError: Raised if timeout exceeded before element found.

        Returns:
            Awaitable[ElementHandle]: Newly created
                `mocr.execution.ElementHandle` from found object.
        """
        frame = self._ensure_frame()
