...
from mocr.exceptions import FirefoxNotImplementedError
from mocr.network.security import SecurityDetails
...
class Response():
    def __init__(
        ...
        Args:
            ...
            httpx_response: (httpx.Response | None, optional): If created by a
                `mocr.network.HttpDomain`, the original `httpx.Response` that
    ...
    def security_details(self) -> SecurityDetails | None:
        """
        Initialised `mocr.network.SecurityDetails` if the remote response
    ...
    def request(self) -> Request:
        """The `mocr.network.Request` object bound to this `Response`."""
    ...
    def buffer(self, force: bool = False) -> Awaitable[bytes]:
        ...
        Args:
            force (bool, optional): Whether to bypass checking if the body
                has been cached here. Useful for `mocr.network.FetchDomain`
