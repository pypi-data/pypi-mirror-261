...
from mocr.execution.context import EVALUATION_SCRIPT_URL
...
class NetworkManager(EventEmitter):
    ...
    async def create(
        ...
        Args:
            ...
            frame_manager (FrameManager): A `mocr.frame.FrameManager`
                spawned by the parent `mocr.browser.Page`.
            ignore_https_errors (bool): Ignore site security errors.
                Inherited from parent `mocr.browser.Page`.
            interception_callback_chain (list[Callable]): A list of callables
                for use with "request" event interception. This list is
                shared by the parent `mocr.browser.Page` and all newly created
                `mocr.network.Request` objects in this manager.
    ...
    def _inspect_stack_for_mocr_uuid(self, event: dict) -> str | None:
        mocr_request_uuid = None
        ...
        if call_frames:
            ...
            if EVALUATION_SCRIPT_URL in url:
                mocr_request_uuid = url.replace(EVALUATION_SCRIPT_URL, "")
        return mocr_request_uuid
