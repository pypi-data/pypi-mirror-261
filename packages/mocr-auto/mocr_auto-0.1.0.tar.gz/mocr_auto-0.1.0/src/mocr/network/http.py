...
from mocr.constants import HTTP_METHODS
...
class HttpDomain():
    def __init__(
        ...
        **httpx_client_kwargs,
        ...
        state between itself and the parent `mocr.browser.Page`.

        This client uses `httpx` so requests can be made with HTTP2.

        Args:
            page (Page): Parent `mocr.browser.Page` that this will sync with.
            sync_cookies (Literal["both", "http", "page", "none"], optional):
                Whether to sync cookies between this client and the parent
                `mocr.browser.Page` before/after each request and response.
                Defaults to "both".
        """
        self._page = page
    ...
    async def _http_to_mocr(self, response: httpx.Response) -> Request:
    ...
    async def _transform_http_objs(self, response: httpx.Response) -> Response:
        ...
        requests = [(await self._http_to_mocr(r)) for r in http_redirect_chain]
        if len(requests) > 1:
