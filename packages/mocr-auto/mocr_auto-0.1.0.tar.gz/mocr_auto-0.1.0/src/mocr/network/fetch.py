...
from mocr.exceptions import MocrTimeoutError, NetworkError, PageError
from mocr.execution.handle.javascript import JavascriptHandle
from mocr.network.request import Request
from mocr.network.response import Response
from mocr.waiters import EventWaiter
...
class FetchDomain(EventEmitter):
    def __init__(
        ...
        Chromium requests take advantage of the existing fetch system in its
        `mocr.network.NetworkManager` while Firefox cannot; Chromium could
        do the same as Firefox does here and build the objects directly but
        some features would be lost like redirect chain.

        Args:
            page (Page): Parent `mocr.browser.Page` this was spawned from.
        """
        super().__init__()
        self._page = page
    ...
    async def _request_has_response(self, request: Request) -> Response:
        if not request._mocr_request_uuid:
            return
        while True:
            done = False
    ...
    def _select_response_chromium(self, request_uuid: str) -> Response:
        responses = {
            request_id: response for request_id, response in
            self._request_id_to_response.items()
            if response and response._request._mocr_request_uuid == request_uuid
        }
    ...
    def _make_response_firefox(self, response_data: dict) -> Response:
        ...
        mocr_response._request = mocr_request
        mocr_response._content = response_data["body"]
        mocr_request._response = mocr_response
        return mocr_response

    def _select_response(self, request_uuid: str, data: dict) -> Response:
        if self._is_firefox:
            return self._make_response_firefox(data)
        else:
            return self._select_response_chromium(request_uuid)

    def _check_for_response_data(self, task: asyncio.Task, params: dict) -> dict | None:
        ...
        if error and isinstance(error, _FirefoxResponseReady):
            ...
            raise MocrTimeoutError("Timeout waiting for fetch to resolve.")
