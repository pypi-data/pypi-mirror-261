...
class NetworkEventManager():
    def __init__(self) -> None:
        ...
        `mocr.network.NetworkManager` to track active event status as events can
        be received in multiple different orders.

        See `mocr.network.NetworkManager` for more information.
        """
    ...
    def get_request(self, network_id: str) -> Request:
        """
        Get `mocr.network.Request` stored under given `network_id`.

        Args:
            network_id (str): Request network identifier.

        Returns:
            dict: The stored `mocr.network.Request` under given `network_id`.
        """
        return self.http_requests_map.get(network_id)

    def store_request(self, network_id: str, request: Request) -> None:
        """
        Store a `mocr.network.Request` under given `network_id`.

        Args:
            network_id (str): Request network identifier.
            request (Request): The `mocr.network.Request` received.
        """
        self.http_requests_map[network_id] = request

    def forget_request(self, network_id: str) -> None:
        """
        Remove `mocr.network.Request` stored under given `network_id`.
