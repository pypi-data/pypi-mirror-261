...
from mocr.constants import INPUT_INSERT_TEXT, INPUT_KEY, KEY_DEFINITIONS
...
class Keyboard():
    ...
    async def send_character(self, char: str) -> None:
        """
        Send a character `char` into the parent `mocr.frame.Frame`. Does not
    ...
    async def type_text(self, text: str, delay: int | float = 0) -> None:
        ...
        `Keyboard.down("shift")` and typing `Keyboard.type_text("mocr")`
        will not type "MOCR".
