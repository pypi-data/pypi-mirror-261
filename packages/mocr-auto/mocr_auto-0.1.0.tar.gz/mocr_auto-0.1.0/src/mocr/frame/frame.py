...
from mocr.exceptions import ElementHandleError, PageError
from mocr.execution.context import ExecutionContext, JavascriptHandle
from mocr.execution.handle.element import ElementHandle
from mocr.frame.wait import WaitTask
...
class Frame():
    def __init__(
        ...
        `mocr.frame.FrameManager` in a `mocr.browser.Page`.

        Args:
            client (DevtoolsConnection): `mocr.execution.ExecutionContext` of
                the `mocr.frame.FrameManager` that spawned this.
    ...
    async def execution_context(self) -> ExecutionContext | None:
        """
        Return the newly created `mocr.execution.ExecutionContext`.
        Created by `mocr.frame.FrameManager`.

        Returns:
            ExecutionContext: `mocr.execution.ExecutionContext`.
        """
    ...
    async def evaluate_handle(
        ...
        *args: Any,
        ...
        Runs this `Frame`'s `mocr.execution.ExecutionContext.evaluate_handle`.
    ...
    async def evaluate(
        ...
        *args: Any,
        ...
        """
        Execute a JavaScript function with given arguments.
        Runs this `Frame`'s `mocr.execution.ExecutionContext.evaluate`.
    ...
    async def type_text(
        ...
        `Frame.type_text("input", "mocr")` will not type "MOCR" into the it.
