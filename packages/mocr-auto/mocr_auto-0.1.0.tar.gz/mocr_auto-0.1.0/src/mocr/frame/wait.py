...
from mocr.constants import METHOD_WAIT_FOR_PREDICATE_PAGE
from mocr.exceptions import MocrTimeoutError, NetworkError, PageError
from mocr.utils.remote import is_javascript_method
...
class WaitTask():
    def __init__(
        ...
        *args: Any,
        ...
        Args:
            frame (Frame): `mocr.frame.Frame` that spawned this `WaitTask`.
    ...
    async def rerun(self) -> None:
        ...
        Raises:
            PageError: Raised if no `mocr.execution.ExecutionContext` attached
                to the parent `mocr.frame.Frame`.
        """
