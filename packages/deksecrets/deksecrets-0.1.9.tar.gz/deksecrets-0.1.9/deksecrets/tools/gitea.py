...
class Gitea:
    ...
    def fetch_data(self, org, project, version, environment):
        ...
        remove_path(file)
        data_base = yaml.load(os.path.join(path_out, f"_base{self.data_ext}"), default={})
        empty = object()
        data = yaml.load(os.path.join(path_out, f"{environment}{self.data_ext}"), default=empty)
        remove_path(path_out)
        if data is empty:
            return {}
        return assign(data_base, data or {})

    def fetch_all(self, path, org):
...
class GiteaManager:
    ...
    def fetch_all(self, org):
        self.gitea.fetch_all(self.cfg.path_dir, org)
