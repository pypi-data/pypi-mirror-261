...
from dektools.file import read_chunked
from deknetreq.request.download import download_url
...
def get_file_url(ins: Gitea, org_name, project_name, version, file):
    ...
    url = f"{ins.url}/api/packages/{org_name}/generic/{project_name}/{version}/index{os.path.splitext(file)[-1]}"
...
def fetch_file(ins: Gitea, org_name, project_name, version, file):
    download_url(get_file_url(ins, org_name, project_name, version, file), file)
