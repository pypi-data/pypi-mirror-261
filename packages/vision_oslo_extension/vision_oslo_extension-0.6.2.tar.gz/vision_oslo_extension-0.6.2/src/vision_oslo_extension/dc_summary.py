            ...
            if sheet_name == 'Start':
                ...
                if not option == "8": # do not check option 8 as the format is very different
                    for index, oslo in enumerate(oslolist):
                        if pd.isna(oslo):
                            print('ERROR: Items {} is not assigned with OSLO ID'.format(start_df.iloc[index,0]))
                            flag = True
...
def check_reading_frame(sheet,option):
    ...
    if sheet.cell(row=index, column=column).value is not None:
        ...
        print("ERROR: Wrong data format. No information at B12")
        return False
    ...
    if sheet.cell(row=index, column=column).value is not None:
        ...
        print("ERROR: Wrong data format. No information at Outage List Row 12")
        return False
