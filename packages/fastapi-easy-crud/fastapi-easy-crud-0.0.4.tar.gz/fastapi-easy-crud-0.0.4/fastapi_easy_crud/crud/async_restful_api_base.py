...
CrudInstanceType = TypeVar("CrudInstanceType", bound=AsyncCRUDBase)
...
class AsyncBaseAPI(
    ...
    def init_router(self, router: APIRouter) -> None:
        ...
        async def get_by_id(
            id: int, db: AsyncSession = Depends(deps.get_async_db)
        ) -> Any:
        ...
        async def delete(id: int, db: AsyncSession = Depends(deps.get_async_db)) -> Any:
