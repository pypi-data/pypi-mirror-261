...
from PySide6.QtGui import *
import pcell, scolor, youtil, jfinder, ganada, han
...
class codes_for_jfinder_gui:
	#jfinder의 gui를 만들때 사용하는 자료
	#자주 사용하는 코드를 만들어서 실행하기 하는것
	well_used_code = {}
	menu_n_menual = {}

	common_dic = {}
	common_dic["excel_codes"] = {}
	common_dic["my_codes"] = {}
	common_dic["word_codes"] = {}
	common_dic["xython_recommend_codes"] = {}
	common_dic["hwp_codes"] = {}

	common_dic["excel_codes"]["빈셀 위의것으로 채워넣기"] = """
	# -*- coding: utf-8 -*-
	import pcell, jfinder
	excel = pcell.pcell()
	jf = jfinder.jfinder()
	
	xyxy = excel.read_address_in_selection()
	print(xyxy)
	excel.write_uppercell_value_in_emptycell_in_range('', xyxy)
	"""

	common_dic["excel_codes"]["연속된 같은것 삭제"] = """
	# -*- coding: utf-8 -*-
	import pcell, jfinder
	excel = pcell.pcell()
	jf = jfinder.jfinder()
	xyxy = excel.read_address_in_selection()
	print(xyxy)
	excel.delete_continious_samevalue_in_range('', xyxy)
	"""
	common_dic["excel_codes"]["폴더안의 모든 화일앞에 문자추가"] = """
	# -*- coding: utf-8 -*-
	import pcell, jfinder
	excel = pcell.pcell()
	jf = jfinder.jfinder()
	#실행이 않될경우 \ ==> \ \로 변경해 보세요
	folder_name = "D:\my_code\py_files"  #폴더위치
	add_text = "aaa"  #앞에 추가할 단어
	
	aaa = util.read_all_filename_in_folder(folder_name)
	
	for one in aaa:
			full_path_old = folder_name +"\\"+one
			full_path_new = folder_name +"\\"+add_text+one
			util.change_filename(full_path_old, full_path_new)
	"""

	common_dic["excel_codes"]["폴더안의 모든 화일앞에서부터 글자수 삭제"] = """
	# -*- coding: utf-8 -*-
	import pcell, jfinder
	excel = pcell.pcell()
	jf = jfinder.jfinder()
	#실행이 않될경우 \ ==> \ \로 변경해 보세요
	folder_name = "D:\my_code\py_files" #폴더위치
	del_len = 3 #삭제하고 싶은 갯수
	
	aaa = util.read_all_filename_in_folder(folder_name)
	
	for one in aaa:
			full_path_old = folder_name +"\\"+one
			full_path_new = folder_name +"\\"+one[del_len:]
			util.change_filename(full_path_old, full_path_new)
	"""

	common_dic["excel_codes"]["선택영역에서 전체가 빈 모든 x열 삭제"] = """
	# -*- coding: utf-8 -*-
	import pcell, jfinder
	excel = pcell.pcell()
	jf = jfinder.jfinder()
	#전체가 빈 가로열들 삭제 
	xyxy = [1,1,10,10]
	excel.delete_empty_xline_in_range("",xyxy)
	"""

	common_dic["excel_codes"]["선택영역에서 전체가 빈 모든 y열 삭제"] = """
	# -*- coding: utf-8 -*-
	import pcell, jfinder
	excel = pcell.pcell()
	jf = jfinder.jfinder()
	#전체가 빈 세로열들 삭제 
	xyxy = [1,1,10,10]
	excel.delete_empty_yline_in_range("",xyxy)
	"""


	common_dic["my_codes"]["괄호안의 자료 찾기-선택영역"] = '''
	read_2d_data = excel.read_value_in_selection()
	excel.new_sheet()
	
	for index, list_1d in enumerate(read_2d_data):
		excel.write_value_in_cell("",[index+1, 1], list_1d[0] )
		result = jf.search_between_brackets_by_jf_sql(list_1d[0])
		changed_data = util.change_list_2d_to_list_1d(result)
		excel.write_list_1d_from_cell("", [index+1, 2], changed_data)
	'''

	common_dic["my_codes"]["중괄호와 안의 내용을 찾기-선택영역"] = '''
	read_2d_data = excel.read_value_in_selection()
	excel.new_sheet()
	
	for index, list_1d in enumerate(read_2d_data):
		excel.write_value_in_cell("",[index+1, 1], list_1d[0] )
		result = jf.search_between_word1_word2_by_jf_sql(list_1d[0], "[", "]")
		changed_data = util.change_list_2d_to_list_1d(result)
		excel.write_list_1d_from_cell("", [index+1, 2], changed_data)
	'''

	common_dic["my_codes"]["공백숫자영어가 1~10개까지 있는 것 찾기-선택영역"] = '''
	read_2d_data = excel.read_value_in_selection()
	excel.new_sheet()
	
	jf_sql = "[공백&숫자&영어:1~10]"
	for index, list_1d in enumerate(read_2d_data):
		excel.write_value_in_cell("",[index+1, 1], list_1d[0] )
		result = jf.search_all_by_jf_sql(jf_sql, list_1d[0])
		changed_data = util.change_list_2d_to_list_1d(result)
		excel.write_list_1d_from_cell("", [index+1, 2], changed_data)
	'''

	common_dic["my_codes"]["원하는 정규표현식 실행하기-선택영역"] = '''
	read_2d_data = excel.read_value_in_selection()
	excel.new_sheet()
	
	jf_sql = "여기에 jf-sql형식의 글을 넣으세요"
	for index, list_1d in enumerate(read_2d_data):
		excel.write_value_in_cell("",[index+1, 1], list_1d[0] )
		result = jf.search_all_by_jf_sql(jf_sql, list_1d[0])
		changed_data = util.change_list_2d_to_list_1d(result)
	...
	common_dic["my_codes"]["가로세로 자료위치 바꾸기"] = """
	read_2d_data = excel.read_value_in_selection()
	excel.new_sheet()
	changed_data = util.change_xylist_to_yxlist(read_2d_data)
	excel.write_value_in_range_priority_by_value("", [1,1], changed_data)
	"""

	common_dic["my_codes"]["1줄씩 가로줄 삽입하기"] = """
	line_no = 1
	excel.insert_xline_in_range_by_step("", "", line_no)
	"""

	common_dic["my_codes"]["빈 가로줄 삽제하기"] = """
	excel.delete_empty_xline_in_range("", "")
	"""

	common_dic["my_codes"]["대문자로 바꾸기"] = """
	[x1, y1, x2, y2] = excel.read_address_in_selection()
	
	for x in range(x1, x2+1):
		for y in range(y1, y2+1):
			try:
				value = excel.read_value_in_cell("", [x, y])
				excel.write_value_in_cell("",[x, y],value.upper())
			except:
				pass
	"""

	common_dic["my_codes"]["소문자로 바꾸기"] = """
	[x1, y1, x2, y2] = excel.read_address_in_selection()
	...
	for x in range(x1, x2+1):
		for y in range(y1, y2+1):
			try:
				value = excel.read_value_in_cell("", [x, y])
				excel.write_value_in_cell("",[x, y],value.lower())
			except:
				pass
	"""


	common_dic["xython_recommend_codes"]["빈셀 위의것으로 채워넣기"] = """
	...
	common_dic["xython_recommend_codes"]["연속된 같은것 삭제"] = """
	xyxy = excel.read_address_in_selection()
	print(xyxy)
	excel.delete_continious_samevalue_in_range('', xyxy)
	"""
	common_dic["xython_recommend_codes"]["폴더안의 모든 화일앞에 문자추가"] = """
	#실행이 않될경우 \ ==> \ \로 변경해 보세요
	folder_name = "D:\my_code\py_files"  #폴더위치
	add_text = "aaa"  #앞에 추가할 단어
	...
	common_dic["xython_recommend_codes"]["폴더안의 모든 화일앞에서부터 글자수 삭제"] = """
	#실행이 않될경우 \ ==> \ \로 변경해 보세요
	folder_name = "D:\my_code\py_files" #폴더위치
	del_len = 3 #삭제하고 싶은 갯수
	...
	common_dic["xython_recommend_codes"]["선택영역에서 전체가 빈 모든 x열 삭제"] = """
	#전체가 빈 가로열들 삭제 
	xyxy = [1,1,10,10]
	excel.delete_empty_xline_in_range("",xyxy)
	"""

	common_dic["xython_recommend_codes"]["선택영역에서 전체가 빈 모든 y열 삭제"] = """
	#전체가 빈 세로열들 삭제 
	xyxy = [1,1,10,10]
	excel.delete_empty_yline_in_range("",xyxy)
	"""

class sample_codes_for_boribori:
	def code_excel(self):
		sample_code_excel = {}

		sample_code_excel["셀(2,2)에 값넣기"] = """
excel.write_value_in_cell("", [2, 2], "테스트입니다")
"""

		sample_code_excel["셀(2,3)에 빨강색 넣기"] = """
excel.paint_scolor_in_cell("", [2, 3], "red")
"""
		return sample_code_excel

	def code_word(self):
		sample_code_word = {}

		sample_code_word["워드에 값넣기"] = """
excel.write_value_in_cell("", [2, 2], "테스트입니다")
"""

		sample_code_word["워드에 빨강색 넣기"] = """
excel.paint_scolor_in_cell("", [2, 3], "red")
"""
		return sample_code_word

	def code_well_used(self):
		well_used_code = {}

		well_used_code["선택영역에서 전체가 빈 모든 x열 삭제"] = """
#전체가 빈 가로열들 삭제 
xyxy = [1,1,10,10]
excel.delete_empty_xline_in_range("",xyxy)
"""

		well_used_code["선택영역에서 전체가 빈 모든 y열 삭제"] = """
#전체가 빈 세로열들 삭제 
xyxy = [1,1,10,10]
excel.delete_empty_yline_in_range("",xyxy)
"""
		return well_used_code

class sample_codes_for_boribori_01:
	def __init__(self):
		self.result = {}
		self.menu_n_menual = {}

		self.result["빈셀 위의것으로 채워넣기"] = """
		...
		self.result["연속된 같은것 삭제"] = """
		xyxy = excel.read_address_in_selection()
		print(xyxy)
		excel.delete_continious_samevalue_in_range('', xyxy)
		"""
		self.result["폴더안의 모든 화일앞에 문자추가"] = """
		#실행이 않될경우 \ ==> \ \로 변경해 보세요
		folder_name = "D:\my_code\py_files"  #폴더위치
		add_text = "aaa"  #앞에 추가할 단어
		...
		self.result["폴더안의 모든 화일앞에서부터 글자수 삭제"] = """
		#실행이 않될경우 \ ==> \ \로 변경해 보세요
		folder_name = "D:\my_code\py_files" #폴더위치
		del_len = 3 #삭제하고 싶은 갯수
		...
		self.result["선택영역에서 전체가 빈 모든 x열 삭제"] = """
		#전체가 빈 가로열들 삭제 
		xyxy = [1,1,10,10]
		excel.delete_empty_xline_in_range("",xyxy)
		"""

		self.result["선택영역에서 전체가 빈 모든 y열 삭제"] = """
		#전체가 빈 세로열들 삭제 
		xyxy = [1,1,10,10]
		excel.delete_empty_yline_in_range("",xyxy)
		"""

		self.menu_n_menual["Tab 설명들"] = """
		* Pcell/Easy Use
		 - Pcell을 쉽게 사용하자 => 양복의 3개 단추처엄, 3단계로 엑셀의 기능을 실행시킨다는 뜻

		* xython files
		 - xython 추천 화일들 => 이 프로그램을 만든곳에서 추천하는 python화일들
		 - 위치 : boribori.exe의 위치=>xython_py_folder에 저장된것을 실행시킨다
		 - xython.co.kr에 zip화일로 별도로 최신의 자료를 관리할 예정입니다

		* User Files
		 - 사용자 작성 화일들 => 자신이 자주 사용하는 python화일을 다시 사용하기위하여 조장해 놓은것
		 - 위치 : boribori.exe의 위치=>user_py_folder

		* Respect 3rd Files
		 - 사용자 작성 화일(입력가능) => 입력형식이 가능하도록 만든 python화일로, 입력후 실행
		 - 위치 : boribori.exe의 위치=>respect_3rd_py_folder

		* Manual Run
		 - Pcell 코드 직접 실행 => 코드를 직접 타이핑해서 실행 시키는것, 별도의 설치없이도 사용가능

		* How to Use
		 - Manual => 사용자 메뉴얼

		"""
		self.menu_n_menual["기본 설명들"] = """
		이 실행화일이 있는 폴더안의 user_folder안의 파이썬 화일을 실행 시키는 것이다
		입력받으면서 실행될 부분은 별도로 처리하여야 합니다. 단 외부 코드를 실행시키기 위해서는

		 - python 3.8 이후 버전설치
		 - pywin32 설치 - xython 모듈 설치

		설정 : 현재 이 실행화일이 위치한곳을 기준으로
		사용자화일들 위치 => ./user_py_folder 밑에 저장
		기타 화일들 위치 => ./xython_py_folder 밑에 저장
		만드는 화일들 위치 => ./bori_py_folder 밑에 저장

		엑셀에 어떤 기능을 실행하는 방법으로
		이미 만들어 놓은 메뉴들을 이용하여,  
		3단계로 버튼을 누르듯이 메소드를 선택한후
		입력이 필요한 경우는 상단의 입력창에 
		필요 인자를 넣은후 "실행"버튼을 누르면
		관련된 메소드가 실행되는 것입니다	"""

		self.menu_n_menual["xython패키지의 모듈이름들"] = """
		** 화일 이름의 의미

		화일 이름을 만들때, 가족들의 약어를 기준으로 이름을 연결해서 지었다
		Pcell은 박씨성의 내이름을 기준으로 => excel로 객체를 만듦
		Scolor는 아들의 이름중에 한글자를 => color로 객체를 만듦
		Jfinder는 딸의 이름중에서 한글자를 => finder로 객체를 만듦
		Youtil은 집사람의 이름에서 한글자를 => util로 객체를 만듦
		Pynal은 final과 같은 발음으로 만듦  => nal로 객체를 만듦
		그리고 최종으로 Bori라는 이름은 우리딸이 지은 우리집 강아지 이름입니다"""

		self.menu_n_menual["동사들 설명"] = """
		read : 눈에 보이는 값을 읽을 때
		get : 입력값이 없는 조건에서의 자료를 갖고올때, read를 제외한 모든 자료를 갖고올 때	

		write : 눈에 보이는 값을 쓸 때, 기존값을 대치하면서 쓰기
		set : write를 제외한 모든 자료를 쓸때
		append 새로운 같은 형태를 추가하는것
		...
		self.menu_n_menual["좌표 용어설명"] = """
		x, y : cell의 기본좌표, X: 가로줄번호, Y: 세로줄 번호, [x,y]	
		xx, yy : [x, x2], [y, y2]
		xyxy : 셀의 영역에 대한 좌표 	[x,y,x2,y2]
		tx : cell의 번호에 대한것, T : table의 약어, table에서 몇번째 셀이라는 쯧	

		cpx : 셀의 좌표번호, C : Cell의 약어, P : pixel의 약어
		px : 좌표, 그림을 그리고 싶은 좌표, P : pixel의 약어, 커서의 픽셀 좌표	
		pwh : 넓이, 높이의 길이를 픽셀단위로 나타낸것	
		dpx, dpy : 번호, 두지점간의 차이, D : Difference 또는 Delta
		mpx, mpy : 마우스의 위치, M : mouse의 약어

		tbl_px : 테이블의 x	
		win_px : 윈도우의 x	
		sel_px : 셀렉션된 영역	
		curr_px : 키보드등으로 움직이는 현재의 셀	

		px, py, px1, py1 : 컴퓨터안에서의 영역	

		stx1, sty1, stx3, sty3 : 현재 선택된 table의 셀번호 (select), 여러테이블 자료를 다룰 때
		"""

		self.menu_n_menual["메소드의 3단계 구성"] = """
		메소드는 대부분 총 3단계로 만들었읍니다

			- 1단계(액션) - 2단계(영역이나 대상물) - 3단계(실행항목)
			- 입력폼에 변수값들을 입력
			- 실행 버튼을 누른다

		오른쪽 아랫부분의 입력폼은 변수들을 넣는 부분으로 입력필요하고 된 부분은 꼭 입력을 해야 
		실행이 되는 부분입니다, 입력후 '실행'버튼을 누르면 실행이 됩니다
			- 기본값 : 입력변수의 기본 설정 값으로 들어가는 것
			- sheet_name의 기본값 : 현재 활성화된 시트
		"""

	def get_well_used_codes(self):
		return self.result

	def get_menu_n_menual(self):
		return self.menu_n_menual

class main_menu_for_boribori:
	def make_menu(self):
		self.menu = {}
		self.tree_menu = {}
		self.title_menu = {}
...
class main_menu_for_boribori_01:
	def make_menu(self):
		self.menu = {}
		self.tree_menu = {}
		self.title_menu = {}
		...
		self.menu["add_text_in_range_at_left"] = {'pcell_method_name': 'add_text_in_range_at_left',
												  'params': {'sheet_name': '', 'xyxy': '', 'input_text': "'입력필요'"},
												  'manual': ' 선택한 영역의 왼쪽에 입력한 글자를 추가 ', '1st': 'add', '2nd': 'text',
												  '3rd': 'in_range_at_left', 'folder': '', 'filename': ''}
		self.menu["add_text_in_range_at_right"] = {'pcell_method_name': 'add_text_in_range_at_right',
												   'params': {'sheet_name': '', 'xyxy': '', 'input_text': "'입력필요'"},
												   'manual': ' 선택한 영역의 오른쪽에 입력한 글자를 추가 ', '1st': 'add', '2nd': 'text',
												   '3rd': 'in_range_at_right', 'folder': '', 'filename': ''}
		self.menu["add_text_in_range_by_step"] = {'pcell_method_name': 'add_text_in_range_by_step',
												  'params': {'sheet_name': '', 'xyxy': '', 'input_text': '',
															 'step': ''}, 'manual': ' 선택한 영역의 시작점부터 n번째 셀마다 값을 넣기 ',
												  '1st': 'add', '2nd': 'text', '3rd': 'in_range_by_step', 'folder': '',
												  'filename': ''}
		self.menu["add_text_in_range_by_xystep"] = {'pcell_method_name': 'add_text_in_range_by_xystep',
													'params': {'sheet_name': '', 'xyxy': '', 'input_text': '',
															   'xystep': '[1, 1]'},
													'manual': ' 선택한 영역의 시작점부터 x,y 번째 셀마다 값을 넣기 ', '1st': 'add',
													'2nd': 'text', '3rd': 'in_range_by_xystep', 'folder': '',
													'filename': ''}
		self.menu["change_last_char_only_in_range"] = {'pcell_method_name': 'change_last_char_only_in_range',
													   'params': {'sheet_name': '', 'xyxy': '', 'input_2d_list': '[]'},
													   'manual': ' 엑셀값중, 맨앞의 글자만 변경하는 것 사용법 : change_first_char("", [1,1,100,1], [["\'", ""], ["*", ""], [" ", ""],]) ',
													   '1st': 'change', '2nd': 'last', '3rd': 'char_only_in_range',
													   'folder': '', 'filename': ''}
		self.menu["change_value_in_range_as_lower"] = {'pcell_method_name': 'change_value_in_range_as_lower',
													   'params': {'sheet_name': '', 'xyxy': ''},
													   'manual': ' 선택영역안의 모든글자를 소문자로 만들어 주는것 screen update를 off로 사용 값 읽어오는것을 range형태로 변경 ',
													   '1st': 'change', '2nd': 'value', '3rd': 'in_range_as_lower',
													   'folder': '', 'filename': ''}
		self.menu["change_value_in_range_to_capital"] = {'pcell_method_name': 'change_value_in_range_to_capital',
														 'params': {'sheet_name': None, 'xyxy': None},
														 'manual': ' change_text_as_capital(sheet_name, xyxy) 선택한 영역의 값들을 첫글자만 대문자로 변경 입력값 : 입력값없이 사용가능 ',
														 '1st': 'change', '2nd': 'value', '3rd': 'in_range_to_capital',
														 'folder': '', 'filename': ''}
		self.menu["change_value_in_range_to_lower"] = {'pcell_method_name': 'change_value_in_range_to_lower',
													   'params': {'sheet_name': '', 'xyxy': ''},
													   'manual': ' 선택영역안의 모든글자를 소문자로 만들어 주는것 ', '1st': 'change',
													   '2nd': 'value', '3rd': 'in_range_to_lower', 'folder': '',
													   'filename': ''}
		self.menu["change_value_in_range_to_ltrim"] = {'pcell_method_name': 'change_value_in_range_to_ltrim',
													   'params': {'sheet_name': '', 'xyxy': ''},
													   'manual': ' 왼쪽끝의 공백을 삭제하는 것 ', '1st': 'change', '2nd': 'value',
													   '3rd': 'in_range_to_ltrim', 'folder': '', 'filename': ''}
		self.menu["change_value_in_range_to_rtrim"] = {'pcell_method_name': 'change_value_in_range_to_rtrim',
													   'params': {'sheet_name': '', 'xyxy': '',
																  'scolor_name': "'yel+'"},
													   'manual': ' 선택한 영역의 셀 값들의 오른쪽 공백을 없앤것 ', '1st': 'change',
													   '2nd': 'value', '3rd': 'in_range_to_rtrim', 'folder': '',
													   'filename': ''}
		self.menu["change_value_in_range_to_swapcase"] = {'pcell_method_name': 'change_value_in_range_to_swapcase',
														  'params': {'sheet_name': '', 'xyxy': ''},
														  'manual': ' 선택한 영역의 값들을 대소문자를 변경 ', '1st': 'change',
														  '2nd': 'value', '3rd': 'in_range_to_swapcase', 'folder': '',
														  'filename': ''}
		self.menu["change_value_in_range_to_trim"] = {'pcell_method_name': 'change_value_in_range_to_trim',
													  'params': {'sheet_name': '', 'xyxy': '', 'scolor_name': "'yel'"},
													  'manual': ' 왼쪽끝의 공백을 삭제하는 것 ', '1st': 'change', '2nd': 'value',
													  '3rd': 'in_range_to_trim', 'folder': '', 'filename': ''}
		self.menu["change_value_in_range_to_underline"] = {'pcell_method_name': 'change_value_in_range_to_underline',
														   'params': {'sheet_name': '', 'xyxy': ''},
														   'manual': ' 선택한 영역의 셀 값들에 밑줄을 넣는 것 ', '1st': 'change',
														   '2nd': 'value', '3rd': 'in_range_to_underline', 'folder': '',
														   'filename': ''}
		self.menu["change_value_in_range_to_upper"] = {'pcell_method_name': 'change_value_in_range_to_upper',
													   'params': {'sheet_name': '', 'xyxy': ''},
													   'manual': ' 선택한 영역의 값들을 대문자로 변경 입력값 : 입력값없이 사용가능 ',
													   '1st': 'change', '2nd': 'value', '3rd': 'in_range_to_upper',
													   'folder': '', 'filename': ''}
		self.menu["check_sheet_password"] = {'pcell_method_name': 'check_sheet_password',
											 'params': {'isnum': "'yes'", 'istext_small': "'yes'",
														'istext_big': "'yes'", 'isspecial': "'no'", 'len_num': '10'},
											 'manual': ' 시트의 암호를 찾아주는것 ', '1st': 'check', '2nd': 'sheet',
											 '3rd': 'password', 'folder': '', 'filename': ''}
		self.menu["delete_all_draw_line_in_range"] = {'pcell_method_name': 'delete_all_draw_line_in_range',
													  'params': {'sheet_name': '', 'xyxy': ''},
													  'manual': ' 시트의 모든 라인을 지우는 것 ', '1st': 'delete', '2nd': 'all',
													  '3rd': 'draw_line_in_range', 'folder': '', 'filename': ''}
		self.menu["delete_all_drawing_in_sheet"] = {'pcell_method_name': 'delete_all_drawing_in_sheet',
													'params': {'sheet_name': ''}, 'manual': ' 시트안의 모든 객체를 삭제하는 것 ',
													'1st': 'delete', '2nd': 'all', '3rd': 'drawing_in_sheet',
													'folder': '', 'filename': ''}
		self.menu["delete_all_line_in_range"] = {'pcell_method_name': 'delete_all_line_in_range',
												 'params': {'sheet_name': '', 'xyxy': ''}, 'manual': ' 영역의 모든선을 지운다 ',
												 '1st': 'delete', '2nd': 'all', '3rd': 'line_in_range', 'folder': '',
												 'filename': ''}
		self.menu["delete_all_samevalue_in_range"] = {'pcell_method_name': 'delete_all_samevalue_in_range',
													  'params': {'sheet_name': '', 'xyxy': ''},
													  'manual': ' 영역안에서 같은것이 있으면 모두 지우고, 고유한것만 남기는것 2개가 같으면 2개모두 지우는 것이다 ',
													  '1st': 'delete', '2nd': 'all', '3rd': 'samevalue_in_range',
													  'folder': '', 'filename': ''}
		self.menu["delete_continious_samevalue_in_range"] = {
			'pcell_method_name': 'delete_continious_samevalue_in_range', 'params': {'sheet_name': '', 'xyxy': ''},
			'manual': ' 대상 : 선택한 영역 밑으로 같은 값들이 있으면 지우는것 ', '1st': 'delete', '2nd': 'continious',
			'3rd': 'samevalue_in_range', 'folder': '', 'filename': ''}
		self.menu["delete_link_in_range"] = {'pcell_method_name': 'delete_link_in_range',
											 'params': {'sheet_name': '', 'xyxy': ''}, 'manual': ' 영역안의 링크를 삭제하는 것 ',
											 '1st': 'delete', '2nd': 'link', '3rd': 'in_range', 'folder': '',
											 'filename': ''}
		self.menu["delete_memo_in_range"] = {'pcell_method_name': 'delete_memo_in_range',
											 'params': {'sheet_name': '', 'xyxy': ''}, 'manual': ' 영역안의 메모를 삭제하는 것 ',
											 '1st': 'delete', '2nd': 'memo', '3rd': 'in_range', 'folder': '',
											 'filename': ''}
		self.menu["delete_patial_value_in_range_as_0toN"] = {
			'pcell_method_name': 'delete_patial_value_in_range_as_0toN',
			'params': {'sheet_name': '', 'xyxy': '', 'num': "'입력필요'"},
			'manual': ' 선택영역의 값중에서 일부분만 지우는것 앞에서부터 N개까지의 글자를 삭제하는것 ', '1st': 'delete', '2nd': 'patial',
			'3rd': 'value_in_range_as_0toN', 'folder': '', 'filename': ''}
		self.menu["delete_samevalue_in_range"] = {'pcell_method_name': 'delete_samevalue_in_range',
												  'params': {'sheet_name': '', 'xyxy': ''},
												  'manual': ' 영역안의 같은 값을 지우는 것이다 ', '1st': 'delete', '2nd': 'samevalue',
												  '3rd': 'in_range', 'folder': '', 'filename': ''}
		self.menu["delete_shape_all_in_sheet"] = {'pcell_method_name': 'delete_shape_all_in_sheet',
												  'params': {'sheet_name': None}, 'manual': ' 모든객체를 삭제 ',
												  '1st': 'delete', '2nd': 'shape', '3rd': 'all_in_sheet', 'folder': '',
												  'filename': ''}
		self.menu["delete_value_in_range_between_specific_letter"] = {
			'pcell_method_name': 'delete_value_in_range_between_specific_letter',
			'params': {'sheet_name': '', 'xyxy': '', 'input_list': "['(', ')']"},
			'manual': ' (sheet_name="", yxyx="", input_list=["(",")"]) 입력형식 : ["(",")"] 입력자료의 두사이의 자료를 포함하여 삭제하는것 예: abc(def)gh ==>abcgh ',
			'1st': 'delete', '2nd': 'value', '3rd': 'in_range_between_specific_letter', 'folder': '', 'filename': ''}
		self.menu["delete_value_in_range_by_continious_samevalue"] = {
			'pcell_method_name': 'delete_value_in_range_by_continious_samevalue',
			'params': {'sheet_name': '', 'xyxy': ''}, 'manual': ' 영역안에서 연속된 같은 값을 삭제 한다 ', '1st': 'delete',
			'2nd': 'value', '3rd': 'in_range_by_continious_samevalue', 'folder': '', 'filename': ''}
		self.menu["delete_value_in_range_by_step"] = {'pcell_method_name': 'delete_value_in_range_by_step',
													  'params': {'sheet_name': '', 'xyxy': '', 'step_no': ''},
													  'manual': ' 삭제 : 선택자료중 n번째 가로열의 자료를 값만 삭제하는것 일하다보면 3번째 줄만 삭제하고싶은경우가 있다, 이럴때 사용하는 것이다 ',
													  '1st': 'delete', '2nd': 'value', '3rd': 'in_range_by_step',
													  'folder': '', 'filename': ''}
		self.menu["delete_xline_in_range_by_step"] = {'pcell_method_name': 'delete_xline_in_range_by_step',
													  'params': {'sheet_name': '', 'xyxy': '', 'step_no': "'입력필요'"},
													  'manual': ' 영역안의 자료에서 선택한 가로행을 삭제한다 값만 삭제하는것이 아니다 위에서부터 삭제가 되게 만든것 ',
													  '1st': 'delete', '2nd': 'xline', '3rd': 'in_range_by_step',
													  'folder': '', 'filename': ''}
		self.menu["delete_xline_value_in_range_by_step"] = {'pcell_method_name': 'delete_xline_value_in_range_by_step',
															'params': {'sheet_name': '', 'xyxy': '',
																	   'step_no': "'입력필요'"},
															'manual': ' 삭제 : 2 ==> 기존의 2번째 마다 삭제 (1,2,3,4,5,6,7 => 1,3,5,7) 삭제 : 선택자료중 n번째 세로줄의 자료를 값만 삭제하는것 일하다보면 3번째 줄만 삭제하고싶은경우가 있다, 이럴때 사용하는 것 ',
															'1st': 'delete', '2nd': 'xline',
															'3rd': 'value_in_range_by_step', 'folder': '',
															'filename': ''}
		self.menu["delete_yline_in_range_by_step"] = {'pcell_method_name': 'delete_yline_in_range_by_step',
													  'params': {'sheet_name': '', 'xyxy': '', 'step_no': "'입력필요'"},
													  'manual': ' 선택한 영역안의 세로줄중에서 선택한 몇번째마다 y라인을 삭제하는것 (선택한 영역안에서 3번째 마다의 y라인을 삭제하는것) ',
													  '1st': 'delete', '2nd': 'yline', '3rd': 'in_range_by_step',
													  'folder': '', 'filename': ''}
		self.menu["delete_yline_value_in_range_by_step"] = {'pcell_method_name': 'delete_yline_value_in_range_by_step',
															'params': {'sheet_name': '', 'xyxy': '',
																	   'step_no': "'입력필요'"},
															'manual': ' 선택한 영역안의 세로줄중에서 선택한 몇번째마다 y라인의 값을 삭제하는것 (선택한 영역안에서 3번째 마다의 y라인의 값을 삭제하는것) ',
															'1st': 'delete', '2nd': 'yline',
															'3rd': 'value_in_range_by_step', 'folder': '',
															'filename': ''}
		self.menu["fill_emptycell_as_uppercell_in_range"] = {
			'pcell_method_name': 'fill_emptycell_as_uppercell_in_range', 'params': {'sheet_name': '', 'xyxy': ''},
			'manual': ' 빈셀을 위의것으로 채우는 것 ', '1st': 'fill', '2nd': 'emptycell', '3rd': 'as_uppercell_in_range',
			'folder': '', 'filename': ''}
		self.menu["fill_emptycell_in_range_as_uppercell"] = {
			'pcell_method_name': 'fill_emptycell_in_range_as_uppercell', 'params': {'sheet_name': '', 'xyxy': ''},
			'manual': ' 문제가 있어서, 아랫것으로 변경 ', '1st': 'fill', '2nd': 'emptycell', '3rd': 'in_range_as_uppercell',
			'folder': '', 'filename': ''}
		self.menu["find_word_in_range"] = {'pcell_method_name': 'find_word_in_range',
										   'params': {'sheet_name': '', 'xyxy': '', 'input_text': ''},
										   'manual': ' 영역안의 글자를 찾는다 ', '1st': 'find', '2nd': 'word', '3rd': 'in_range',
										   'folder': '', 'filename': ''}
		self.menu["make_vba_module"] = {'pcell_method_name': 'make_vba_module',
										'params': {'vba_code': None, 'macro_name': None},
										'manual': ' 텍스트로 만든 매크로 코드를 실행하는 코드이다 ', '1st': 'make', '2nd': 'vba',
										'3rd': 'module', 'folder': '', 'filename': ''}
		self.menu["merge_left_2_ylines_in_range"] = {'pcell_method_name': 'merge_left_2_ylines_in_range',
													 'params': {'sheet_name': '', 'xyxy': ''},
													 'manual': ' 선택 영역중 바로 위의것과 아랫것만 병합하는것 왼쪽의 2줄을 병합하는 것이다 ',
													 '1st': 'merge', '2nd': 'left', '3rd': '2_ylines_in_range',
													 'folder': '', 'filename': ''}
		self.menu["merge_top_2_xlines_in_range"] = {'pcell_method_name': 'merge_top_2_xlines_in_range',
													'params': {'sheet_name': '', 'xyxy': ''},
													'manual': ' 선택 영역중 바로 위의것과 아랫것만 병합하는것 제일위의 2줄만 세로씩 병합하는 것이다 가로줄 갯수만큰 병합하는것 위와 아래에 값이 있으면 알람이 뜰것이다 ',
													'1st': 'merge', '2nd': 'top', '3rd': '2_xlines_in_range',
													'folder': '', 'filename': ''}
		self.menu["merge_top_2_ylines_in_range"] = {'pcell_method_name': 'merge_top_2_ylines_in_range',
													'params': {'sheet_name': '', 'xyxy': ''},
													'manual': ' 선택 영역중 바로 위의것과 아랫것만 병합하는것 제일위의 2줄만 가로씩 병합하는 것이다 세로줄 갯수만큰 병합하는것 위와 아래에 값이 있으면 알람이 뜰것이다 ',
													'1st': 'merge', '2nd': 'top', '3rd': '2_ylines_in_range',
													'folder': '', 'filename': ''}
		self.menu["paint_cell_by_rgb"] = {'pcell_method_name': 'paint_cell_by_rgb',
										  'params': {'sheet_name': None, 'xy': None, 'input_color': "'입력필요'"},
										  'manual': ' paint_color(sheet_name, xyxy, input_data="입력필요") 선택 셀에 색깔을 넣는다 ',
										  '1st': 'paint', '2nd': 'cell', '3rd': 'by_rgb', 'folder': '', 'filename': ''}
		self.menu["paint_cell_in_range_by_specific_text"] = {
			'pcell_method_name': 'paint_cell_in_range_by_specific_text',
			'params': {'sheet_name': '', 'xyxy': '', 'input_text': "'입력필요'", 'input_color': "'입력필요'"},
			'manual': ' 영역안에 입력받은 글자와 같은것이 있으면 색칠하는것 ', '1st': 'paint', '2nd': 'cell',
			'3rd': 'in_range_by_specific_text', 'folder': '', 'filename': ''}
		self.menu["paint_color_in_range_bywords"] = {'pcell_method_name': 'paint_color_in_range_bywords',
													 'params': {'sheet_name': '', 'xyxy': ''},
													 'manual': ' 입력값을 받는데 영역안에 입력받은 글자가 있으면 색칠하는것 ', '1st': 'paint',
													 '2nd': 'color', '3rd': 'in_range_bywords', 'folder': '',
													 'filename': ''}
		self.menu["paint_maxvalue_in_range_in_each_xline"] = {
			'pcell_method_name': 'paint_maxvalue_in_range_in_each_xline', 'params': {'sheet_name': '', 'xyxy': ''},
			'manual': ' 각 x라인별로 최대값에 색칠하는 것 ', '1st': 'paint', '2nd': 'maxvalue', '3rd': 'in_range_in_each_xline',
			'folder': '', 'filename': ''}
		self.menu["paint_minvalue_in_range_in_each_xline"] = {
			'pcell_method_name': 'paint_minvalue_in_range_in_each_xline', 'params': {'sheet_name': '', 'xyxy': ''},
			'manual': ' 각 x라인별로 최소값에 색칠하는 것 ', '1st': 'paint', '2nd': 'minvalue', '3rd': 'in_range_in_each_xline',
			'folder': '', 'filename': ''}
		self.menu["paint_range_for_samevalue_by_excel_colorno"] = {
			'pcell_method_name': 'paint_range_for_samevalue_by_excel_colorno',
			'params': {'sheet_name': '', 'xyxy': '', 'excelcolorno': '4'}, 'manual': ' 영역에서 같은 값을 색칠하는 것 ',
			'1st': 'paint', '2nd': 'range', '3rd': 'for_samevalue_by_excel_colorno', 'folder': '', 'filename': ''}
		self.menu["paint_samevalue_in_range_by_scolor"] = {'pcell_method_name': 'paint_samevalue_in_range_by_scolor',
														   'params': {'sheet_name': '', 'xyxy': '',
																	  'input_color': "'gray'"},
														   'manual': ' 선택한 영역에서 2번이상 반복된것만 색칠하기 ', '1st': 'paint',
														   '2nd': 'samevalue', '3rd': 'in_range_by_scolor',
														   'folder': '', 'filename': ''}
		self.menu["paint_spacecell_in_range_by_scolor"] = {'pcell_method_name': 'paint_spacecell_in_range_by_scolor',
														   'params': {'sheet_name': '', 'xyxy': '',
																	  'input_color': "'입력필요'"},
														   'manual': ' 빈셀처럼 보이는데 space문자가 들어가 있는것 찾기 선택한 영역의 셀을 하나씩 읽어와서 re모듈을 이용해서 공백만 있는지 확인한다 ',
														   '1st': 'paint', '2nd': 'spacecell',
														   '3rd': 'in_range_by_scolor', 'folder': '', 'filename': ''}
		self.menu["split_partial_value_in_range_by_step_from_start"] = {
			'pcell_method_name': 'split_partial_value_in_range_by_step_from_start',
			'params': {'sheet_name': None, 'xyxy': None, 'n_char': None},
			'manual': ' 어떤 자료중에 앞에서 몇번째것들만 갖고오고 싶을때 예:시군구 자료에서 앞의 2글자만 분리해서 얻어오는 코드 ', '1st': 'split', '2nd': 'partial',
			'3rd': 'value_in_range_by_step_from_start', 'folder': '', 'filename': ''}
		self.menu["split_value_as_engnum"] = {'pcell_method_name': 'split_value_as_engnum', 'params': {'data': None},
											  'manual': '  단어중에 나와있는 숫자, 영어를 분리하는기능 ', '1st': 'split', '2nd': 'value',
											  '3rd': 'as_engnum', 'folder': '', 'filename': ''}
		self.menu["split_value_by_special_string"] = {'pcell_method_name': 'split_value_by_special_string',
													  'params': {'sheet_name': '', 'input_text': "'입력필요'"},
													  'manual': ' split_inputvalue_as_special_string( input_text="입력필요"): 선택한 1줄의 영역에서 원하는 문자나 글자를 기준으로 분리할때 2개의 세로행을 추가해서 결과값을 쓴다 ',
													  '1st': 'split', '2nd': 'value', '3rd': 'by_special_string',
													  'folder': '', 'filename': ''}
		self.menu["split_value_to_str_num"] = {'pcell_method_name': 'split_value_to_str_num',
											   'params': {'input_text': None}, 'manual': ' ', '1st': 'split',
											   '2nd': 'value', '3rd': 'to_str_num', 'folder': '', 'filename': ''}
		self.menu["swap_cap_small"] = {'pcell_method_name': 'swap_cap_small', 'params': {'sheet_name': '', 'xyxy': ''},
									   'manual': ' 영역안의 값을 대/소문자를 바꾸는 것 ', '1st': 'swap', '2nd': 'cap', '3rd': 'small',
									   'folder': '', 'filename': ''}
		self.menu["swap_data_for_list2d_by_2_index"] = {'pcell_method_name': 'swap_data_for_list2d_by_2_index',
														'params': {'input_list2d': None, 'input_no_list': None},
														'manual': ' 2차원 리스트의 자료에서 각 라인별 2개의 위치를 바꾼는것 change_position_for_list2d_by_2_index([[1,2,3], [4,5,6]], [0,2]) [[1,2,3], [4,5,6]] ==> [[3,2,1], [6,5,4]] ',
														'1st': 'swap', '2nd': 'data', '3rd': 'for_list2d_by_2_index',
														'folder': '', 'filename': ''}
		self.menu["write_range_formula"] = {'pcell_method_name': 'write_range_formula',
											'params': {'sheet_name': None, 'xyxy': None}, 'manual': ' ', '1st': 'write',
											'2nd': 'range', '3rd': 'formula', 'folder': '', 'filename': ''}
		self.menu["write_serial_no_by_step_to_y_line"] = {'pcell_method_name': 'write_serial_no_by_step_to_y_line',
														  'params': {'xyxy': None, 'start_no': '1', 'step': '1'},
														  'manual': ' 선택한 영역에 시작번호, 간격으로 이루어진 연속된 숫자를 쓰는것 예 :  0,2,4,6,8.... ',
														  '1st': 'write', '2nd': 'serial',
														  '3rd': 'no_by_step_to_y_line', 'folder': '', 'filename': ''}
		self.menu["write_value_in_range_by_trans"] = {'pcell_method_name': 'write_value_in_range_by_trans',
													  'params': {'sheet_name': '', 'xyxy': '', 'input_list2d': ''},
													  'manual': ' 입력자료의 xy를 바꿔서 입력하는 것 ', '1st': 'write',
													  '2nd': 'value', '3rd': 'in_range_by_trans', 'folder': '',
													  'filename': ''}
		self.menu["write_value_in_range_xystep"] = {'pcell_method_name': 'write_value_in_range_xystep',
													'params': {'sheet_name': '', 'xyxy': '', 'input_text': '',
															   'xystep': '[1, 1]'},
													'manual': ' 선택한 영역의 시작점부터 x,y 번째 셀마다 값을 넣기 ', '1st': 'write',
													'2nd': 'value', '3rd': 'in_range_xystep', 'folder': '',
													'filename': ''}
		self.menu["write_value_to_left"] = {'pcell_method_name': 'write_value_to_left', 'params': {'input_text': ''},
											'manual': ' ', '1st': 'write', '2nd': 'value', '3rd': 'to_left',
											'folder': '', 'filename': ''}
...
class basic_data_for_pcell:
	...
	def __init__(self):
		...
		self.vars["css_basic_btn"] = "background-color: light gray; text-align: left;"
		self.vars["css_selected_btn_1"] = "background-color: #A1EBFA;font-weight: bold; text-align: left;"
		self.vars["css_selected_btn_2"] = "background-color: #FFCCFF;font-weight: bold; text-align: left;"
		self.vars["css_selected_btn_3"] = "background-color: #FFFFCC;font-weight: bold; text-align: left;"
		self.vars["css_selected_btn_4"] = """
						 QPushButton {
						font: 50 17pt;
						font-weight: bold;
						border: 1px solid;
		...
		self.vars["css_btn_1"] = """
						...
						border-radius: 20px;
						"""
		self.vars["message_how_to_run_external_files"] = """
			** 외부 화일을 실행시키는 방법

			자체적으로 저장되어있지 않은 화일을 실행시키는 방법은
			boribori.exe가 있는 위치의 user_py_folder안에 Python으로 만든 화일을 넣으면 된다

			만약 화일을 실행하는데, 좀 복잡하다고 생각하시는 분은
			boribori.exe의 화일을 D:\의 몰더에 복사해서 사용하셔도 잘 작동합니다
			물론, 관련화일도 같이 복사하시면, 직접 만드신 화일을 관리하시는데 쉽게 사용하실것입니다
			"""

		self.vars["p0_basic_manual"] = """
			** 자주 사용하는 예제

			엑셀에서 사용하면 좋을 예제들을 나열합니다

			1번째 메뉴 - 역활을 기본적으로 설명
			2번째 메뉴 - 어떤 일을 하는지 전체적인 설명 
			"""

		self.vars["p1_basic_manual"] = """** pcell실행 탭
			** 엑셀에 적용가능한 함수를 함수별로 실행시키는 것

			함수이름을 3단계로 구분하여, 왼쪽의 메뉴처럼 만듦 
			1. 버튼을 누르듯 3개의 이름을 선택
			2. 입력변수의 값을 입력 
			3. "실행"버튼을 누르면

			관련된 메소드가 실행됨"""

		self.vars["p2_basic_manual"] = """** user files 탭
			** 사용자가 만든 python화일

			사용자가 직접 만든 python화일을 다시 사용이 가능하도록 
			이 화일과 같은 폴더안의 user_py_folder에 저장해 놓으면,
			버튼이 만들어 져서, 그 버튼을 누르면 실행되는 것

			이 화일들은 boribori폴더/user_py_folder안에 
			저장해 놓은것만 가능하다
			"""

		self.vars["p3_basic_manual"] = """** xython 폴더 Tab 
			** xython의 작성자가 추천하는 파이썬 화일이며
			boribori가 있는 폴더에 xython_code라는 폴더안의 자료가 나타나는 것이다

			각 python이름으로 버튼이 자동으로 만들어 지며, 
			이것을 누르면 실행이 되는 것이다
		...
		self.vars["basic_simple_head_code"] = """# -*- coding: utf-8
from xython import pcell, ganada, scolor, youtil
excel = pcell.pcell()
color = scolor.scolor()
util = youtil.youtil()
word = ganada.ganada()
"""
		...
		self.vars["basic_head_code"] = """# -*- coding: utf-8
from xython import pcell, ganada, han, mailmail, pynal, anydb, scolor, youtil, mygrid, jfinder, basic_data
excel = pcell.pcell()
color = scolor.scolor()
util = youtil.youtil()
jf = jfinder.jfinder()
...
base_data = basic_data.basic_data()
"""

		self.vars["p7_basic_manual"] = """#** manual run 탭

			#직접 이곳에 코드를 넣어서 사용하면 실행이 되는 것입니다
			#자주 사용하는 모듈중에서 아래의 코드는 기본으로 그냥 들어갑니다
			#그러니 별도로 아래의 코드는 써넣지 않아도 됩니다 

			# -*- coding: utf-8
			from xython import pcell, youtil, scolor, jfinder
			excel = pcell.pcell()
			util = youtil.youtil()
			color = scolor.scolor()
			jf = jfinder.jfinder()
			"""
		self.vars["p22_basic_manual"] = """
			3단계로 나누는 것입니다
			"""
		self.vars["p8_basic_manual"] = """** 함수찾기 탭
			현재까지 만든 저의 모듈의 함수들을 설명해주는 부분입니다
			1. 제일 왼쪽의 모듈을 선택한다
			2. 다른 그리드 첫번째 라인에 그 모듈 함수들의 첫번째 단어가 나온다
			3. 선택한 함수의 내용이 밑에 나온다

			왼쪽의 버튼을 누르면,
			함수중에서 원하는 함수들을 찾아서 2번째 행에 버튼을 만들어 주며,

			버튼을 누르면, 그 함수의 doc부분을 불러와서 텍스트박스에 나타납니다
			함수의 doc는 각 함수의 이름 바로밑의 설명부분을 말하는 것입니다
		...
		self.vars["message_how_to_run_external_files"] = """
			외부화일을 실행하는 방법입니다
		"""
		self.vars["message_how_to_use"] = """
			엑셀을 컨트롤 하는 pcell을 잘 사용할수있도록 만든것입니다
			기본적으로 
			몇가지의 형식으로 사용하는 방법을 나누어 보았읍니다

			pcell중에서 잘 사용하는 방법을 나나태는 페이지가 1개
			파일형식으로 만든 것중에
				- 프로그램을 만든사람이 공통으로 공유하는 파일들을 1개
				- 사용자가 직접 만들어서 사용하는 것 1개
				- 경우에 따라서 입력하는것이 필요한것을 만드는 형태로 사용하는 파일들

			이렇게 4가지의 형식으로 만들어서 구분하여 사용할 것입니다
					"""

		self.vars["message_001"] = """
			** 누가 만들었나요?
			여기까지 오는데 20년의 시간이 흘렀네요
			그기간동안 이런저런것들도 많이 했네요

			맨처음 sourceforge에 올렸던것은 이젠 찾지도 못하겠고...

			개인적으로 활용하기 위해 만든 것들을 공유하는 목적이며
			SJ.Park/SC.Park라고 합니다
			"""

		self.vars["message_002"] = """** 화일 이름의 의미
			제일 앞글자에 가족들의 이니셜을 하나씩 집어 넣으면 어떨까라는 기본 생각에서 만들었읍니다
			물론 의미상으로 연관도 있어야 할것 같다는 생각도 같이 했구요

			Pcell은 박씨성의 내이름을 기준으로
			Scolor는 아들의 이름중에 한글자를
			Jfinder는 딸의 이름중에서 한글자를
			Youtil은 집사람의 이름에서 한글자를
			그리고 최종으로 Bori라는 이름은 우리딸이 지은 우리집 강아지 이름입니다
			"""

		self.vars["message_003"] = """
			** 참고 사이트들
		...
		self.vars["p6_basic_manual"] = """
			** 기초 sample 
			시작하시는 분들을 위한 기본적인 사용방법을 위한 코드들
			"""
		...
		self.vars["check_breakthrough"] = {"취소선": "strikethrough", "strikethrough": "strikethrough",
										   "취소": "strikethrough", "strike": "strikethrough"}
		...
		self.vars["color"]["check_color_kor"] = self.vars["check_color_kor"] = {
			...
			"주": "오렌지", "주황": "오렌지", "주황색": "오렌지", "ora": "오렌지", "yr": "오렌지", "yellowred": "오렌지", "yellow_red": "오렌지",
			"yel_red": "오렌지", "yellow red": "오렌지", "o": "오렌지", "orange": "오렌지",
			"노": "노랑", "노랑": "노랑", "노랑색": "노랑", "황색": "노랑", "y": "노랑", "yellow": "노랑", "yel": "노랑",
			"연": "연두", "연두": "연두", "연두색": "연두", "gy": "연두", "greenyellow": "연두", "green_yellow": "연두", "gre_yel": "연두",
			"green yellow": "연두", "green-yellow": "연두", "yg": "연두",
			"초": "초록", "초록": "초록", "초록색": "초록", "g": "초록", "녹색": "초록",
			"연초록": "연초록", "greencyan": "연초록", "green_cyan": "연초록", "gre_cya": "연초록", "green cyan": "연초록", "cg": "연초록",
			"gc": "연초록",
			"옥": "하늘", "옥색": "하늘", "하늘색": "하늘", "하늘": "하늘", "c": "하늘", "cya": "하늘", "cyan": "하늘",
			"청록": "군청색", "청록색": "군청색", "청": "군청색", "군청색": "군청색", "청색": "군청색", "bluecyan": "군청색", "blue_cyan": "군청색",
			"blu_cya": "군청색", "blue cyan": "군청색", "cb": "군청색", "bg": "군청색", "bluegreen": "군청색",
			"파": "파랑", "파랑": "파랑", "파랑색": "파랑", "b": "파랑", "blue": "파랑", "blu": "파랑",
			"남": "남색", "남색": "남색", "bluemagenta": "남색", "blue_magenta": "남색", "blu_mag": "남색", "blue magenta": "남색",
			"bm": "남색", "pb": "남색", "purpleblue": "남색", "purple_blue": "남색", "pur_blu": "남색", "purple blue": "남색",
			"핑크": "핑크", "핑크색": "핑크", "분홍": "핑크", "분홍색": "핑크", "pink": "핑크",
			"자홍": "자주", "자주": "자주", "자주색": "자주", "redmagenta": "자주", "red_mag": "자주", "red_magenta": "자주",
			"red magenta": "자주", "rm": "자주", "mr": "자주",
			"보": "보라", "보라": "보라", "보라색": "보라", "magenta": "보라", "m": "보라", "mag": "보라", "purple": "보라", "자색": "보라",
			"violet": "보라", "vio": "보라",
		...
		self.vars["color"]["check_color_eng"] = self.vars["check_color_eng"] = self.vars["check_color_name"] = \
		self.vars["color"]["check_color_eng_name"] = {
			"빨": "red", "적": "red", "빨강": "red", "red": "red", "r": "red", "빨간색": "red", "빨강색": "red", 0: "red",
			"주": "ora", "주황": "ora", "주황색": "ora", "오렌지": "ora", "yr": "ora", "yellowred": "ora", "yellow_red": "ora",
			"yel_red": "ora", "yellow red": "ora", "o": "ora", "orange": "ora", "ora": "ora", 1: "ora", 30: "ora",
			"노": "yel", "노랑": "yel", "노랑색": "yel", "황색": "yel", "y": "yel", "yellow": "yel", "yel": "yel", 60: "yel",
			"연": "gre_yel", "연두": "gre_yel", "연두색": "gre_yel", "gre_yel": "gre_yel", "gy": "gre_yel",
			"greenyellow": "gre_yel", "green_yellow": "gre_yel", "green yellow": "gre_yel", "green-yellow": "gre_yel",
			"yg": "gre_yel", 3: "gre_yel", 90: "gre_yel",
			"초": "gre", "초록": "gre", "초록색": "gre", "g": "gre", "green": "gre", "gre": "gre", "녹색": "gre", 4: "gre",
			120: "gre",
			"연초록": "gre_cya", "gre_cya": "gre_cya", "greencyan": "gre_cya", "green_cyan": "gre_cya",
			"green cyan": "gre_cya", "cg": "gre_cya", "gc": "gre_cya", 5: "gre_cya", 150: "gre_cya",
			"옥": "cya", "옥색": "cya", "하늘색": "cya", "하늘": "cya", "c": "cya", "cya": "cya", "cyan": "cya", 6: "cya",
			180: "cya",
			"청록": "blu_cya", "청록색": "blu_cya", "청": "blu_cya", "군청색": "blu_cya", "청색": "blu_cya", "blu_cya": "blu_cya",
			"bluecyan": "blu_cya", "blue_cyan": "blu_cya", "blue cyan": "blu_cya", "cb": "blu_cya", "bg": "blu_cya",
			"bluegreen": "blu_cya", "bc": "blu_cya", 7: "blu_cya", 210: "blu_cya",
			"파": "blu", "파랑": "blu", "파랑색": "blu", "b": "blu", "blue": "blu", "blu": "blu", 8: "blu", 240: "blu",
			"남": "blu_mag", "남색": "blu_mag", "blu_mag": "blu_mag", "bluemagenta": "blu_mag", "blue_magenta": "blu_mag",
			"blue magenta": "blu_mag", "bm": "blu_mag", "pb": "blu_mag", "purpleblue": "blu_mag",
			"purple_blue": "blu_mag", "purple blue": "blu_mag",
			"핑크": "pin", "핑크색": "pin", "분홍": "pin", "분홍색": "pin", "pink": "pin", "pin": "pin",
			"자홍": "red_mag", "자주": "red_mag", "자주색": "red_mag", "redmagenta": "red_mag", "red_mag": "red_mag",
			"red_magenta": "red_mag", "red magenta": "red_mag", "rm": "red_mag", "mr": "red_mag", 11: "red_mag",
			330: "red_mag",
			"보": "mag", "보라": "mag", "보라색": "mag", "magenta": "mag", "m": "mag", "mag": "mag", "purple": "mag",
			"자색": "mag", "violet": "mag", "vio": "mag", 10: "mag", 300: "mag",
		...
		self.vars["color"]["check_color_style"] = self.vars["check_color_style"] = self.vars["color"][
			"check_color_tone"] = self.vars["check_color_tone"] = {
		...
		self.vars["shape_style_base_dic"] = {
			"star": 147, "별": 147,
			"diamond": 4, "다이아몬드": 4, "마름모": 4,
			"arc": 25, "원호": 25,
			"balloon": 137, "풍선": 137,
			"can": 13, "캔": 13,
			"cloud": 179, "구름": 179,
			...
			"donut": 18,  # Donut
			"doublebrace": 27,  # Double brace
			"doublebracket": 26,  # Double bracket
			"doublewave": 104,  # Double wave
			"downarrow": 36,  # block arrow that points down
			...
			"Isoscelestriangle": 7,  # Isosceles triangle
			"leftarrow": 34,  # block arrow that points left
			"leftarrowcallout": 54,  # callout with arrow that points left
			"leftbrace": 31,  # left brace
			"leftbracket": 29,  # left bracket
			"leftcirculararrow": 176,  # circular arrow pointing counter-clockwise
			"leftrightarrow": 37,  # block arrow with arrowheads that point both left and right
			...
			"leftrightuparrow": 40,  # block arrow with arrowheads that point left, right, and up
			"leftuparrow": 43,  # block arrow with arrowheads that point left and up
			"lightningbolt": 22,  # lightning bolt
		...
		self.vars["color"]["tone_kor"] = self.vars["color_tone_kor"] = ["검은", "하얀", "연한", "밝은회색", "회색", "어두운회색", "옅은",
																		"부드러운", "탁한", "어두운", "밝은", "강한", "짙은", "선명한",
																		"파스텔"]
		self.vars["color"]["tone"] = self.vars["color_tone"] = self.vars["pccs_style"] = list(
			set(self.vars["check_color_tone"].values()))
		self.vars["color"]["style"] = self.vars["color_style"] = self.vars["color"]["tone_eng"] = self.vars[
			"color_tone_eng"] = list(set(self.vars["check_color_tone"].values()))
		...
		self.vars["line_position"] = {
									  "left": [7], "왼쪽": [7],
									  "top": [8], "위쪽": [8],
									  "bottom": [9], "아래쪽": [9],"b": [9],
									  "right": [10], "오른쪽": [10],
									  "inside-h": [11], "inh": [11], "안쪽세로": [11], "|": [11],
									  "inside-v": [12], "inv": [12], "안쪽가로": [12], "-": [12],
									  "all_position": [7, 8, 9, 10, 11, 12], "전부": [7, 8, 9, 10, 11, 12],
									  "all": [7, 8, 9, 10, 11, 12],
									  "테두리": [7, 8, 9, 10], "outline": [7, 8, 9, 10], "box": [7, 8, 9, 10],
									  "/": [6], "대각선오른쪽": [6],
									  "\\": [5], "대각선왼쪽": [5],
			"a": [7, 8, 9, 10, 11, 12],
			"o": [7, 8, 9, 10], "": [7, 8, 9, 10],
		...
		self.vars["line_thick"] = {	"가는점선": 1,"굵은실선": 6,"hairline": 1,"basic": -4138,"thick": 4,
									   "thin": 2,"t-2": 1,"t0": -4138,"t1": 4,"t-1": 2,"매우가는선": 1,
									   "일반": -4138,"굵은선": 4,	"가는선": 2, "": 1,}

		self.vars["line_style"] = {"basic": 1, "": 1, "-": 1, "-.": 4,	"-..": 5, "--": -4115,".": -4118, "=": -4119, "no": -4142, "/.": 13, "/..": 13,"실선": 1, "점선": 2, "가는점선": 1, "굵은실선": 6, }

		self.vars["line"] = self.vars["line_position"]
		self.vars["line"].update(self.vars["line_thick"])
		self.vars["line"].update(self.vars["line_style"])
		...
		self.vars["자음"] = self.vars["list_자음_19"] = ["ㄱ", "ㄲ", "ㄴ", "ㄷ", "ㄸ", "ㄹ", "ㅁ", "ㅂ", "ㅃ", "ㅅ", "ㅆ", "ㅇ", "ㅈ",
													 "ㅉ", "ㅊ", "ㅋ", "ㅌ", "ㅍ", "ㅎ"]  # 19 글자
		self.vars["모음"] = self.vars["list_모음_21"] = ["ㅏ", "ㅐ", "ㅑ", "ㅒ", "ㅓ", "ㅔ", "ㅕ", "ㅖ", "ㅗ", "ㅘ", "ㅙ", "ㅚ", "ㅛ",
													 "ㅜ", "ㅝ", "ㅞ", "ㅟ", "ㅠ", "ㅡ", "ㅢ", "ㅣ"]  # 21 글자
		self.vars["받침"] = self.vars["list_받침_28"] = ["", "ㄱ", "ㄲ", "ㄳ", "ㄴ", "ㄵ", "ㄶ", "ㄷ", "ㄹ", "ㄺ", "ㄻ", "ㄼ", "ㄽ",
		...
		self.vars["month"] = ['JANUARY', 'FEBRUARY', 'MARCH', 'APRIL', 'MAY', 'JUNE', 'JULY', 'AUGUST', 'SEPTEMBER',
		...
		self.vars["dic_no_mon"] = {1: 'january', 2: 'february', 3: 'march', 4: 'april', 5: 'may', 6: 'june', 7: 'july',
								   8: 'august',
								   9: 'september', 10: 'october', 11: 'november', 12: 'december'}
		self.vars["dic_mon_no_s"] = {'jan': 1, 'feb': 2, 'mar': 3, 'apr': 4, 'jun': 6, 'jul': 7, 'aug': 8, 'sep': 9,
									 'oct': 10,
									 'nov': 11, 'dec': 12}
		self.vars["dic_day_th_no"] = {'1st': 1, '2nd': 2, '3rd': 3, '4th': 4, '5th': 5, '6th': 6, '7th': 7, '8th': 8,
									  '9th': 9, '10th': 10,
		...
		self.vars["dic_week_no"] = {'monday': 1, 'tuesday': 2, 'wendsday': 3, 'thursday': 4, 'friday': 5, 'saturday': 6,
									'sunday': 7,
		...
		self.vars["sl_small_step_vs_sl_no"] = {"1": [0, 12], "2": [0, 9], "3": [0, 6], "4": [0, 3],
											   "5": [0, 0], "6": [0, -3], "7": [0, -6], "8": [0, -9], "9": [0, -12],
											   "0": [0, -14], }

		self.vars["color"]["h"] = self.vars["h12"] = [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330]

		self.vars["color"]["sl10"] = self.vars["sl10"] = [[100, 0], [90, 10], [80, 20], [70, 30], [60, 40], [50, 50],
														  [40, 60], [30, 70], [10, 80], [10, 90], [0, 100]]

		self.vars["color"]["sl"] = self.vars["sl11"] = [[100, 0], [100, 10], [100, 20], [100, 30], [100, 40], [100, 50],
														[100, 60], [100, 70], [100, 80], [100, 90], [100, 100]]

		self.vars["color"]["sl12"] = self.vars["sl12"] = [[90, 50], [80, 30], [80, 50], [80, 70], [50, 20], [50, 40],
														  [50, 60], [50, 80], [20, 20], [20, 40], [20, 60], [20, 80]]

		self.vars["color"]["hsl"] = self.vars["hsl12"] = [[0, 100, 50], [30, 100, 50], [60, 100, 50], [90, 100, 50],
														  [120, 100, 50], [150, 100, 50], [180, 100, 50],
														  [210, 100, 50], [240, 100, 50], [270, 100, 50],
														  [300, 100, 50], [330, 100, 50]]

		self.vars["color"]["rgb12"] = self.vars["rgb12"] = [[255, 0, 0], [255, 128, 0], [255, 255, 0], [128, 255, 0],
															[0, 255, 0], [0, 255, 128], [0, 255, 255], [0, 128, 255],
															[0, 0, 255], [128, 0, 255], [255, 0, 255], [255, 0, 128], ]

		self.vars["color"]["sl_big_step"] = self.vars["sl_big_step"] = [[100, 80], [100, 77], [100, 68], [100, 59],
																		[100, 50], [90, 41], [80, 32], [70, 23],
																		[60, 14]]

		self.vars["color"]["sl_no_vs_sl_small_step"] = self.vars["sl_no_vs_sl_small_step"] = {"1": [0, 12], "2": [0, 9],
																							  "3": [0, 6], "4": [0, 3],
																							  "5": [0, 0], "6": [0, -3],
																							  "7": [0, -6],
																							  "8": [0, -9],
		...
		self.vars["color"]["color_tone_vs_no"] = self.vars["color_tone_12_names_vs_no"] = {"연한": 0, "밝은회색": 1, "회색": 2,
																						   "어두운회색": 3, "옅은": 4,
																						   "부드러운": 5, "탁한": 6, "어두운": 7,
																						   "밝은": 8, "강한": 9, "짙은": 10,
																						   "선명한": 11}

		self.vars["color"]["excel56_rgb"] = self.vars["excel56_rgb"] = [[255, 0, 0], [0, 255, 0], [0, 0, 255],
																		[255, 255, 0], [255, 0, 255],
																		[0, 255, 255], [153, 51, 102], [255, 255, 204],
																		[204, 255, 255], [102, 0, 102],
																		[255, 128, 128], [0, 102, 204], [204, 204, 255],
																		[0, 204, 255], [204, 255, 204],
																		[255, 255, 153], [153, 204, 255],
																		[255, 153, 204], [204, 153, 255],
																		[255, 204, 153],
																		[51, 102, 255], [51, 204, 204], [153, 204, 0],
																		[255, 204, 0], [255, 153, 0],
																		[255, 102, 0], [102, 102, 153], [150, 150, 150],
																		[0, 51, 102], [51, 153, 102],
																		[0, 51, 0], [51, 51, 0], [153, 51, 0],
																		[51, 51, 153], [51, 51, 51],
																		[128, 0, 0], [0, 128, 0], [0, 0, 128],
																		[128, 128, 0], [128, 0, 128],
																		[0, 128, 128], [192, 192, 192], [128, 128, 128],
																		[153, 153, 255], [0, 0, 0],
																		[255, 255, 255], [102, 102, 153],
																		[150, 150, 150], [0, 51, 102], [51, 153, 102],
																		[0, 51, 0], [51, 51, 0], [153, 51, 0],
		...
		self.vars["color"]["mode"] = self.vars["color_mode"] = ["단색조합", "등간격 3색 조화", "보색", "인접색", "근접보색조합",
																...
																"중명도배색", "중명도배색+약간더 부드러운 이미지", "저중명도배색",
																"저명도배색+약간더 어두운 이미지",
																"명도차가 큰 배색", "명도차가 큰 배색 + 근접보색",
																"고채도 배색", "채도차가 큰 배색", "저채도 배색", ]

		self.vars["color"]["색강도_vs_sl"] = self.vars["색강도_vs_sl"] = {"+++++": [0, 15], "++++": [0, 12], "+++": [0, 9],
																	"++": [0, 6], "+": [0, 3], "-": [0, -3],
																	"--": [0, -6], "---": [0, -9], "----": [0, -12],
																	"-----": [0, -14], }
		self.vars["color"]["step"] = self.vars["color"]["strong"] = self.vars["색강도"] = ["++++++++++", "+++++++++",
																						...
																						"-------", "--------",
																						"---------"]
		self.vars["color"]["color_vs_h"] = self.vars["color_vs_h"] = {"red": 0, "ora": 30, "yel": 60, "gre_yel": 90,
																	  "gre": 120, "gre_cya": 150, "cya": 180,
																	  "blu_cya": 210, "blu": 240, "blu_mag": 270,
																	  "mag": 300, "red_mag": 330, "gra": 0, "whi": 0,
																	  "bla": 0, }
		self.vars["color"]["color_vs_index"] = self.vars["color_vs_index"] = {"red": 0, "ora": 1, "yel": 2,
																			  "gre_yel": 3, "gre": 4, "gre_cya": 5,
																			  "cya": 6, "blu_cya": 7, "blu": 8,
																			  "blu_mag": 9, "mag": 10, "red_mag": 11,
																			  "gra": 12, "whi": 13, "bla": 14}
		self.vars["color"]["no_vs_sl"] = self.vars["no_vs_sl"] = {"1": [0, 12], "2": [0, 9], "3": [0, 6], "4": [0, 3],
																  "5": [0, 0], "6": [0, -3], "7": [0, -6], "8": [0, -9],
																  "9": [0, -12], "0": [0, -14], }

		self.vars["color"]["color_eng_all"] = self.vars["color_eng"] = list(set(self.vars["check_color_eng"].values()))

		self.vars["color"]["color_eng"] = self.vars["color_eng_12"] = ['red', 'ora', 'yel', 'gre_yel', 'gre', 'gre_cya',
																	   'cya', 'blu_cya', 'blu', 'blu_mag', 'pin',
																	   'red_mag']

		self.vars["color"]["color_kor_all"] = self.vars["color_kor"] = list(set(self.vars["check_color_kor"].values()))

		self.vars["color"]["color_kor"] = self.vars["color_kor_12"] = ["빨강", "오렌지", "노랑", "연두", "초록", "연초록", "하늘",
																	   "군청색", "파랑", "남색", "핑크", "자주"]
		...
		self.vars["color"]["basic"] = [[255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0], [255, 0, 255],
									   ...
									   [255, 255, 153], [153, 204, 255], [255, 153, 204], [204, 153, 255],
									   [255, 204, 153],
									   ...
									   [0, 51, 0], [51, 51, 0], [153, 51, 0], [153, 51, 102], [51, 51, 153],
									   [51, 51, 51]]
		...
		self.vars["color"]["no_vs_color_mode"] = self.vars["no_vs_color_mode"] = {1: "단색조합", "단색조합": 1,
																				  "강력한 대기효과와 넓은공간감": 1,
																				  2: "등간격 3색 조화", "등간격 3색 조화": 2,
																				  "활동적인 인상과 이미지": 2,
																				  3: "보색", "보색": 3, "강력한비교": 3,
																				  4: "인접색", "인접색": 4, "비슷한색 찾기": 4,
																				  5: "근접보색조합", "근접보색조합": 5,
																				  "보색의 강한 인상이 부담스러울때 보색의 근처에있는 색": 5,
																				  6: "고명도배색", "고명도배색": 6,
																				  7: "고명도배색+약간더 부드러운 이미지",
																				  "고명도배색+약간더 부드러운 이미지": 7,
																				  8: "중명도배색", "중명도배색": 8,
																				  9: "중명도배색+약간더 부드러운 이미지",
																				  "중명도배색+약간더 부드러운 이미지": 9,
																				  10: "저중명도배색", "저중명도배색": 10,
																				  11: "저명도배색+약간더 어두운 이미지",
																				  "저명도배색+약간더 어두운 이미지": 11,
		...
		self.vars["color"]["johannes_hsl"] = self.vars["hsl_by_johannes"] = [
			[[0, 100, 0], [0, 100, 10], [0, 100, 20], [0, 100, 30], [0, 100, 40], [0, 100, 50], [0, 100, 60],
			 [0, 100, 70],
			...
			[[120, 100, 0], [120, 100, 10], [120, 100, 20], [120, 100, 30], [120, 100, 40], [120, 100, 50],
			 [120, 100, 60],
			 [120, 100, 70], [120, 100, 80], [120, 100, 90], [120, 100, 100]],
			[[150, 100, 0], [150, 100, 10], [150, 100, 20], [150, 100, 30], [150, 100, 40], [150, 100, 50],
			 [150, 100, 60],
			 [150, 100, 70], [150, 100, 80], [150, 100, 90], [150, 100, 100]],
			[[180, 100, 0], [180, 100, 10], [180, 100, 20], [180, 100, 30], [180, 100, 40], [180, 100, 50],
			 [180, 100, 60],
			 [180, 100, 70], [180, 100, 80], [180, 100, 90], [180, 100, 100]],
			[[210, 100, 0], [210, 100, 10], [210, 100, 20], [210, 100, 30], [210, 100, 40], [210, 100, 50],
			 [210, 100, 60],
			 [210, 100, 70], [210, 100, 80], [210, 100, 90], [210, 100, 100]],
			[[240, 100, 0], [240, 100, 10], [240, 100, 20], [240, 100, 30], [240, 100, 40], [240, 100, 50],
			 [240, 100, 60],
			 [240, 100, 70], [240, 100, 80], [240, 100, 90], [240, 100, 100]],
			[[270, 100, 0], [270, 100, 10], [270, 100, 20], [270, 100, 30], [270, 100, 40], [270, 100, 50],
			 [270, 100, 60],
			 [270, 100, 70], [270, 100, 80], [270, 100, 90], [270, 100, 100]],
			[[300, 100, 0], [300, 100, 10], [300, 100, 20], [300, 100, 30], [300, 100, 40], [300, 100, 50],
			 [300, 100, 60],
			 [300, 100, 70], [300, 100, 80], [300, 100, 90], [300, 100, 100]],
			[[330, 100, 0], [330, 100, 10], [330, 100, 20], [330, 100, 30], [330, 100, 40], [330, 100, 50],
			 [330, 100, 60],
		...
		self.vars["line_position_vs_enum"] = {"/": 6, "\\": 5, "left": 7, "top": 8, "bottom": 9, "right": 10,
											  "inside-h": 11, "inside-v": 12, "대각선오른쪽": 6, "대각선왼쪽": 5, "왼쪽": 7, "위쪽": 8,
											  "아래쪽": 9, "오른쪽": 10, "안쪽세로": 11, "안쪽가로": 12, }
		self.vars["line_style_vs_enum"] = {"basic": 1, "": 1, "-": 1, ".": -4118, "-.": 4, "-..": 5, "..": -4115,
										   "=": 4119, "no": -4142, "/..": 13, "/.": 13, "실선": 1, "점선": 2, "가는점선": 1,
										   "굵은실선": 6, }
		...
		self.vars["base_cell_data"] = {"sheet_name": "", "xyxy": [100, 100, 0, 0], "color": 10058239,
									   "line_style": "-.", "thickness": 0.5, "transparency": 0, "head_style": ">",
									   "head_length": "0", "head_width": "0", "tail_style": ">", "tail_length": "0",
									   "tail_width": "0", }

		self.vars["check_keyboard_hex_value"] = {"LBUTTON": 0x01, "RBUTTON": 0x02, "CANCEL": 0x03,
												 "MBUTTON": 0x04, "XBUTTON1": 0x05, "XBUTTON2": 0x06, "BACK": 0x08,
												 "TAB": 0x09, "CLEAR": 0x0C,
												 "RETURN": 0x0D, "SHIFT": 0x10, "CONTROL": 0x11, "MENU": 0x12,
												 "PAUSE": 0x13, "CAPITAL": 0x14,
												 "HANGUEL": 0x15, "KANA": 0x15, "JUNJA": 0x17, "FINAL": 0x18,
												 "KANJI": 0x19, "HANJA": 0x19,
												 "ESCAPE": 0x1B, "CONVERT": 0x1C, "NONCONVERT": 0x1D, "ACCEPT": 0x1E,
												 "MODECHANGE": 0x1F, "SPACE": 0x20,
												 "PRIOR": 0x21, "NEXT": 0x22, "END": 0x23, "HOME": 0x24, "LEFT": 0x25,
												 "UP": 0x26,
		...
		self.vars["action_key_list"] = [
			...
			'alt', 'altleft', 'altright', 'ctrl', 'ctrlleft', 'ctrlright', 'shift', 'shiftleft', 'shiftright', 'win',
			'winleft', 'winright',
		...
		self.vars["dic_jfinder_gui_dic_4"] = {"abc중 c만 1번이상 반복되는것 찾기": [4, "abc[1~]", "type_z0"],
											  ...
											  "후방탐색 : (?<=\$)[0-9.]+ => $뒤의 숫자": [4, "(?<=\$)[0-9.]+", "type_z0"],
											  "Html태그중 <H숫자>찾을문자<\H숫자>안의 글자": [4, "<H(0-30])>.*?</H\\1>", "type_z0"],
											  "문장중 핸드폰번호 찾기": [4, "0[1~1][숫자:2~2]-[0~1][숫자:3~4]-[0~1][숫자:4~4]",
															   "type_z0"],
											  "2022-01-01형식의 날짜찾기": [4, "[0-9]{4})-([0-9]{2})-([0-9]{2}", "type_z0"],
											  "이메일찾기": [4, "[\w\.-]+@[\w\.-]+", "type_z0"],
											  "괄호안의 숫자 찾기": [4, "([모든문자:0~])", "type_z0"],
		...
		self.vars["dic_shape_enum"] = {
			...
			'FlowchartData': 64, 'FlowchartPredefinedProcess': 65, 'FlowchartInternalStorage': 66,
			'FlowchartDocument': 67,
			'FlowchartMultidocument': 68, 'FlowchartTerminator': 69, 'FlowchartPreparation': 70,
			'FlowchartManualInput': 71,
			'FlowchartManualOperation': 72, 'FlowchartConnector': 73, 'FlowchartOffpageConnector': 74,
			'FlowchartCard': 75,
			...
			'FlowchartDelay': 84, 'FlowchartSequentialAccessStorage': 85, 'FlowchartMagneticDisk': 86,
			...
			'LineCallout4': 112, 'LineCallout1AccentBar': 113, 'LineCallout2AccentBar': 114,
			'LineCallout3AccentBar': 115,
			'LineCallout4AccentBar': 116, 'LineCallout1NoBorder': 117, 'LineCallout2NoBorder': 118,
			...
			'LineCallout4BorderandAccentBar': 124, 'ActionButtonCustom': 125, 'ActionButtonHome': 126,
			'ActionButtonHelp': 127,
			'ActionButtonInformation': 128, 'ActionButtonBackorPrevious': 129, 'ActionButtonForwardorNext': 130,
			'ActionButtonBeginning': 131,
			'ActionButtonEnd': 132, 'ActionButtonReturn': 133, 'ActionButtonDocument': 134, 'ActionButtonSound': 135,
			'ActionButtonMovie': 136, 'Balloon': 137, 'FlowchartOfflineStorage': 139,
...
class basic_data_for_xy_manual:
	def xy_manual_dic(self):
		my_dic_01 = {}
		my_dic_01["Tab 설명들"] = """
* Pcell/Easy Use
	 - Pcell을 쉽게 사용하자 => 양복의 3개 단추처엄, 3단계로 엑셀의 기능을 실행시킨다는 뜻
...
* 함수찾기
 - 단어를 입력하면, Pcell 모듈에서 관련된 함수를 찾아주는 것

* How to Use
 - Manual => 사용자 메뉴얼
"""
		my_dic_01["기본 설명들"] = """
이 실행화일이 있는 폴더안의 user_py_folder안의 파이썬 화일을 실행 시키는 것이다
입력받으면서 실행될 부분은 별도로 처리하여야 합니다. 단 외부 코드를 실행시키기 위해서는
...
ixy : x, y의 좌표이며, 0부터시작 <-> xy
ixyxy : 영역을 나타내는 4자리 리스트 <-> xyxy
...
class sample_data_for_all:
	"""
	가끔 코딩을 해보니, 테스트용데이터가 필요하더라구요
	"""
	vars = {}

	vars["nim"] = """한용운의 시, 님의 침묵
...
"""
	...
	vars["empty_sample"] = [['가나','','가이아나','감비아','','그레나다','그루지야'],
										  ['그리스','기니','기니비사우','나미비아','나우루','나이지리아','남수단','남아프리카 공화국'],
										  ['네덜란드','네팔','','뉴질랜드','니제르','니카라과','덴마크','도미니카 연방'],
										  ['도미니카 공화국','','동티모르','라오스','라이베리아','','러시아','레바논'],
										  ['레소토','','룩셈부르크','르완다','리비아','리투아니아','리히텐슈타인','마다가스카르'],
										  ['마셜 제도','말라위','말레이시아','말리','멕시코','','모로코','모리셔스'],
										  ['모리타니','','몬테네그로','몰도바','몰디브','','몽골','미국'],
										  ['미얀마','','바누아투','바레인','바베이도스','바티칸','바하마','방글라데시'],
										  ['베네수엘라','','베트남','','벨라루스','벨리즈','','보츠와나'],
										  ['볼리비아','부룬디','부르키나파소','부탄','북마케도니아','','불가리아','브라질'],
										  ['브루나이','산마리노','사모아','사우디아라비아','상투메 프린시페','세네갈','세르비아', '세이셸'],]


	vars["list_2d"] = [['가나','가봉','가이아나','감비아','과테말라','그레나다','그루지야'],
										  ['그리스','기니','기니비사우','나미비아','나우루','나이지리아','남수단','남아프리카 공화국'],
	...
	vars["all_country_name"] = ['가나','가봉','가이아나','감비아','과테말라','그레나다','그루지야',
										  '그리스','기니','기니비사우','나미비아','나우루','나이지리아','남수단','남아프리카 공화국',
										  '네덜란드','네팔','노르웨이','뉴질랜드','니제르','니카라과','덴마크','도미니카 연방',
										  '도미니카 공화국','독일','동티모르','라오스','라이베리아','라트비아','러시아','레바논',
										  '레소토','루마니아','룩셈부르크','르완다','리비아','리투아니아','리히텐슈타인','마다가스카르',
	...
	vars["df_1"] = {"1번째열": {'Country': 'Russia', 'Capital': 'Moscow', 'Area(Sq.Miles)': 6601670, 'Population': 146171015},
	...
	vars["list_1d"] =["1번째열", 'A1', "", None, [], (),'러시아', 'Capital', 'Moscow', '면적', 6601670, '인구', 146171015]

	vars["list_1d_001"] =[12345, 12345.67, "1234567", 'A1B2', "가나다#$%@", None, [], (),'abcd=efgh', 'Capital', 'Moscow', '면적---', 6601670, '인구', "14617_1015"]

	vars["list_1d_002"] =["1번째열_2번째", 'A1', "", None, [], (),'러시아_인가?', 'Capital', 'Moscow', '면적---', 6601670, '인구', "14617_1015"]
	...
	vars["dic_2"] = [{"x": 2, "y": 1, "kind_1": "basic", "kind_2": "date", "value": datetime.datetime(2002, 2, 2)},
						{"x": 2, "y": 3, "kind_1": "basic", "kind_2": "basic", "text": "값"},
						{"x": 2, "y": 5, "kind_1": "basic", "kind_2": "basic", "text": "값"},
						{"x": 2, "y": 7, "kind_1": "basic", "kind_2": "date", "value": datetime.datetime(2002, 2, 2)},
						{"x": 2, "y": 6, "kind_1": "tool_tip", "text": "tool_tip", "tool_tip": "툴팁입니다"},
	...
	vars["machine_names"] = ['fitting', 'ball', 'flowmeter', 'on-off', 'battery', 'flange', 'electric', 'cable', 'gasket',   'seal',
	...
	vars["changed_dic"] = {"밸브": "valve", "교반기": "agitator", "아지테이터": "agitator", "송풍기": "blower", "블로어": "blower", "s/p": "spare part",
