...
class INNClient(BaseClient):
    ...
    def do_login(self, user: str, password: str, require_success: bool = True):
        self.api_key = self.api_key or UUID("372625ea-6457-4bb0-8061-d9d257d14de9")
        login_body = {
            "user": user,
    ...
    def reset_password(self, reset_code: str, new_pass: str, other_fields: Optional[Dict] = None):
        body = {
    ...
    def check_balances(
        self, coins: List[int], block_height: int = -1, other_fields: Optional[Dict] = None
    ):
    ...
    def get_wallet_assets(self, coins: Optional[List[int]] = None,
    ...
    def request_nft_refund(self, description: str, nft_uri: str,
                           client_id: Optional[str] = None,
                           other_fields: Optional[Dict] = None):
        if not client_id:
            client_id = str(uuid4())
    ...
    def request_token_refund(
            self, description: str, amount: int, coin_id: int,
            client_id: Optional[str] = None,
            other_fields: Optional[Dict] = None
    ):
        if not client_id:
    ...
    def get_support_verification(self, user: Optional[str] = None,
    ...
    def test_bundle(self, coin_id: int, amount: int = 1,
                    other_fields: Optional[Dict] = None):
        body = {
            "uuid": self.uuid,
    ...
    def test_bundle_mint(
        ...
        if not TESTNET and not body.get("clientId", None):
            client_id = generate_client_id()
            body["clientId"] = client_id
            body["checksum"] = generate_client_hash(body.get("bundleCoinId", 17),
                                                    client_secret=self.api_key, amount=1, client_id=client_id)
