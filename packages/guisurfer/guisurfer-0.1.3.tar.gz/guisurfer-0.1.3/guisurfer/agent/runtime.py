...
from .env import HUB_SERVER_ENV, AGENTD_ADDR_ENV, AGENTD_PRIVATE_SSH_KEY_ENV
...
class AgentRuntime(WithDB):
    ...
    def run(
        ...
        if self.provider == "gke":
            ...
            k8s_envs.append(
                client.V1EnvVar(name=HUB_SERVER_ENV, value=SERVER_ADDRESS),
                client.V1EnvVar(name=AGENTD_ADDR_ENV, value=desktop_vm.addr),
            )
            k8s_secret_envs = [
                client.V1EnvFromSource(
                    secret_ref=client.V1SecretEnvSource(name=name, optional=False)
                )
            ]
            print("creating pod...")
