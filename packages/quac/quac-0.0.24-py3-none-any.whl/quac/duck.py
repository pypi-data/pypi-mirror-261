...
# %% auto 0
__all__ = ['iterable', 'itertype', 'iterq', 'listq', 'tupleq', 'nlen', 'len1', 'len2', 'len3', 'len4', 'npbool', 'boolic',
           'bool01', 'bools', 'boolq', 'intq', 'ints', 'floatq', 'floats', 'real', 'realq', 'reals', 'num', 'numq',
           'nums', 'numstr', 'numstrq', 'none', 'notnone', 'na', 'anyq', 'pynan', 'pdna', 'npnan', 'nan', 'nanq',
           'nans', 'pattern', 'rematch', 'strtmpl', 'patternq', 'nilstr', 'notnilstr', 'strbytes', 'notstrbytes',
           'notiterstr', 'strq', 'strs', 'ospathlike', 'plibpath', 'posixpath', 'pathlike', 'pathq', 'paths',
           'walktuple', 'walkparts', 'moduleloader', 'modulespec', 'moduletype', 'wrappertype', 'builtinfunctype',
           'trackbacktype', 'functiontype', 'mapping', 'moduleq', 'callq', 'literal', 'enumauto', 'enumdict',
           'dictkeys', 'dictvalues', 'dictitems', 'setq', 'dictq', 'sig', 'param', 'paramkind', 'logger', 'loggerq',
           'ichain', 'iprod', 'ziplong', 'artist', 'colormap', 'lcolormap', 'lscolormap', 'mplpatch', 'mplaxbase',
           'mplaxes', 'nparray', 'npmatrix', 'recarray', 'npempty', 'npdatasource', 'sparray', 'spmatrix', 'cscarray',
           'cscmatrix', 'cooarray', 'coomatrix', 'csrarray', 'csrmatrix', 'diaarray', 'diamatrix', 'dokarray',
           'dokmatrix', 'lilarray', 'lilmatrix', 'bsrarray', 'bsrmatrix', 'sparsearray', 'sparsematrix', 'sparse',
           'array', 'matrix', 'sparseq', 'arrayq', 'matrixq', 'cv2vidcap', 'pilimage', 'vidcap', 'image', 'groupkey',
           'pddelegate', 'pdcataccessor', 'pdindex', 'pdcatindex', 'pdseries', 'boolindex', 'catindex', 'boolindexq',
           'catindexq', 'indextype', 'indexlike', 'seriestype', 'serieslike', 'pddataframe', 'dataframe', 'pttype',
           'pttypeq', 'memfmt', 'presfmt', 'ptsize', 'ptsizeq', 'ptdevice', 'ptdeviceq', 'pttensor', 'inttensor',
           'booltensor', 'floattensor', 'longtensor', 'doubletensor', 'pttensorq', 'inttensorq', 'booltensorq',
           'floattensorq', 'longtensorq', 'doubletensorq', 'nantensor', 'nantensorq', 'tensor', 'tensorq', 'ptmodule',
           'plmodule', 'nnmodule', 'ptdataset', 'pldatamodule', 'ptdataloader', 'neuralode', 'neuralsde',
           'neuraldynamics', 'augmenter', 'odefunc', 'sdefunc', 'sdeito', 'brownianintervial', 'dtype', 'dtypeq',
           'device', 'deviceq', 'devcpu', 'devmps', 'devcuda', 'size', 'sizeq', 'datasource', 'adata', 'adatas',
           'graphtype', 'datatype', 'nnum', 'nint', 'nfloat', 'int2', 'int3', 'int4', 'shaped', 'dimmed', 'dim1',
           'dim2', 'dim3', 'dim4', 'genset', 'setstr', 'setnum', 'kvdict', 'pttndict', 'annreturn', 'slicespec',
           'slicespecs', 'richtree', 'richtext', 'richcons', 'richprog', 'richtask', 'tqdmauto', 'typerapp']
...
from typing import (
    Any, Dict, List, Set, Self, Type, Union, Tuple, Generic, Callable, Literal, 
    TypeVar, ClassVar, TypeAlias, ParamSpec, Optional,
    Iterable, ForwardRef, Mapping, TypeGuard, Generic, Protocol
)
...
from .cons import NIL, NDIM, SHAPE, KEYS, VALUES, __K__, __V__, __N__, __RETANNOT__, __KVTYPES__
...
# %% ../nbs/40_duck.ipynb 24
from typing import Any
class nlen(itertype[*Ts], Duck):
...
# %% ../nbs/40_duck.ipynb 35
class intq(Duck):
    '''`int` or None'''
    __ducktype__ = (int, )
    __species__ = Species.OPT
    
class ints(itertype[*Ts], Generic[*Ts]):
    '''iterable of `int`s'''
    __expected__ = dict(dtype=int)

# %% ../nbs/40_duck.ipynb 37
class floatq(Duck):
    '''`float` or None'''
    __ducktype__ = (float, )
    __species__ = Species.OPT
    
class floats(itertype[*Ts], Generic[*Ts]):
    '''iterable of `float`s'''
...
class reals(itertype[*Ts], Generic[*Ts]):
    '''iterable of `real`'''
    __expected__ = dict(dtype=real)

# %% ../nbs/40_duck.ipynb 41
class num(Duck):
...
class nums(itertype[*Ts], Generic[*Ts]):
    '''iterable of `num`'''
    __expected__ = dict(dtype=num)

# %% ../nbs/40_duck.ipynb 45
class numstr(Duck):
    '''`num` or str'''
...
class notiterstr(itertype[*Ts], Generic[*Ts]):
...
# %% ../nbs/40_duck.ipynb 75
class walktuple(len3, itertype[pathlike, paths, paths]):
...
class walkparts(itertype[walktuple]):
...
class arrayq(array): 
    '''Optional array type (e.g. `np.array`, `np.recarray`, `sp.sparray`, `itertype`, or `list`)'''
    __species__ = Species.OPT
    
class matrixq(matrix): 
    '''Optional matrix type(e.g. `np.matrix`, `sp.matrix`, `itertype`, or `list`)'''
    __species__ = Species.OPT

# %% ../nbs/40_duck.ipynb 114
...
class nantensorq(nantensor, Generic[*Ts]): 
...
class sizeq(size[*Ts]): 
    '''Optional size type (e.g. `tuple`, `torch.Size`, `itertype`)'''
    __species__ = Species.OPT

# %% ../nbs/40_duck.ipynb 176
class datasource(Duck, Generic[*Ts]): 
    '''`numpy.DataSource`''';
    __ducktype__ = (eggs.np_datasource, )   

# %% ../nbs/40_duck.ipynb 178
class adata(Duck): 
    '''`anndata.AnnData`''';
...
class slicespecs(itertype[slicespec]):
