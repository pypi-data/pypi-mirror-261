...
class AIF:
    ...
    def _add_segment(self, Lnode_ID, segment):       
        ...
        self.xaif['aif']['nodes'].append({'text': segment, 'type':'L','nodeID': i_id})

        y_id = self.get_next_max_id('nodes', 'nodeID')	
        self.xaif['aif']['nodes'].append({'text': 'Default Illocuting', 'type':'YA','nodeID': y_id})	

        edge_id = self.get_next_max_id('edges', 'edgeID')	
        self.xaif['aif']['edges'].append({'toID': y_id, 'fromID':node_id,'edgeID': edge_id})
        edge_id = self.get_next_max_id('edges', 'edgeID')	
        self.xaif['aif']['edges'].append({'toID': i_id, 'fromID':y_id,'edgeID': edge_id})
