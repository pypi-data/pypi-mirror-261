...
class Manager:
    def __init__(self, priority:int=-1, maxLogCount:int=1000):
        ...
        :param maxLogCount: Maximum number of logs to store in memory, Values available as list in `Manager().logs`
        """
    ...
    def __log(self, string: str, back: tuple[int,int,int], fore: tuple[int,int,int]):
        ...
        if len(self.logs) > self._maxLogCount:
            self.logs.pop(0)
    ...
    def __formString(category: str, *args:*list[str]):
        ...
        :return:
        """
        return f"[{ctime()}] [{category}] {' '.join(args)}"
