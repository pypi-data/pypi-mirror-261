...
def stockQuarterlyEstimates(stockName):
    ...
    driver.get("https://www.marketwatch.com/investing/stock/{stockName}/analystestimates?mod=mw_quote_tab".format(stockName = stockName))
    # Load the website
    time.sleep(3)
    # Switch to the cookies iframe
    iframe = driver.find_element(By.CSS_SELECTOR, "[title = 'SP Consent Message']")
    # Switch to the iframe
    driver.switch_to.frame(iframe)
    # Click the agree to cookies button
    driver.find_element(By.CSS_SELECTOR, "[title*='YES, I AGREE']").click()
    # Switch back to the default content
    driver.switch_to.default_content()
    # Wait for the page to load
    driver.implicitly_wait(2)
    # Get the page source after it's loaded dynamically
    page_source = driver.page_source

    # Close the virtual window (optional)
    driver.quit()
    # Memory Management
    del driver, iframe
