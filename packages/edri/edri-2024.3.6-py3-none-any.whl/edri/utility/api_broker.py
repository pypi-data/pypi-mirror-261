...
from edri.dataclass.client import Client
from edri.dataclass.event import Event, api_events
...
class ApiBroker(ManagerBase):
    """
        The ApiBroker class serves as a central component for managing API interactions,
        client registrations, and event dispatching in a system that utilizes
        multiprocessing. It inherits from ManagerBase to leverage common management
        functionalities and extends it with API-specific capabilities.

        Attributes:
            api_broker_queue (Queue): A multiprocessing queue used to receive events
            specifically for the API broker.
            clients (Optional[Storage[Client]]): A storage mechanism for registered
            clients, allowing for quick access and management.
            db: Placeholder attribute for potential database connections or operations,
            not implemented by default.
            events (dict): A mapping of event resources to event classes, derived from
            the global api_events definition.

        Methods:
            __init__(router_queue: Queue, api_broker_queue: Queue): Constructor for the
            ApiBroker class.
            name (property): Returns the name of the ApiBroker instance.
            send_specific(event: Event): Sends an event to a specific client based on
            the event's key.
            send_all(event: Event): Broadcasts an event to all clients of a specific
            API type.
            client_register(event: client.Register): Registers a new client and sends
            a confirmation event.
            client_unregister(event: Event): Unregisters a client based on the event's
            key.
            event_register(event: manage.Register): Registers a client for specific
            events and parameters.
            solve(event: Event): Determines the appropriate method to handle an incoming
            event.
            _prepare(): Prepares the ApiBroker for operation by initializing storage for
            clients.
            additional_pipes (property): Returns a set of additional pipes to monitor
            for incoming events.
            run_resolver(): Main loop for processing incoming events and dispatching them
            to the appropriate handlers.
            resolve_callback(event: Event, pipe: Pipe): Callback method for handling
            events received from clients.

        Usage:
            This class should be instantiated with the appropriate queues and used as
            part of a larger multiprocessing application to manage API interactions and
            client communications. It requires subclassing ManagerBase and implementing
            any additional abstract methods or properties as needed.
        """
    def __init__(self, router_queue: Queue[Event], api_broker_queue: Queue[Event]) -> None:
        """
    ...
    def event_register(self, event: manage.Register) -> None:
        """
          ...
          """
        if not event._key:
            raise KeyError("Event key is missing!")
        client = self.clients[event._key]
