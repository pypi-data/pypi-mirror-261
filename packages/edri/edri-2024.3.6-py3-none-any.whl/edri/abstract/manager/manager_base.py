...
from typing import Optional, Type, Tuple, Callable, Union, Generic, TypeVar
...
from edri.dataclass.event import Response, Event
...
class ManagerBase(ABC, Process):
    ...
    def _prepare_resolvers(self) -> None:
        ...
        The 'event' parameter in the signature of the methods being registered must be annotated with the type of
        event it handles. If a method does not have an 'event' parameter as its second parameter, or if the 'event'
        parameter is not properly annotated, the method logs an error and raises a KeyError.

        Exceptions:
            KeyError: Raised if the 'event' parameter is missing from the method's signature or if the 'event'
            parameter is not the second parameter. The error message will include the name of the parameter found
        ...
        def get_event(sig: Signature) -> Type[Event]:
            try:
                ...
                raise e
            return event_sig.annotation

        methods_name = (method_name for method_name in dir(self) if not method_name.startswith("__"))
    ...
    def _resolve_callback_stream_close(self, event: StreamClose, worker: Worker) -> None:
        ...
        """
        raise NotImplementedError("yet")
        # if len(worker.streams) == 1 and event.message is None:
        #     event, stream_key = worker.streams.popitem()
        #     stream_message = event()
        #     stream_message._stream = stream_key + STREAM_CLOSE_MARK
        # elif event.message:
        #     stream_key = worker.streams.pop(event.message.__class__)
        #     stream_message = event.message()
        #     stream_message._stream = stream_key + STREAM_CLOSE_MARK
        # else:
        ...
        #     return
        #
        # request = self._requests.get(stream_message.__class__, None)
        # if not request:
        #     response = self._responses.get(stream_message.__class__, None)
        #     if not response:
    ...
    def start(self, router_queue: Optional[Queue[Event]] = None) -> None:
        """
