...
class Router:
    ...
    def __init__(self, queue: "Queue[Event]", components: Set[ManagerBase]) -> None:
        ...
        self.subscribed_requests: Dict[Type[Event], Set[Connection]] = {}
        self.subscribed_responses: Dict[Type[Event], Set[Connection]] = {}
    ...
    #         self.logger.error("Nelze přiřadit connector!")
    #         return
    #     if event.request:
    #         if (subscribers := self.subscribed_requests.get(event.event, None)) is not None:
    #             subscribers.add(self.connector_pipe)
    #         else:
    #             self.subscribed_requests[event.event] = {self.connector_pipe}
    #     else:
    #         if (subscribers := self.subscribed_responses.get(event.event, None)) is not None:
    #             subscribers.add(self.connector_pipe)
    #         else:
    #             self.subscribed_responses[event.event] = {self.connector_pipe}
    ...
    def subscribe(self, event: router.Subscribe) -> Optional[Connection]:
        ...
        if event.request:
            if (subscribers := self.subscribed_requests.get(event.event_type, None)) is not None:
                subscribers.add(event.pipe)
            else:
                self.subscribed_requests[event.event_type] = {event.pipe}
        else:
            if (subscribers := self.subscribed_responses.get(event.event_type, None)) is not None:
                subscribers.add(event.pipe)
            else:
                self.subscribed_responses[event.event_type] = {event.pipe}
    ...
    def unsubscribe(self, event: router.Unsubscribe) -> None:
        ...
        """
        if event.request:
            if (connections := self.subscribed_requests.get(event.__class__, None)) is not None:
                connections.remove(event.pipe)
        else:
            if (connections := self.subscribed_responses.get(event.__class__, None)) is not None:
                connections.remove(event.pipe)
    ...
    def send_from(self, event: router.SendFrom) -> None:
        ...
        """
        if event.id:
            cached_events = self.cache.events_from(event.id)
        else:
            cached_events = [item.event for item in self.cache.items]
        self.logger.debug("Odesílání zpožděných zpráv: %s", len(cached_events))
        self.logger.debug("Requests: %s", self.subscribed_requests)
        self.logger.debug("Responses: %s", self.subscribed_responses)
        for cache_event in cached_events:
            if cache_event.get_response() is None:
                if self.connector_pipe in self.subscribed_requests.get(cache_event.__class__, []):
                    self.logger.debug("Odesílání switchi: %s", cache_event)
                    self.connector_pipe.send(event)
            else:
                if self.connector_pipe in self.subscribed_responses.get(cache_event.__class__, []):
                    self.logger.debug("Odesílání switchi: %s", cache_event)
                    self.connector_pipe.send(cache_event)
    ...
    def send(self, event: Event, connections: Set[Connection]) -> None:
        ...
        for connection in connections:
            if event._switch and self.connector_pipe == connection:  # This supposes to stop event storm
                continue
            try:
