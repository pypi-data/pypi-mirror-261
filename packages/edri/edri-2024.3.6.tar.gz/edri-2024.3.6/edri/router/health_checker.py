...
class HealthChecker:
    ...
    class Record:
        ...
        Attributes:
            ...
            message (Optional[str]): Additional message or information about the component's health.
        """
        name: str
        pipe: Connection
        ...
        if name not in self.state:
            ...
            for component in self.components:
                if component.name == name:
                    break
            self.state[name] = self.Record(name, pipe, component)
            self.logger.debug("Přidán manager %s %s", name, pipe)
    ...
    def control_result(self, name: str, status: ResponseStatus) -> None:
        ...
        """
        self.logger.debug("Přidán záznam k %s status %s", name, status)
    ...
    def save_status(self) -> None:
        ...
        for name, record in self.state.items():
            if record.status is None:
                self.logger.debug("%s nebyl zatím zkontrolován", record.name)
                continue
            if (record.timestamp + timedelta(seconds=HEALTH_CHECK_TIMEOUT * 2)) < now:
                self.logger.warning("%s neodeslal stavovou zprávů - poslední stav %s - %s",
