...
from cassandra.cluster import Cluster, Session, HostDistance, NoHostAvailable, ResponseFuture, \
    ExecutionProfile
...
class ORMInstance(IDatabaseService):
        ...
        for _ in range(self._attempts):
            try:
                ...
                self.cluster = Cluster(
                    ...
                    execution_profiles=self._execution_profile
                )
    ...
    def select_from_json(self, json_input: str, table_name: str) -> ResponseFuture:
        return self.select(json.loads(json_input), table_name)

    def select(self, dictionary_input: Dict[str, Any], table_name: str) -> ResponseFuture:
        try:
    ...
    def insert(self, dictionary_input: Dict[str, Any], table_name: str) -> ResponseFuture:
        try:
            ...
            return self._bind_delete_or_insert(filtered_columns, prepared_statement)
        except Exception as e:
            self.log.error(f"Failed to insert data: {e}, in table {table_name}")
            raise ApolloORMException(e)

    def delete(self, dictionary_input: Dict[str, Any], table_name: str) -> ResponseFuture:
        try:
    ...
    def _bind_delete_or_insert(self, filtered_columns: Dict[str, Column],
                               prepared_statement: PreparedStatement) -> ResponseFuture:
    ...
    def _execute_async_query(self, statement: PreparedStatement, values: List[Any]) -> ResponseFuture:
        ...
        try:
            return self._future(statement, values)
    ...
    def _future(self, statement: PreparedStatement, values: List[Any]) -> ResponseFuture:
        self.log.info("Initializing future query...")
        return self.session.execute_async(statement.bind(values))

    def _on_success(self, rows: List, *args) -> None:
        self.log.info(f"Query executed successfully: {args[0]} with values: {args[1]}")
        print("Rows: ", rows)
        if isinstance(rows, list):
            for row in rows:
                args[2].append(row)

    def _on_error(self, error: Exception, *args) -> None:
        self.log.error(f"Failed to execute query: {args[0]} with values {args[1]} - {error}")
        raise ApolloORMException(error)
