...
def process_report():
    if args.report:
        ...
        testdata = tabulate.tabulate(tabular_data=[
            ["",""],
            ["Execution START __________: ", "10:30"],
            ["Execution STOP ___________: ", "10:31"],
            ["Execution DURATION _______: ", "8sec (=0d 0h 0min 8sec)"],
            ["",""],
            ["Execution Result _________: ", "[x] OK        [ ] Failed        [ ] Failed on internal error"],
            ["",""],
            ["Python / Platform ________: ", "%d.%d / %s" % (sys.version_info.major, sys.version_info.minor, sys.platform)],
            ["Hostname _________________: ", socket.gethostname()],
            ["User (UID) _______________: ", "%s (%d)" % (getpass.getuser(), os.getuid())],
            ["Browser Mode _____________: ", driver_mode],
            ["Test Engine ______________: ", "dcx-" + const_appversion],
            ["",""],
            ["Playbook Title (if set) __: ","-"],
            ["Playbook Directory _______: ",envdata["PWD"]],
        ...
        for report_i in range(0, len(play)):
            ...
            pdf.page(the_page).text(7,10, "TARGET:  " + str(report_play_part[0]))
            pdf.page(the_page).text(8,10, "COMMAND: " + str(report_play_part[1]))
            pdf.page(the_page).text(9,10, "PARAMS: " + str(report_play_part[2:]))




            pdf.page(the_page).text(30,5, "_"*55 + " [ BEFORE  ] " + "_"*55)
            pdf.page(the_page).text(65,5, "_"*55 + " [ AFTER   ] " + "_"*55)
            ...
            if jpg_landscape:
                ...
                calclulated_height = int(250.0 / jpg_ratio)
                image_params = [15, 200, 250, calclulated_height, jpg]
            else:
                #TODO: porttrait mode
                pass
            ...
            if jpg_landscape:
                ...
                jpg_ratio = float(jpg_width) / float(jpg_height) # 1.XXXX
                calclulated_height = int(250.0 / jpg_ratio)
                image_params = [15, 20, 250, calclulated_height, jpg]
            else:
                #TODO: porttrait mode
                pass
            pdf.page(the_page).image_manual_mm(*image_params)
