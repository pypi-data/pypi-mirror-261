...
class Api:
    ...
    def get_room_emote(self, room_id: str, pack: str, name: str) -> str | None:
        if (not room_id in self.room_emotes.keys()
            or
            not pack in self.room_emotes[room_id].keys()
            or
            not name in self.room_emotes[room_id][pack]['images'].keys()):
            return None
        return self.room_emotes[room_id][pack]['images'][name]['url']

    def find_emote(self, name: str) -> str | None:
        """
        Find emote mxc url in Api.user_emotes and Api.room_emotes
        """
        for emote, obj in self.user_emotes['images'].items():
            if emote == name: return obj['url']
        for _, packs in self.room_emotes.items():
            for pack in packs.keys():
                for emote, obj in packs[pack]['images'].items():
                    if emote == name: return obj['url']
    ...
    def _add_emotes(self, formatted_body: str, emotes: list[str]) -> str:
        ...
        for emote in emotes:
            emote_url = self.find_emote(emote)
            if emote_url is None: continue

            formatted_body = formatted_body.replace(
                f":{emote}:",
                FileUrl(emote, emote_url, FileType.Image, {'height' : 32}).to_html()
            )
    ...
    async def send(self, room_id: str, body: str | File,
        ...
        """
        return await self.__send(room_id, body, use_html, mentions=mentions, emotes=emotes)
