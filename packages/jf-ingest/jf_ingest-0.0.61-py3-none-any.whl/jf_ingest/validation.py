...
def validate_git(git_configs: list[GitConfig]) -> list[GitConnectionHealthCheckResult]:
    ...
    for i, git_config in enumerate(git_configs, start=1):
        print(f"\nGit Configuration details for instance {i}/{len(git_configs)}:")
        print(f"  Instance slug: {git_config.instance_slug}")
        print(f"  Provider: {git_config.git_provider}")
        print(f"  Organizations: {len(git_config.git_organizations)}")
        if len(git_config.excluded_organizations) > 0:
            print(f"  Excluded organizations: {git_config.excluded_organizations}")
        if len(git_config.included_repos) > 0:
            print(f"  Included repos: {git_config.included_repos}")
        if len(git_config.excluded_repos) > 0:
            print(f"  Excluded repos: {git_config.excluded_repos}")
        if len(git_config.included_branches_by_repo) > 0:
            print(f"  Included Branches: {git_config.included_branches_by_repo}")

        print('==> Testing Git connection...')

        git_adapter = GitAdapter.get_git_adapter(git_config, hide_progress_bar=True)
        for org in sorted(git_adapter.get_organizations(), key=lambda org: org.login):
            ...
            print(f'\nFor Organization {org.login} we have the following API Scopes:')
            print(f'  {api_scopes}\n')
            try:
                (
                ...
                for repo in sorted(repos, key=lambda repo: repo.name):
                    ...
                    if (
                        ...
                        print(f'    PRs:      {repo_access_result.pull_requests}')
                        print(f'    Commits:  {repo_access_result.commits_on_default_branch}')
                        print(f'    Branches: {repo_access_result.branches}')
