...
def list_branches():
    #command = "git branch --list"  # lists local only
    # list remote branches, take out the annoying origin/ prefix, and remove the HEAD branch
    command = "git branch -r | grep -v 'HEAD -> origin' | sed 's/origin\///'"  
...
def find_commits_by_phrase(search_phrase):
   ...
   for line in result.stdout.splitlines():
        ...
        if message_matches:
            tags = get_tags_for_commit(commit_hash, None)  # don't try to match the search_phrase in tags, just get all tags
        else:
            tags = get_tags_for_commit(commit_hash, search_phrase) # find tags that match the search_phrase
...
def deploying_workflow():
    ...
    if permissions:
        ...
        if selected_branch == default_remote_branch:
            print(colorize(f"Error: Deployment to the default remote branch '{default_remote_branch}' is not allowed.", 41))
            main_menu()
