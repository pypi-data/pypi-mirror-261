...
class RabbitMQCommunicator:
    def __init__(self,
        ...
        if producer_config and not callback:
        ...
        elif consumer_config and callback:
            ...
            raise ValueError("Incorrect RabbitMq configuration.")
    ...
    async def send_message(self, message):
        await self.attempt_with_retry(lambda: self._rabbitmq_producer.send_message(message),
                                      ...
                                      lambda: self._rabbitmq_producer.connect())
    ...
    async def connect_rabbitmq_producer(self):
        await self.attempt_with_retry(lambda: self._rabbitmq_producer.connect(),
                                      "connect to RabbitMQ producer",
                                      self._producer_reconnect_interval_sec)

    async def connect_rabbitmq_consumer(self):
        await self.attempt_with_retry(lambda: self._rabbitmq_consumer.connect(),
