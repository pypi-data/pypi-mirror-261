    ...
    def other_submission(result_directory, args):
        ...
        for i in range(len(manifestFile[head[0]])):
            if '#' not in manifestFile[head[0]][i]:
                ...
                if not pd.isna(manifestFile['STUDY_REF'][i]):
                    if manifestFile['STUDY_REF'][i].startswith('PRJEB') or manifestFile['STUDY_REF'][i].startswith('ERP'):
                        study_ref = etree.SubElement(analysis, 'STUDY_REF', accession=manifestFile['STUDY_REF'][i])
                    else : 
                        study_ref = etree.SubElement(analysis, 'STUDY_REF', refname=manifestFile['STUDY_REF'][i])
                        
                if not pd.isna(manifestFile['SAMPLE_REF'][i]):
                    if manifestFile['SAMPLE_REF'][i].startswith('ERS') :    
                        sample_ref = etree.SubElement(analysis, 'SAMPLE_REF', accession=manifestFile['SAMPLE_REF'][i])
                    else : 
                        sample_ref = etree.SubElement(analysis, 'SAMPLE_REF', refname=manifestFile['SAMPLE_REF'][i])

                
                if not pd.isna(manifestFile['RUN_REF'][i]):
                    if manifestFile['RUN_REF'][i].startswith('ERR') :
                        run_ref = etree.SubElement(analysis, 'RUN_REF', accession=manifestFile['RUN_REF'][i])
                    else :
                        run_ref = etree.SubElement(analysis, 'RUN_REF', refname=manifestFile['RUN_REF'][i])
