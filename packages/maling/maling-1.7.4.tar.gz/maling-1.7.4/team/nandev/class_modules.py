...
async def refresh_modules():
    ...
    for modul in USER_MOD:
        imported_module = importlib.import_module("modular." + modul)
        if hasattr(imported_module, "__modles____") and imported_module.__modles____:
            imported_module.__modles____ = imported_module.__modles____
        if hasattr(imported_module, "__modles____") and imported_module.__modles____:
            if not imported_module.__modles____.replace(" ", "_").lower() in CMD_HELP:
                CMD_HELP[imported_module.__modles____.replace(" ", "_").lower()] = imported_module
            else:
