...
def _do_eigen_post_request(requesturl,data,auth):
    headers = {}

    if settings._azure_tenant_id_ and settings._azure_client_id_ and settings._azure_auth_enabled_ and auth:
        headers = _authenticate_azure_user('/'.join(requesturl.split("/")[0:3]))
    
    resp = requests.post(requesturl, json=data, verify=False, headers=headers)
    return resp

def _post_eigen_events(requesturl,data,auth):
    headers = {}
    success = []
    failures = []
