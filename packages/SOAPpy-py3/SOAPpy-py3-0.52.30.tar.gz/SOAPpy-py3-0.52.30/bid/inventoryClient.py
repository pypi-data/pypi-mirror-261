...
def SimpleBuy(serv, sa, epname):
    ...
    return serv.SimpleBuy(
        ProductName="widget", Quantity=50, Address="this is my address"
    )  # JHawk, Phalanx require this order of params


def RequestForQuote(serv, sa, epname):
    serv = serv._sa(sa % {"methodname": "RequestForQuote"})
    return serv.RequestForQuote(
        Quantity=3, ProductName="thing"
    )  # for Phalanx, JHawk
