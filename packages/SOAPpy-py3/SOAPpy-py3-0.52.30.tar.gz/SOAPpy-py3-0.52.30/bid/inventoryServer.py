...
def SimpleBuy(Address, ProductName, Quantity):
    ...
    NUMSIMPLEBUYS += 1
    if Quantity < 1:
        raise ValueError("must order at least one")
    else:
        return "Receipt for %d %s(s) bought from %s" % (
            int(Quantity),
            ProductName,
            serverstring,
        )
...
def RequestForQuote(ProductName, Quantity):
    ...
    NUMREQUESTS += 1
    if Quantity < 1:
        raise ValueError("must order at least 1")
    else:
        import whrandom
...
def Buy(**kw):
    ...
    try:
        ...
        if POkeys != POkeys_expected:
            raise ValueError(
                "struct 'PurchaseOrder' needs %s, %s, %s, %s, and %s"
                % tuple(POkeys_expected)
            )
    except:
...
def Monitor(str):
    if str == "actzero":
        ...
        global NUMSIMPLEBUYS
        global NUMPINGS
        return "(Buys, RequestForQuote(s),SimpleBuy(s), Ping(s)) = " + repr(
            (NUMBUYS, NUMREQUESTS, NUMSIMPLEBUYS, NUMPINGS)
        )
    else:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        try:
            port = int(sys.argv[1])
            if port not in list(range(2000, 15000)):
                raise ValueError
        except:
