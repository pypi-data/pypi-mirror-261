...
DEFAULT_METHODS = (
    ...
    "echoVerySmallFloat",
    "echoVoid",
    "mustUnderstandEqualsOne",
    "mustUnderstandEqualsZero",
)
...
def methodUsage():
    ...
    print(
        "Methods are specified by number. Multiple methods can be "
    ...
    for i in range(half):
        ...
        if i + half < len(DEFAULT_METHODS):
            print(
                "%4d. %-25s" % (i + 1 + half, DEFAULT_METHODS[i + half]),
                end=" ",
            )
        print()
...
def testActorShouldPass(server, action, harsh):
    ...
    hd = SOAP.headerType()
    hd.InteropTestHeader = SOAP.stringType(
        "This shouldn't fault because " "the mustUnderstand attribute is 0"
    )
    hd.InteropTestHeader._setMustUnderstand(0)
    hd.InteropTestHeader._setActor("http://schemas.xmlsoap.org/soap/actor/next")
...
def testActorShouldFail(server, action, harsh):
    ...
    hd = SOAP.headerType()
    hd.InteropTestHeader = SOAP.stringType(
        "This should fault because " "the mustUnderstand attribute is 1"
    )
...
def testEchoFloatArray(server, action, harsh):
    ...
    for i in range(len(test)):
        ...
        if not nearlyeq(result[i], test[i]):
            raise Exception(
                "@ %d expected %s, got %s" % (i, repr(test), repr(result))
            )
...
def testEchoStructArray(server, action, harsh):
    test = [
        {"varFloat": -5.398, "varInt": -546, "varString": "West Virginia"},
...
def testEchoVoid(server, action, harsh):
    ...
    for k in list(result.__dict__.keys()):
        if k[0] != "_":
            raise Exception(
                "expected an empty structType, got %s" % repr(result.__dict__)
            )
...
def testMustUnderstandEqualsOne(server, action, harsh):
    ...
    server = server._sa(action % {"methodname": "echoInteger"})
    hd = SOAP.headerType()
    hd.MustUnderstandThis = SOAP.stringType(
        "This should fault because " "the mustUnderstand attribute is 1"
    )
...
def main():
        ...
        for num in methodnums:
            ...
            try:
                ...
                if name in s["nonfunctional"]:
                    print(title, "succeeded despite being marked nonfunctional")
                if "s" in output:
    ...
    if stats:
        ...
        if stats > 0 or fail > 0:
            print(
                "Failed unexpectedly: %d (%3.2f%%)"
                % (fail, 100.0 * fail / total)
            )
        if stats > 0:
            print(
                " Failed as expected: %d (%3.2f%%)"
                % (failok, 100.0 * failok / total)
            )
        if stats > 0 or notimp > 0:
            print(
                "    Not implemented: %d (%3.2f%%)"
