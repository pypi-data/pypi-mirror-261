...
class SOAPParser(xml.sax.handler.ContentHandler):
    ...
    def startElementNS(self, name, qname, attrs):
        ...
        if self._next == "E":
            if name[1] != "Envelope":
                raise Error(
                    "expected `SOAP-ENV:Envelope', " "got `%s'" % toStr(name)
                )
            if name[0] != NS.ENV:
                raise faultType(
            ...
            if name == (NS.ENV, "Body"):
                ...
                raise Error(
                    "expected `SOAP-ENV:Body', " "got `%s'" % toStr(name)
                )
        elif self._next == "":
        ...
        while 1:
            ...
            if cur.rules != None:
                ...
                if isinstance(rule, collections.Callable):
                    ...
                    data = self.convertType(
                        string.join(self._data, ""), rule, attrs
                    )
            ...
            if (
                ...
                and kind == None
                and len(cur) == 0
                and (self._data is None or "".join(self._data).strip() == "")
            ):
                data = structType(name=(ns, name), attrs=attrs)
                break
    ...
    def endDocument(self):
        if len(self._refs) == 1:
            ...
            raise Error(
                "unresolved references " + ", ".join(list(self._refs.keys()))
            )
    ...
    class DATETIMECONSTS:
        ...
        TIMEZONEre = (
            "(?P<zulu>Z)|(?P<tzsign>[-+])(?P<tzhour>\d{2}):"
            "(?P<tzminute>\d{2})"
        )
        BOSre = "^\s*"
        EOSre = "\s*$"
        ...
        dateTime = (
            "%(b)s%(sign)s%(century)s%(year)s-%(month)s-%(day)sT"
            "%(hour)s:%(minute)s:%(second)s(%(timezone)s)?%(e)s" % __allres
        )
        timeInstant = dateTime
        timePeriod = dateTime
        time = (
            "%(b)s%(hour)s:%(minute)s:%(second)s(%(timezone)s)?%(e)s" % __allres
        )
        date = (
            "%(b)s%(sign)s%(century)s%(year)s-%(month)s-%(day)s"
            "(%(timezone)s)?%(e)s" % __allres
        )
        century = "%(b)s%(sign)s%(century)s(%(timezone)s)?%(e)s" % __allres
        gYearMonth = (
            "%(b)s%(sign)s%(century)s%(year)s-%(month)s"
            "(%(timezone)s)?%(e)s" % __allres
        )
        gYear = (
            "%(b)s%(sign)s%(century)s%(year)s(%(timezone)s)?%(e)s" % __allres
        )
        year = gYear
    ...
    def convertDateTime(self, value, kind):
        ...
        def applyZoneOffset(months, zoffs, date, minfield, posday=1):
            ...
            while 1:
                ...
                leap = (
                    ...
                    and date[1] == 3
                    and date[0] % 4 == 0
                    and (date[0] % 100 != 0 or date[0] % 400 == 0)
                )
        ...
        try:
            ...
            if kind == "recurringInstant":
                ...
                return tuple(
                    applyZoneOffset(
                        self.DATETIMECONSTS.months, getZoneOffset(d), r, fn, 0
                    )
                )
    ...
    intlimits = {
        ...
        "long": (1, -9223372036854775808, 9223372036854775807),
...
def _parseSOAP(
    ...
    inpsrc.setByteStream(BytesIO(xml_str.encode()))
...
def parseSOAPRPC(
    xml_str, header=0, body=0, attrs=0, rules=None, ignore_ext=None
):
