...
def align_moldatas(moldatas: list[MolData], template: Mol) -> list[MolData]:
    ...
    # anchor_mol = dm.sanitize_mol(dm.fix_mol(anchor.mol))

    # alinhadas = [anchor_mol]

    # atoms = [[int(a)-1 for a in df[1][j].split(',')] for j in range(len(df[1]))]

    # refAtoms = atoms[0]
    # Chem.MolToPDBFile(anchor_mol,filename=os.path.join(directory,mols[0][:-len(extension)]+"pdb"))

    # for i in range(1,len(mols)):
    #     prbMol = Chem.MolFromMol2File(os.path.join(directory,mols[i]),removeHs=False)
    #     prbAtoms = atoms[i]
    # #     print(prbAtoms)
    #     alinhamento = AlignMol(prbMol,anchor_mol,atomMap=list(zip(prbAtoms,refAtoms)))
