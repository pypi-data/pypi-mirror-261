...
class EulithWeb3(Web3):
    ...
    def eulith_contract_address(self, account: str) -> str:
        ...
        for c in contracts:
            if c['authorized_address'].lower() == account.lower() and c['chain_id'] == self.chain_id:
...
class v0:
    ...
    def bundle_and_commit(
            self, auth_address: str, txs: List[Any], commit_options: Optional[atomic.CommitRequest] = None
    ) -> TxParams:
        """
        Equivalent to calling `ew3.eth.send_transaction` for each transaction, followed by `commit_atomic_transaction()`
        """
        request = atomic.BundleRequest(auth_address=auth_address, transactions=txs, commit_options=commit_options)
    ...
    def get_armor_and_safe_addresses(self, authorized_trading_address: str) -> (str, str):
        ...
        for c in contracts:
            if c['authorized_address'].lower() == authorized_trading_address.lower() and c[
                'chain_id'] == self.ew3.chain_id:
    ...
    def append_to_draft_client_whitelist(self, auth_address: str, addresses: List[str],
                                         chain_id: Optional[int] = None) -> int:
        """
    ...
    def append_to_draft_client_whitelist_v2(self, list_id: int,
                                            addresses: List[whitelists_v2.AddressOnChain]) -> whitelists_v2.Whitelist:
        """
    ...
    def start_activate_whitelist(self, list_id: int, auth_address: str, safe_address: str,
                                 chain_id: int) -> whitelists_v2.rpc.StartActivateResponse:
        request = whitelists_v2.rpc.StartActivateRequest(list_id=list_id, auth_address=auth_address,
                                                         safe_address=safe_address, chain_id=chain_id)
        return self.ew3.eulith_service.start_activate_whitelist(request)

    def submit_activate_whitelist_signature(self, signer: Signer,
                                            activate_request: whitelists_v2.rpc.StartActivateResponse) -> whitelists_v2.rpc.SubmitActivateSignatureResponse:
        ...
        request = whitelists_v2.rpc.SubmitActivateSignatureRequest(activation_id=activate_request["activation_id"],
    ...
    def submit_light_simulation_signature(self, signer: Signer, proposal_id: int) -> LightSimulationSubmitResponse:
        ...
        response, error = self.ew3.eulith_service.submit_light_simulation_signature(proposal_id, signer.address,
                                                                                    serialized_signature,
                                                                                    hash_response["hash"])
        if error:
            raise EulithRpcException(error)
    ...
    def check_tx_receipt_diag(self, tx_receipt: TxReceipt):
        if tx_receipt["status"] != ETHEREUM_STATUS_SUCCESS:
            self.ew3.eulith_service.eulith_provider.make_request("trace_transaction",
