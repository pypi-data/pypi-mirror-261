...
def get_hyper_liquid_create_order_typed_data(message: HyperliquidCreateOrderHashInput) -> dict:
    types = {
        "EIP712Domain": [
...
def get_hyper_liquid_create_order_hash(message: HyperliquidCreateOrderHashInput) -> bytes:
    signable_message = encode_typed_data(full_message=get_hyper_liquid_create_order_typed_data(message))
...
def get_hyper_liquid_cancel_order_typed_data(message: HyperliquidCancelOrderHashInput) -> dict:
    types = {
        "EIP712Domain": [
            {"name": "name", "type": "string"},
            {"name": "version", "type": "string"},
...
def get_hyper_liquid_cancel_order_hash(message: HyperliquidCancelOrderHashInput) -> bytes:
    signable_message = encode_typed_data(full_message=get_hyper_liquid_cancel_order_typed_data(message))
    return keccak(b"\x19\x01" + signable_message.header + signable_message.body)
