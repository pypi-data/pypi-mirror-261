...
from dronebuddylib.models import AtomicEngineConfigurations
...
from dronebuddylib.utils.utils import overwrite_file, read_from_file
...
class FaceRecognitionKNNImpl(IFaceRecognition):
    ...
    def remember_face(self, image=None, name=None) -> bool:
        ...
        self.create_data_set(face_classifier, name, 0, 900)
        ...
        for i in range(5):
            logger.log_info(self.get_class_name(), 'Starting test set creation in ' + str(5 - i) + ' seconds...')
            time.sleep(1)

        logger.log_info(self.get_class_name(), 'Starting to create testing data set for ' + name)
        self.create_data_set(face_classifier, name, 1, 100)
        logger.log_info(self.get_class_name(), 'Successfully created testing data set for ' + name)

        logger.log_info(self.get_class_name(), 'Get ready for the test set creation ' + name)

        for i in range(5):
            logger.log_info(self.get_class_name(), 'Starting validation set creation in ' + str(5 - i) + ' seconds...')
            time.sleep(1)

        logger.log_info(self.get_class_name(), 'Starting to create validation data set for ' + name)

        self.create_data_set(face_classifier, name, 2, 50)
    ...
    def create_memory(self):
        ...
        self.train(training_path_name,
                   model_save_path=classifier_path_name, n_neighbors=2,
                   verbose=True)
        ...
        accuracy_score = self.test_classifier()
    ...
    def create_data_set(self, face_classifier, participant_name, data_mode, data_set_size=900):
    ...
    def get_optional_params(self) -> list:
        ...
        """
        return [AtomicEngineConfigurations.FACE_RECOGNITION_ALGORITHM_NEIGHBORS,
                AtomicEngineConfigurations.FACE_RECOGNITION_ALGORITHM_NAME,
                AtomicEngineConfigurations.FACE_RECOGNITION_MODEL_SAVING_PATH,
                AtomicEngineConfigurations.FACE_RECOGNITION_TESTING_DATA_SET_SIZE,
                AtomicEngineConfigurations.FACE_RECOGNITION_TESTING_DATA_SET_SIZE,
                AtomicEngineConfigurations.FACE_RECOGNITION_VALIDATION_DATA_SET_SIZE, ]
