       ...
       with connect(self.websocket,user_agent_header=ua.random) as websocket:
         ...
         while True:
           ...
           if event['t'] == "GUILD_BAN_ADD":
              user = User(self,event['d']['user'],self.cdn,self.token,self.base_url,event['d']['guild_id'])
              for g in self.allGuilds:
                if g.id == event['d']['guild_id']:
                  guild = g
              await self.events.on_guild_ban_add(guild,user)
            if event['t'] == "GUILD_BAN_REMOVE":
              user = User(self,event['d']['user'],self.cdn,self.token,self.base_url,event['d']['guild_id'])
              for g in self.allGuilds:
                 if g.id == event['d']['guild_id']:
                   guild = g
              await self.events.on_guild_ban_remove(guild,user)
