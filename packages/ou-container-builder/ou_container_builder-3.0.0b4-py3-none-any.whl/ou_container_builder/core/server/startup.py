...
<body>
  ...
  <script>
    ...
    const ws = new WebSocket(((window.location.protocol === "https:") ? "wss://" : "ws://") + window.location.host + "/ocl-status");
...
async def main():
    ...
    application = tornado.web.Application(
        [("/ocl-status", ProgressHandler, {"queue": queue}), ("/.*", IndexRequestHandler)]
    )
