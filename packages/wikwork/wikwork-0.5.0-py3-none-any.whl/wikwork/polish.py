...
class PolishEntry():
    ...
    adj_comp: str
        ...
        are done:
            'brak' -> '[no comp.]' (ie, the adjective has no comparative form)
            'bardziej' -> 'bardziej ~'
    ...
    verb_def_id: str
        ...
        See `adj_def_id` for additional notes.
    verb_ref_etal: str
        Information on the line between the verb_def_id and the verb template.
    verb_aspect: str
    ...
    noun_gender_subcat: str
        ...
        below. These tend to occur after the '{{znaczenia}}' template and in
        the future the logic might be updated to restrict to strings only in
        this section.
        "''rzeczownik, rodzaj żeński''", "''rzeczownik, rodzaj nijaki''",
        The following strings are processed and mapped to the following output:
    ...
    @classmethod
    def build_new(cls) -> 'PolishEntry':
        """ Build new PolishEntry instance """
        return cls()
...
# Private functions
#------------------
def _parse_verb_template(self: PolishWord, line: str,
                         entry_index: int) -> bool:
...
def _parse_noun_template(self: PolishWord, line: str,
                         entry_index: int) -> bool:
...
def _parse_adjective_template(self: PolishWord, line: str,
                              entry_index: int) -> bool:
    ...
    if res:
        ...
        if not self.entries[entry_index].adj_comp and adj_comp:
            current_entry.adj_def_id = adj_def_id
            current_entry.adj_comp = adj_comp
            return True
        elif not self.entries[entry_index].adj_comp2 and adj_comp:
...
#        '{{XXXX}}' : 'word_separations',
        ...
        '{{przykłady}}' : 'examples',
        '{{frazeologia}}' : 'expressions',
        '{{kolokacje}}' : 'word_combos',
        '{{pokrewne}}' : 'word_formations',
        '{{źródła}}' : 'references',
        '{{składnia}}' : 'syntax',
        '{{meronimy}}' : 'meronyms',
        '{{holonimy}}' : 'holonyms',
    ...
    def do_on_each_line(self: PolishWord, entry_index: int, line: str) -> None:
    ...
    def process_other_templates(self: PolishWord,
                                line: str, entry_index: int) -> bool:
        if line.find('odmiana-przymiotnik-polski') != -1:
            return _parse_adjective_template(self, line, entry_index)
        elif line.find('odmiana-rzeczownik-polski') != -1:
            return _parse_noun_template(self, line, entry_index)
        elif line.find('odmiana-czasownik-polski') != -1:
            return _parse_verb_template(self, line, entry_index)
        else:
            return False

    parse._parse_multi(self, temp_to_attrib_dict=temp_to_attrib_dict,
        ...
        entry_constructor=PolishEntry.build_new,
