...
class GermanEntry():
    ...
    @classmethod
    def build_new(cls) -> 'GermanEntry':
        """ Build new GermanEntry instance """
        return cls()
...
class GermanWord(page_media.Headword):
    ...
    # Private methods
    #------------------
    def _parse_noun_template(self, line: str, entry_index: int) -> None:
    ...
    def _parse_verb_template(self, line: str, entry_index: int) -> None:
    ...
    def _parse_adjective_template(self, line: str, entry_index: int) -> None:
    ...
    def _parse_wikitext(self) -> None:
        ...
        def do_on_each_line(self: GermanWord, # pylint: disable=unused-argument
                entry_index: int,       # pylint: disable=unused-argument
        ...
        def process_other_templates(self: GermanWord,
                                    line: str, entry_index: int) -> bool:
            if line.startswith('{{Deutsch Verb Übersicht|'):
                self._parse_verb_template(line, entry_index)
                return True
            elif line.startswith('{{Deutsch Substantiv Übersicht|'):
                self._parse_noun_template(line, entry_index)
                return True
            elif line.startswith('{{Deutsch Adjektiv Übersicht|'):
                self._parse_adjective_template(line, entry_index)
                return True
        ...
        parse._parse_multi(self, temp_to_attrib_dict=temp_to_attrib_dict,
            ...
            entry_constructor=GermanEntry.build_new,
