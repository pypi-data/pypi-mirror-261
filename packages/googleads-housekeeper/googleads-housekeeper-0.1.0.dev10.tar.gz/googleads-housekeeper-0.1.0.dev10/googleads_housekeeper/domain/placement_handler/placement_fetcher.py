...
class PlacementFetcher:
    ...
    def get_placements_for_account(
        ...
        Args:
            ...
            task: A task defining parameters of fetching.
        Returns:
        ...
        placements = self._get_placement_data(task=task, account=account)
        ...
        if runtime_options.is_conversion_query:
            placements_by_conversion_name = self._get_placement_data(
                task=task,
                account=account,
                query_class=entities.PlacementsConversionSplit)
            if 'YOUTUBE_VIDEO' in task.placement_types:
                if (detail_placements_by_conversion_name :=
                        self._get_placement_data(
                            task=task,
                            account=account,
                            query_class=entities.PlacementsConversionSplit)):
                    placements_by_conversion_name = (
                        placements_by_conversion_name +
                        detail_placements_by_conversion_name)
            if placements_by_conversion_name:
                ...
                placements_by_conversion_name = (
                    conversion_split_exclusion_specification
                    .apply_specifications(placements_by_conversion_name))
                placements = self._join_conversion_split(
                    placements, placements_by_conversion_name,
                    runtime_options.conversion_name)
        return self._aggregate_placements(placements, task.exclusion_level)
    ...
    def get_already_excluded_placements(
        ...
        for row in already_excluded_placements:
            row['placement'] = (
                row.website_url or row.app_id or row.video_id or row.channel_id)
