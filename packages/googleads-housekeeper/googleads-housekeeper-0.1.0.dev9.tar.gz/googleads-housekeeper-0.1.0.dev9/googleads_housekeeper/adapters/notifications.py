...
class GoogleCloudAppEngineEmailNotifications(BaseNotifications):
    ...
    def _prepare_message(self, payload: MessagePayload) -> mail.Message:
        ...
        if placements_excluded := payload.placements_excluded_sample:
            subject = (f'{payload.total_placement_excluded} '
                       'placements were excluded')
            body = self._generate_notification_html(task_name,
                                                    placements_excluded)
        else:
    ...
    def _generate_notification_html(
            self, task_name: str, placements: gaarf.report.GaarfReport) -> str:
        html_body = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
        ...
        html_body += f"""
                ...
                <p><strong>Formula:</strong> impressions > 10 and clicks = 0
                (<a href="[Direct Task Link]" class="task-link">View Task</a>)
                </p>"""

        for placement in placements:
            exclusion_reason = getattr(placement, 'reason', '')
            html_body += f"""
                <ul class="placement-list">
                    <li><strong>Placement Name:</strong> {placement["name"]}
                    (<a href="[Placement Link]">{placement["placement"]}</a>) -
                    Reason: {exclusion_reason}</li>
                </ul>
            """
        html_body += """
            </div>
            ...
            <div class="logo-placeholder">
                <img src="rsc/gtech_logo_emblem_rgb_White.png" alt="gPS Logo">
            </div>
