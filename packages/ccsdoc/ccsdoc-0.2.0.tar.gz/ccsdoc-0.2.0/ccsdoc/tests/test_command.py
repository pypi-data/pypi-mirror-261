...
def test_command_simple():
    cmd = Command(name=NAME, cmd_type=TYPE, level=LEVEL, description=DESCRIPTION)
...
def test_command_with_arguments():
    cmd = Command(name=NAME, cmd_type=TYPE, level=LEVEL, description=DESCRIPTION, args=[ARGUMENT])
