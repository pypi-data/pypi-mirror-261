...
from inspect import isasyncgen, isclass, iscoroutine, isfunction, isgenerator
...
def patch_in_out(func: Callable):
    from .utils.proxy import to_js, to_py

    @wraps(func)
    def wrapper(*args, **kwargs):
        res = func(*(to_py(i) for i in args), **{i: to_py(j) for i, j in kwargs.items()})
