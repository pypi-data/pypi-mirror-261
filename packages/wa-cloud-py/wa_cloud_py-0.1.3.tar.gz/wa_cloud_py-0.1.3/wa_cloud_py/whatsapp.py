...
from wa_cloud_py.components.flows import FlowCategory
...
class WhatsApp:
    ...
    ) -> Union[InteractiveListMessage, TextMessage, OrderMessage, None]:
        """
    ...
    def create_flow(self, name: str, categories: list[FlowCategory]):
        res = requests.post(
            self.messages_url,
            headers=self.headers,
        ...
        if res.status_code == 200:
            if self.verbose:
                logger.success(
                    f"Flow created successfully with ID {res.json().get('id')}"
                )
            return True, res.json()

        if self.verbose:
            logger.error(f"Failed to create flow.\nReason: {res.json()}")
        return False, res.json()
