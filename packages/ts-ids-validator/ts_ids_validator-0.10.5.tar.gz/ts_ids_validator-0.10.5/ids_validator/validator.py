...
class Validator:
    ...
    def __init__(
        ...
        self.is_tetra_data_schema = (
            validator_parameters.convention_version is Conventions.V1_0_0
        )
        if checks_list is not None:
            ...
            self.checks_list = (
                v1_checks if self.is_tetra_data_schema else generic_checks
            )
        ...
        if self.is_tetra_data_schema:
            for node_checker in ENFORCED_NODE_CHECKS:
            ...
            if not checker.bulk_checker:
                if checker is SchemaBreakingChangeChecker:
                    ...
                    if checker in ENFORCED_NODE_CHECKS:
                        _node = str(node.path)
                        if _node.startswith(checker.reserved_root):
                            if _node in self.missing_nodes:
                                self.missing_nodes.remove(_node)
                        else:
                            continue  # Skip validating nodes that are not intended for these checks
    ...
    def display_validator_info(self):
        ...
        if self.parameters.previous_artifact is None:
            info_logs.append(
                CheckResult.warning(
                    "No previous IDS artifact is present in the validator input, "
                    "so validation is running without versioning validation. "
                    "This is expected for the first version of an IDS artifact. "
                    "If this is unexpected, check whether a previous IDS artifact "
                    "is being passed in for comparison."
                )
            )
        else:
            info_logs.append(
                CheckResult.info(
                    f"Previous IDS artifact for versioning validation: {self.parameters.previous_artifact.identity}"
                )
            )
        info_logs.append(
            CheckResult.info(
    ...
    def validate_ids(self):
        ...
        if self.missing_nodes:
            ...
            format_spaces = " " * 8
            missing_nodes = f"\n{format_spaces}".join(self.missing_nodes)
            result = [
    ...
    def log(
        ...
        for result in sorted(messages) if sort_logs else messages:
            ...
            msg_level = {
                Log.INFO: "Info",
                Log.WARNING: "Warning (may ignore)",
                Log.CRITICAL: "Error",
            }[result.level]
