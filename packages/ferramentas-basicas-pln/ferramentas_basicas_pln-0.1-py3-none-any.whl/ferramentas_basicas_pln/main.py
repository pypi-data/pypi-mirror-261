...
def padronizarDatas(texto: str,
    ...
    digito_ano = r'\d{2,4}'
    texto_transformado = re.sub(r'\b{dia}\/{mes}\/{ano}\b|\b{ano}\/{mes}\/{dia}\b|\b{dia}\-{mes}\-{ano}\b|\b{ano}\-{mes}\-{dia}\b'.format(dia=digito_dia,mes=digito_mes,ano=digito_ano),padrao_data,texto)
    return texto_transformado

def padronizarEmails(texto : str,
                     padrao_email : str = 'EMAIL') -> str:
    texto_transformado = re.sub(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b',padrao_email,texto)
    return texto_transformado
...
def padronizarTelefoneCelular(texto : str,
    ...
    texto_transformado = re.sub(r'({pais})?({estado})?({celular})|({pais}(\s+)?)?({estado}(\s+)?)?({celular})'.format(pais=ddd_pais,estado=ddd_estado,celular=ddd_celular),
                                padrao_tel,
                                texto)
    return texto_transformado
...
def padronizarLinks(texto : str,
    ...
    texto_transformado = re.sub(r'https?://\S+',padrao_link,texto)
    return texto_transformado

def padronizarNumeros(texto : str,
                      padrao_numeros : str = 'NUM') -> str:
    texto_transformado = re.sub(r'(\b)?\d+(\S+)?(,\d+)?(\b)?|(\b)?\d+(\S+)?(.\d+)?(\b)?',padrao_numeros,texto)
    return texto_transformado

def padronizarDinheiros(texto : str,
                        padrao_dinheiro : str = '$') -> str:
    texto_transformado = re.sub(r'R\$(\s+)?\d+(\S+)?(,\d{2})?|\$(\s+)?\d+(\S+)?(\.\d{2})?',padrao_dinheiro,texto)
    return texto_transformado

def padronizarParaUnidecode(texto : str) -> str:
    texto_transformado = unidecode(texto)
    return texto_transformado
...
def formatacaoTotalDeTexto(texto : str,
                           ...
                           padrao_tel : str = 'TEL',
                           padronizar_com_unidecode : bool = False) -> str:
    ...
    if padronizar_dinheiros:
        for caracter_padrao_dinheiro in padrao_dinheiro:
            if caracter_padrao_dinheiro in string_com_os_caracteres_especiais:
                ...
                texto = padronizarDinheiros(texto=texto,padrao_dinheiro='codpdtzzqaio')
                break
        else:
    ...
    if padronizar_datas:
        for caracter_padrao_data in padrao_data:
            if caracter_padrao_data in string_com_os_caracteres_especiais:
                ...
                texto = padronizarDatas(texto=texto,padrao_data='codpdtzzqaio')
                break
        else:
    ...
    if padronizar_com_unidecode:
        texto = padronizarParaUnidecode(texto)
    ...
    if padronizar_dinheiros and caracteres_especiais_impacta_padrao_dinheiro:
        texto = transformarTextoSubstituindoCaracteres(texto=texto,caracteres='codpdtzzqaio',substituir_por=padrao_dinheiro)
    ...
    if padronizar_datas and caracteres_especiais_impacta_padrao_data:
        texto = transformarTextoSubstituindoCaracteres(texto=texto,caracteres='codpdtzzqaio',substituir_por=padrao_data)
