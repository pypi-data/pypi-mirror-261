...
class Dataset:
    ...
    def upload_files(
        ...
        transaction = self.__transaction_manager.begin_manifest_transaction(
        ...
        upload_prefix = credentials.required_prefix

        upload_mappings = [
            (
                path,
                f"{upload_prefix}/{path.name}",
            )
            for path in files
        ]
        ...
        with progress_monitor_factory.upload_monitor(
            ...
            self.__file_delegate.upload_many_files(
                bucket=credentials.bucket,
                file_generator=upload_mappings,
