...
from .constants import BEARER_TOKEN_HEADER
...
class HttpClient:
    ...
    def __request_headers(self, request_ctx: HttpRequest) -> dict[str, str]:
        ...
        if (
            BEARER_TOKEN_HEADER not in headers.keys()
            and default_env.bearer_token is not None
        ):
            headers[BEARER_TOKEN_HEADER] = default_env.bearer_token
