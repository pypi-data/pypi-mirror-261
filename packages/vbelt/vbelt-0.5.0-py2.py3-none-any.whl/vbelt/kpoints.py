...
class Kpoints:
    ...
    def from_file(cls, path):
        ...
        with open(path) as f:
            ...
            if n == 0:
                if mode not in {"m", "g"}:
                    raise ValueError(f"Unknown k-point generation automatic mode {mode}")
                ...
                if mode == "l":
                    ...
                    if line_coord != "f" and line_coord != "r":
                        raise ValueError("I don't want to deal with anything but fractional k points.")
                    ...
                    for line in f:
                        if l := line.strip():
                            if first:
                                segments.append((first, tuple(map(float, l.split()[:3]))))
                                first = None
                            else:
                    ...
                    raise ValueError("Please don't use cartesian mode, it's a dumb mode I don't like it.")
