...
class BYTETracker:
    ...
    def get_tracks(self):
        ids = np.zeros(shape=(len(self.tracked_stracks)), dtype=int)
        tlbrs = np.zeros(shape=(len(self.tracked_stracks), 4), dtype=int)
        confs = np.zeros(shape=(len(self.tracked_stracks)), dtype=float)
        clsses = np.zeros(shape=(len(self.tracked_stracks)), dtype=int)

        for idx, track in enumerate(self.tracked_stracks):
            ids[idx] = track.track_id
            tlbrs[idx] = track.tlbr
            confs[idx] = track.score
            clsses[idx] = track.cls

        return ids, tlbrs, confs, clsses
    ...
    def update(self, output_results):
        ...
        if len(dets) > 0:
            """Detections"""
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c) for (tlbr, s, c) in zip(dets, scores_keep, clsses)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [
                ...
                for (tlbr, s, c) in zip(dets_second, scores_second, clsses_second)
            ]
        else:
            detections_second = []
