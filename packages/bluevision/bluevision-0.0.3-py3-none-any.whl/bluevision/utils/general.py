...
coco_class_names = {0: 'person', 1: 'bicycle', 2: 'car', 3: 'motorcycle', 4: 'airplane', 5: 'bus', 6: 'train',
...
def make_labels(detections, class_names=None):
    ...
    if detections.tracker_id is None:
        labels = [f"{class_names[class_id]} {conf:.2f}" for class_id, conf in
                  zip(detections.class_id, detections.confidence)]
    else:
        labels = [f"{tracker_id} {class_names[class_id]} {conf:.2f}" for tracker_id, class_id, conf in
                  zip(detections.tracker_id, detections.class_id, detections.confidence)]
    return labels


def to_supervision_detections(detections):
    # tracks format
    # (ids, xyxy, confs, classes): tracker outputs
    # np.array([x1,y1,x2,y2,conf,class]): detection outputs

    tracker_id = None

    if isinstance(detections, tuple):
        tracker_id = detections[0]
        xyxy = detections[1]
        confidence = detections[2]
        class_id = detections[3]
    else:
...
def resize(image, input_size):
    ...
    # Scale ratio (new / old)
    r = min(min(input_size / shape[0], input_size / shape[1]), 1.0)
