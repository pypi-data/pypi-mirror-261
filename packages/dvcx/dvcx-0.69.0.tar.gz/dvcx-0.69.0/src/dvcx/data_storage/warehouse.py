...
class AbstractWarehouse(ABC):
    ...
    def drop_dataset_rows_table(
        ...
        table_name = self.dataset_table_name(dataset.name, version=version)
        table = Table(table_name, self.db.metadata)
        self.db.drop_table(table, if_exists=if_exists)

    def insert_into_shadow_dataset(
        self, dataset: DatasetRecord, uri: str, path: str, recursive: bool = False
    ) -> None:
        """Inserts data to shadow dataset based on bucket uri and glob path"""
        assert dataset.shadow

        dst = self.dataset_rows(dataset)
        assert uri == self.uri
        src = self.nodes
        src_cols = {c.name for c in src.c}

        # Not including id and parent_id
        dst_cols = {c.name for c in dst.c if c.name not in ("id", "parent_id")}
    ...
    def get_dataset_row(
        ...
        dr = self.dataset_rows(dataset, dataset_version)
        row = next(
            self.db.execute(
    ...
    def dataset_stats(self, dataset: DatasetRecord, version=None) -> tuple[int, int]:
        """
