...
@pytest.mark.parametrize("use_cache", [False, True])
def test_extract(cloud_test_catalog, dogs_shadow_dataset, use_cache):
    catalog = cloud_test_catalog.catalog
...
def test_rrow_generator_with_new_columns(cloud_test_catalog, dogs_shadow_dataset):
    catalog = cloud_test_catalog.catalog
...
def test_index_tar(cloud_test_catalog):
    ...
    rows = list(catalog.ls_dataset_rows("extracted", version=1))
...
def test_dataset_dependencies_one_storage_as_dependency(
    ...
    assert [
        ...
        for d in catalog.get_dataset_dependencies(ds_name, version=1, indirect=indirect)
    ] == [
        {
            "id": ANY,
...
def test_dataset_dependencies_one_shadow_dataset_as_dependency(
    ...
    assert catalog.get_dataset_dependencies(ds_name, version=1) == [None]
...
def test_dataset_dependencies_multiple_direct_dataset_dependencies(
    ...
    assert sorted(
        (
            dataset_dependency_asdict(d)
            for d in catalog.get_dataset_dependencies(ds_name, version=1, indirect=True)
        ),
        key=lambda d: d["name"],
    ...
    assert sorted(
        (
            dataset_dependency_asdict(d)
            for d in catalog.get_dataset_dependencies(ds_name, version=1)
        ),
    ...
    assert catalog.get_dataset_dependencies(ds_name, version=1) == [None, None]
