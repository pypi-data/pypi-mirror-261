...
from PyENVI.utils import *
from PyENVI.dists import *
...
class ENVI():
    ...
    def __init__(self,
        ...
        if(num_HVG == 0):
            sc_data.var.highly_variable = False
        if(len(sc_genes) > 0):
            sc_data.var['highly_variable'][np.intersect1d(sc_genes, self.sc_data.var_names)] = True
        
        if(self.sc_data.raw is None):   
            self.sc_data.raw = self.sc_data
            
        self.sc_data = self.sc_data[:, np.union1d(sc_data.var_names[sc_data.var.highly_variable], self.spatial_data.var_names)]
        ...
        self.spatial_data.obsm['COVET'], self.spatial_data.obsm['COVET_SQRT'], self.CovGenes = GetCov(self.spatial_data, self.k_nearest, num_cov_genes, cov_genes, spatial_key = spatial_key, batch_key = batch_key)
