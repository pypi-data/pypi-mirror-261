...
def CalcCovMats(spatial_data, kNN, spatial_key = 'spatial', batch_key = -1):
...
def GetCov(spatial_data, k, g, genes, spatial_key = 'spatial', batch_key = -1):
    ...
    if(g == -1):
        ...
        spatial_data.var.highly_variable = spatial_data.var.highly_variable
    
    CovGeneSet = np.where(np.asarray(spatial_data.var.highly_variable))[0]
    CovGenes = spatial_data.var_names[CovGeneSet]
    
    CovMats, kNNGraphIndex = CalcCovMats(spatial_data, k, spatial_key = spatial_key, batch_key = batch_key)
