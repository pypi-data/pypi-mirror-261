...
def start_encrypt(
    ...
    :param except_main_file: 不加密包含__main__的文件(主文件加密后无法启动), 值为0、1。 默认为1
    :param pyc: 默认未so或者pyd格式加密，pyc为真时编译成pyc码
    :return:
    """
    ...
    logger.debug("加密完成 total_count={}, success_count={}, 生成到 {}".format(len(need_encrypted_py), len(encrypted_py),
                                                                        output_file_path))
