...
import mymark.adapter_server as adapter
...
class Interpreter:
    def __init__(
        ...
        self.server_adapter = adapter.ServerAdapter()
        self.query_index = 1
        ...
        self.mark_scheme = (
            ...
            else self.server_adapter.get_mark_scheme(module_name, exercise_name)
        )
    ...
    def get_gpt_response(self, query: str) -> str:
        ...
        response = self.server_adapter.get_response(
            adapter.GPTRequest(
                [{"content": query, "role": GPTRole.USER}],
                temperature=0,
                model=(
                    adapter.GPTModel.GPT3_5TURBO_UNLIMITED
                    if os.getenv("GPT_VERSION") == "3.5"
                    else adapter.GPTModel.GPT4LATEST
                ),
            )
        )
