...
def create_events_for_next_blocks(url, master_url, output, error_data, job):
    ...
    if status:
        ...
        send_subtask_success_callback(
            master_url, job["data"]["task_id"], job["data"]["algo_name"],job["data"]["level"]
        )
...
def send_subtask_success_callback(url, task_id, algo_name,level):
    if url == "":
        return
    ...
    headers = {
        ...
        "authUsername":"qc.user@increff.com",
        "authPassword":"Qcuser1@increff",
