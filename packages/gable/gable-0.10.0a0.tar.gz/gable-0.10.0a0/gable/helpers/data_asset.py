...
def gather_pyspark_asset_data(
    ...
    try:
        sca_results_dict = cast(
            ...
            json.loads(json.loads(json.loads(sca_results)["message"])),
        )
    except json.JSONDecodeError as exc:
        logger.opt(exception=exc).debug(f"Error processing PySpark job: {sca_results}")
        raise click.ClickException(
            f"Error processing PySpark job: {sca_results}"
        ) from exc
    project_repo = get_git_repo_info(project_root)
    source_names = [f"{project_repo['gitSSHRepo']}" for x in sca_results_dict]
    return source_names, sca_results_dict
