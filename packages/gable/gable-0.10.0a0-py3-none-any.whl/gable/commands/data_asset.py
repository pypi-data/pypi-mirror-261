...
def register_data_asset(
    ...
    if source_type in SCHEMA_SOURCE_TYPE_VALUES:
        ...
        if not spark_job_entrypoint or not project_root or not connection_string:
            raise click.ClickException(
        ...
        source_names, sca_results_dict = gather_pyspark_asset_data(
            get_abs_project_root_path(project_root),
        ...
        formatted_event = "\n".join(
            [f"{event['eventName']} - {event['type']}" for event in events]
        )
        logger.info(
