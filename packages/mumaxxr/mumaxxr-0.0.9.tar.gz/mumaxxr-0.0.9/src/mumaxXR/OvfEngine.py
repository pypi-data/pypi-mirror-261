...
class OvfBackendArray(xr.backends.BackendArray):
    ...
    def generate_array_file_link(self):
        ...
        if (len(self.dirListToConcat) == 0):
            if (len(self.wavetype) == 0):
                dims = ['t', 'z', 'y', 'x', 'comp']
                fileList, tmaxArray = self.get_corresponding_files(self.filename_or_obj, returnTData=True)
                coords = [tmaxArray, self.mesh.get_axis(2), self.mesh.get_axis(1), self.mesh.get_axis(0), np.arange(self.mesh.n_comp)]
                shape = [value.size for value in coords]
                fileList = np.asarray(fileList)
            else:
                dims = ['wavetype', 't', 'z', 'y', 'x', 'comp']
                fileList[0], tmaxArray = self.read_data_from_ovf(self.filename_or_obj, type=self.wavetype[0], returnTData=True)

                for type in self.wavetype[1:]:
                    fileList.append(self.get_corresponding_files(self.filename_or_obj, type=type))
                
                coords = [np.array(self.wavetype), tmaxArray, self.mesh.get_axis(2), self.mesh.get_axis(1), self.mesh.get_axis(0), np.arange(self.mesh.n_comp)]
                shape = [value.size for value in coords]
                fileList = np.asarray(fileList)
        else:
            if (len(self.wavetype) == 0):
                if (isinstance(self.sweepName, str)):
                    ...
                    fileList[0], tmaxArray = self.get_corresponding_files(self.dirListToConcat[0], returnTData=True)
                    ...
                    coords = [coordsParam, tmaxArray, self.mesh.get_axis(2), self.mesh.get_axis(1), self.mesh.get_axis(0), np.arange(self.mesh.n_comp)]
                    shape = [value.size for value in coords]
                    fileList = np.asarray(fileList)
                else:
                    dims = self.sweepName + ['t', 'z', 'y', 'x', 'comp']
                ...
                if (isinstance(self.sweepName, str)):
                    ...
                    if (self.dirListToConcat[0].suffix != '.ovf'):
                        self.dirListToConcat[0] = self.dirListToConcat[0].joinpath(Path(startingFileName))
                    tmaxArray = self.get_corresponding_files(self.dirListToConcat[0], returnTData=True, returnMeshData=False)
                    ...
                    coords = [coordsParam, np.array(self.wavetype), tmaxArray, self.mesh.get_axis(2), self.mesh.get_axis(1), self.mesh.get_axis(0), np.arange(self.mesh.n_comp)]
                    shape = [value.size for value in coords]
                    fileList = np.asarray(fileList)
                else:
                    dims = self.sweepName + ['wavetype', 't', 'z', 'y', 'x', 'comp']
