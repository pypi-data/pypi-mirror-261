...
class Recording:
    ...
    def get_start_time(self, plots=False):
        ...
        if self.is_audio():
            self.sound_without_TicTacCode = self._strip_yaltc()
        return start_UTC
    ...
    def _strip_yaltc(self):
        ...
        TicTacCode_channel = self.TicTacCode_channel + 1 # sox channels start at 1
        ...
        if len(self.silent_channels) + 1 == n_channels:
            print('\nWarning, file [gold1]%s[/gold1] doesnt have any active channel,'%input_file)
            print('will keep at least one silent track.\n')
            self.silent_channels.pop(-1)
            logger.debug('new silent chans (1st = #1) %s'%self.silent_channels)
        all_channels = range(1, n_channels + 1) # eg [1,2,3,4]
        excluded_chans = self.silent_channels + [TicTacCode_channel]
