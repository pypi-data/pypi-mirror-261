...
class Recording:
    ...
    Attributes:
        ...
        sound_without_TicTacCode : pathlib.path
    ...
    def __init__(self, media):
        ...
        self.sound_without_TicTacCode = None
    ...
    def get_duration(self):
        ...
        """
        if self.sound_without_TicTacCode:
            val = sox.file_info.duration(_pathname(self.sound_without_TicTacCode))
            logger.debug('duration of sound_without_TicTacCode %f'%val)
            return val
        else:
            if self.probe is None:
                return 0
    ...
    def get_start_time(self, plots=False):
        ...
        if not returns None.
        If successful AND self is audio, sets self.sound_without_TicTacCode
        """
        ...
        if self.is_audio():
            self.sound_without_TicTacCode = self._strip_TTC()
        return start_UTC

    def _strip_TTC(self):
        """        
        Returns a tempfile.NamedTemporaryFile of sound file stripped
        of TicTacCode channel and of any silent channel. Criteria for
        silence is sox stats "RMS lev db" value less than DB_RMS_SILENCE_SOX

        Beware of track numerotation: sox starts at 1 but this classe
        starts at track #0.
        ...
        silent_channels = [i for i,db in enumerate(sox_RMS_lev_dB)
                                            if float(db) < DB_RMS_SILENCE_SOX]
        logger.debug('silent chans (1st = #1) %s'%silent_channels)
        trax = self.device.tracks
        logger.debug('trax %s'%trax)    
        def _get_muted_trx() -> list:
            """
            This fct verifies coherency between tracks information given by the
            user (in self.device.tracks, parsed by
            device_scanner._parse_track_values()) and tracks information known
            here: silent_channels and yaltc.Recording.TicTacCode_channel

            Tries to accommodate light discrepancies for unused tracks (but
            always warn user).

            Returns values according to sox numbering (1st = index 1)
            """
            all_channels_nbr = self.get_audio_channels_nbr()
            user_declared_trx_nb = len(trax.rawtrx)
            logger.debug('all_channels_nbr: %i user_declared_trx_nb %i'%
                (all_channels_nbr, user_declared_trx_nb))
            logger.debug('trax: %s'%trax)
            if all_channels_nbr != user_declared_trx_nb:
                print('\nError: [gold1]%i[/gold1] tracks are present in [gold1]%s[/gold1]'%
                    (all_channels_nbr, self.AVpath))
                print('but [gold1]%i[/gold1] tracks are declared in [gold1]%s[/gold1], quitting.'%
                    (user_declared_trx_nb, device_scanner.TRACKSFN))
                quit()
            auto_excluded_chans = silent_channels + [self.TicTacCode_channel]
            declared_excluded_chans = [idx for idx, tag in enumerate(trax.rawtrx)
                if tag in ['0','ttc','tc']]
            logger.debug('auto_excluded_chans: %s declared_excluded_chans: %s (ZBIDX)'%
                    (auto_excluded_chans, declared_excluded_chans))
            all_chans = range(all_channels_nbr)
            auto_kept_chans = [idx for idx in all_chans 
                if idx not in auto_excluded_chans]
            declared_ketp_chans = [idx for idx in all_chans
                if idx not in declared_excluded_chans]
            logger.debug('auto_kept_chans: %s, declared_ketp_chans: %s'%
                (auto_kept_chans,declared_ketp_chans))
            TTC_does_match = trax.ttc == sox_TicTacCode_channel
            excl_does_match = sorted(auto_excluded_chans) == \
                sorted(declared_excluded_chans)
            all_does_match = TTC_does_match and excl_does_match
            if all_does_match:
                logger.debug('every track does match user declaration, muted: %s'%
                    auto_excluded_chans)
                return auto_excluded_chans
            table = Table(title="Tracks status")
            table.add_column("track #", justify="center", style='gold1')
            table.add_column("Detected", justify="center", style='gold1')
            table.add_column("Declared", justify="center", style='gold1')
            detected_chan_status = ['audio'] * all_channels_nbr
            for n in silent_channels:
                detected_chan_status[n] = 'silent'
            detected_chan_status[self.TicTacCode_channel] = 'ttc'
            logger.debug('detected_chan_status: %s'%detected_chan_status)
            for n in range(all_channels_nbr):
                table.add_row(str(n), detected_chan_status[n], trax.rawtrx[n])
            if excl_does_match: # simply warn but proceed 
                print('\nWarning: for file [gold1]%s[/gold1] declaration in [gold1]%s[/gold1] doesn\'t match exactly:\n'%
                    (self.AVpath, device_scanner.TRACKSFN))
                console = Console()
                console.print(table)
                print('\nWill proceed anyway, keeping tracks %s'%auto_kept_chans)
                return auto_excluded_chans
            else:
                print('\nError: for file [gold1]%s[/gold1] declaration in [gold1]%s[/gold1] doesn\'t match:\n'%
                    (self.AVpath, device_scanner.TRACKSFN))
                console = Console()
                console.print(table)
                print('\nMake necessary corrections and rerun: quitting.')
                quit()
        if trax:
            muted_chans = _get_muted_trx()
        all_channels = range(1, n_channels + 1) # from 1 to n_channels included
        sox_muted_chans = [n+1 for n in muted_chans]
        logger.debug('excluded chans %s (not ZBIDX)'%sox_muted_chans)
        kept_chans = [[n] for n in all_channels if n not in sox_muted_chans]
        # eg [[1], [3], [4]]
...
if __name__ == '__main__':
    ...
    parser.add_argument("mediafile", nargs="?", help="path of media file with TicTacCode")
