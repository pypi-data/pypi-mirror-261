...
from bliss.common.logtools import log_debug, log_error, user_print, user_warning
...
class GalilDMC(Controller):
    ...
    def initialize(self):
        ...
        try:
            ...
            user_warning(f"cannot determnine model serie for {self.model_id}...")

        if self.model_id not in model_str:
            user_warning(
                f"Ohhh oh, it seems your controller ({model_str})",
    ...
    def get_TS(self, axis):
        ...
        ts_status["pos_latch"] = bool(TS & (1 << 0))  # Position latch has occured
