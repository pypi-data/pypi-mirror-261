...
class SyriusAPI:
    ...
    def __init__(self):
        ...
        self.base_url = os.environ.get('SYRIUS_BASE_URL', "https://api.m7r.com/api")

    def flow_exist(self, name: str, hash: str) -> bool:
        response = httpx.get(self._set_url("flow", "name", name, "hash", hash), auth=SyriusAuth(self.api_key))
        if response.status_code == 200:
            return True
        return False

    def add_flow(self, flow: Dict[str, Any]):
        response = httpx.post(self._set_url("flow"), json=flow, auth=SyriusAuth(self.api_key))
        if response.status_code == 200:
            return response.json()
        raise FlowAPIException("Flow add process failed")

    def run(self, name: str, hash: str):
        response = httpx.post(self._set_url("flow", "name", name, "hash", hash, "run"), auth=SyriusAuth(self.api_key))
        if response.status_code == 200:
            data = response.json()
            return data['runner']
    ...
    def check_flow_status(self, runner: str):
        response = httpx.get(self._set_url("flow", "status", runner), auth=SyriusAuth(self.api_key))
        if response.status_code == 200:
            return response.json()
    ...
    def upload_file(self, path_file: str):
        ...
        response = httpx.post(self._set_url("file"), files=_files, auth=SyriusAuth(self.api_key))
        if response.status_code == 200:
            return response.json()
