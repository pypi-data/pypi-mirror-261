...
from afeng_tools.baidu_pan_tool.tools import file_meta_tools
from afeng_tools.fastapi_tool.common.enum import ResourceUrlEnum, ResourceFormatEnum
from afeng_tools.fastapi_tool.common.po_service.resource_po_service import ResourcePoService
...
class ResourceService(BaseService):
    ...
    def get_by_code(self, resource_code: int) -> Any:
        """
        通过编码查询
    ...
    def query_in_code(self, resource_code_list: list[int]) -> list[Any]:
        """通过编码列出"""
        return self.po_service.query_more(self.po_model_type.resource_code.in_(resource_code_list))

    @classmethod
    def get_resource_url(cls, url: str, resource_code: int) -> str:
        if url:
            return url
        if resource_code:
    ...
    def refresh_baidu_img_access_url(self, resource_info_po) -> Any:
        ...
        if resource_info_po and resource_info_po.resource_format == ResourceFormatEnum.image and resource_info_po.baidu_fs_id:
            ...
            file_info_list = file_meta_tools.get_file_metas(baidu_access_token, [resource_info_po.baidu_fs_id], thumb=1)
            if file_info_list:
    ...
    def get_baidu_download_url(cls, resource_info_po) -> str:
        ...
        if resource_info_po and resource_info_po.baidu_fs_id and resource_info_po.resource_format == ResourceFormatEnum.file:
            baidu_access_token = baidu_pan_tools.get_access_token()
            file_info_list = file_meta_tools.get_file_metas(baidu_access_token, [resource_info_po.baidu_fs_id],
                                                            d_link=1)
            if file_info_list:
    ...
    @classmethod
    def run_local_cache(cls, resource_code: int, resource_url: str, subfix: str = None, local_cache_path: str = None):
        """运行本地缓存"""
