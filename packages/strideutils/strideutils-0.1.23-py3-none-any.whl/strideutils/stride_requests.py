...
def get_celestia_delegation_records(include_archived: bool = False) -> List[Dict]:
    ...
    """
    url = f"{config.stride.api_endpoint}/{MULTISIG_DELEGATION_RECORDS_QUERY.format(module=STAKETIA)}"
...
def get_celestia_unbonding_records(include_archived: bool = False) -> List[Dict]:
    ...
    """
    url = f"{config.stride.api_endpoint}/{MULTISIG_UNBONDING_RECORDS_QUERY.format(module=STAKETIA)}"
    params = {"include_archived": include_archived}
...
def get_celestia_redemption_records(
    ...
    url = f"{config.stride.api_endpoint}/{MULTISIG_REDEMPTION_RECORDS_QUERY.format(module=STAKETIA)}"
...
def get_empirical_annual_fees(ticker: str) -> float:
    ...
    """
    empirical_dollar_fees = stride_upstash.get(
        "sheet_STRD_STAKING_ST{}_DOLLARS".format(ticker.upper()), db_name="frontend"
    )
    return float(empirical_dollar_fees)
