...
minimal flux patterns from elementary flux vectors (EFVs) [2].
...
def find_mps(model, subset=set(), method='iterative', graph=True, random=False,
    ...
    Args:
        ...
        subset: Iterable specifying the subset of reactions that can be part of
                MPs. Can be reaction IDs, Gurobi variables, or COBRA reactions.
                If empty, all reactions are included in the subset.
        method: Minimization method used to find MPs (iterative or direct).
        graph: Use graph to accelerate enumeration (only iterative)?
        random: Randomize order of MPs (only possible for iterative method)?
        bounds: Flux bounds to replace the flux bounds in the model (dict with
                reaction IDs as keys and tuples of flux bounds as values).
        tighten: Use flux variability analysis to tighten bounds?
        tol: Numerical tolerance (1e-9 is the minimum and is recommended).
        inf: Maximal flux bound ("infinity") to replace any larger flux bound.
        ...
        max_t: Maximum time in seconds (0 for no limit, must be non-zero for
               random sampling if no MP limit is specified).
        verbose: Print information as MPs are found?
    ...
    Returns:
        ...
        complete: Were all MPs (and MCs) found (cannot be checked if random)?
    ...
    if export:
        ...
        save_set(subset, os.path.join(output_dir, filename))

    # Convert subset to variables if necessary
    if subset:
        s = set()
        for x in subset:
