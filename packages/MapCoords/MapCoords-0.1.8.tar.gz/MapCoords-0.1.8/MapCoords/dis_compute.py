...
5. 关于效率，尝试了通过multiprocessing模块多进程并行计算，没有任何用处耗时还变多了不少，幸运的是目前串行计算的速度足够快，符合实际使用需求。
...
def compute_arc_dis_matrix(lgts1, lats1, lgts2, lats2, is_radian=False):
    ...
    e.g. input: points1: x1, x2, x3   points2: y1, y2
         output: np.array([[dis(x1, y1), dis(x1, y2)],
                           [dis(x2, y1), dis(x2, y2)],
                           [dis(x3, y1), dis(x3, y2)]])
    效率测试：21763*21763 cost 24s, compute: 14s, assign: 10s
    ...
    # compute_t = assign_t = 0
    dist_m = np.zeros((len(lgts1), len(lgts2)))
    for i in range(len(lgts2)):
        dist_m[:, i] = compute_arc_dis(lgts1, lats1, lgts2[i], lats2[i], True)
    #     t0 = time.time()
    #     tmp = compute_arc_dis(lgts1, lats1, lgts2[i], lats2[i], True)
    #     t1 = time.time()
    #     compute_t += t1 - t0
    #     t2 = time.time()
    #     dist_m[:, i] = tmp
    #     assign_t += time.time() - t2
    # print(compute_t, assign_t)
