...
class LoLMatch:
        ...
        async with get_session() as session:
            ...
            if sub_tournament := results.one_or_none():
                ...
                if group_id_list is None:
                    sub_tournament.group_id = [group_id]
                elif group_id in group_id_list:
                    return False
                else:
    ...
    async def tournament_cancel(cls, tournament_id: int, group_id: int) -> bool:
        ...
        async with get_session() as session:
            ...
            if tournament is None:
                return False
            elif tournament.group_id is None:
                return False
            elif group_id not in tournament.group_id:
                return False
            else:
    ...
    async def cancel_tournament_group(cls, tournament_id: Optional[int], group_id: int):
        ...
        Args:
            tournament_id (Optional[int]): 联赛ID 为None则取消所有
            group_id (int): 群号
