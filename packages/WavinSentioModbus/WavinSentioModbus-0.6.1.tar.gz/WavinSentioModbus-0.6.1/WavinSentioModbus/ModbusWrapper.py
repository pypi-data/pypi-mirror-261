            ...
            if(registerMapObject.regType == RegisterType.INPUT_REGISTER):
                ...
                if not response.isError(): 
                    if(registerMapObject.dataType == RegisterDataType.NUMERIC):
                        ...
                        returnValue = returnValue.decode('utf-8').replace('\0', '')
                    ...
                    logging.trace("Error while reading register {0} type INPUT".format(address)) #not really error, could be register is not used. just return error
                    returnValue = None
                ...
                if not response.isError(): 
                    ...
                    logging.trace("Error while reading register") #not really error, could be register is not used. just return error
                    returnValue = None
        except Exception as e:
            logging.exception("error occured ==>  {0}".format(e))
            return None
