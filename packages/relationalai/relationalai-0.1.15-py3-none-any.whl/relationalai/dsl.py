...
class Type(Producer):
    def __init__(self, graph:'Graph', name:str, builtins:List[str] = []):
        ...
        if graph._config.get("compiler.use_value_types", False):
            ...
            self._add_action(build.install(self._type))
    ...
    def _make_sub(self, name: str, existing=None):
        if existing is not None:
            return existing
        return Property(self._graph, name, [self._type], self)
...
class RelationNS():
    def __init__(self, ns:List[str], name:str):
        self._name = name
        self._ns = ns
        self._subs = {}
...
class Graph:
    ...
    def _action(self, action:Action|List[Action], pre=False):
