...
class MVDParser:
    ...
    def __enter__(self):
        ...
        if self.proxy:
            self.client.proxies.update(self.proxy, timeout=TIMEOUT_SEC)
        return self
    ...
    def _get_csrf_token(self, html_str) -> str:
    ...
    def _fetch_get_request(self, url: str, headers) -> requests.Response:
    ...
    def _fetch_post_request(self, url: str, params, headers) -> requests.Response:
    ...
    def initialize(self) -> str:
        ...
        csrf_token = self._get_csrf_token(self._fetch_get_request(BASE_URL, HEADERS["h1"]).text)
        self.params["sec_csrftoken"] = csrf_token

        response = self._fetch_get_request(CAPTCHA_URL, HEADERS["h2"])
    ...
    def get_result(self, captcha_word: str) -> dict:
        ...
        response = self._fetch_post_request(BASE_URL, self.params, HEADERS["h3"])
