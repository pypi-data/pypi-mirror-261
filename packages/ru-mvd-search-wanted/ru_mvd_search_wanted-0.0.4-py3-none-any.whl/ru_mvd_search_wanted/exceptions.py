...
class MVDParserException(Exception):
    """Base Exception class, returned when nothing more specific applies"""

    def __init__(self, message: Optional[str] = None) -> None:
        super(MVDParserException, self).__init__(message)
...
class MVDParserAPIStatusException(MVDParserException):
...
class MistralAPIStatusException(MVDParserException):
    """Returned when we receive a non-200 response from the API that we should retry"""

class MistralConnectionException(MVDParserException):
