...
class DeltaPosition:
    ...
    def prepare_greek_frame_with_options(
        self, options: OptionCollection, is_call: bool
    ) -> np.ndarray:
        greeks = options.get_greeks(informative=True)
        array = np.array([option for option in options])
        greeks = prepare_filtered_greeks_frame(
            greeks, self.underlying.get_synthetic_future_price(), 0.65, is_call
        )
    ...
    def set_recommendation_for_first_side(
        ...
        idx = np.abs(greeks[:, 3] - 0.1).argmin()
        target_gamma = greeks[idx, 4]
        ...
        calibrated_delta = np.dot(weights, greeks[:, 3])
    ...
    def set_recommendation_for_second_side(
        ...
        try:
            weights = optimize_option_weights(
                greeks[:, 3], greeks[:, 4], target_deltas[0], target_deltas[1]
            )
        except Exception as e:
            logger.error(
                f"Error in optimization: {e}, Traceback: {traceback.format_exc()}"
            )
            raise OptimizationError(f"Second side optimization failed")
    ...
    def set_optimized_recommended_qty(self) -> None:
        try:
            target_deltas = self.set_recommendation_for_first_side(
                self.prospective_calls, is_call=True
            )
            self.set_recommendation_for_second_side(
                self.prospective_puts, is_call=False, target_deltas=target_deltas
            )
        except Exception as e:
            if "Second side optimization failed" in str(e):
                ...
                self.set_recommendation_for_second_side(
                    self.prospective_calls, is_call=True, target_deltas=target_deltas
                )
            else:
                notifier(
