...
def mid_on_mid_off(
    ...
    exposure: int | float,
    hedge_steps: int,
    take_profit_amount: int | float,
    stop_loss_amount: int | float,
    entry_time: tuple[int, int] = (9, 16),
    scan_exit_time: tuple[int, int] = (14, 30),
    universal_exit_time: tuple[int, int] = (15, 00),
    webhook_url: Optional[str] = None,
):
    """The amounts should be absolute values.
    Stop loss will be converted to negative value so provide positive value.
    """
    ...
    # Converting take profit and stop loss amounts to points
    take_profit_points = take_profit_amount / underlying.lot_size
    stop_loss_points = stop_loss_amount / underlying.lot_size
    logger.debug(
        f"Midon-Midoff take profit points: {take_profit_points}, stop loss points: {stop_loss_points}"
    )

    # Setting the entry and exit times
    entry_time = time(*entry_time)
    scan_exit_time = time(*scan_exit_time)
    universal_exit_time = time(*universal_exit_time)

    position_monitor = PositionMonitor(underlying)
    position_monitor.exit_triggers["exit_time_reached"] = False

    while current_time().time() < entry_time:
        sleep(1)

    while position_monitor.position_active or current_time().time() < scan_exit_time:
        if (
            not position_monitor.position_active
        ):  # If no position is active, identify the position and enter it
            if current_time().time() > scan_exit_time:
                logger.debug(f"Midon-Midoff scan exit time reached.")
                break

            if position_monitor.counter > 0:
                sleep(
                    120
                )  # Sleep for 2 minutes before entering the next position if its the second or later position

            position_monitor.counter += 1  # Increment the counter
            order_tag = f"Midon-Midoff {position_monitor.counter}"

            position_monitor.main_strangle = underlying.get_strangle(
                expiry
            )  # Fetches atm straddle
            position_monitor.hedge_strangle = underlying.get_strangle(
                expiry, steps=hedge_steps, invert=True
            )  # Fetches hedge strangle

            # Placing of main orders
            h_call_avg_price, h_put_avg_price = place_option_order_and_notify(
                position_monitor.hedge_strangle,
                Action.BUY,
                quantity_in_lots,
                "MARKET",
                order_tag,
            ...
            m_call_avg_price, m_put_avg_price = place_option_order_and_notify(
                position_monitor.main_strangle,
                Action.SELL,
                quantity_in_lots,
                "MARKET",
                order_tag,
            ...
            position_monitor.total_points_collected = (
                m_call_avg_price + m_put_avg_price - h_call_avg_price - h_put_avg_price
            )
            position_monitor.position_active = True

            logger.debug(
                f"Midon-Midoff position entered. Main strangle: {position_monitor.main_strangle}, "
                f"Hedge strangle: {position_monitor.hedge_strangle}, "
                f"Total points collected: {position_monitor.total_points_collected}"
            )

        else:  # We have an active position with total points collected defined
            if current_time().time() > universal_exit_time:
                logger.debug(f"Midon-Midoff universal exit time reached.")
                position_monitor.exit_triggers["exit_time_reached"] = True

            hedge_call_ltp, hedge_put_ltp = position_monitor.hedge_strangle.fetch_ltp()
            (
                hedge_call_best_bid,
                hedge_put_best_bid,
            ) = position_monitor.hedge_strangle.fetch_ltp("SELL")

            main_points = position_monitor.main_strangle.fetch_total_ltp("BUY")
            hedge_points = hedge_call_best_bid + hedge_put_best_bid

            current_points = main_points - hedge_points
            mtm = position_monitor.total_points_collected - current_points

            logger.info(
                f"Midon-Midoff position active\nCurrent points: {current_points}\nMTM: {mtm}\n"
                f"Hedge call bid: {hedge_call_best_bid}, Hedge put bid: {hedge_put_best_bid}\n"
                f"Hedge call ltp: {hedge_call_ltp}, Hedge put ltp: {hedge_put_ltp}\n"
            )

            if (
                mtm > take_profit_points
                or mtm < -stop_loss_points
                or position_monitor.exit_triggered()
            ):  # If an active position's exit is triggered
                logger.debug(f"Midon-Midoff position exit triggered. MTM: {mtm}")
                place_option_order_and_notify(
                    position_monitor.main_strangle,
                    Action.BUY,
                    quantity_in_lots,
                ...
                position_monitor.position_active = False


def rolling_straddle(
    underlying: Index | Stock,
    exposure: int | float,
    stop_loss_points: int | float = 5,
    reenter_points: int | float = 10,
    universal_profit_points: int | float = 25,
    entry_time: tuple[int, int] = (9, 16),
    scan_exit_time: tuple[int, int] = (15, 00),
    universal_exit_time: tuple[int, int] = (15, 13),
):
    quantity_in_lots = convert_exposure_to_lots(
        exposure, underlying.fetch_ltp(), underlying.lot_size
    )
    ...
    while position_monitor.position_active or current_time().time() < scan_exit_time:
        if not position_monitor.position_active:
            if current_time().time() > scan_exit_time:
                logger.debug(f"Rolling straddle scan exit time reached.")
                break
            straddle = underlying.get_strangle(expiry)
