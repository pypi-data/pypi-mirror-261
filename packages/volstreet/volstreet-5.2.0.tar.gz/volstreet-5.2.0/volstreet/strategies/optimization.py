...
def add_time_value_to_greeks_frame(
    ...
    greeks = np.column_stack((greeks, time_value))
    return greeks
...
def filter_greeks_frame(
    ...
    if is_call:
        mask = (greeks[:, 3] < delta_threshold) & (greeks[:, 3] > 0.01)
    else:
        mask = (greeks[:, 3] > -delta_threshold) & (greeks[:, 3] < -0.01)

    greeks_fil = greeks[mask]

    diffs = np.diff(greeks_fil[:, -2])  # Difference of time value
...
def prepare_filtered_greeks_frame(
    greeks: np.ndarray,
    implied_spot: float,
    delta_threshold: float,
    is_call: bool,
) -> np.ndarray:
    greeks = add_time_value_to_greeks_frame(greeks, implied_spot, is_call)
    greeks_fil = filter_greeks_frame(greeks, delta_threshold, is_call)
    return greeks_fil
