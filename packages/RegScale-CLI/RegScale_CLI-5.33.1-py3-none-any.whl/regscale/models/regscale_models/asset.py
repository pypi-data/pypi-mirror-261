...
class Asset(RegScaleModel):
    ...
    def bulk_insert(
        ...
        """
        warnings.warn(
            "bulk_insert is deprecated and will be removed in a future version. "
            "Use batch_create instead.",
            DeprecationWarning,
        )
        url = urljoin(Asset.config["domain"], "/api/assets/batchcreate")
        api = Api()
        results = []
        if batch:
    ...
    def bulk_update(
        ...
        """
        warnings.warn(
            "bulk_update is deprecated and will be removed in a future version. "
            "Use batch_update instead.",
            DeprecationWarning,
        )
        url = urljoin(Asset.config["domain"], "/api/assets/batchupdate")
        api = Api()
        results = []
        if batch:
            # Chunk list into batches
...
class AssetMapping(RegScaleModel):
    ...
    def find_by_unique(self) -> Optional["AssetMapping"]:  # type: ignore
        """
        ...
        for mapping in mappings:
            if mapping.componentId == self.componentId:
                return mapping
        return None
    ...
    @classmethod
    def _handle_list_response(cls, response, component_id: int) -> List[T]:
        """
        ...
        if response.ok:
            ...
            return cast(
                List[T],
                [cls(componentId=component_id, **o) for o in json_response],
            )
        else:
