...
class RegScaleModel(BaseModel, ABC):
    ...
    def find_by_unique(self) -> Optional[T]:
        ...
        if not (
            parent_control_id := getattr(
                self,
                "parentControlId",
                getattr(self, "ParentId", getattr(self, "securityPlansId", None)),
            )
        ):
            raise NotImplementedError(
    ...
    def _handle_list_response(
        cls, response: Response, suppress_error: bool = False
    ) -> List[T]:
        """
    ...
    @classmethod
    def _handle_response(cls, response) -> Optional[T]:
        """
    ...
    def batch_create(cls, items: List[T]) -> List[T]:
        ...
        :param List[cls] items: List of Asset Objects
    ...
    def batch_update(cls, items: List[T]) -> List[T]:
        """
        Use bulk_create method to create assets.
        :param List[cls] items: List of cls Objects
        :return: List of cls items from RegScale
        :rtype: List[cls]
        """
    ...
    @classmethod
    def parse_response(cls, response, suppress_error: bool = False) -> Optional[T]:
        """
        Parse a response.
        :param response: The response
        :param suppress_error: Whether to suppress the error, defaults to False
    ...
    @classmethod
    def log_response_error(cls, response: Response, suppress_error: bool = False):
        """
