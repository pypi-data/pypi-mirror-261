...
class WizInventory(Inventory):
    ...
    def fetch_wiz_data_if_needed(self) -> List[Dict]:
        ...
        fetch_interval = timedelta(
            hours=self.config.get("wizFullPullLimitHours", 8)
        ...
        if os.path.exists(INVENTORY_FILE_PATH):
            file_mod_time = datetime.fromtimestamp(
                os.path.getmtime(INVENTORY_FILE_PATH)
            )
    ...
    def create_wiz_assets(
        ...
        :param data: Wiz data to process.
        ...
        data_objects = {}
        map_job = map_progress.add_task(
            "[#f68d1f]Mapping Wiz inventory data to RegScale...", total=len(data)
        )
        ...
        created_assets, updated_assets = finalize_assets(
            wiz_assets_to_create, wiz_assets_to_update
        )
        all_assets = []
        ...
        create_asset_component_mappings(created_assets, components)
...
def pull_resource_info_from_props(wiz_entity_properties: Dict) -> tuple[int, int]:
    ...
    :param wiz_entity_properties: Wiz entity properties
    :return: Memory, CPU
    """
...
def parse_memory(memory_str: str) -> int:
    ...
    :param memory_str: Memory string
...
def get_software_name_from_cpe(wiz_entity_properties: Dict, name: str) -> Dict:
    ...
    if "cpe" in wiz_entity_properties.keys() and wiz_entity_properties.get("cpe", ""):
        cpe_info_dict = extract_product_name_and_version(
            wiz_entity_properties.get("cpe", "")
        )
...
def get_notes_from_wiz_props(wiz_entity_properties: Dict, external_id: str) -> str:
    ...
    (
        notes.append(
            f"Provider Unique ID: {wiz_entity_properties.get('providerUniqueId')}"
        )
    ...
    (
        notes.append(
            f"Severity Description: {wiz_entity_properties.get('severity_description')}"
        )
...
def create_asset_type(asset_type: str) -> str:
    ...
    meta_data_list = Metadata.get_metadata_by_module_field(
        module="assets", field="Asset Type"
    )
...
def create_or_get_component(
    ...
    :param List wiz_projects: Wiz project ID
    :param list existing_components: Existing components
...
def map_asset(
    ...
    r_asset = Asset(
        name=software_name_dict.get("name"),
        ...
        ipAddress=network_dict.get("ip4_address"),
        ipv6Address=network_dict.get("ip6_address"),
...
def handle_cpe_name(wiz_entity_properties: Dict, row: Dict) -> Dict:
    if "cpe" in wiz_entity_properties:
        ...
        software_name_dict = get_software_name_from_cpe(
            wiz_entity_properties, row.get("name")
        )
        software_name_dict["name"] = cpe_title if cpe_title else row.get("name")
        software_name_dict["software_name"] = (
            cpe_title if cpe_title else row.get("name")
        )
    else:
        software_name_dict = get_software_name_from_cpe(
            wiz_entity_properties, row.get("name")
...
def handle_management_type(wiz_entity_properties: Dict) -> str:
    ...
    return (
        ...
        if wiz_entity_properties.get("isManaged")
        else "Internally Managed"
    )


def handle_software_version(
    wiz_entity_properties: Dict, asset_category: str
) -> Optional[str]:
    """
    Handle software version
    :param wiz_entity_properties: Wiz entity properties
    :param asset_category: Asset category
...
def get_cloud_identifier(
    ...
    provider_unique_id = (
        wiz_entity_properties.get("providerUniqueId").lower()
        if wiz_entity_properties.get("providerUniqueId")
        else ""
    )
...
def process_row(
    ...
    :param row: Row
    :param params: Process data parameters
    :return: None
    """
    try:
        ...
        wiz_entity = row.get("graphEntity") or dict()
        wiz_entity_properties = wiz_entity.get("properties", dict())
        if wiz_entity_properties.get("cloudProviderURL"):
...
def handle_component_title(row: Dict, components_to_create: List[str]):
    """
    Handle component title
    :param row: Row
    :param components_to_create: Components to create
    :return: Components to create
    :rtype: None
    """
    try:
        component_title = (
            row.get("type").title().replace("_", " ") if row.get("type") else None
        )
...
def handle_port_mapping(
    ...
    Handle port mapping
    :param wiz_entity_properties: Wiz entity properties
    :param wiz_entity: Wiz entity
    :param wiz_id: Wiz ID
    :param parent_id: Parent ID
    :param parent_module: Parent module
    :param ports_to_create: Ports to create
    :return: None
    """
    wiz_entity_properties.get("portStart")
    if start_port := wiz_entity_properties.get("portStart"):
        protocol = wiz_entity_properties.get(
            "protocols", wiz_entity_properties.get("protocol")
        )
        end_port = (
            wiz_entity_properties.get("portEnd")
            if wiz_entity_properties.get("portEnd")
            else start_port
        )
...
def handle_removed_assets(existing_assets: List[Asset], all_assets: List[Asset]):
    """
    ...
    delete_job = delete_progress.add_task(
        "[#f68d1f]Deleting assets that are no longer in Wiz...", total=len(to_delete)
    )
...
def create_data_objects(data_objects: Dict, assets: List[Asset]):
    """
    Create data objects for Wiz assets.
    :param data_objects: Data objects
    :param assets: Created assets
    """
    app = Application()
...
def create_links_for_assets(
    ...
    :param links_to_create_for_assets: Links to create for assets
...
def create_properties(wiz_prop_map: Dict, assets: List[Asset]):
    """
    Create properties for Wiz assets.
    :param wiz_prop_map: Wiz property map
...
def create_asset_component_mappings(
    created_assets: List[Asset], components: List[Component]
):
    """
    Create mappings between assets and components.

    :param created_assets: List of created assets.
    ...
    with map_asset_comp_progress:
        for asset in created_assets:
            component_title = asset.assetType.upper().replace(" ", "_")
            component = component_dict.get(component_title)
            if component:
                create_asset_component_mapping(asset, component)
            map_asset_comp_progress.advance(map_asset_comp_task, advance=1)


def create_asset_component_mapping(asset: Asset, component: Component):
    """
...
def create_missing_components(
    ...
    :param components_to_create: List of component titles to create or get.
    :param parent_id: Parent ID for the components.
    :param user_id: User ID for creating components.
    :param wiz_projects: List of Wiz project IDs associated with the components.
    :param existing_components: List of existing Component objects.
...
def log_existing_assets_info(existing_assets: List[Asset]):
    """
    Log information about existing assets.

    :param existing_assets: List of existing assets.
    """
    if existing_assets:
...
def collect_components_to_create(
    data: List[Dict[str, Any]], components_to_create: List[str]
):
    """
    Collect unique component titles to create from the data.

    :param data: Wiz data to process.
    :param components_to_create: List of component titles to create.
    :return: Updated list of component titles to create.
    """
    for row in data:
...
def process_data(params: ProcessDataParams):
    ...
    :param params: ProcessDataParams object containing the necessary parameters.
    :return: None
    """
...
def threaded_process_rows(params: ProcessDataParams, max_workers: int = 10):
    """
...
def save_ports(ports_to_create: Dict, assets: List[Asset]):
    """
    Create ports from the list of ports to create.

    :param ports_to_create: Dictionary of ports to create.
    :param assets: List of all assets.
    :return: None
    """
    create_ports_progress = create_progress_object()
    create_ports_job = create_ports_progress.add_task(
        "[#f68d1f]Creating ports for assets...", total=len(assets)
    )
...
def finalize_assets(
    ...
    :param wiz_assets_to_create: List of Wiz assets to create.
    :param wiz_assets_to_update: List of Wiz assets to update.
...
def log_final_asset_counts(wiz_assets_to_create, wiz_assets_to_update):
    """
...
def fetch_wiz_data(
    ...
    :param str api_endpoint_url: Wiz GraphQL URL (optional, defaults to app.config["wizUrl"]).
    ...
    app = Application()
    api_endpoint_url = (
        app.config["wizUrl"] if api_endpoint_url is None else api_endpoint_url
    )
