...
def stig_component_title(stig_title: str) -> str:
    ...
    :param stig_title: The STIG title.
...
def find_stig_files(directory) -> List[Path]:
    """
    Finds all STIG (.ckl) files within the specified directory.

    :param directory: Path to the directory where STIG files are located.
...
def parse_element_to_dict(
    ...
    :param key_tag: Optional; the tag name that contains the key for key-value parsing mode.
    :param value_tag: Optional; the tag name that contains the value for key-value parsing mode.
            ...
            STIG(
                stig_info=STIGInfo(
                    **parse_element_to_dict(
                        istig_elem, key_tag="SID_NAME", value_tag="SID_DATA"
                    )
                ),
                vulns=[
                    Vuln(
...
def get_components_from_checklist(
    ...
    """
    return (
        {stig.stig_info.stigid: stig_component_title(stig.stig_info.title)}
...
def get_all_components_from_checklists(checklists: List[Checklist]) -> dict[str, str]:
    ...
    :param checklists: The list of checklist objects.
