...
class PierOne(Service):
    def get_tag_info(self, image: DockerImage) -> list:
        ...
        """
        if image.is_zcr_registry():
            return self.get_oci_tag_info(image)
        return self.get_pierone_tag_info(image)

    def get_pierone_tag_info(self, image: DockerImage) -> list:
        """
    ...
    def get_oci_tag_info(self, image: DockerImage) -> list:
        """
        Gets detailed tag information
        """
        path = f"/v2/{image.team}/{image.artifact}/manifests/{image.tag}"
        ...
        # check whether an image is multi-arch
        if "manifests" in tag_info:
            return {
                "created_by": None,
                "created": None
            }
        
        digest = tag_info["config"]["digest"]
        path = f"/v2/{image.team}/{image.artifact}/blobs/{digest}"
    ...
    def get_image_tags(self, image: DockerImage, limit: int) -> list:
        """
        Gets all tags for an image.
        """

        if image.is_zcr_registry():
            path = "/v2/{team}/{artifact}/tags/list?n={limit}".format(
                team=image.team, artifact=image.artifact, limit=limit
            )
        else:
        ...
        if image.is_zcr_registry():
            return [
                parse_zcr_artifact_dict(entry, image.team, image.artifact)
                for entry in response.json()["tags"]
            ]
        else:
            return [
                parse_pierone_artifact_dict(entry, image.team, image.artifact)
                for entry in response.json()[-limit:]
            ]
...
def parse_zcr_artifact_dict(tag, team, artifact) -> dict:
    """
    Prepare a dictionary in the format suitable for display.

    Args:
        tag (str): The tag value.
        team (str): The team associated with the artifact.
        artifact (str): The artifact name.

    Returns:
        dict: A dictionary containing team, artifact, and tag information for CLI display.
    """    
    parsed_dict = {}
    parsed_dict["team"] = team
    parsed_dict["artifact"] = artifact
