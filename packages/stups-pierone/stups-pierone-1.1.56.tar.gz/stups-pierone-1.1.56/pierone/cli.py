...
def tags(config, team: str, artifact, url, output, limit):
    '''List all tags for a given team and optionally artifact'''
    url = set_pierone_url(config, url)
    ...
    for art in artifact:
        ...
        try:
            rows = api.get_image_tags(image, limit)
...
def describe(config, team, artifact, tag, url):
    ...
    if created_by := tag_info.get("created_by") is not None:
        details_box.set("General Information", "Author", created_by)

    if created_in := tag_info.get("created_by") is not None:
        details_box.set("General Information", "Created in", created_in)
