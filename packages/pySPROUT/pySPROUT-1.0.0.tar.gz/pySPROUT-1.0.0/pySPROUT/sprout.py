...
class SPROUT:
    ...
    def _check_params(self):
        ...
        self.weight, self.st_exp, self.st_coord = utils.check_spots_match(self.weight, self.st_exp, self.st_coord)
    ...
    def select_sc(self, num_per_spot = 10, mode = 'strict', max_rep = 1, repeat_penalty = 10):
        ...
        if self.num_per_spot == 0:	
            ...
            print(f'\t num_per_spot == 0; Using the exact cell number in each spot provided in weight.')
        elif self.num_per_spot == 1:
            self.num = self.weight.apply(lambda x: x.eq(x.max()).astype(int), axis=1)
            print(f'\t num_per_spot == 1; Using the idxmax celltype for each spot.')
        else:
        ...
        for key, value in self.new_picked_index.items():
            ...
            tmp[1] = key
            picked_index_df = pd.concat([picked_index_df, tmp], ignore_index=True)
    ...
    def affinity_sparse(self, chunk_size = 12):
        ...
        sparse.save_npz(f"{self.save_path}sparse_A.npz", sparse_A)
        #### TODO ###
    ...
    def spatial_recon(self, left_range = 0, right_range = 20, steps = 10, dim = 2,max_dist = 1):
        ...
        new_coord.to_csv(f'{self.save_path}SC_coord.tsv',sep = '\t',header=True,index=True)
        return new_coord