    ...
    # save all reconstructed result
        for i in range(int(left_range),int(right_range)):
            for j in range(steps):
                ...
                pd.DataFrame(coord).to_csv(f'{path}tmp_{i}_{j}.tsv',index = True, header= True, sep = '\t')
                if cor > max_shape:
                    max_shape = cor
                    best_in_shape = coord
    else:
        ...
        for i in range(int(left_range),int(right_range)):
            for j in range(steps):
                ...
                if cor > max_shape:
                    ...
                    best_in_shape = coord
            #print(i,':',max_shape)
    pd.DataFrame(best_in_shape,columns = ['x','y']).to_csv(f'{path}SC_coord_raw.tsv',index = True, header= True, sep = '\t')
