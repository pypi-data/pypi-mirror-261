...
def exchange_remote_token(client, access_token):
    """
    Exchange remote client's access token for a token of OpenID Connect profile.

    :param client: current client
    :param access_token: remote access token
    :rtype: dict
    """
    issuer = django_keycloak_lma.services.realm.get_issuer(client.realm)
    # remote token decoding check
    ...
    oidc_profile = django_keycloak_lma.services.oidc_profile.get_from_id_token(client, access_token)

    try:
        # try exchange
        token_response = client.openid_api_client.token_exchange(
            subject_token=access_token,
            requested_token_type='urn:ietf:params:oauth:token-type:refresh_token'
        )
    except Exception as ex:
        raise Exception(messages.REMOTE_REFRESH_TOKEN_ERROR)

    initiate_time = timezone.now()

    oidc_profile = django_keycloak_lma.services.oidc_profile.update_tokens(token_model=oidc_profile,
