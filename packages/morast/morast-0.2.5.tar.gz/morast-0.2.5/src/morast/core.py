...
METHOD_TARGETS: Dict[int, str] = {
    CLASS_METHOD: KW_CLASS,
    STATIC_METHOD: KW_CLASS,
    INSTANCE_METHOD: KW_INSTANCE,
}
...
MORAST_VERBATIM = morast_prefix("verbatim")
...
class SuperConfig:
    ...
    def __init__(
        ...
        self._extracted_sections: collections.OrderedDict[
            str, overrides.OverridesSection
        ] = collections.OrderedDict()

    @property
    def extracted_override_sections(
        self,
    ) -> collections.OrderedDict[str, overrides.OverridesSection]:
        """The ectracted overrides sections as a list"""
        return self._extracted_sections.copy()

    def extract_overrides(
        self, namespaced_name: str, section: overrides.OverridesSection
    ):
        """Store the overrides section"""
        self._extracted_sections[namespaced_name] = section
...
class MorastDocumentableItem:
    ...
    def set_docstring_from_override(self) -> None:
        ...
        """
        override_section = self.sc.mor[self.namespaced_name]
        ...
        self.sc.extract_overrides(self.namespaced_name, override_section)
...
class MorastAttribute(MorastDocumentableItem):
    ...
    supported_scopes = (KW_CLASS, KW_INSTANCE, KW_MODULE)
    ...
    def __init__(
        ...
        prefix_by_scope: Dict[str, str] = {
            KW_CLASS: f"{superconfig.emoji.class_attributes_prefix}"
            f" {namespace}",
            KW_INSTANCE: f"{superconfig.emoji.instance_attributes_prefix} ",
            KW_MODULE: f"{superconfig.emoji.constants_prefix} {namespace}",
        }
        ...
        #
        super().__init__(name, namespace=namespace, superconfig=superconfig)
        self.check_private()
...
class MorastAttributesList(MorastSection):
    ...
    supported_scopes = (KW_CLASS, KW_MODULE)
    ...
    def __init__(
        ...
        the internal collection of contained items.
        """
        level = 2 if scope == KW_MODULE else 4
        super().__init__(
            name,
            namespace="",
...
class MorastClassDef(MorastSection):
    ...
    * _superconfig_: ...
    ...
    # xylint: disable=too-many-branches
    # xylint: disable=too-many-statements
    # xylint: disable=too-many-locals
    ...
    def __init__(
        ...
        for scope in (KW_CLASS, KW_INSTANCE):
    ...
    def _add_attribute(
        ...
        _scope_ may be KW_CLASS or KW_INSTANCE
        """
        try:
        ...
        if (
            scope == KW_INSTANCE
            and new_attribute.name in self.existing_attributes[KW_CLASS]
        ):
            self.attribute_lists[KW_CLASS].remove(new_attribute.name)
            self.existing_attributes[KW_CLASS].remove(new_attribute.name)
        #
    ...
    def _add_class_attribute(
        ...
        #
        self._add_attribute(element, KW_CLASS)
    ...
    def _add_instance_attribute(
        ...
        """Add an instance attribute if it does not exist yet"""
        self._add_attribute(element, KW_INSTANCE, check_self=check_self)
    ...
    *   _superconfig_: a **SuperConfig** instance (passed through
    ...
    def __init__(
        ...
        *   classes
        ...
        #
        self.set_docstring_from_override()
        self.add_subnode(MORAST_DOCSTRING, nodes.DocString(self.docstring))
    ...
    def markdown_elements(self) -> Iterator[mde.BaseElement]:
        ...
        try:
            ...
            refs = self.sc.mor[MORAST_VERBATIM]
        except KeyError:
            pass
        else:
    ...
    def get_extracted_overrides(self) -> str:
        """Return extracted overrides (for the future extract subcommand)"""
        extracted = self.sc.extracted_override_sections
        descendants = collections.defaultdict(list)

        for name in extracted.keys():
            name_parts = name.split(commons.DOT)
            current_namespace = []
            previous_full_name = ""
            for part in name_parts:
                current_namespace.append(part)
                full_name = commons.DOT.join(current_namespace)
                if previous_full_name:
                    descendants[previous_full_name].append(full_name)
                #
                previous_full_name = full_name
            #
        #
        # start with self.name
        next_items = [self.name]
        for name, sub_items in descendants.items():
            if name not in next_items:
                next_items.append(name)
            #
            next_items.extend(sub_items)
        #
        overrides_list: List[str] = []
        for name in next_items:
            try:
                current_section = extracted.pop(name)
            except KeyError:
                continue
            else:
                # print(current_section.name)
                overrides_list.extend(
                    (str(current_section).rstrip(), commons.EMPTY)
                )
            #
        #
