...
class H2OGPTEAsync:
    ...
    def __init__(
        ...
        self._openid_connect_done = False

        additional_headers = {}
        if self._api_key is not None:
            self._auth_header = f"Bearer {self._api_key}"
        elif self._refresh_token is not None:
            self._auth_header = f"Token-Bearer {self._refresh_token}"
        ...
        if self._api_key is not None and self._refresh_token is not None:
            print(
                "Warning: The refresh_token parameter will be ignored in favor of the provided api_key"
            )
    ...
    async def openid_connect(self) -> str:
        """Tests openid connection.

        Returns:
            A session ID string.
        """
        return await self._get("/auth/oidc/connect", skip_openid_init=True)
    ...
    async def upload(self, file_name: str, file: Union[IO[bytes], bytes]) -> str:
        ...
        Raises:
            Exception: The upload request was unsuccessful.
        """
        await self._openid_connect_init()
    ...
    async def _openid_connect_init(self):
        if self._session_id is not None and not self._openid_connect_done:
            new_session_id = await self.openid_connect()
            if self._session_id != new_session_id:
                raise RuntimeError("Unable to setup connection to server")
            self._openid_connect_done = True
    ...
    async def _get(self, endpoint: str, skip_openid_init=False) -> Any:
        if not skip_openid_init:
            await self._openid_connect_init()
        res = await self._client.get(
