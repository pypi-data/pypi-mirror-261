...
class H2OGPTE:
    ...
    def __init__(
        ...
        self._base_headers.update(additional_headers)

        self._openid_connect_init()
        self._check_version(strict_version_check)

    def _openid_connect_init(self):
        if self._session_id is not None:
            new_session_id = self.openid_connect()
    ...
    def openid_connect(self) -> str:
        """Tests openid connection.

        Returns:
            A session ID string.
        """
        return self._get("/auth/oidc/connect")
