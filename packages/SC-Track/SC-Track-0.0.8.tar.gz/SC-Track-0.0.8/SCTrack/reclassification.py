...
class TreeParser(object):
    ...
    @staticmethod
    def check_ctype_start(type_name, start_index, linage_cell,
                          threshold=config.PROB_THRESHOLD * config.SMOOTH_WINDOW_LEN):
        """
        ...
        for i in range(len(cell_node_line) - 1):
            ...
            if current_cell_node.cell.area / before_cell_node.cell.area >= area_size_t:
                if self.check_mitosis_start(i, cell_node_line,
                                            m_predict_threshold=config.PROB_THRESHOLD * config.SMOOTH_WINDOW_LEN):
...
def relink_tree(tracker, gap_threshold=3):
    """When the frame index of the root node of a tree is greater than the frame index of the leaf node of another tree,
    and the difference between the two is within a certain range(gap_threshold), it will judged whether the two trees can
    be relinked,according to the similarity of this two cell"""
    root_nodes = {}
    leaves_nodes = {}
...
if __name__ == '__main__':
    ...
    annotation = r"G:\raw data\demo\annotation.json"
    mcy_img = r"G:\raw data\demo\image.tif"
    ...
    run(annotation=annotation, output_dir=r"G:\raw data\demo\test",
