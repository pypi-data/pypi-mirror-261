...
def peak_checking_area_precursor_frag_swath(peak_ref, ms2, precursor_ion_start_mass=99.5, profile=True, mz_overlap=1,
    ...
    for i, (k, v) in enumerate(items):
        ...
        # . xxxxxx
        peak_area = peak_checking_area_split(ref_all, target_ms2, name, profile=profile, orbi=True, message=message1,
                                             sn_info=sn_info)
...
def FT_ICRMS_process(mzs, intensities, mode='neg', atoms=['C', 'H', 'O', 'N', 'S'],
  ...
  Args:
      mzs: A list containing the raw m/z values from the mass spectrum.
      intensities: A list containing the raw intensities for each m/z value.
          The length of this list must match the length of `mzs`.
