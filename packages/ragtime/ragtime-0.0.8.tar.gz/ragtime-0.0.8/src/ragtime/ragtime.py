...
FOLDER_FACTS:Path = FOLDER_EXPE / Path('03. Facts')
FOLDER_ANSWERS:Path = FOLDER_EXPE / Path('02. Answers')
FOLDER_QUESTIONS:Path = FOLDER_EXPE / Path('01. Questions')
...
logger:RagtimeLogger
try:
    logger
except NameError: # set it to None only if not already defined - otherwise could overwrite later conf
    logger = None
