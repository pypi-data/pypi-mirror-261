...
class IssueImplementationS2(Issue):
    ...
    def all_lite():
        ...
        issues = IssueImplementationS2.get_issue_list_from_jira(fields)
        return [IssueImplementationS2(key=issue["key"], updated=issue["fields"].get("updated", None),status_updated=issue["fields"].get("statuscategorychangedate")) for issue in
                issues]
    ...
    def all_key():
       fields = ["updated"]
       return [issue.get('key') for issue in IssueImplementationS2.get_issue_list_from_jira(fields)]
    ...
    @staticmethod
    def all_jql():
        return [IssueImplementationS2(issue["key"], issue["fields"]["updated"]) for issue in
                IssueImplementationS2.find_by(config.jql_query)]
