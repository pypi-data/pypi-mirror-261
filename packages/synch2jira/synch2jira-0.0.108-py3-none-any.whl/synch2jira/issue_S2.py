...
class IssueImplementationS2(Issue):
    ...
    def get(self):
        try:
            ...
            if issue is not None:
                return IssueImplementationS2(key=issue.key, summary=issue.summary, description=issue.description, status_updated=issue.status,
    ...
    def find_by(jql_query):
        ...
        response = requests.get(config.jira_url_all, params=payload, headers=config.headers, auth=config.auth)           #à modifier 
    ...
    def all_lite():
        ...
        return[IssueImplementationS2(key = issue["key"],updated=issue["fields"].get("updated",None)) for issue in issues]
    ...
    def get_issue_list_from_jira():
        ...
        start_at =0
        while True:
            params = {'jql': '',"startAt": start_at, "maxResults": batch_size,"fields": [  "summary"]}
    ...
    def create_isssue_and_move_to_done_for_performance_test():
        ...
        for i in range(307,500):
            ...
            issue = IssueImplementationS2(summary= summary,description= description)
            key = issue.save()
            print(key)
            key_list.append(key)
            issue = IssueImplementationS2.find_by_id(key)
