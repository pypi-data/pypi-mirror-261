...
class WorkFlow():
    ...
    def get_all_workflow(state1,state2):
        issue_list = [issue.key for issue in IssueImplementationS2.all_lite()]  #S2 qui renvoit les clés
    ...
    def get_all_wokflow_in_csv(state1,state2):
        ...
        entete = ["Issue",f"{state1}_last_date",f"{state2}_last_date","Duration"]
    ...
    def get_rate(begin_time,end_time):
        ...
        workflow_list = [workflow for workflow in workflow_list if WorkFlow.is_time_in_period(WorkFlow.convert_jira_date_to_datime(workflow.to_time),begin_time,end_time)]
