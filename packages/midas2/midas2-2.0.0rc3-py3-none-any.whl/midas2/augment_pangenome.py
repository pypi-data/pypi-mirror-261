...
OUTPUT:
    ...
    - {centroid_xx}_info.tsv: centroid_xx, centroid_xx_length, marker_id, centroid_prevalence, cluster_annotation
    - augment/{centroid_xx}_prevalence.tsv: this also contrains centroid_length <= TODO: UPDATE to voted length
...
def augment_pangenome_master(args):
    ...
    def species_work(species_id):
        ...
        dest_file = midas_db.get_target_layout("cluster_xx_info", True, species_id, "", "99")
        local_file = midas_db.get_target_layout("cluster_xx_info", False, species_id, "", "99")
...
def augment_pangenome_worker(args):
    ...
    for xx in ["99", "95", "90", "85", "80", "75"]:
        ...
        centroid_xx_info = render_full_stacked_cxx_by_genome(genes_info, centroid_xx)
        centroid_xx_info.to_csv(f"{centroid_xx}_stacked_long_matrix.tsv", sep='\t', index=False)

        cxx_lifted_to_qry = list_cxx_coordinate_to_genome(genes_info, centroid_xx, repgenome)
        cxx_lifted_to_qry.to_csv(f"{centroid_xx}_lifted_to_{repgenome}.tsv", sep='\t', index=False)

        if args.scratch_dir != ".":
            copy_tasks.append((f"{centroid_xx}_lifted_to_{repgenome}.tsv", localpath(midas_db, species_id, f"augment/{centroid_xx}_lifted_to_{repgenome}.tsv")))
            copy_tasks.append((f"{centroid_xx}_stacked_matrix.tsv", localpath(midas_db, species_id, f"augment/{centroid_xx}_stacked_matrix.tsv")))
