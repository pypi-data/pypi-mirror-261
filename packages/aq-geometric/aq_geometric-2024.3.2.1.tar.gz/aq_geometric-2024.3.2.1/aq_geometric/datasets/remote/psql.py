...
class AqGeometricDataset(Dataset):
    ...
    def _load_next_graph(self) -> None:
        ...
        if self.current_graph_index+1 < len(self.graph_index_ranges):
            ...
            node_features = torch.cat((g1.x, g2.x), dim=0)
            node_targets = torch.cat((g1.y, g2.y), dim=0)
            node_features_mask = torch.cat((g1.node_features_mask, g2.node_features_mask), dim=0)
            node_targets_mask = torch.cat((g1.node_targets_mask, g2.node_targets_mask), dim=0)
            valid_measurement_mask = torch.cat((g1.valid_measurement_mask, g2.valid_measurement_mask), dim=0)
