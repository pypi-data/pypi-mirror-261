...
class CliActionRunner:  # pylint: disable=too-few-public-methods
    ...
    def _install_setup(self):
        if Util.is_windows():
            ...
            result = subprocess.run(["wsl", "--version"], capture_output=True, text=True, check=False)  # nosec
            if "WSL version" not in ("".join(filter(lambda x: x != "\x00", result.stdout)).split(":")):
                commands = [
                    (
                        ...
                        "Subsystem-Linux /all /norestart' -Wait"
                    )
                ] + commands
...
aws_secret_access_key = {mount_storage_credentials_body["credentials"]["secretAccessKey"]}
    ...
    def _mount_storage(self, credentials_content):
        ...
        for directory in ["~/.aws", "~/.fovus", "/fovus-storage/files", "/fovus-storage/jobs"]:
            if Util.is_windows():
                ...
                if ~os.path.exists(os.path.expanduser(f"{directory}")):
                    os.system(f"mkdir -p {directory}")  # nosec
        ...
        if Util.is_windows():
            ...
            launch_wsl_script_content = r"""Set shell = CreateObject("WScript.Shell")
            shell.Run "wsl -d Fovus-Ubuntu", 0
            Set FSO = CreateObject("Scripting.FileSystemObject")

            If FSO.DriveExists("M:\") Then
                WScript.Echo "M: drive is already mapped."
            Else
                shell.Run "cmd /c net use M: \\wsl.localhost\Fovus-Ubuntu /persistent:yes", 0
                WScript.Echo "WSL directory mapped to M: drive."
            End If
            """
            ...
            mount_fovus_storage_script_content = r"""Set shell = CreateObject("WScript.Shell")
            shell.Run "fovus -ms", 0"""
            ...
            print(
                r"Your Fovus storage is successfully mounted under the path of M:\fovus-storage\ as a network "
                r'file system. The path to "My files" is M:\fovus-storage\files, '
                r'and the path to "Job files" is M:\fovus-storage\jobs. Job files are read-only.'
                "\n\n"
            ...
            print(
                "\033[91mA system reboot may be required. If you do not see a new M drive gets mounted under "
                '"This PC", please reboot your system for it to take effect.\033[0m'
            )
        elif Util.is_unix():
            subprocess.run(  # nosec
                f"echo '{credentials_content.strip()}' > ~/.aws/credentials",
            ...
            os.system("echo 'fovus -ms' > /etc/profile.d/fovus-storage-init.sh")  # nosec
    ...
    def _unmount_storage(self):
        ...
        if Util.is_windows():
            ...
            subprocess.run(  # nosec
                "net use M: /delete /persistent:yes",
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
                check=False,
                shell=True,
            ...
            for mount_dir in ["files", "jobs"]:
                result = subprocess.run(
                    f"umount /fovus-storage/{mount_dir}", capture_output=True, text=True, check=False, shell=True
                )  # nosec
                if result.stderr:
