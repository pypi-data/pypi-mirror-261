...
class Edges:
    ...
    def define_edges(self, include_all=False):
        ...
        if self.is_2D:
            ...
            self.define_edges_from_2D(include_all=True)
        ...
        for edge in edges:
            ...
            self.add_edge(edge[0], edge[1], bo=1,
                          visible=True, dij=dij, rigid=False)

        self._add_terminal_attribute()

        self._add_stick_attribute()

    def define_edges_from_2D(self, include_all=False):

        edges, rij0 = self.get_edges_from_2D(include_all=include_all)

        for edge in edges:

            self.add_edge(edge[0], edge[1], bo=1,
                          visible=True, dij=rij0[edge[0]][edge[1]], rigid=False)

    def get_edges_from_2D(self, include_all=False):
        try:
            with open(self.filepath, 'r') as f:
                for _ in range(3):
                    comment = f.readline()

                line_natoms = f.readline().split()
                n_atoms = int(line_natoms[0])
                for i in range(n_atoms):
                    line = f.readline()

                edges = []

                rij0 = self._define_rij0(include_all=include_all)
        ...
        # self.return_node_data(attribute="cn_fak", include_all=include_all)
    ...
    def return_edges(self, attributes=None, values=None, nodes_visible=False, include_all=False) -> list:
        """
        ...
        elif (isinstance(attributes, Iterable) and isinstance(values, Iterable)) and (len(attributes) != len(values)):
        ...
        if include_all:
            ...
            if nodes_visible:
                ...
                visible_edges = [(node1, node2) for node1, node2, data in self.edges(data=True) if all(
                    [node in visible_nodes for node in (node1, node2)]) and (data.get("visible") == True)]

            else:

                visible_edges = [(node1, node2) for node1, node2, data in self.edges(
                    data=True) if (data.get("visible") == True)]

            return [(node1, node2) for node1, node2, data in self.edges(data=True) if all([data.get(attribute) == value for attribute, value in zip(attributes, values)]) and ((node1, node2) in visible_edges)]

    def return_edge_data(self, attribute: str, nodes_visible=False, include_all=False) -> np.ndarray:
        """
        ...
        if include_all:
            ...
            if nodes_visible:
                ...
                edge_data = [data.get(attribute) for node1, node2, data in self.edges(data=True) if all(
                    [node in visible_nodes for node in (node1, node2)]) and (data.get("visible") == True)]

            else:

                edge_data = [data.get(attribute) for node1, node2, data in self.edges(
                    data=True) if (data.get("visible") == True)]
    ...
    def return_visible_edges(self, nodes_visible=False, include_all=False) -> list:
        ...
        if include_all:
            ...
            if nodes_visible:
                ...
                edges = [(node1, node2) for node1, node2, data in self.edges(data=True) if all(
                    [node in visible_nodes for node in (node1, node2)]) and (data.get("visible") == True)]

            else:

                edges = [(node1, node2) for node1, node2, data in self.edges(
                    data=True) if (data.get("visible") == True)]
    ...
    def return_unvisible_edges(self, nodes_visible=False, include_all=False) -> list:
        ...
        if include_all:
            ...
            if nodes_visible:
                ...
                edges = [(node1, node2) for node1, node2, data in self.edges(data=True) if all(
                    [node in visible_nodes for node in (node1, node2)]) and (data.get("visible") == False)]

            else:

                edges = [(node1, node2) for node1, node2, data in self.edges(
                    data=True) if (data.get("visible") == False)]
    ...
    def set_edges_visible(self, attributes=None, values=None, visible_edges=None, include_all=False) -> None:
        """
        ...
        elif isinstance(visible_edges, Iterable) and isinstance(visible_edges, str) == False:
        ...
        if visible_edges is not None:
            ...
            visible_edges = [(node1, node2) for node1, node2, data in self.edges(data=True) if all(
                [data.get(attribute) == value for attribute, value in zip(attributes, values)])]
    ...
    def set_edges_unvisible(self, attributes=None, values=None, include_all=False, default_all_visible=True):
        """
        ...
        unvisible_edges = [(node1, node2) for node1, node2, data in self.edges(data=True) if all(
            [data.get(attribute) == value for attribute, value in zip(attributes, values)])]
    ...
    def get_distance_matrix(self, include_all=False) -> dict:
        """
        ...
        for source in visible_nodes:
            ...
            distances[source] = {node: matrix[source, node]
    ...
    def get_all_shortest_paths(self, include_all=False) -> dict:
        ...
        if include_all:
            ...
            return {source: {target: nx.shortest_path(self, source, target) for target in visible_nodes if source != target} for source in visible_nodes}
            ...
            return {source: {target: nx.shortest_path(subgraph, source, target) for target in visible_nodes if source != target} for source in visible_nodes}
    ...
    def _add_terminal_attribute(self) -> None:
        ...
        for edge in self.edges:
            ...
            if degree1 == 1 or degree2 == 1:
                ...
                self.add_edge(edge[0], edge[1], terminal=True)
    ...
    def _add_stick_attribute(self) -> None:
        ...
        nodes_with_cn2 = [node for node in visible_nodes if degrees[node] == 2]

        self.set_nodes_visible(visible_nodes=nodes_with_cn2)
        ...
        if self.constraints.get("constraints") != None:
            ...
            if extra_constraints != None:
                ...
                for extra_constraint in extra_constraints:
                    ...
                    self.add_edge(nodes[0]-1, nodes[1]-1, bo=1,
                                  visible=True, dij=extra_constraint["value"])
