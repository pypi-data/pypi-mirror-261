import numpy as np
import itertools
import molbar.molgraph.edges.bo_matrix as bo
...
class BondOrder:
    ...
    def assign_bond_orders(self):
        ...
        is_unsaturated = np.array([True if (node_data[1].get("visible") == True) and (
            degree[i] < valences[i][0]) else False for i, node_data in enumerate(self.return_visible_nodes(return_data=True))])
        ...
        self.add_node_data(attribute="possible_unsaturated",
                           new_data=is_unsaturated)
    ...
    def _get_bond_order_for_fragment(self, core_nodes: list, visible_nodes: list) -> None:
        """
        ...
        visible_nodes = self.return_visible_nodes()
        #print(np.array(visible_nodes)+1)
        ...
        atomic_valences = [list(valence) if is_core == 1 else [
            degree] for valence, degree, is_core in zip(atomic_valences, degrees, are_core)]
        ...
        all_valence_combinations = self._get_combinations_of_valences(
            elements, atomic_valences, degrees, are_core)
        ...
        fragment_bo_matrix = bo.bond_order_detection.get_best_bo_matrix(
            n_atoms, cn_matrix, degrees, atomic_numbers, all_valence_combinations, are_core)
        # print(np.argwhere(fragment_bo_matrix == 2)+1)
                ...
                if (j > i) and (bo_matrix[i, j] > 1):
                    ...
                    if (ith_element == "C") & (jth_element == "C"):
                        ...
                        if degrees[i] == 3 and degrees[j] == 2:
                            ...
                            self.add_edge(
                                visible_nodes[i], visible_nodes[j], bo=2)
                            ...
                            self.add_edge(
                                visible_nodes[i], visible_nodes[j], bo=bo_matrix[i, j])
        ...
        for edge in visible_edges:
            ...
            if "N" in elements_in_edge and "C" in elements_in_edge:
                ...
                if len(adjacent_to_carbon) == 3:
                    ...
                    if "O" in elements_adjacent_to_carbon and counts[np.where(unique_elements == "O")[0][0]] == 1:

                        oxygen_index = adjacent_nodes[np.where(
                            elements_adjacent_to_carbon == "O")[0][0]]

                        bond_order_C0 = self.edges[carbon_index,
                                                   oxygen_index]["bo"]

                        if bond_order_C0 == 2:

                            self.add_edge(carbon_index, nitrogen_index, bo=2)

    def _get_combinations_of_valences(self, atoms, atomic_valences: list, degrees: list, are_core: list) -> list:
        """
        ...
        return np.asfortranarray([list for list in itertools.product(*all_possible_valences)]).T

    def _saturate_groups(self, is_unsaturated: list, visible_nodes: list, elements: list, degree: list, valences: list):
        """
        ...
        for unsaturated_node in unsaturated_nodes:
            ...
            is_unsaturated = self._saturate_carbon_cn2(
                unsaturated_node, elements, degree, visible_nodes, is_unsaturated, valences)
    ...
    def _saturate_carbonyl_group(self, unsaturated_node, elements, degree, visible_nodes, is_unsaturated):
        """
        ...
        if elements[unsaturated_node] == "C" and degree[unsaturated_node] == 3:
            ...
            adjacent_oxygen_index = [i for i, node in enumerate(
                adjacent_nodes) if elements[visible_nodes.index(node)] == "O"]
            ...
            if adjacent_oxygen_index:
                for index in adjacent_oxygen_index:
                    ...
                    if oxygen_degree == 1:
                        self.add_edge(carbon_node, oxygen_node, bo=2)
                        is_unsaturated[unsaturated_node] = False
                        is_unsaturated[visible_nodes.index(
                            oxygen_node)] = False

        return is_unsaturated

    def _saturate_carbon_cn2(self, unsaturated_node: int, elements: list, degree: list, visible_nodes: list, is_unsaturated: list, valences: list):
        """
        ...
        if elements[unsaturated_node] == "C" and degree[unsaturated_node] == 2:
            ...
            for adjacent_node in adjacent_nodes:
                try:
                    ...
                    adjacent_valence = valences[adjacent_index][0]
                    adjacent_element = elements[adjacent_index]
                except ValueError:
                    continue

                if adjacent_degree + 1 == adjacent_valence or (adjacent_element == "N" and adjacent_degree == 2) or (adjacent_element == "C" and adjacent_degree == 2):

                    is_unsaturated[unsaturated_node] = False

                    #is_unsaturated[adjacent_index] = False

                    self.add_edge(carbon_node, adjacent_node, bo=2)
                    

        return is_unsaturated

    def _saturate_diazo(self, unsaturated_node: int, elements: list, degree: list, visible_nodes: list, is_unsaturated: list):
        """
        ...
        if elements[unsaturated_node] == "N" and degree[unsaturated_node] == 1:
            ...
            if (adjacent_element == "N" and adjacent_degree == 2) or (adjacent_element == "C" and adjacent_degree == 2):

                is_unsaturated[unsaturated_node] = False

                #is_unsaturated[adjacent_index] = False

                self.add_edge(nitrogen_node, adjacent_node, bo=2)
                    
        return is_unsaturated
    
    def _saturate_nitroxyl(self, unsaturated_node: int, elements: list, degree: list, visible_nodes: list, is_unsaturated: list):
        """
        ...
        if elements[unsaturated_node] == "N" and degree[unsaturated_node] == 3:
            ...
            if adjacent_elements.count("O") == 1:
                ...
                if adjacent_oxygen_degree == 1:
                    ...
                    self.add_edge(nitrogen_node, global_oxygen_index, bo=2)
    ...
    def _saturate_allene(self, unsaturated_node: int, elements: list, degree: list, visible_nodes: list, is_unsaturated: list):
        """
        ...
        if elements[unsaturated_node] == "C" and degree[unsaturated_node] == 2:
            ...
            if (first_element == "C" and first_adjacent_degree == 3) and (second_element == "C" and second_adjacent_degree == 3):
                ...
                self.add_edge(central_node, visible_nodes[first_adjacent_index], bo=2)

                self.add_edge(central_node, visible_nodes[second_adjacent_index], bo=2)
