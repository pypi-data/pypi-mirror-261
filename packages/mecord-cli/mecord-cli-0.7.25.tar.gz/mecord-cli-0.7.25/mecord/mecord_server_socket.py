...
def restart_mecord_cli():
    ...
    if platform.system() == 'Windows':
        ...
        result = subprocess.Popen(['schtasks', '/create', '/sc', 'ONCE', '/st', f'{win_hour}:{win_minute}', '/tn', f'MecordUpdate-{calendar.timegm(time.gmtime())}', '/tr', f"\"{time_task_file}\""], shell=True)
...
class MecordLongConnectThread(Thread):
    ...
    def receive_GetTask(self, body):
        ...
        if len(rsp.list) > 0:
            if self.max_counter - self.cur_counter > 0:            
                self.GetTask()
        ...
        while self.is_running:
            try:
                ...
                try:
                    ...
                    taskUtils.taskPrint(taskUUID, f"{current_thread().name}-{self.service_country} === receive task : {taskUUID}")
                ...
                taskUtils.taskPrint(taskUUID, f"{current_thread().name}-{self.service_country} === exception : {e}")
                pass
        print(f"   {current_thread().name}-{self.service_country} taskRunning stop")
    ...
    def GetTask(self):
        ...
        req.apply = True
        print(f"{current_thread().name}-{self.service_country} waiting next {req.limit} task")
