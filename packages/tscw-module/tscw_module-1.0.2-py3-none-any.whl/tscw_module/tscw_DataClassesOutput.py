...
class TSCW_Output():
    ...
    def create_pT_plot(self, data, meta:dict):
        ...
        colors = np.unique([line.get_color() for line in lines])
        labels = np.unique([line.get_label() for line in lines])
        ...
        lg = ax1.legend(handles = legend_lines, bbox_to_anchor = (0.99, 0.99), loc = 'upper right')
...
class TSCW_TBHC(TSCW_Output):
    ...
    def plot_forces_difference(self, is_export, depths, xlimits, *args):
        ...
        axes = flatten(axes) # make a list or return list
        if nrows == 1:
        ...
        for idx_arg, color in enumerate(colors):
                 ...
                 temp_path = Path(arg.path)
                 ln = axes[0].plot(arg.t_total, arg.forces_df.Fz_ges, color=color,
                                label='%s' %(temp_path.stem) ,linestyle='-', alpha = ALPHA)
        ...
        if is_export:
            fig.set_size_inches(self.plot_meta['width'], self.plot_meta['height'])
            save_path = Path(self.path).parent.joinpath(Path(self.path).stem + '_Fz_difference.png')
            fig.savefig(save_path, dpi = self.plot_meta['dpi'])
            print('Exported %s' %(save_path))
    ...
    def plot_pt_difference(self, depths, is_export, *args):
        ...
        for idx_arg, color in enumerate(colors):
            ...
            lns.append(Line2D([], [], color=color, linestyle='-', label=Path(arg.path).stem))
        ...
        if is_export:
            fig.set_size_inches(self.plot_meta['width'], self.plot_meta['height'])
            save_path = Path(self.path).parent.joinpath(Path(self.path).stem + '_pt_difference.png')
            fig.savefig(save_path, dpi = self.plot_meta['dpi'])
            print('Exported %s' %(save_path))
...
class TSCW_TFBH(TSCW_Output):
    ...
    def create_movie(self, range_radial:list = None, 
        ...
        ax.set(xlabel='x [m]', ylabel='z [m]', xlim = [range_radial[0], range_radial[1]]) #, ylim = [y[0], y[-1]])
        ax.invert_yaxis()
