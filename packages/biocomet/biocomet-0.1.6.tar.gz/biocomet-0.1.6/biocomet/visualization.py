...
def plot_nv(G, sigPartition, min_comm_size=3, plot_dir='.', legend=True, kind='ArcPlots', show=True, background='transparent'):
    ...
    if len(small_communities) > 0:
        print(f"The following communities have been removed as showing less nodes"
              f" than specified in min_comm_size: {small_communities}")

    G_trunc = G.copy()
    print(G_trunc.nodes(data='community'))
    ...
    if full_network:
        ...
        for commNum, commGeneSet in allCommGeneSets.items():
            ...
            if len(commGeneSet) < PPIGraph.min_comm_size:
                print('Skipped community number ' + str(commNum) + ' as it contains '
                                                                   'less nodes than specified in min_comm_size parameter.')
                continue
    ...
    if full_network: #community parameter ignored here
        ...
        for comm in communities:
            ...
            if len(nodes_in_community) < min_comm_size:
                print('Skipped community number ' + str(comm) + ' as it contains '
                                                                   'less nodes than specified in min_comm_size parameter.')
                continue
