...
class PPIGraph:
    ...
    def community_detection(self, iterations=10, algorithm='leiden', seed=None):
        if algorithm == 'leiden':
            ...
            nx.set_node_attributes(self.network, self.partition, 'community')
        elif algorithm == 'louvain':
            self.partition = apply_louvain(self.network, iterations=iterations, seed=seed)
            nx.set_node_attributes(self.network, self.partition, 'community')


    def get_functional_annotation(self, full_network=False):
        ...
        if full_network:
            self.func_annotation_full_network = checkFuncSignificanceFullNetwork(self)

        else:
            if self.partition is None:
                print('Community detection necessary first. Starting community detection now with default parameters.')
                self.community_detection()
            self.func_annotation = checkFuncSignificance(self)
    ...
    def plot_Wordclouds(self, full_network=False, categories='default', show=True, background='transparent'):
        if full_network:
            if self.func_annotation_full_network == None:
                print('Functional annotation necessary first. Starting functional annotation now with default parameters.')
                self.get_functional_annotation(full_network=True)
            ...
            if self.func_annotation == None:
                print('Functional annotation necessary first. Starting functional annotation now with default parameters.')
                self.get_functional_annotation(full_network=False)
