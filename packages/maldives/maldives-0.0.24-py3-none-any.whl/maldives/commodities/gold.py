...
class GoldPriceModel(object):
    ...
    def display(self, return_figures=False):
        ...
        self.df = self.df.join(self.df.Prediction.shift(self.shift).rename('PredictedPrice'), how='outer')
    ...
    def backtest(self, window):
        data = self.df[['CPI', 'Treasury10Y', 'ClosingPrice']].dropna().drop_duplicates()
        X, y = data[['CPI', 'Treasury10Y']], data['ClosingPrice']
