...
class StackedMultiLCA:
    ...
    def __init__(
        ...
        for row, func_unit in enumerate(self.func_units):
            logger.debug(f"Run functional unit: {row}/{len(self.func_units)}")
            ...
            for col, cf_matrix in enumerate(self.method_matrices):
                ...
                # self.stored_lcas[]
                # TODO: store func_unit-name method-name !!
                func_key = list(func_unit.keys())[0]["name"]
                self.stored_lcas[(func_key, self.methods[col])] = self.lca.characterized_inventory
