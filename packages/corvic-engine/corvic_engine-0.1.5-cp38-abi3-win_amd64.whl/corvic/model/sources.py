...
from corvic.model.resources import Resource
...
class Source(WrappedOrmObject[SourceID, orm.Source]):
    ...
    def _make(self, orm_self: orm.Source, derived_from_id: SourceID) -> Self:
    ...
    ) -> Ok[Self] | system.DataMisplacedError:
        client = client or get_default_client()
    ...
    @classmethod
    def from_polars(
        cls, name: str, data_frame: pl.DataFrame, client: system.Client | None = None
    ) -> Self:
        ...
        return cls.from_resource(resource, name=name, client=client).unwrap_or_raise()

    def _with_new_table(self, table: Table) -> Source:
        orm_source = dataclasses.replace(self._orm_self)
        orm_source.table_op_graph = table.op_graph.to_bytes()
        return Source(self._client, orm_source, self.derived_from_id)
