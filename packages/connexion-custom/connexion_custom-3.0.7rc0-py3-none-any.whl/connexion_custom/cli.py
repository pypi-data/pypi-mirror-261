...
import connexion_custom
from connexion_custom.apps import AbstractApp
from connexion_custom.mock import MockResolver
...
ASYNC_APP = "async"
AVAILABLE_APPS = {
    FLASK_APP: "connexion_custom.apps.flask.FlaskApp",
    ASYNC_APP: "connexion_custom.apps.asynchronous.AsyncApp",
}


def run(app: AbstractApp, args: argparse.Namespace):
    app.run("connexion_custom.cli:create_app", port=args.port, host=args.host, factory=True)
...
parser.add_argument(
    "--version",
    action="version",
    version=f"Connexion {importlib.metadata.version('connexion_custom')}",
)
...
def create_app(args: t.Optional[argparse.Namespace] = None) -> AbstractApp:
    ...
    app_cls = connexion_custom.utils.get_function_from_name(AVAILABLE_APPS[args.app_framework])
