...
from connexion_custom.context import operation, receive, scope
from connexion_custom.decorators.parameter import (
    AsyncParameterDecorator,
    BaseParameterDecorator,
    SyncParameterDecorator,
)
from connexion_custom.decorators.response import (
    AsyncResponseDecorator,
    BaseResponseDecorator,
    NoResponseDecorator,
    SyncResponseDecorator,
)
from connexion_custom.frameworks.abstract import Framework
from connexion_custom.frameworks.starlette import Starlette as StarletteFramework
from connexion_custom.uri_parsing import AbstractURIParser
from connexion_custom.utils import not_installed_error

try:
    from connexion_custom.frameworks.flask import Flask as FlaskFramework
except ImportError as e:
    _flask_not_installed_error = not_installed_error(
        e, msg="Please install connexion_custom using the 'flask' extra"
    )
    FlaskFramework = _flask_not_installed_error  # type: ignore


class BaseDecorator:
    """Base class for connexion_custom decorators."""
...
class WSGIDecorator(BaseDecorator):
    ...
    we get the request via the connexion_custom context provided by WSGI middleware.
...
class ASGIDecorator(BaseDecorator):
    ...
    we get the request via the connexion_custom context provided by ASGI middleware.
