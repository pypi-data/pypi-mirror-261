...
from connexion_custom.exceptions import MissingMiddleware, ResolverError
from connexion_custom.http_facts import METHODS
...
ROUTING_CONTEXT = "connexion_custom_routing"
...
class RoutedMiddleware(SpecMiddleware, t.Generic[API]):
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            connexion_custom_context = scope["extensions"][ROUTING_CONTEXT]
        except KeyError:
        ...
        api_base_path = connexion_custom_context.get("api_base_path")
        if api_base_path is not None and api_base_path in self.apis:
            for api in self.apis[api_base_path]:
                operation_id = connexion_custom_context.get("operation_id")
                try:
