...
class YargenPlugin(idaapi.plugin_t):
    ...
    def get_cache_dir(self):
        ida_user_dir = Path(idaapi.get_user_idadir())

        # 创建 yargen_cache 目录
        yargen_cache_dir = ida_user_dir / "yargen_cache"
        yargen_cache_dir.mkdir(exist_ok=True)
        return yargen_cache_dir

    def get_desktop_dir(self):
        ida_user_dir = Path.home() / 'Desktop'
    ...
    def ask_user_for_path(self):
        ...
        return self.get_desktop_dir() if choice == 1 else self.get_cache_dir()
    ...
    def run(self, arg):
        ...
        sample_instructions_path = save_path / "sample_instructions"
