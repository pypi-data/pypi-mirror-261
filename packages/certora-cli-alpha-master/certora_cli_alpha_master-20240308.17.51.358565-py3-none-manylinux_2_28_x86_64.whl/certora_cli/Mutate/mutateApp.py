...
class WebUtils:
    ...
    def config_server(args: SimpleNamespace) -> str:
        ...
        if Constants.SERVER in args.prover_dict:
            return args.prover_dict.get(Constants.SERVER)
...
class MutateApp():
    ...
    def submit(self) -> None:
        ...
        if not self.orig_run:
            ...
            success, certora_run_result = self.run_certora_prover(self.conf, msg=Constants.ORIGINAL)
            ...
            if not success or not certora_run_result or not result_link or not sources_dir:
                try:
                    assert self.dump_failed_collects
        ...
        # runs = [(mutant, sources_dir, trg_dir, prover_conf,
    ...
    def generate_gambit_conf_file(self) -> None:
        ...
        for gambit_obj in self.gambit:
            parent_dir = Path(gambit_obj[Constants.FILENAME]).parent.resolve()
            if parent_dir != Path.cwd():
                gambit_obj[Constants.SOURCEROOT] = str(parent_dir)
        ...
        for mutant in self.manual_mutants:
            ...
            if not mutants_location.is_dir():
                raise Util.CertoraUserInputError(
                    f"Manual mutations directory '{mutants_location}' was not found at {mutants_location}")
    ...
    def validate_empty_directories(self) -> None:
        ...
        if self.outdir:
            if self.outdir == Constants.GAMBIT_OUT:  # the default value
                shutil.rmtree(str(Constants.GAMBIT_OUT), ignore_errors=True)
            else:
                validate_non_existent_dir(self.outdir, "gambit output directory")
    ...
    def delete_artifacts() -> None:
        ...
        """
        for path in [Constants.ORIG_RUN_PROVER_CONF, Constants.ZIP_PATH]:
            try:
                if path.is_file():
                    path.unlink()
                elif path.is_dir():
    ...
    def cleanup(self) -> None:
        ...
        if not self.debug:
            ...
            shutil.rmtree(str(self.orig_run_dir), ignore_errors=True)
            shutil.rmtree(str(Constants.APPLIED_MUTANTS_DIR), ignore_errors=True)
            shutil.rmtree(str(self.outdir), ignore_errors=True)
