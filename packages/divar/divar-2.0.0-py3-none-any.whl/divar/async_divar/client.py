...
class Client:
	...
	async def GetCategory(self ,city: str, category_name: str, pagination: int=1, header: str | dict=None , timeout: int=None) -> list:
		...
		return await get_category(city, category_name, pagination, header , timeout)

	async def GetPost(self ,Token:str , header: str | dict=None , timeout: int=None) -> dict:
		"""Getting post info.
		...
		return await get_post(Token , header)

	async def Search(self ,query: str,cities: list=None, header: dict=None, timeout: int=3) -> list:
		"""Search for posts.
		...
		return await search_posts(query , cities , header , timeout)

	async def __sign(self , phone_number: str, header: dict=None, timeout: int=None) -> bool:
		"""Otp requester.
		...
		return await authmaker(phone_number , header , timeout)

	async def login(self, phone_number: str, otp_code: str | int, header: dict=None, timeout: int=None) -> str:
		...
		if timeout is None:
			timeout = self.timeout
		return await auth(phone_number , otp_code , header , timeout)

	async def GetNumber(self ,Token: str , _authorization: str | dict=None, header: dict=None, timeout: int=None) -> int | str:
		...
		if _authorization is not None:
			return await get_number(Token , _authorization , header , timeout)
		
		elif (_authorization is None) and Client.Unique_JWT is not None:
			return await get_number(Token , Client.Unique_JWT , header , timeout)
			...
			while not co_loop:
				try:
					...
					code_req = await self.__sign(number , header , timeout)
					code = input(f'Sent OTP code to {number}' + '\nPlease insert the otp code to login : ')
					jwt = await auth(number , code , self.header , self.timeout)
