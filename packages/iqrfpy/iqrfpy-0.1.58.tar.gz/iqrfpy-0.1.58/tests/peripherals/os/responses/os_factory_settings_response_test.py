...
from iqrfpy.peripherals.os.responses.factory_settings import FactorySettingsResponse
from tests.helpers.json import generate_json_response
from iqrfpy.response_factory import ResponseFactory

data_ok: dict = {
    'mtype': OSMessages.FACTORY_SETTINGS,
    'msgid': 'factorySettingsTest',
    'nadr': 2,
    'hwpid': 0,
    'rcode': 0,
    'dpa_value': 64,
    'dpa': b'\x02\x00\x02\x8d\x00\x00\x00\x40'
}
...
data_ok_1: dict = {
    ...
    'rcode': 0,
    'dpa_value': 35,
    'dpa': b'\x02\x00\x02\x8d\x02\x04\x00\x23'
}
...
data_error: dict = {
    ...
    'rcode': 8,
    'dpa_value': 35,
    'dpa': b'\x00\x00\x02\x8d\x02\x04\x08\x23'
}
...
class ResetResponseTestCase(unittest.TestCase):
    ...
    def test_factory_methods_ok(self, _, response_data, response, json):
        ...
        with self.subTest():
            self.assertEqual(response.pcmd, OSResponseCommands.FACTORY_SETTINGS)
        with self.subTest():
        ...
        if json:
            with self.subTest():
                self.assertEqual(response.mtype, OSMessages.FACTORY_SETTINGS)
            with self.subTest():
    ...
    def test_from_dpa_invalid(self):
        with self.assertRaises(DpaResponsePacketLengthError):
            FactorySettingsResponse.from_dpa(b'\x02\x00\x02\x8d\x00\x00\x00\x22\x32')
