...
def convert_file(
    ...
    logging.info(f"Wrote tab-delimited file '{outfile}'")
...
@click.option(
    "--infile",
    help="The input comma-separated file to be parsed and loaded into the SQLite3 database",
)
...
def main(
    ...
    if infile.endswith("csv"):
        logging.info(f"Will attempt to convert '{infile}' to a tab-delimited file")
        outfile = os.path.join(outdir, os.path.splitext(os.path.basename(infile))[0] + ".tsv")
        convert_file(infile, outfile, verbose)
        infile = outfile
