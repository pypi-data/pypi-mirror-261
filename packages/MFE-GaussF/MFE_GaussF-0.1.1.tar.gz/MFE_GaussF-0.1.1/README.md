...
```
import argparse
from MFEGaussF import kmer_frequency_distribution_mini_shared
...
To install the package `MFEGaussF` using pip, run the following command in your terminal:
```bash
pip install MFEGaussF
```
vim kmer_counting.py
```
from MFEGaussF import kmer_counting_loop
...
```
from MFEGaussF import merge_normalize_isoform_count_v1
from MFEGaussF import merge_normalized_isoform_count_TPM
...
```
import argparse
from MFEGaussF import MFE_GaussF_esti
...
```
import argparse
from MFEGaussF import pipeline_abundance_GaussF_esti_loop
