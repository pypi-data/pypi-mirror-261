...
def test_wetted_surface_area_regression_marinetraffic(tmpdir):
    ...
    with DBConn(dbpath) as dbconn, warnings.catch_warnings(), vinfoDB as trafficDB:
        ...
        tracks = vessel_info(
            encode_greatcircledistance(track_gen.TrackGen(rowgen, decimate=True), distance_threshold=250000, ),
            dbconn=trafficDB, )
