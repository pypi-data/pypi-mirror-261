...
tracks_single = [dict(lon=np.array([x1]), lat=np.array([y1]), time=[0], dynamic=set(["time"]), )]
track1K = [dict(lon=(np.random.random(1000) * 90) - 90, lat=(np.random.random(1000) * 90) + 0, time=range(1000),
                dynamic=set(["time"]), )]
