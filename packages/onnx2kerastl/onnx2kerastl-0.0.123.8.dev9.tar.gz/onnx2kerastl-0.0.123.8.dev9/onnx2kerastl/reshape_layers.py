    ...
    if is_numpy(layers[node.input[0]]) and is_numpy(layers[node.input[1]]) and not 'is_embedding' in params:
        ...
        if "is_embedding" in params:
            ...
            if isinstance(indices, (int, np.integer)) and indices < 0:
                indices += dim_len
            if tf.is_tensor(indices) or (isinstance(indices, np.ndarray) and np.sum(indices < 0) > 0):
                if isinstance(indices, np.ndarray):
                    indices = tf.convert_to_tensor(indices)
                indices = tf.cast(tf.where(indices < 0, indices + dim_len, indices), dtype=indices.dtype)
