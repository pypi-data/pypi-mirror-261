...
def threshold_image(filename="test.fits"):
    ...
    # azcam.console.plot.plt.hist(r, bins=50)
    ...
    azcam.console.plot.plt.hist(values, bins=bins)
    azcam.console.plot.plt.xlim(threshold * 0.95, threshold * 1.2)
    ...
    azcam.console.plot.plt.clf()
    azcam.console.plot.plt.xlabel('X')
    azcam.console.plot.plt.ylabel('Pixel value above background')
    azcam.console.plot.plt.plot(x,fitfunc(parameters,x))
    azcam.console.plot.plt.errorbar(x,y,yerr=numpy.sqrt(y),fmt='ro')
    """
