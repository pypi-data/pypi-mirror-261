...
export default class EnvironmentsSidebar extends React.Component {
  ...
  handleCreateEnvironmentClick = async () => {
    ...
    let envData = {
      ...
      tags: tagArray,
      visibility: this.state.newEnvVisibility,
    ...
    try {
      ...
      console.log(err);
      alert(`Sorry unable to create new environment`);
      this.handleOpenPopup(
        'Sorry something went wrong, we are unable to create new environment',
        'error'
      );
    }
    setTimeout(() => {
  ...
  cloneEnvironment = env => {
    ...
    this.setState({
      ...
      newEnvCode: env?.packagesInImage
        ...
        : '',
      img: env?.logoUrls[0] ? env?.logoUrls[0] : DEFAULT_IMG,
      filterQuery: '',
      filteredEnvironments: [],
      editingNewEnvForm: true,
      tagFilter: false
      ...
      .concat([
        envs.length <= 0 && (
          <div style={{ padding: '1em' }}>
            <p style={{ fontSize: 24, color: 'var(--jp-ui-font-color2)' }}>
                    ...
                    >
                      <form onSubmit={this.handleAddChips}>
                        <TextField
                          ...
                          label="Tagged with"
                          variant="outlined"
                          placeholder="Filter environment by tag"
