...
class PIH:
    ...
    class INPUT_WAIT:
        ...
        def has(group_name: str, recipient: str) -> bool:
            ...
            return nn(result) and (
                n(result.timeout)
                or (DateTimeTool.now() - result.timestamp).total_seconds() < result.timeout
            )
    ...
    class EXECUTOR:
        ...
        def execute_python_for_windows(
            ...
            for executor_name in (CONST.PYTHON.EXECUTOR_ALIAS, CONST.PYTHON.EXECUTOR):
                command_list: tuple[str, ...] = (
                    executor_name,
    ...
    class RESULT(ResultTool):
        ...
        class ZABBIX:
            ...
            @staticmethod
            def items(
                host_id: int, item_ids: int | tuple[int, ...] | None = None, fields: tuple[str, ...] | None = None
            ) -> Result[list[ZabbixItem]]:
                def mapper(item: ZabbixItem) -> ZabbixItem:
            ...
            @staticmethod
            def item(host_id: int, item_id: int) -> Result[ZabbixValue]:
                return ResultTool.with_first_item(PIH.RESULT.ZABBIX.items(host_id, item_id))

            @staticmethod
            def values(
                host_id: int, item_id: int | tuple[int, ...], limit: int | None = None
            ) -> Result[list[ZabbixValue]]:
                def mapper(item: ZabbixValue) -> ZabbixValue:
            ...
            @staticmethod
            def value(host_id: int, item_id: int) -> Result[ZabbixValue]:
