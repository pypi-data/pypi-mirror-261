...
class ConsoleAppsApi:
        ...
        with self.output.make_indent(1):
            ...
            if CheckableSections.MATERIALIZED_RESOURCES in checkable_section_list:
                if len(checkable_section_list) > 1:
                    ...
                    self.output.separated_line()
                    self.output.write_line(
                        js((A.CT_V.BLUE_ROMB, self.bold("Материальные ресурсы")))
                    )
            ...
            if section in checkable_section_list:
                A.R_F.execute("show_polibase_database_dump_statistics")
                event_description: EventDescription = (
                    A.CT_E.POLIBASE_CREATION_DB_DUMP_COMPLETE
                )
                event: EventDS = one(A.R_E.get_last(event_description))
                hours: int = round((A.D.now() - event.timestamp).total_seconds() / 60 / 60)
                with self.output.make_separated_lines():
                    self.output.write_line(
                        j(
                            (
    ...
    def show_zabbix(self) -> None:
        def label_function(
            item: ZabbixHost | ZabbixItem | ZabbixValue, item2: ZabbixItem | int
        ) -> str:
            if isinstance(item, ZabbixHost):
                return j((item.name, " (", item.host, ")"))
            if isinstance(item, ZabbixItem):
            ...
            if isinstance(item, ZabbixValue):
                return j(
                    (
                        item2.name,
                        ": ",
        ...
        host_id: int = int(
            self.input.item_by_index(
                "Выберите хост", A.R_Z.hosts().data, label_function
            ).id
        )
        item: ZabbixItem = self.input.item_by_index(
            "Выберите элемент", A.R_Z.items(host_id).data, label_function
        )
        value: ZabbixValue = one(A.R_Z.value(host_id, item.itemid))
        self.output.write_line(label_function(value, item))
