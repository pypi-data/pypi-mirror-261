...
#            'rsi','stoch','roc','cci','willr','ao','stochrsi','ppo',  # Momentum
...
def get_tech_hist(ticker, window):
    ...
    try:
        df_hist['feature'] = signal.detrend(df_hist['Close'])  # 통계 정확성을 위한 주기성 시그널 제거용
            ...
            time.sleep(3.1)
            base.logger2.error(f'{ticker} get_tech_hist Value error 1: {e}')  # get_tech_hist Value error 1: array must not contain infs or NaNs: 채권에서처럼 detrend 를 만들수 없는 오류
            df_hist = ticker_obj.history(period='36mo') # test: 10mo, real: 36mo
...
# yfinance 를 이용한 ticker statesments 데이터 가져오기
def get_tech_fin(ticker, window, opt='All'):

    ticker_obj = yf.Ticker(ticker)
    # Financial Data: 이 정보는 finviz 에서 분기단위 + 4개분기 더 제공하여 대체됨. (finviz fobidden 시 활용가능) 20240309
    pnl = ticker_obj.financials  # 손익계산서
    bs = ticker_obj.balancesheet  # 재무재표, 재정상태표
    ...
    df_fin.columns = ['Date', *df_fin.columns[1:]]
...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    for group, techs  in TECH_LIST.items():
        for tech in techs:
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        # buttom에서 매수시그널 리포트, 분할매수 시작, 기준선까지가 분할매수 마무리하고,
        # 그 상태로 홀딩하다다가,
        # top 에서 매도시그널 리포트, 분할매도 시작, 기준선까지 분할매도 마무리하는 방식으로 전환: 20240305
        _df['SellAndBuy'] = _df[tech_type].ewm(span=5, adjust=False, min_periods=5).mean()
        try:
            slope_before = base.trend_detector(_df, 'SellAndBuy', base.get_working_day_before(5))
        except Exception as e:  # lastest_date 가 짧아 np.polyfit 함수 오류로 인한 수작업 기울기 구하기.
            slope_before = (df[tech_type] - df[tech_type].shift(3)) / 3
        print('        ', slope_before)

        # Here is Buttom, 분할매수 시작, slope 이 마이너스(-)에서 0 로 전환된 날 리포트 : 2
        _df['pivot'] = np.where((slope_before <= abs(permit_range)) & (_df['SellAndBuy'].shift(1) < _df['SellAndBuy']) & \
                                _df['SellAndBuy'] < 0, 2, 0)
        # 분할매수 끝, slope 이 플러스(+) 인 상태에서, 이동평균값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((slope_before > permit_range) & (_df['SellAndBuy'].shift(1) < _df['SellAndBuy']) & \
                                _df['SellAndBuy'] < 0, 1, _df['pivot'])

        # Here is Top, 분할매도 시작, slope 이 플러스(+)에서 0 으로 전환된 날 리포트 : -2
        _df['pivot'] = np.where((slope_before <= abs(permit_range)) & (_df['SellAndBuy'].shift(1) > _df['SellAndBuy']) & \
                                _df['SellAndBuy'] > 0, -2, _df['pivot'])
        # 분할매도 끝, slope 이 마이너스(-)인 상태에서 , 이동평균값이 기준선 0 아래로 전환된 날 리포트: -1
        _df['pivot'] = np.where((slope_before < permit_range*(-1)) & (_df['SellAndBuy'].shift(1) > _df['SellAndBuy']) & \
                                _df['SellAndBuy'] > 0, -1, _df['pivot'])

        _1 = _df[_df['pivot'] == 2] # 분할매수 시작
        _2 = _df[_df['pivot'] == 1] # 분할매수 마무리
        _3 = _df[_df['pivot'] == -2] # 분할매도 시작
        _4 = _df[_df['pivot'] == -1] # 분할매도 마무리
        ...
        if tech_type == 'bbands':  # 
            ...
            _df['pivot'] = np.where((_df[tech_type] >= _df['BBU_5_2.0']) & (_df[tech_type].shift(1) <= _df['BBU_5_2.0']).shift(1), -2, 0)
            # 분할매도 마무리
            _df['pivot'] = np.where((_df[tech_type] >= _df['BBU_5_2.0']) & (_df[tech_type].shift(1) >= _df['BBU_5_2.0']).shift(1) & \
                                    (slope_before <= abs(permit_range)), -1, _df['pivot'])
            # 분할매수 시작은 Buttom 에서 시작해서 기준선 하단 통과시 매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['BBL_5_2.0']) & (slope_before <= abs(permit_range)) & \
                                    (_df[tech_type].shift(1) <= _df['BBL_5_2.0']).shift(1), 2, _df['pivot'])
            # 분할매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['BBL_5_2.0']) & (_df[tech_type].shift(1) <= _df['BBL_5_2.0']).shift(1), 1, _df['pivot'])            
            ...
            # print(_df)
            # 분할매도는 상단기준선 넘으면서 바로 시작 (Not top) 해서 top 에서 매도 마무리
            _df['pivot'] = np.where((_df[tech_type] >= _df['DCU_20_20']) & (_df[tech_type].shift(1) <= _df['DCU_20_20']).shift(1), -2, 0)
            # 분할매도 마무리
            _df['pivot'] = np.where((_df[tech_type] >= _df['DCU_20_20']) & (_df[tech_type].shift(1) >= _df['DCU_20_20']).shift(1) & \
                                    (slope_before <= abs(permit_range)), -1, _df['pivot'])
            # 분할매수 시작은 Buttom 에서 시작해서 기준선 하단 통과시 매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['DCL_20_20']) & (slope_before <= abs(permit_range)) & \
                                    (_df[tech_type].shift(1) <= _df['DCL_20_20']).shift(1), 2, _df['pivot'])
            # 분할매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['DCL_20_20']) & (_df[tech_type].shift(1) <= _df['DCL_20_20']).shift(1), 1, _df['pivot'])  
        elif tech_type == 'kc':  # 
            plt.plot(_df.index, _df['KCUe_20_2'], color='orange')  # Upper line
            plt.plot(_df.index, _df['KCLe_20_2'], color='orange')  # Lower line
            # print(_df)
            # 분할매도는 상단기준선 넘으면서 바로 시작 (Not top) 해서 top 에서 매도 마무리
            _df['pivot'] = np.where((_df[tech_type] >= _df['KCUe_20_2']) & (_df[tech_type].shift(1) <= _df['KCUe_20_2']).shift(1), -2, 0)
            # 분할매도 마무리
            _df['pivot'] = np.where((_df[tech_type] >= _df['KCUe_20_2']) & (_df[tech_type].shift(1) >= _df['KCUe_20_2']).shift(1) & \
                                    (slope_before <= abs(permit_range)), -1, _df['pivot'])
            # 분할매수 시작은 Buttom 에서 시작해서 기준선 하단 통과시 매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['KCLe_20_2']) & (slope_before <= abs(permit_range)) & \
                                    (_df[tech_type].shift(1) <= _df['KCLe_20_2']).shift(1), 2, _df['pivot'])
            # 분할매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['KCLe_20_2']) & (_df[tech_type].shift(1) <= _df['KCLe_20_2']).shift(1), 1, _df['pivot'])  
        else:
        ...
        for date in _1.index:
            plt.axvline(date, color='g', linestyle='--', lw=1,  alpha=1)
        # for date in _2.index:
        #     plt.axvline(date, color='g', linestyle='--', lw=1,  alpha=1)

        for date in _3.index:
            plt.axvline(date, color='r', linestyle='--', lw=1,  alpha=1) 
        # for date in _4.index:
        #     plt.axvline(date, color='r', linestyle='--', lw=1,  alpha=1)             
 
    signal = get_tech_types(tech_type)
    # print(signal)

    if _1.empty & _2.empty & _3.empty & _4.empty:
        latest_date = '2050-12-31' # 임시
        result = 0
        decision = '대기'
    else:
        if _1.empty & _2.empty & _3.empty:
            latest_date = _4.index[-1]
        elif _1.empty & _2.empty & _4.empty:
            latest_date = _3.index[-1]
        elif _1.empty & _3.empty & _4.empty:
            latest_date = _2.index[-1]
        elif _2.empty & _3.empty & _4.empty:
            latest_date = _1.index[-1]              
        elif _1.empty & _2.empty:
            latest_date = max(_3.index[-1], _4.index[-1])
        elif _1.empty & _3.empty:
            latest_date = max(_2.index[-1], _4.index[-1])
        elif _1.empty & _4.empty:
            latest_date = max(_2.index[-1], _3.index[-1]) 
        elif _2.empty & _3.empty:
            latest_date = max(_1.index[-1], _4.index[-1])
        elif _2.empty & _4.empty:
            latest_date = max(_1.index[-1], _3.index[-1])
        elif _1.empty:
            latest_date = max(_2.index[-1], _3.index[-1], _4.index[-1])
        elif _2.empty:
            latest_date = max(_1.index[-1], _3.index[-1], _4.index[-1])
        elif _3.empty:
            latest_date = max(_1.index[-1], _2.index[-1], _4.index[-1])
        elif _4.empty:
            latest_date = max(_1.index[-1], _2.index[-1], _3.index[-1])                
        else:
            latest_date = max(_1.index[-1], _2.index[-1], _3.index[-1], _4.index[-1])

        try:
            if latest_date == _1.index[-1]:  # 분할매수 시작
                result = 2
                if signal == 'trend':
                    decision = '추세상승, 분할매수 시작'
                elif signal == 'overBoughtSold':
                    decision = '과매도, 분할매수 시작'
                elif signal == 'trendReversal':
                    decision = '상승반전, 분할매수 시작'
                elif signal == 'kReversal':
                    decision = 'k 상승반전 분할매수 시작'
                else:
                    decision = '분할매수 시작'
        ...
        try:    
            if latest_date == _2.index[-1]:  # 분할매수 마무리
                result = 1
                if signal == 'trend':
                    decision = '상승추세 유지하지만 분할매수 마무리 '
                elif signal == 'overBoughtSold':
                    decision = '적정매도매수 분할매수 마무리'
                elif signal == 'trendReversal':
                    decision = '반전없음, 분할매수 마무리'
                elif signal == 'kReversal':
                    decision = 'k 반전없음, 분할매수 마무리'
                else:
                    decision = '분할매수 마무리'
        except IndexError as e:
            pass
        ...
        try:
            if latest_date == _3.index[-1]:  # 분할매도 시작
                result = -2
                if signal == 'trend':
                    decision = '추세하락, 분할매도 시작'
                elif signal == 'overBoughtSold':
                    decision = '과매수, 분할매도 시작'
                elif signal == 'trendReversal':
                    decision = '하락반정, 분할매도 시작'
                elif signal == 'kReversal':
                    decision = 'k 하락반전 분할매도 시작'
                else:
                    decision = '분할매도 시작'
        except IndexError as e:
            pass
        ...
        try:
            if latest_date == _4.index[-1]:  # 분할매도 마무리
                result = -1
                if signal == 'trend':
                    decision = '하락추세 유지하지만 분할매도 마무리'
                elif signal == 'overBoughtSold':
                    decision = '적정매수매도 분할매도 마무리'
                elif signal == 'trendReversal':
                    decision = '반전없음, 분할매도 마무리'
                elif signal == 'kReversal':
                    decision = 'k 반전없음, 분할매도 마무리'            
                else:
                    decision = '분할매도 마무리'
        except IndexError as e:
            pass
...
def sma(ticker, df, tech_type, TECH_LIST, idx):
    try:
        ...
        base.logger2.error(f'{ticker} Technical.sma Value error 1: {e}')
        return df, '2050-12-31', 0, 0
    
    latest_date, sellandbuy = make_tech_plot(ticker, df, tech_type, TECH_LIST, idx)
    ...
    slope = (from_latest_date_slope / from_5day_ago_slope) * 5
    
    return df, latest_date, slope, sellandbuy
...
def ema(ticker, df, tech_type, TECH_LIST, idx):
    try:
        ...
        base.logger2.error(f'{ticker} Technical.ema Value error 1: {e}')
        return df, '2050-12-31', 0, 0
