...
const onConfirm = (action: ConfirmOption) => {
  const entryPayload: SpecificEntryPayload | BlanketEntryPayload = {
    ...
    start_hour: values.allDay ? '00:00:00' : moment(values.range?.[0]).format('HH:mm:ss'),
    end_hour: values.allDay ? '23:59:59' : moment(values.range?.[1]).format('HH:mm:ss'),
    ...
    if (values.repeatFreq === 'weekly') {
      ...
      if (action === 'only') {
        if ('excluded_dates' in entryPayload) {
          if (isDelete.value) {
            const formattedDate = moment(props.event?.start)
              .tz('UTC')
              .format('YYYY-MM-DD')
...
const genPayloadBased = (isExcludedPayload = true, tmpDate?: string | null, weeks?: number[]) => {
  const payload: BlanketEntryPayload = {
    ...
    end_date: moment(entry.value.end_date).format('YYYY-MM-DD'),
...
<template>
  <BModal v-model="open" :no-close-on-backdrop="isPending" centered @hide="emit('hide')">
    <BForm>
      ...
      <div class="flex mb-[.5rem]" v-if="mode !== 'quick-add-sickness-absence'">
        <label class="col-form-label">
        ...
        <div class="flex-1">
          <div class="mb-[.5rem]">
            <RangePickr
              ...
              :max-diff="24"
              :disabled-allday="values.entry_type?.requires_full_workday"
              :disabled="mode === 'edit'"
            />
