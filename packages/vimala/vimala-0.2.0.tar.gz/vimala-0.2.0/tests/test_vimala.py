...
@params("lineno,commands", [(None, []), (123, ["foo_cmd", "bar_cmd"])])
def test_vim(
    tmp_path: Path, lineno: int | None, commands: Iterable[str]
) -> None:
    ...
    assert echo_args.pop(0) == str(tmp_path)
    if lineno is not None:
        assert echo_args.pop(0) == f"+{lineno}"
    for cmd in commands:
        assert echo_args.pop(0) == "-c"
        assert echo_args.pop(0) == cmd
