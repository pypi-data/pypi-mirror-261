...
class Deconv(object):
    ...
    def __init__(self, image_size, number_of_sources, scale,
        ...
        if psf is None:
            ...
            if not len(psf.shape) == 3:
                raise RuntimeError(
                    'We expect your psf to have shape (epochs, nx, ny). Your psf does not have the right number of dimensions, with shape: {psf.shape}')
            # psf = np.flip(psf, (1, 2))
    ...
    def get_mask_pts_source(self, kwargs, mask_size=3, nan_mask=False):
        """
        Return an array containing a mask of size 'mask_size' around the point sources. Ouput array is at the
