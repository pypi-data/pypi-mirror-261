...
class Dataset:
    ...
    def __obj_embedding(self, x):
        try:
            ...
            print("Embedding column is not in the correct format. Required format: '[-90,7883,28282....]'")
            sys.exit(1)
    ...
    def __obj_mistake_scores(self, x):
        try:
            ...
            print("mistake_score column is not in the correct format. Required format: {{'classId':{'mistake_score':value}}")
            sys.exit(1)


    def add_embeddings(self, userDataFrame:pd.DataFrame, model:str):
        ...
        if "imageId" in df.columns and "imageId" in userDataFrame.columns:
            merged_df = pd.merge(df, userDataFrame, on='imageId', how='left')
        if "imageId" not in df.columns:
            num_rows_to_concat = min(userDataFrame.shape[0], df.shape[0])
            merged_df = pd.concat([df.iloc[:num_rows_to_concat, :], userDataFrame],axis=1)
            schema.add("imageId", PredictionSchemaElement())
        merged_df["embedding"] = merged_df["embedding"].apply(self.__obj_embedding)
        self.raga_dataset = merged_df
        schema.add("embedding", ImageEmbeddingSchemaElement(model=model))
        self.raga_schema = schema
        self.raga_extracted_dataset = data_frame_extractor(self.raga_dataset)


    def add_mistake_scores(self, userDataFrame:pd.DataFrame, refColName:str=""):
        df = self.raga_dataset
        schema = self.raga_schema
        if "mistake_score" not in userDataFrame:
            print("Your dataframe doesn't have mistake_score column")
            sys.exit(1)
        if "mistake_score" in df.columns:
            df.drop(columns=['mistake_score'])
        merged_df = pd.DataFrame()
        if "imageId" in df.columns and "imageId" in userDataFrame.columns:
            merged_df = pd.merge(df, userDataFrame, on='imageId', how='left')
        if "imageId" not in df.columns:
            ...
            merged_df = pd.concat([df.iloc[:num_rows_to_concat, :], userDataFrame],axis=1)
            schema.add("imageId", PredictionSchemaElement())
        merged_df["mistake_score"] = merged_df["mistake_score"].apply(self.__obj_mistake_scores)
        self.raga_dataset = merged_df
        schema.add("mistake_score", MistakeScoreSchemaElement(ref_col_name=refColName))
        self.raga_schema = schema
        self.raga_extracted_dataset = data_frame_extractor(self.raga_dataset)
