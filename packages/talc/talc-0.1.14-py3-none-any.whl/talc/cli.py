...
from talc.utils.cli.io import print_single_case, get_failures_and_warnings
...
def eval(
    ...
    res = poll_results(
        client, run_info.id, print_failed, out_file, loaded_config.scoring_rules
    )
    exit(res)
...
def get_results(api_key: str, run_id: str, print_failed: bool):
    ...
    res = poll_results(
        client, run_id, print_failed, None, DEFAULT_GRADER_CONFIG.scoring_rules
    )
...
def upload_and_grade(api_key: str, in_file: str, name: str, out_file: str | None):
    ...
    res = poll_results(
        client, run_info.id, True, out_file, DEFAULT_GRADER_CONFIG.scoring_rules
    )
...
@click.option("--dataset_name", default=None, help="The name of the dataset.")
@click.option(
    "--sampling_mode",
...
def generate(
    ...
    if dataset_name is None:
        dataset_name = "Generated " + datetime.datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )
