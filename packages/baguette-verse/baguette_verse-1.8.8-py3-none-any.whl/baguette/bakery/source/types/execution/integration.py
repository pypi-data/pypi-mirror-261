...
def integrate_process_creation(c : entities.Call):
    ...
    if c.arguments.process_identifier != 0:
        ...
        for p in Process:
            ...
            p = Process(PID = c.arguments.process_identifier, command = parse_command_line(c.arguments.command_line, executable = c.arguments.filepath if c.arguments.filepath else None), start = c.time)
