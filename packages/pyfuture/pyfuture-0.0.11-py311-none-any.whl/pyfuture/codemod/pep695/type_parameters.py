...
class TransformTypeParametersCommand(VisitorBasedCodemodCommand):
    ...
    __TransformTypeParametersCommand_remove_type_parameters_T = TypeVar("__TransformTypeParametersCommand_remove_type_parameters_T", bound = Union[FunctionDef, ClassDef])
    ...
    def remove_type_parameters(
        ...
        for type_param in type_params.params:
            new_name = type_param.param.name.with_changes(value="{:}{:}{:}".format(prefix, type_param.param.name.value, suffix))
    ...
    def visit_FunctionDef(self, node: FunctionDef):
        ...
        replacemences = {}
        prefix = "__{:}_".format(node.name.value)
        for type_param in type_params.params:
            for scope in [node_scope, body_scope]:
                ...
                for access in set(scope.accesses[type_param.param.name]):
                    ...
                    replacemences[access.node] = Name(value="{:}{:}".format(prefix, type_param.param.name.value))
        ...
        func = SimpleStatementLine(
            [
                Assign(
                    ...
                    value=Call(
                        func=Name(value="__wrapper_func_{:}".format(updated_node.name.value)),
                        args=[],
                    ),
                )
            ]
    ...
    def visit_ClassDef(self, node: ClassDef):
        ...
        for subnode in node.body.body:
            ...
            if isinstance(subnode, FunctionDef):
                ...
                prefix = "__{:}_{:}_".format(node.name.value, subnode.name.value)
    ...
    def leave_ClassDef(self, original_node: ClassDef, updated_node: ClassDef):
        ...
        for subnode in new_node.body.body:
            if isinstance(subnode, FunctionDef):
                prefix = "__{:}_{:}_".format(new_node.name.value, subnode.name.value)
