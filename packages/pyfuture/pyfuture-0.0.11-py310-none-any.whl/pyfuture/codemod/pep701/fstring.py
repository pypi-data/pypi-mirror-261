            ...
            if isinstance(node, cst.FormattedStringExpression):
                ...
                if (format_spec := node.format_spec) is not None:
                    for format_spec_node in format_spec:
                        assert isinstance(format_spec_node, cst.FormattedStringText), "Unknown node type: {:}".format(node)
                        format_spec_str += format_spec_node.value
                string += "{{:{:}}}".format(format_spec_str)
            else:
                assert isinstance(node, cst.FormattedStringText), "Unknown node type: {:}".format(node)
                string += node.value
        return cst.Call(
            func=cst.Attribute(
                value=cst.SimpleString(value="{:}{:}{:}".format(start, string, end)),
