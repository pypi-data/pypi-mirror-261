    ...
    for rule_set in rule_sets:
        match rule_set:
            ...
            case _:  # pragma: no cover
                raise ValueError("Unknown rule set: {:}".format(rule_set))


def transform_bit_or(op: cst.BinaryOperation, use_union: bool = True) -> Union[cst.Subscript, cst.Tuple, None]:
    """
...
def gen_type_param(
    type_param: Union[cst.TypeVar, cst.TypeVarTuple, cst.ParamSpec], type_name: Union[cst.Name, None] = None
    ...
    if isinstance(type_param, cst.TypeVar):
        args = [
            cst.Arg(cst.SimpleString('"{:}"'.format(type_name.value))),
        ]
...
def gen_func_wrapper(node: cst.FunctionDef, type_vars: list[cst.SimpleStatementLine]) -> cst.FunctionDef:
    ...
    wrapper = cst.FunctionDef(
        name=cst.Name(value="__wrapper_func_{:}".format(node.name.value)),
