...
CONFIG_NAME = "ipex_llm_config.json"


def _save_low_bit(self, save_dir, *args, **kwargs):
    invalidInputError(self._ipex_llm_config.get("ipex_llm_transformers_low_bit", False),
    ...
    with open(os.path.join(save_dir, CONFIG_NAME), "w") as json_file:
        json.dump(self._ipex_llm_config, json_file)
...
def low_bit_sanity_check(model_path):
    ...
    invalidInputError(os.path.isfile(os.path.join(model_path, CONFIG_NAME)),
                      "ipex_llm_config.json should be under your model directory,"
                      "please check your input path.")
    with open(os.path.join(model_path, CONFIG_NAME), 'r') as f:
        _config = json.load(f)

    low_bit = _config.get("ipex_llm_transformers_low_bit", None)
...
def optimize_model(model, low_bit='sym_int4', optimize_llm=True, modules_to_not_convert=None,
    ...
    import types
    model._ipex_llm_config = dict()
    model._ipex_llm_config["ipex_llm_transformers_low_bit"] = low_bit
