...
def replace_with_low_bit_linear_for_module(model, qtype, module_name=None,
    ...
    if has_been_replaced:
        if not (getattr(model, "quantization_method", None) == "gptq"):
            if torch_dtype == "auto":
                convert_ipex_llm_other_module(model, torch.float32)
            else:
                convert_ipex_llm_other_module(model, torch_dtype)
    return model
...
def ggml_convert_low_bit(model, qtype, optimize_model=True,
    ...
    # using ipex optimizer before changing to  linear
...
def convert_ipex_llm_other_module(model, dtype):
    # Convert modules outside of ipex_llm linear to corresponding dtype
