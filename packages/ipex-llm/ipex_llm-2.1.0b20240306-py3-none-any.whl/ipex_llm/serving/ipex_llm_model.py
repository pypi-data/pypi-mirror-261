...
def load_model_base(self, model_path: str, from_pretrained_kwargs: dict):
    ...
    print("Customized ipex-llm loader")
...
class IpexLLMAdapter(BaseModelAdapter):
    "Model adapter for ipex-llm backend models"

    def match(self, model_path: str):
        return "ipex-llm" in model_path
...
class IpexLLMLOWBITAdapter(BaseModelAdapter):
    "Model adapter for ipex-llm backend low-bit models"

    def match(self, model_path: str):
        return "ipex-llm-lowbit" in model_path
...
def patch_fastchat():
    ...
    if is_fastchat_patched:
        return
    register_model_adapter(IpexLLMLOWBITAdapter)
    register_model_adapter(IpexLLMAdapter)
