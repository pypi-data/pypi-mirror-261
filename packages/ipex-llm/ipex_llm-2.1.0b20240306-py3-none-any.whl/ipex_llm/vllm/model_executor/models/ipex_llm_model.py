...
class IpexLLMModelForCausalLM(nn.Module):
    ...
    def prepare_kv_cache(
        ...
        if (self.last_kv_cache is not None) and cur_seq_ids == self.last_seq_ids:
            if self.last_kv_cache[0][0].size(2) < max_seq_limit * 1.5:
                ipex_llm_kv_cache = self.last_kv_cache
                # Immediately set it to None to decrease ref-count
                self.last_kv_cache = None
            else:
                ipex_llm_kv_cache = [[tmp.narrow(2, self.last_kv_cache[0][0].size(2)
            ...
            for i in range(num_layers):
                ...
                ipex_llm_kv_cache.append(cur_list)

        return ipex_llm_kv_cache
    ...
    def prepare_kv_cache_selective_batching(
        ...
        # Return ipex_llm_kv_cache in the format of Tuple(List[Tuple(torch.Tensor)])
        ipex_llm_kv_cache = []
        for i in range(num_layers):
            ...
            ipex_llm_kv_cache.append(temp_cache)
        return ipex_llm_kv_cache
