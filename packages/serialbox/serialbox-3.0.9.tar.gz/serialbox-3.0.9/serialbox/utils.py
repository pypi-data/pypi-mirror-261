...
def check_sequential_region_boundaries(region):
    ...
    if region.start >= region.end:
        raise RegionBoundaryException(
            _("The start of the range can not be greater or equal to the end value.")
        )
    if region.state < region.start:
        raise RegionBoundaryException(
            _("The start of the range can not be less than the start value.")
        )
    regions = sb_models.SequentialRegion.objects.filter(pool=region.pool).exclude(
        pk=region.pk
    )
    if regions.count() > 0:
        for current_region in regions:
            if (
                ...
                and region.start <= current_region.end
            ):
                raise RegionBoundaryException()
