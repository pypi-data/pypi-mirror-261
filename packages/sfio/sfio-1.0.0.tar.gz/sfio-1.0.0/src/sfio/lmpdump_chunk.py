...
class Lmpdump(ReadWrite, MultiFrames):
    ...
    def scan(self, size: int = -1):
        sections = [
            (1, 'header', None),
            (1, 'frame', b'ITEM: TIMESTEP'),
            (1, 'box', b'ITEM: BOX BOUNDS'),
            (1, 'atoms', b'ITEM: ATOMS'),
        ]
        ...
        with self.open() as fd:
            ...
            patterns = [s for _, _, s in sections[1:]]
            matches = func.search_in_file(fd, patterns, self.scanned)
            scanned = fd.tell()

            print(matches)
            for j in range(3):
                for i, ((required, sect, _), match) in enumerate(
                    zip(sections[1:], matches)
                ):
                    print(sect, match[j])
                    self.scanned = match[j]
