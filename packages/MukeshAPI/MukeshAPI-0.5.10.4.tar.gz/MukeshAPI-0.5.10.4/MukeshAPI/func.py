...
def methods(class_obj=MukeshAPI):
    methods = [method for method in dir(class_obj)
               if callable(getattr(class_obj, method)) and not method.startswith("__")]
    ...
    for method_name in methods:
        ...
        if len(params) > 1:
            custom_methods[method_name] = list(params.keys())[1:]
