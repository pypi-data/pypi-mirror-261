...
from tecton_proto.args.feature_view_pb2 import TimeWindowSeries as TimeWindowSeriesArgs
...
from tecton_proto.common.time_window_pb2 import TimeWindowSeries
...
class TimeWindowSeriesSpec(TimeWindowSpec):
    ...
    def from_args_proto(cls, proto: TimeWindowSeriesArgs) -> "TimeWindowSeriesSpec":
        time_window_specs = ()
        for i in range(
            proto.start.seconds, (proto.end.seconds - proto.window_duration.seconds) + 1, proto.step_size.seconds
        ):
            time_window_specs += (
    ...
    def to_args_proto(self) -> TimeWindowSeriesArgs:
        return TimeWindowSeriesArgs(
            window_duration=timedelta_to_proto(self.window_duration),
            start=timedelta_to_proto(self.window_series_start),
            end=timedelta_to_proto(self.window_series_end),
    ...
    def step_size(self) -> datetime.timedelta:
        return abs(self.time_windows[0].window_end - self.time_windows[0].window_start)
    ...
    def to_string(self) -> str:
        return f"{self.window_duration_string()}_windows_{self.window_series_start_string()}_{self.window_series_end_string()}_{self.step_size_string()}"
