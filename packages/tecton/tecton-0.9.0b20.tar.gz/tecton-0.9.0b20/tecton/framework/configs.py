...
class TimeWindowSeries(StrictModel):
    ...
    end: datetime.timedelta = datetime.timedelta(seconds=0)
    ...
    def _to_proto(self) -> feature_view_pb2.TimeWindowSeries:
        ...
        return feature_view_pb2.TimeWindowSeries(
            start=time_utils.timedelta_to_proto(self.start),
            end=time_utils.timedelta_to_proto(self.end),
            step_size=time_utils.timedelta_to_proto(self.step_size),
            window_duration=time_utils.timedelta_to_proto(window_size),
        )
...
class Aggregation(StrictModel):
    ...
    def _default_name(self, aggregation_interval: datetime.timedelta, is_continuous: bool) -> str:
        ...
        if isinstance(self.time_window, TimeWindowSeries):
            column_name = f"{column_name}_windows_{window_spec.window_series_start_string()}_{window_spec.window_series_end_string()}_{window_spec.step_size_string()}"
        else:
