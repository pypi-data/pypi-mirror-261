...
from openweather_sdk.exceptions import ClientAlreadyExistsError, ClientDoesntExistError
...
class Client:
    ...
    def __init__(self, token, mode="on-demand", language="en", units="metric", cache_size=10, ttl=600, **kwargs):
        """
    ...
    @property
    def is_alive(self):
        if self.token in _ACTIVE_TOKENS:
            return True
        return False
    ...
    def remove(self):
        ...
        if self.is_alive:
            _ACTIVE_TOKENS.discard(self.token)
        else:
    ...
    def health_check(self):
        """Check if available 'http://api.openweathermap.org/'."""
        if not self.is_alive:
            raise ClientDoesntExistError
