...
def create_feature_set_frames_v2(current_index, directory_name, feature_images_urls): 
    ...
    current_index = add_images_in_frameset(temp_feature_images_urls, directory_name, current_index,120, 1) 
...
def run_ffmpeg_v2(directory_name, uniq_code):
    ...
    os.system(f"ffmpeg -v verbose -y -framerate 60 -i {directory_name}/frame_%d.jpg  -i {main_audio_path} -i {thanks_audio_path} -filter_complex '[2:a]adelay={delay_for_thanks}|{delay_for_thanks}[a2];[1:a][a2]amix=inputs=2:duration=longest[aout]' -map 0:v -map '[aout]' -c:v -preset ultrafast libx264 -crf 18 -pix_fmt yuv420p -color_primaries bt709 -color_trc bt709 -colorspace bt709 -r 60 -c:a aac -strict experimental -shortest outputs/output_{uniq_code}.mp4")
