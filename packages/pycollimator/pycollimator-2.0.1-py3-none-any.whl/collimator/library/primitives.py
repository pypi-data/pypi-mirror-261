...
def check_state_type(sys: LeafSystem, inp_data: Array, state_data: Array) -> None:
    ...
    if inp_data.shape != state_data.shape:
        ...
        raise ShapeMismatchError(
            system=sys,
            expected_shape=inp_data.shape,
            actual_shape=state_data.shape,
        )
    if inp_data.dtype != state_data.dtype:
        ...
        raise DtypeMismatchError(
            system=sys,
            expected_dtype=inp_data.dtype,
            actual_dtype=state_data.dtype,
        )
...
class Integrator(LeafSystem):
    ...
    def check_types(self, context):
        ...
        check_state_type(self, inp_data=u, state_data=xc)
...
class IntegratorDiscrete(LeafSystem):
    ...
    def check_types(self, context):
        ...
        check_state_type(self, inp_data=u, state_data=xd)
...
class Power(FeedthroughBlock):
    ...
    def _func(self, *inputs, **parameters):
        (u,) = inputs
        return cnp.power(u, parameters["exponent"])
...
class UnitDelay(LeafSystem):
    ...
    def check_types(self, context):
        ...
        xd = context[self.system_id].discrete_state
        check_state_type(self, inp_data=inp_data, state_data=xd)
