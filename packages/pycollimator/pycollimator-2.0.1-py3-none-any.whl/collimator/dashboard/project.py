...
class Project:
    ...
    def get_model(
        self, name: str, parameters: dict[str, model_json.Parameter] = None
    ) -> SimulationContext:
        if name not in self._models:
            raise KeyError(f"Model '{name}' not found")
        model, _globals = self._models.get(name)

        if model.diagram.nodes:
            logger.info('Loading model "%s"', model.name)
            try:
                return from_model_json.loads_model(
                    model.to_json(),
                    namespace=_globals,
    ...
    def get_submodel(
        self, name: str, instance_name: str, instance_parameters: dict[str, Any] = None
    ) -> Diagram:
        if name not in self._submodels:
            raise KeyError(f"Submodel '{name}' not found")
        ref_id = self._submodels.get(name)
        if ref_id is None:
            raise ValueError(f"Submodel '{name}' not found")
        return ReferenceSubdiagram.create_diagram(
            ref_id, block_name=instance_name, instance_parameters=instance_parameters
        )

    def save_model(self, *args, **kwargs):
        return save_model(self.summary.uuid, *args, **kwargs)

    def save_submodel(self, *args, **kwargs):
        return save_submodel(self.summary.uuid, *args, **kwargs)
...
def get_project_by_uuid(project_uuid: str, project_dir=None) -> Project:
    ...
    for model_summary in project_summary.models:
        if model_summary.kind == ModelKind.MODEL:
            ...
            models[model_summary.name] = (model, _globals)

    return Project(
        summary=project_summary, models=models, files=files, submodels=submodels
    )
...
def _save_submodel(
    ...
    submodel = model_api.get_reference_submodel_by_name(project_uuid, model.name)
...
def save_submodel(project_uuid: str, diagram: Diagram):
    """
    ...
    Returns:
        None
    """
    model_json, ref_submodels = to_model_json.convert(diagram)

    for ref_id, ref_submodel in ref_submodels.items():
        _save_submodel(project_uuid, ref_submodel, ref_id)

    _save_submodel(project_uuid, model_json, diagram.ref_id)
...
def save_model(
    ...
    if model is None:
        if model_uuid is not None:
            logger.info(
                "Model '%s' (%s) not found, a new model will be created",
                diagram.name,
                model_uuid,
            )
...
async def simulate(
    ...
    if parameters:
        body["parameters"] = {pname: p.to_dict() for pname, p in parameters.items()}
