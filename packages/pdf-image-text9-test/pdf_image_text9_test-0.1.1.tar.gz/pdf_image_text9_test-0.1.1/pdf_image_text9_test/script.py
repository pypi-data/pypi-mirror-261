...
class PdfToText:
    ...
    def get_pdf_content(self, image_filter: ImageFilter, page_index: int | None = None,
                        exclude_formatted_data: bool = False) -> Page | List[Page]:
        """
        Get pdf content
        :param image_filter:
        :param page_index:
        :param exclude_formatted_data:
        :return:
        """
        ...
        output = self.processor.get_pdf_text(pdf_content=self.pdf_content,
                                             ...
                                             exclude_formatted_data=exclude_formatted_data)
        return output


class ImageToText:

    def __init__(self, open_ai_key=None, open_ai_vision_model=None):
        self.open_ai_key = open_ai_key or os.environ.get("OPEN_AI_KEY")
        self.open_ai_vision_model = open_ai_vision_model or os.environ.get("OPEN_AI_KEY") or "gpt-4-vision-preview"
