...
EMPTY_CONFIG_PATH = os.path.join(
    os.path.dirname(__file__), "test_config_files", "empty.yaml",
)
...
NAME_AND_GIBBERISH_CONFIG_PATH = os.path.join(
    os.path.dirname(__file__), "test_config_files", "name_and_gibberish.yaml",
)
...
class TestDeploy:
    ...
    def test_deploy_from_file_override_options(self, fake_service_sdk):
        ...
        # Override image.
        result = runner.invoke(
            deploy, ["-f", FULL_CONFIG_PATH, "--image-uri", "override-image"]
        )
        assert result.exit_code == 0
        assert fake_service_sdk.deployed_config is not None
...
class TestStatus:
    ...
    def test_config_file_not_found(self, fake_service_sdk):
        runner = CliRunner()
    ...
    def test_config_file_missing_name(self, fake_service_sdk):
        runner = CliRunner()
