...
def get_cluster_compute_from_name(
    ...
    if enable_compute_config_versioning:
        (
            parsed_cluster_compute_name,
            parsed_version,
        ) = parse_cluster_compute_name_version(cluster_compute_name)

        if parsed_version and version:
            # TODO: after cleaning up the separate version argument, remove this check.
            raise ValueError(
                "Please provide either version in the name or as a separate argument, not both."
            )
        elif parsed_version:
            cluster_compute_name = parsed_cluster_compute_name
            version = parsed_version
        else:
            # version is not None and parsed_version is None.
            # Use non-parsed cluster_compute_name and version.
            pass
