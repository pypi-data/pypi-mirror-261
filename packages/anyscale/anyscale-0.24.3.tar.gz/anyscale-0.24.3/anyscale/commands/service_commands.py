...
@service_cli.command(
    ...
    help=(
        ...
        "To deploy a single application, the import path and optional arguments can "
...
def deploy(  # noqa: PLR0913
    ...
    if config_file is not None:
        if import_path is not None or len(arguments) > 0:
            raise click.ClickException(
                "When config file is provided, import path and application arguments can't be."
            )
    ...
    anyscale.service.deploy(
        config.options(
            ...
            excludes=[e for e in exclude] if exclude else None,
