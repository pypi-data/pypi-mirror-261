...
class Cataloguer:
    ...
    flux_perc: float
        ...
        required Num_target. Default value is 50th upper percentile of all stars (0.5).
    morph_perc: float
        ...
        Default values is the lower 25th percentile (0.25).
    timestamp: str
    ...
    '''
    def __init__(self, g_path, r_path, ccd_names, gaia_path=None, num_target=20, edge_crit=0.05, iso_perc=0.99,
                flux_perc=0.5, morph_perc=0.25, timestamp=None, generate_plot=True, perform_cosmicray_removal=False):
        '''
        Initialises Cataloguer with file paths, ccd_names, pixscale, num_target, edge_crit,
    ...
    def tap_query_gaia(self, fits_header):
        '''
        Runs a TAP query to GAIA DR 3 to get Parallax, Gmag and VCR values.
        Saves output as "gaia_catalogue.csv".

        Parameters
        ----------
        fits_header: ``astropy.io.fits.header.Header`` class object.
            FITS header, to get the ra and dec values to be used as the centre of the search.
        ...
        query = """SELECT g.RAJ2000, g.DEJ2000, g.Plx, g.Gmag, v.VCR \
        ...
        df.to_csv('gaia_catalogue.csv', index=False)
        logging.info("Successfully saved GAIA catalogue to 'gaia_catalogue.csv'.")
    ...
    def strip_sources(self, ccd_img_data, all_peaks_table):
        ...
        edge_a = ccd_img_data.shape[0]*self.edge_crit
        edge_b = ccd_img_data.shape[1]*self.edge_crit

        #Strip edge sources from detected peaks
        central_peaks = all_peaks_table[(all_peaks_table['x'] < ccd_img_data.shape[0]-edge_a) & (all_peaks_table['x'] > edge_a) & 
                    (all_peaks_table['y'] < ccd_img_data.shape[1]-edge_b) & (all_peaks_table['y'] > edge_b)]

        #Strip neighbouring sources closer than the Iso_Perc percentile
        sep_pix = all_peaks_table['pix_sep']
        sep_cut = np.quantile(sep_pix,(self.iso_perc))    #the most isolated stars
        ...
        flux_cut = np.quantile(iso_peaks['peak_value'].data,(self.flux_perc)) #upper 50% of brightest stars
    ...
    def select_reference_stars(self, pixscale, wcs_fits, ccd_img_data):
        '''
        ...
        matches: int array
            Indices in the base catalogue where there are matched points for the peaks/stars.
        '''

        all_peaks = self.find_all_peaks(pixscale, wcs_fits, ccd_img_data)
        ...
        matches = all_peaks['neighbour']

        #Extract all intermediate stars
        star_nddata = NDData(data=ccd_img_data) 
        all_stars = extract_stars(star_nddata, inter_stars, size=15) #TODO, estimate size from image and pixelscale        

        #Mask unuseable extracted stars (from overlapping cutout regions)
        extract = pd.DataFrame(all_stars.center_flat, columns = ['x', 'y'])

        df_all = inter_stars_df[['x','y']].merge(extract.drop_duplicates(), on=['x','y'], 
                            how='left', indicator=True)

        ref_stars = inter_stars_df[df_all['_merge'] == 'both']

        #Provide morphological properties of stars THIS COULD BE BETTER
        # UPDATE: hopefully this is better..
        semimajor_sigmas = []
        semiminor_sigmas = []
        fwhms = []
        ellipticities = []
        ...
        all_df_with_ref_indicator = pd.merge(all_peaks_df, ref_stars, on=['ref_id'], how='left', indicator='ref_star')
        all_df_with_ref_indicator['ref_star'] = np.where(all_df_with_ref_indicator.ref_star == 'both', True, False)

        return ref_sample, all_df_with_ref_indicator, all_stars, matches
    ...
    def plot_psf_stars(self, sample_df, all_stars, ccd_name):
        ...
        # Saves into 'psf_stars_{timestamp}.png'
        ...
        fig_save = plt.gcf()
        output_fig_path = "psf_stars-{}-{}.png".format(ccd_name, timestamp)
        fig_save.savefig(output_fig_path)
        logger.info("Succesfully saved sample reference stars plot as {}".format(output_fig_path))
        #plt.show()


    def pointed_catalogue(self, ccd_name, ccd_img_data, ccd_img_hdr, background, wcs_fits,
                        cosmicray_removed_mask, stars_df, match):
        '''
        ...
        ----------
        ccd_img_data: float ``numpy.ndarray`` of the image.
        ccd_img_hdr: ``astropy.io.fits.hdu.compressed.header.CompImageHeader`` class object.
        background: float
            The estimated background value.
        wcs_fits: ``astropy.wcs.WCS`` class object.
        cosmicray_removed_mask: boolean ``numpy.ndarray``
            The cosmic ray mask (boolean) array with values of True where there are cosmic ray detections.
        ...
        psf_fwhm = sigma_clipped_stats(stars_df['fwhm'].values, sigma=3.0)[1]
        ...
        finder = DAOStarFinder(threshold=background, fwhm=psf_fwhm, sigma_radius=sigma_rad)
        photometry = IterativePSFPhotometry(psf_model=psf_model, fit_shape=(7,7), 
                                   finder=finder, grouper=grouper, fitter=LevMarLSQFitter(),
                                    aperture_radius=aperture_rad, fitter_maxiters=fit_maxiters)
        ...
        sky_positions = wcs_fits.pixel_to_world(table['x_fit'], table['y_fit'])

        #Make final frame
        ids = [ccd_name + '-' + str(i) for i in table['id']]
        comb_phot = pd.DataFrame()
        comb_phot['chp'] = [ccd_name]*len(sky_positions)
        comb_phot['id'] =  ids
        comb_phot['x'] = table['x_fit']
        comb_phot['y'] = table['y_fit']
        comb_phot['flux'] = table['flux_fit']
    ...
    def generate_master_catalogue(self, filepath, ccd_name):
        ...
        if self.gaia_path:
            ...
            gaia_cat = self.tap_query_gaia(fits_header)

        sample_reference_stars, all_stars_df, all_stars, matches = self.select_reference_stars(pixscale, wcs_fits, data)

        if self.generate_plot:
            self.plot_psf_stars(sample_reference_stars, all_stars, ccd_name)
        
        photometry_cat = self.pointed_catalogue(ccd_name, ccd_data, ccd_header, wcs_fits, crmask,
                                                all_stars_df, matches)
    ...
    def compile_g_r_catalogues(self, ccd_name):
        ...
        match_3, sep_3, d3sep_3 = match_coordinates_sky(SkyCoord(g_cat['ra_GAIA'].values, g_cat['dec_GAIA'].values, unit="deg"),
                                                        SkyCoord(r_cat['ra_GAIA'].values, r_cat['dec_GAIA'].values, unit="deg"),
                                                        nthneighbor = 1)

        gsub_r = r_cat.drop(match_3)

        ccd_cat = pd.concat([g_cat, gsub_r])
    ...
    def generate_whole_catalogue(self):
        '''
        Generates catalogues for all CCDs in both g and r files and concatenates them into *the* 'whole catalogue'.
        ...
        output_csv_path = 'master_catalogue_g_r.csv'
