...
def sample_sim_new(d_overlay, sig_overlay, G,D,sig_W,sig_SiC,
                   D_Cr, sig_Cr, sig_Si_Bot,water=True, bilayer = True):
...
def multilayer_ref_new(theta,I0,thoff,bg,foot,G,D,d_overlay, sig_overlay, 
    sig_W,sig_SiC,Energy,res,D_Cr,sig_Cr,sig_Si_Bot,new_cell=True,water=True,bilayer=True):
    if res == 0:
        norm = 1
        nspan = 1
        dth_span = [0]
    ...
    for i in range(nspan):
        ...
        Intensity[frange] *= theta[frange]/foot
        if new_cell:
            # Correct for kapton absorption
            d_kap = 2*1e-3*scc.inch/theta/scc.degree 
            d_kap[d_kap>10*scc.milli] = 10*scc.milli
            mu = xdb.material_mu('kapton',Energy)/scc.centi
            Intensity *= np.exp(-d_kap*mu)
        tres = Intensity*mul[i]/norm
        result_list.append(tres)       
    return(I0*np.sum(np.array(result_list),0)+bg)
...
multilayer_ref_new_model = Model(multilayer_ref_new,
    independent_vars = ['theta','Energy','new_cell','water','bilayer'])
