...
class SchemaValidator:
    ...
    def check_class_attributes(self, cls: type[T]) -> None:
        ...
        if self.__schema_type == SchemaType.SCHEMA:
            id_field_names = [
                name
                for name, type_ in cls.__annotations__.items()
                if issubclass(type_, IdField)
            ]
            if len(id_field_names) > 1:
                raise MultipleIdFieldException("A schema cannot have more than 1 ids.")

    def check_unannotated_members(self, cls: T) -> None:
