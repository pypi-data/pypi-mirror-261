...
class Space(ABC):
    ...
    def __init__(self, fields: SIT | list[SIT], type_: type) -> None:
        ...
        self.__validate_fields_types(field_list, type_)
        self.field_set = set(field_list)
    ...
    def __validate_fields_types(self, field_list: list[SIT], type_: type) -> None:
        if incompatible_fields := [
            field.schema_obj._schema_name
            for field in field_list
            if not isinstance(field, type_)
        ]:
            raise InvalidSpaceParamException(
                f"The following field schema(s) are not compatible with this space: {incompatible_fields}"
            )
