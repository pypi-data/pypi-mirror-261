...
class Effect:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.affected_schema_reference, SchemaReference):
            raise InitializationException(
                f"{self.__class__.__name__} must have an affected_schema_reference"
                + f" of the type {SchemaReference.__name__}"
            )
