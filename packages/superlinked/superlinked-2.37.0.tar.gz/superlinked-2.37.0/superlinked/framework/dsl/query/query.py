...
class QueryObj:
    ...
    def with_vector(
        ...
        if not isinstance(schema_obj, IdSchemaObject):
            raise InvalidSchemaException(
                f"'find' ({type(self.schema)}) is not a schema."
            )
        filters = self.filters.copy()
        filters.append(
            BinaryPredicate(
...
class Query:
    ...
    def __init__(
        self,
        index: Index,
        weights: dict[Space, FloatParamType] | None = None,
    ) -> None:
        """
    ...
    def find(self, schema: IdSchemaObject | T) -> QueryObj:
        ...
        """
        self.__check_find_input(schema)
        query_obj = QueryObj(self, schema)
        self._queries.append(query_obj)
        return query_obj

    def __check_find_input(self, schema: IdSchemaObject | T) -> None:
