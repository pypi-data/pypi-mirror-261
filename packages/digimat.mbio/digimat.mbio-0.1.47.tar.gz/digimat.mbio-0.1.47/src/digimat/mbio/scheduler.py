...
class Schedulers(object):
    ...
    def hasConstant(self, name):
        try:
            self._constants[str(name).strip().lower()]
            return True
        except:
            pass
        return False
            ...
            for row in reader:
                try:
                    ...
                    if action in ['on', 'off']:
                        ...
                        self.updateConstant(self.getField(row, 1), self.getField(row, 2))
                except:
                    if logger:
...
class MBIOTaskScheduler(MBIOTask):
            ...
            if items:
                for item in items:
                    ...
                    if self._schedulers.setConstant(name, v):
                        self.value('cst_%s' % name, unit=0xff, default=v)
    ...
    def onLoad(self, xml: XMLConfig):
        ...
        if item and item.child('url'):
            try:
                ...
                self.config.separator=item.get('separator', ',')
                self.config.dialect=item.get('dialect')
    ...
    def run(self):
        ...
        setpoints=self._schedulers._setpoints.copy()
        ...
        try:
            for sp in setpoints:
                ...
                if value is not None:
                    value.updateValue(setpoints[sp])
        except:
            pass
