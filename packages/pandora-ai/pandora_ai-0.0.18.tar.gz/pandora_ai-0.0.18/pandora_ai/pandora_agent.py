...
class Pandora:
    ...
    def decide(self):
        ...
        [Assignement]
        Based on the immediate context of the conversation (described above) between the user and the ai-agent:
        - Output "1" if the agent should continue generating or perform more action.
        - Output "0" if the agent should stop here and let the user make a move.
        
        Response:
        
        """)
        response=self.completion(prompt=prompt,context=self.get_messages()[-6:],max_tokens=1,temperature=0,model="gpt-4-turbo-preview")
        if int(response)==1:
            return True
        else:
            return False
