...
class BzuTech:
    ...
    def send_reading(self, sensoref:str, DeviceID: str, reading: float, date: str):
        url = "https://back-dev.bzutech.com.br/logs/home_assistence/"
        data = "{'bci':'HA-81564', 'date':'{date}', 'data':'['ref':'{sensorref}','med':{reading}".format(date = date, sensorref = sensoref, reading = reading)
        print(data)
        client = ClientSession()
